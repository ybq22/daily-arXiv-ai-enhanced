...
TL;DR: 提出PC-UNet模型和PVMC-Loss损失函数，用于处理低剂量PET成像中的泊松噪声问题，提高图像保真度


<details>
  <summary>Details</summary>
Motivation: PET成像临床应用中，高剂量会增加辐射暴露风险，而降低剂量会导致泊松噪声增加，现有去噪方法无法有效处理这种噪声且会产生失真和伪影

Method: 开发Poisson Consistent U-Net模型，引入新的Poisson Variance and Mean Consistency Loss损失函数，该损失函数结合物理数据，在方差和梯度自适应方面具有统计无偏性

Result: 在PET数据集上的测试表明，PC-UNet提高了物理一致性和图像保真度

Conclusion: PC-UNet能够有效整合物理信息，为低剂量PET成像提供了一种有效的解决方案
...
TL;DR: DeLeaker是一种轻量级、无需优化的推理时方法，通过直接干预扩散模型的注意力图来缓解语义泄漏问题，无需外部输入即可有效抑制实体间的过度交互。


<details>
  <summary>Details</summary>
Motivation: 现有的文本到图像模型存在语义泄漏问题，即不同实体间的语义特征会意外传递。现有缓解方法通常基于优化或依赖外部输入，需要更高效的解决方案。

Method: 在扩散过程中动态重新加权注意力图，抑制实体间的过度交互，同时强化每个实体的身份特征。

Result: DeLeaker在所有基线方法中表现最佳，即使基线方法使用外部信息，DeLeaker也能在不影响保真度或质量的情况下有效缓解语义泄漏。

Conclusion: 注意力控制在缓解语义泄漏方面具有重要价值，为开发更语义精确的文本到图像模型开辟了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 现有的手工制作或程序生成的仿真场景缺乏可扩展性或无法捕捉真实世界复杂性，需要多样化的高保真城市环境来训练城市AI代理。

Method: UrbanVerse包含UrbanVerse-100K（10万+带语义和物理属性的城市3D资产库）和UrbanVerse-Gen（从视频提取场景布局并使用检索资产实例化度量尺度3D仿真的自动管线）。

Result: UrbanVerse构建了来自24个国家的160个高质量场景，在导航任务中相比先前方法在仿真中成功率提升+6.3%，在零样本仿真到真实迁移中提升+30.1%，仅需两次干预即可完成300米真实世界任务。

Conclusion: UrbanVerse场景保持了真实世界的语义和布局，达到了与手工制作场景相当的人类评估真实感，在训练城市导航策略方面表现出扩展幂律和强泛化能力。
...
TL;DR: Nano3D是一个无需训练的3D物体编辑框架，通过FlowEdit和TRELLIS实现精确的局部编辑，并引入Voxel/Slat-Merge策略保持结构一致性，同时构建了首个大规模3D编辑数据集Nano3D-Edit-100k。


<details>
  <summary>Details</summary>
Motivation: 当前3D物体编辑方法效率低下、不一致，且难以保持未编辑区域，多视图渲染编辑方法会引入伪影，限制了实际应用。

Method: 将FlowEdit集成到TRELLIS中，通过前视图渲染指导局部编辑，并引入区域感知合并策略Voxel/Slat-Merge自适应保持结构保真度。

Result: 实验表明Nano3D在3D一致性和视觉质量方面优于现有方法，并构建了包含10万+高质量3D编辑对的大规模数据集。

Conclusion: 该工作解决了算法设计和数据可用性的长期挑战，显著提升了3D编辑的通用性和可靠性，为前馈3D编辑模型的发展奠定了基础。
...
TL;DR: 提出了ECHO框架，通过社交媒体帖子构建图像生成模型基准测试，发现现有基准未能捕捉的新兴用例，并更好地区分最先进模型。


<details>
  <summary>Details</summary>
Motivation: 现有图像生成基准往往滞后于模型发展，无法捕捉新兴使用场景，导致社区对进展的认知与正式评估之间存在差距。

Method: 从展示新颖提示和用户定性判断的社交媒体帖子中构建基准测试框架，收集了超过31,000个提示构建数据集。

Result: ECHO发现现有基准中缺失的创意复杂任务，更清晰地区分最先进模型，并通过社区反馈为模型质量指标设计提供信息。

Conclusion: ECHO框架能够有效捕捉图像生成模型的新兴能力，为更准确评估模型进展提供了新方法。
...
TL;DR: 提出了一种基于子模框架的方法，用于从海量LoRA适配器中选择最相关且多样化的模型，解决用户选择困难的问题。


<details>
  <summary>Details</summary>
Motivation: LoRA模型虽然实现了预训练扩散模型的个性化定制，但平台上超过10万个适配器缺乏结构化组织，用户难以有效选择和利用最合适的模型。

Method: 将LoRA模型选择任务构建为组合优化问题，并提出了新颖的子模框架来解决这一挑战。

Result: 定量和定性实验表明，该方法能够在广泛领域内生成多样化的输出结果。

Conclusion: 提出的子模框架有效解决了从大规模LoRA适配器数据库中选择相关且多样化模型的挑战。
...
TL;DR: MOBIUS是一个用于通用实例分割的轻量化基础模型系列，通过瓶颈像素解码器、语言引导的不确定性校准损失和统一训练策略，在保持SOTA性能的同时显著降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有基础模型虽然性能优异但计算成本高昂，难以在资源受限的边缘设备上部署。需要开发既能保持高性能又能高效部署的实例分割模型。

Method: 提出瓶颈像素解码器用于高效多尺度和多模态融合；语言引导的不确定性校准损失用于自适应解码器剪枝；统一的训练策略简化训练流程。

Result: MOBIUS将像素和transformer解码器的FLOPs分别降低55%和75%，仅需三分之一训练迭代即可保持SOTA性能，在高低端设备上均建立高效分割新基准。

Conclusion: MOBIUS实现了在保持性能的同时显著降低计算复杂度的目标，为资源受限平台提供了可行的实例分割解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型在图表、文档、网页等人工图像领域缺乏大规模人工标注推理数据集的问题，这些领域在实践中丰富但标注困难。

Method: 将每个种子问题分解为原始感知和推理因子，然后系统性地与新图像重组生成大量合成问答对，每个生成的问题都配有子问题和中间答案，支持基于因子级过程奖励的强化学习。

Result: 在图表推理任务上，COGS显著提升了未见问题的性能，特别是在推理密集和组合性问题上的提升最大。使用不同种子数据的因子级混合训练在多个数据集上实现了更好的迁移效果。
...
TL;DR: GDGen是一个统一物理动力学生成框架，从势能角度将刚体、关节体和软体动力学整合到几何无关的系统中，通过引入方向刚度扩展经典弹性动力学，能够从简单运动观察推断物理属性。


<details>
  <summary>Details</summary>
Motivation: 构建具有真实交互动力学的数字孪生世界，为在可扫描环境中开发通用具身智能体提供机会，需要统一处理复杂物理行为的框架。

Method: 从势能原理出发，将世界视为整体实体，引入方向刚度扩展经典弹性动力学，使用专门网络建模扩展材料属性，采用神经场以几何无关方式表示变形。

Result: 实验表明GDGen能够稳健地统一多种仿真范式，为创建交互式虚拟环境和在复杂动态丰富场景中训练机器人智能体提供多功能基础。

Conclusion: GDGen通过势能视角成功构建了统一物理动力学生成框架，能够处理多种物理系统，为数字孪生和具身智能体开发提供了有力工具。
...
TL;DR: 提出COLIPRI编码器家族，通过引入报告生成目标和结合视觉-语言与纯视觉预训练，解决3D医学影像领域数据不足问题，在多项任务中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 3D医学影像领域的数据稀缺限制了当前视觉-语言编码器的能力，需要缓解数据不足的问题。

Method: 引入额外的归纳偏置：报告生成目标，并将视觉-语言预训练与纯视觉预训练相结合，利用图像-文本配对数据和纯图像数据。

Result: COLIPRI编码器在报告生成、分类探测和零样本分类中达到最先进性能，在语义分割中保持竞争力。

Conclusion: 通过额外的归纳偏置和3D医学影像领域的最佳实践，成功开发出性能优越的COLIPRI编码器家族。
...
TL;DR: 提出DRIFT方法，通过梯度空间转移推理知识来提升多模态大语言模型的推理能力，避免模型合并的性能不稳定问题


<details>
  <summary>Details</summary>
Motivation: 现有提升多模态大语言模型推理能力的方法依赖大规模监督微调或强化学习，资源消耗大。模型合并方法效果不稳定，在不同模型家族中表现差异显著

Method: DRIFT方法预计算推理先验作为推理模型与多模态变体在参数空间的差异，在微调过程中用该先验偏置梯度，实现推理知识转移而不破坏多模态对齐

Result: 在MathVista和MathVerse等多模态推理基准测试中，DRIFT相比朴素合并和监督微调持续提升推理性能，以较低成本达到或超越训练密集型方法的性能

Conclusion: DRIFT提供了一种轻量级方法，在标准监督微调流程中高效转移推理知识，解决了模型合并的不稳定性问题
...
TL;DR: 婴儿日常视觉体验的统计特性支持早期类别学习，其视觉输入呈现块状相似性结构，这种结构有助于从小数据集中实现泛化。


<details>
  <summary>Details</summary>
Motivation: 探索婴儿如何从有限经验中实现稳健的类别学习和泛化，解决婴儿学习能力与数据限制之间的明显矛盾。

Method: 分析14名7-11个月婴儿的自我中心视角图像，计算视觉输入的相似性结构，并进行计算实验验证这种结构对机器学习泛化的影响。

Result: 婴儿日常视觉输入呈现块状相似性结构，包含高度相似图像簇和较少见的可变图像，这种结构在机器学习中能提高小数据集的泛化能力。
...
TL;DR: SaLon3R是一个结构感知的长期3D高斯溅射重建框架，通过引入紧凑锚点基元和3D点Transformer，有效消除冗余并解决跨帧几何不一致性，实现超过50个视图的实时重建。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS方法在处理长时视频序列时存在大量冗余和几何不一致问题，需要一种能够在线重建且消除冗余的高效方法。

Method: 使用3D重建骨干网络预测密集高斯和显著图，通过可微分显著感知高斯量化将冗余高斯压缩为紧凑锚点，利用3D点Transformer学习空间结构先验来精炼锚点属性和显著度。

Result: 在多个数据集上实现了最先进的性能，能够重建超过50个视图且速度超过10FPS，冗余去除率达50%-90%，在新型视图合成和深度估计方面表现优异。

Conclusion: SaLon3R无需已知相机参数或测试时优化，在单次前向传播中有效解决伪影并修剪冗余3DGS，展示了高效、鲁棒和泛化能力强的长期可泛化3D重建方法。
...
TL;DR: TGT框架通过文本引导的轨迹控制视频生成，使用位置感知交叉注意力和双CFG方案，在复杂多对象场景中实现精确的视觉实体控制和运动控制。


<details>
  <summary>Details</summary>
Motivation: 现有文本到视频生成方法在控制场景中多个对象的精确位置和运动轨迹方面存在局限，特别是在复杂多对象设置下，缺乏明确的轨迹与视觉实体对应关系。

Method: 提出文本引导轨迹框架，结合位置感知交叉注意力整合轨迹和局部文本信号，采用双CFG方案分别调制局部和全局文本指导，并开发数据管道生成带跟踪实体描述的轨迹数据。

Result: 实验表明TGT在视觉质量、文本对齐准确性和运动可控性方面优于现有方法，能够使用点轨迹作为直观的运动控制手柄。

Conclusion: TGT框架通过文本引导的轨迹控制，显著提升了视频生成中对多个对象外观和运动的精确控制能力。
...
<details>
  <summary>Details</summary>
Motivation: 结合数据驱动的扩散模型与领域知识来指导脑成像问题是一个重要挑战。传统的贝叶斯逆问题方法虽然成功，但其解剖建模通常依赖经典数学先验，难以捕捉脑解剖的复杂结构。

Method: 使用基于分数的扩散先验，在大规模多样的脑MRI数据上训练，配合灵活的前向模型来处理各种图像处理任务。该方法还能优化现有深度学习方法的输出以提高解剖保真度。

Result: 在异质临床和研究MRI数据上的实验表明，该方法达到了最先进的性能，无需配对训练数据集即可产生一致、高质量的解决方案。

Conclusion: 这些结果凸显了扩散先验作为脑MRI分析多功能工具的潜力，为医学成像逆问题提供了新的解决方案。
...
TL;DR: FFT-MIL是一个用于全切片图像分类的框架，通过在频域分支中提取低频信息来增强多实例学习，有效捕获全局依赖关系。


<details>
  <summary>Details</summary>
Motivation: 现有的多实例学习方法难以捕获全切片图像的全局依赖关系，因为图像尺寸巨大且补丁嵌入具有局部性，这限制了粗粒度结构的建模能力。

Method: 使用快速傅里叶变换从WSI中提取低频信息，通过FFT-Block模块（包含卷积层和Min-Max归一化）处理频域数据，并将学习到的全局频域特征与空间补丁特征融合。

Result: 在三个公共数据集上评估了六种最先进的MIL方法，FFT-Block集成使宏观F1分数平均提高3.51%，AUC提高1.51%。
...
TL;DR: 提出了XModBench基准来评估全模态大语言模型的跨模态一致性，发现即使是Gemini 2.5 Pro在空间时间推理、模态差异和方向不平衡方面仍存在显著问题。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估一般跨模态问答能力，但不清楚全模态大语言模型是否真正实现模态不变推理或存在模态特定偏见。

Method: 构建包含60,828个多选题的大规模三模态基准XModBench，涵盖五个任务家族和所有六种模态组合，用于细粒度诊断模态不变推理、模态差异和方向不平衡。

Result: 实验显示最强模型Gemini 2.5 Pro：(i)空间时间推理准确率低于60%；(ii)存在持续模态差异，音频模态性能显著下降；(iii)显示系统性方向不平衡，视觉作为上下文时一致性较低。
...
TL;DR: 提出了UniFilter方法，通过训练多模态大语言模型作为统一的数据质量分类器，来筛选高质量的图像-文本描述数据和交错文档数据。
...
Method: 采用半合成方法，利用原始图像生成不同质量级别的文本，创建样本-分数对来训练UniFilter分类器，然后应用该分类器筛选高质量数据。

Result: 在筛选数据上预训练的MLLM表现出显著增强的能力，包括更强的零样本推理和上下文学习能力，在视觉监督微调后在多个基准测试中表现更优。

Conclusion: 高质量多模态预训练数据对下游任务性能有显著提升，作者发布了合成训练数据、UniFilter模型检查点和高质量交错文档子集OBELICS-HQ供社区使用。
...
Method: 在QUILT-1M数据集上训练CLIP模型，系统评估不同超参数设置（如学习率）和数据增强策略（如RandomResizedCrop）在三个下游组织病理学数据集（PatchCamelyon、LC25000-Lung、LC25000-Colon）上的影响。

Result: 发现RandomResizedCrop值在0.7-0.8时表现最佳，分布式训练无局部损失提高了稳定性，学习率低于5.0e-5在所有数据集上都会降低性能，LC25000（Colon）数据集提供了最可重复的基准。
...
Method: 使用显著概念感知图像嵌入模型来减少合成过程中无关视觉细节的影响，保持图像和文本输入之间的直观对齐，生成更好保留类别判别特征并具有额外受控变化的图像。

Result: 在八个细粒度视觉数据集上表现出色，在常规和长尾设置下分别以0.73%和6.5%的平均分类准确率提升优于最先进的增强方法。

Conclusion: 该框架通过减少无关视觉细节的影响，有效增强了训练数据集的多样性，从而提高了下游模型的鲁棒性。
...
TL;DR: 提出了首个公开的多模态数据集CARDIUM，结合胎儿超声心动图和临床数据，并开发了跨注意力多模态transformer架构，显著提升了先天性心脏病的产前检测性能。


<details>
  <summary>Details</summary>
Motivation: 先天性心脏病产前诊断面临数据稀缺、质量差、多源信息整合不足的问题，限制了AI模型的临床应用。

Method: 构建CARDIUM多模态数据集，包含胎儿超声图像、超声心动图和母体临床记录；提出跨注意力多模态transformer架构融合图像和表格数据特征。

Result: 在CARDIUM数据集上，多模态方法比单模态图像和表格方法分别提升11%和50%，F1得分达到79.8±4.8%。

Conclusion: CARDIUM数据集和多模态方法为先天性心脏病产前诊断研究提供了重要资源，显著提升了检测性能。
...
TL;DR: 研究文本到图像模型在广告定制中的潜力，分析广告主题的人口统计偏见，评估不同性别/种族人物形象对广告说服力的影响，并实验针对特定国家的广告定向技术。


<details>
  <summary>Details</summary>
Motivation: 探索文本到图像模型在定制视觉广告和针对特定人群方面的应用潜力，特别关注广告中的人口统计偏见问题。

Method: 通过分析不同广告主题的人口统计偏见，评估相同广告内容中仅改变人物性别/种族对说服力的影响，并实验针对特定国家的广告定向方法。

Result: 发现了广告中存在的性别和种族偏见，以及这些人口统计特征对广告说服力的不同影响。
...
TL;DR: DriveGen3D是一个生成高质量可控动态3D驾驶场景的新框架，通过结合加速长期视频生成和大规模动态场景重建来解决现有方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有驾驶场景合成方法存在计算需求过高、仅关注视频合成而缺乏3D表示、或局限于静态单场景重建等问题，需要一种能够生成长时间动态3D场景的高效方法。

Method: 采用统一流水线，包含两个专门组件：FastDrive-DiT（高效视频扩散变换器，用于在文本和鸟瞰图布局指导下合成高分辨率时间连贯视频）和FastRecon3D（前馈重建模块，快速构建跨时间的3D高斯表示，确保时空一致性）。

Result: 实现了实时生成长达12FPS的扩展驾驶视频（424×800分辨率）和相应的动态3D场景，在新视角合成上达到SSIM 0.811和PSNR 22.84，同时保持参数效率。

Conclusion: DriveGen3D成功解决了动态3D驾驶场景生成的关键挑战，通过多模态条件控制实现了高质量、高可控性的场景生成，为自动驾驶仿真等领域提供了有效工具。
...
Method: 采用CUDA加速的GPU并行化技术，支持姿态优化、地图构建、先验地图定位和外参精炼，专为离线处理设计以最大化计算资源利用。

Result: 实验结果表明，cuSfM在多种测试场景下相比广泛使用的COLMAP方法，在精度和处理速度上均有显著提升，同时保持了离线SfM应用所需的高精度和全局一致性。
...
TL;DR: 本文提出了一种结合模型集成和高效后处理策略的脑部MRI图像修复方法，通过中值滤波、直方图匹配、像素平均和轻量级U-Net增强，提高了肿瘤区域修复的解剖合理性和视觉保真度。


<details>
  <summary>Details</summary>
Motivation: 传统MRI分析工具主要针对健康解剖结构，在处理大病灶（如肿瘤）时经常失败。图像修复技术旨在合成肿瘤区域的健康脑组织，使通用工具能够可靠应用。

Method: 结合模型集成与高效后处理策略（中值滤波、直方图匹配、像素平均），并通过轻量级U-Net增强阶段进行解剖细化。

Result: 综合评估表明，该管道提高了修复区域的解剖合理性和视觉保真度，比单个基线模型具有更高的准确性和更稳健的结果。
...
<details>
  <summary>Details</summary>
Motivation: 当前人脸识别系统存在两个主要问题：可识别性仅通过软边界约束部分捕获，导致质量表示较弱；特征方向和幅度的梯度重叠导致优化不稳定和泛化能力差。
...
Conclusion: 硬边界策略QCFace有效解决了可识别性与身份特征纠缠的问题，实现了更好的特征表示和泛化能力。
...
### [28] [Spatial457: A Diagnostic Benchmark for 6D Spatial Reasoning of Large Multimodal Models](https://arxiv.org/abs/2502.08636)
...
<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注2D空间理解，缺乏评估6D空间推理能力的框架，需要开发更全面的评估方法。

Method: 创建了包含4种关键能力的合成数据集，采用级联评估结构，构建了7种问题类型和5个难度级别，并引入相对性能下降率(RPDR)指标。

Result: 评估显示模型性能随任务复杂度增加而下降，特别是在3D推理和6D空间任务中表现较差，并发现了预测偏差问题。

Conclusion: 当前大型多模态模型在复杂3D空间推理方面存在显著不足，需要进一步改进空间理解能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决单正多标签学习中只能获得单个正标签标注但样本可能属于多个类别的挑战，现有方法缺乏对不同关系类型的显式几何定义。
...
TL;DR: SVG是一种新型的潜在扩散模型，它不使用变分自编码器，而是利用自监督表示进行视觉生成，通过冻结的DINO特征构建具有清晰语义区分度的特征空间，实现了更高效的扩散训练和更高质量的生成。


<details>
  <summary>Details</summary>
Motivation: 传统的VAE+扩散范式存在训练效率低、推理速度慢、对更广泛视觉任务可迁移性差的问题，这些问题源于VAE潜在空间缺乏清晰的语义分离和强判别结构。
...
Conclusion: SVG为任务通用、高质量的视觉表示提供了一条有原则的路径，展示了语义结构潜在空间对于高效视觉生成的重要性。
...
TL;DR: CoMe是一个渐进式层剪枝框架，通过串联合并技术和分层蒸馏后训练过程，解决大语言模型结构化剪枝中的性能退化问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然擅长自然语言处理任务，但其庞大尺寸导致高计算和存储需求。现有结构化剪枝方法往往忽略保留剪枝部分的模型能力。

Method: 提出通道敏感度度量用于细粒度通道选择，采用串联式层合并方法融合相邻层的关键通道，并设计分层蒸馏协议实现高效知识迁移。

Result: 在七个基准测试中，CoMe达到最先进性能；当剪枝LLaMA-2-7b 30%参数时，剪枝模型保留原始平均准确率的83%。

Conclusion: CoMe通过渐进式剪枝和分层蒸馏有效解决了结构化剪枝中的关键限制，在保持模型性能的同时显著减少模型尺寸。
...
Conclusion: Proto-Former通过原型学习成功解决了多数据集面部关键点检测的统一建模问题，为开发通用面部关键点检测模型提供了有效解决方案。
...
TL;DR: SHARE是一种利用场景几何空间线索来精确定位人体运动重建的技术，仅需单目RGB视频即可准确地将人体放置在3D空间中。


<details>
  <summary>Details</summary>
Motivation: 当前的人体运动重建方法在准确将人体放置在3D空间中存在困难，这对于游戏、AR/VR和机器人等自主代理的真实角色交互至关重要。

Method: SHARE首先估计每帧的人体网格和分割掩码，以及关键帧的场景点图。通过比较人体网格与使用掩码从场景中提取的人体点图，迭代优化关键帧中的人体位置。在优化过程中保持非关键帧人体网格相对于关键帧根关节位置的一致性。

Result: 广泛实验表明，SHARE在准确重建周围场景的同时实现了更精确的3D人体定位，在精选数据集和野外网络视频上都表现出色。

Conclusion: SHARE方法优于现有方法，能够更准确地重建3D人体运动并定位在场景中。
...
TL;DR: 提出了Cortical-SSM架构，通过深度状态空间模型捕捉脑电信号的时空频域依赖关系，在运动想象分类任务中优于基线方法。
...
Result: 在三个基准测试中（两个大规模公开EEG数据集和一个临床ECoG数据集）均优于基线方法，并能有效识别神经生理相关区域。

Conclusion: Cortical-SSM能够有效捕捉脑电信号的跨域依赖，为运动想象分类提供了更优的解决方案。
...
TL;DR: 提出一种两阶段自适应微调方法，通过线性探测和渐进冻结来优化预训练CNN模型在手术工具检测任务中的性能，在Cholec80数据集上达到96.4%的mAP。
...
Result: 在Cholec80数据集上验证，使用ImageNet预训练的ResNet-50和DenseNet-121架构，检测性能优于现有方法和传统微调技术，mAP达到96.4%。在CATARACTS数据集上的进一步验证表明该方法具有良好的泛化能力。

Conclusion: 渐进冻结微调是提高多样化手术程序中工具存在检测性能的有前景技术，在通用图像分类任务中具有更广泛的应用潜力。
...
TL;DR: 提出FreqPDE方法，通过频率感知的空间金字塔编码器、跨视图尺度不变深度预测器和位置深度编码器，为2D图像特征注入空间信息，用于3D检测Transformer解码器。


<details>
  <summary>Details</summary>
Motivation: 当前多视图2D图像3D检测方法依赖深度预测恢复空间信息，但存在深度不连续、小物体区分困难、跨视图一致性和尺度不变性被忽视等问题。

Method: 使用频率感知空间金字塔编码器结合高低频特征，跨视图尺度不变深度预测器估计像素级深度分布，位置深度编码器生成3D深度感知特征，并采用混合深度监督。

Result: 在nuScenes数据集上的大量实验证明了该方法的有效性和优越性。

Conclusion: FreqPDE方法能够有效解决现有3D检测方法中的深度预测质量问题，提升检测性能。
...
TL;DR: PFGS是一种姿态感知的3D高斯泼溅框架，通过多姿态图像融合解决单姿态重建不完整的问题，结合全局和局部配准策略，利用基础模型提升配准效果，实现更完整和高质量的3D重建。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅方法通常假设物体在单一静态姿态下捕获，导致重建结果不完整，遗漏被遮挡或自遮挡区域。需要解决从多姿态图像中重建完整物体的实际挑战。

Method: 给定物体在一个主姿态和多个辅助姿态下的图像，PFGS迭代地将每个辅助姿态集融合到主姿态的统一3DGS表示中。采用姿态感知融合策略，结合全局和局部配准，并智能整合基础模型进行配准过程。

Result: 实验结果表明，PFGS在定性和定量评估中始终优于强基线方法，产生更完整的重建结果和更高保真度的3DGS模型。

Conclusion: PFGS通过多姿态融合和智能配准策略，有效解决了单姿态重建不完整的问题，为3D高斯泼溅提供了更完整和高质量的重建解决方案。
...
TL;DR: LILAC是一个实时运动风格化框架，通过流式VAE-扩散架构和因果解码实现长序列的实时任意风格化，解决了现有方法在实时性方面的局限性。
...
Method: 基于高性能离线框架，通过潜在空间流式架构、滑动窗口因果设计和注入解码运动特征来确保平滑过渡，无需依赖未来帧或修改扩散模型架构。

Result: 在基准数据集上的实验表明，该方法在风格化质量和响应性之间取得了良好平衡，能够实现长序列实时任意风格化。

Conclusion: LILAC成功将离线运动风格化框架扩展到在线设置，为需要连续响应角色控制的应用提供了有效的解决方案。
...
TL;DR: 提出了MARIS基准和统一框架，解决水下开放词汇实例分割中的视觉退化和语义错位问题，通过几何先验增强和语义对齐注入机制提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有水下实例分割方法受限于封闭词汇预测，无法识别新的海洋类别，且水下场景存在严重的视觉退化（如颜色衰减）和语义错位问题。

Method: 提出统一框架包含两个互补组件：几何先验增强模块（GPEM）利用稳定的部件级和结构线索保持对象一致性；语义对齐注入机制（SAIM）用领域特定先验丰富语言嵌入，缓解语义模糊。

Result: 实验表明该框架在MARIS基准上一致优于现有开放词汇基线，在域内和跨域设置中均表现出色。
...
TL;DR: LoSP-Prompt是一个用于多器官多切片多方向多b值扩散加权MRI的重建框架，通过物理建模和合成数据驱动的提示学习解决运动引起的相位伪影问题，实现了高分辨率成像。


<details>
  <summary>Details</summary>
Motivation: 多器官多切片多方向多b值的复杂性，加上呼吸、蠕动等生理运动引起的严重相位伪影，限制了多器官扩散加权MRI在临床肿瘤诊断中的应用。

Method: 将帧间相位变化建模为高阶局部平滑相位(LoSP)，集成到低秩Hankel矩阵重建中，通过仅使用模拟生理运动的合成腹部DWI数据进行提示学习来自动设置算法秩参数。

Result: 在10,000+临床图像验证中：实现临床单次DWI两倍空间分辨率；单一模型泛化到7个解剖区域；在图像质量、伪影抑制和噪声降低方面优于最先进方法，获得放射科医生4-5分评价。

Conclusion: 该方法消除了导航信号和真实数据监督需求，为高分辨率多器官多器官DWI提供了可解释、鲁棒的解决方案，其扫描仪无关性能在精准肿瘤学中具有变革潜力。
...
<details>
  <summary>Details</summary>
Motivation: 尽管进行了广泛的对齐努力，大型视觉语言模型仍然容易受到越狱攻击，存在严重的安全风险。现有检测方法要么学习攻击特定参数，限制了泛化能力，要么依赖启发式原则，限制了准确性和效率。

Method: 提出了Learning to Detect (LoD)框架，包括多模态安全概念激活向量模块用于安全导向的表示学习，以及安全模式自动编码器模块用于无监督攻击分类。

Result: 大量实验表明，该方法在多样未知攻击上实现了持续更高的检测AUROC，同时提高了效率。

Conclusion: LoD框架通过将重点从攻击特定学习转向任务特定学习，能够准确检测未知越狱攻击，解决了现有方法的泛化和效率限制问题。
...
TL;DR: 提出Semantic4Safety框架，使用零样本语义分割从街景图像中提取11个可解释的街道指标，结合道路类型分析3万起交通事故记录，通过XGBoost分类器和SHAP解释特征贡献，应用GPS加权和ATE估计量化因果效应。
...
Method: 使用零样本语义分割从街景图像中提取11个可解释的街道指标，结合道路类型信息，训练XGBoost多类分类器，使用SHAP解释特征贡献，应用GPS加权和ATE估计控制混杂因素并量化因果效应。

Result: 发现异质化、事故类型特定的因果模式：捕捉场景复杂性、暴露度和道路几何的特征主导预测能力；更大的可驾驶区域和应急空间降低风险，而过度视觉开放性可能增加风险。
...
TL;DR: 提出了预测-校正（PC）范式来加速深度学习，通过解耦建模任务解决端到端方法收敛慢和数据依赖性强的问题。基于此构建了PCMambaNet网络，在脑MRI分割任务中仅需1-5个epoch即可达到最优精度。


<details>
  <summary>Details</summary>
Motivation: 端到端深度学习方法存在收敛速度慢、严重依赖大规模数据集的问题，限制了其在医学影像等数据稀缺领域的应用效率。

Method: PCMambaNet包含两个协同模块：预测先验模块（PPM）利用解剖知识（双侧对称性）生成低成本的粗略近似，定位诊断相关的非对称区域；校正残差网络（CRN）学习建模残差误差，专注于精炼这些挑战性区域和描绘精确的病理边界。

Result: 在高分辨率脑MRI分割实验中，PCMambaNet实现了最先进的精度，且仅需1-5个epoch即可收敛，这是传统端到端模型无法达到的性能。

Conclusion: 通过显式融入领域知识来简化学习目标，PCMambaNet有效缓解了数据效率低下和过拟合问题，证明了预测-校正范式在加速学习方面的有效性。
...
TL;DR: 提出证据优先的自适应框架EARL，通过强化学习让模型主动选择最相关帧并进行局部重采样，在长视频推理任务中取得SOTA性能。
...
Method: 基于"选择更少，推理更多"理念，提出证据感知强化学习框架，动态选择最相关帧并在关键帧周围进行局部重采样以获取细粒度时间细节。

Result: 在五个视频推理基准测试中达到开源Video LLMs的新SOTA，7B模型在LongVideoBench上达到59.8%，MVBench上69.0%，VideoMME上64.9%。

Conclusion: 证据纯度优先策略的重要性得到验证，提出的框架在长视频推理中表现出色。
...
Conclusion: MAVR-Net通过多视角融合和跨视角注意力机制，有效提升了MAV动作识别的性能，为自主空中蜂群的协同感知和控制提供了可靠解决方案。
...
TL;DR: DPTrack是一种基于提示学习的夜间航拍跟踪器，通过将目标属性特征编码到包含细粒度线索的方向性核中，生成精确的提示来改进夜间跟踪性能。


<details>
  <summary>Details</summary>
Motivation: 现有的夜间航拍跟踪器仅依赖空间定位监督，无法提供指向目标特征的细粒度线索，导致提示模糊，跟踪性能不佳。

Method: DPTrack首先分层捕获目标的拓扑结构，利用拓扑属性丰富特征表示；然后编码器将这些拓扑感知特征压缩到方向性核中；最后基于通道-类别对应关系的核引导提示模块在搜索区域特征中传播核，识别目标特征位置并转换为精确提示。

Result: 在已建立的基准测试上进行广泛评估，证明了DPTrack的优越性能。

Conclusion: DPTrack通过编码细粒度属性线索到方向性核中，有效提升了夜间航拍跟踪的准确性和鲁棒性。
...
TL;DR: 提出了一种基于动态图像的双相位时间增强方法，将微表情序列分解为起始到顶点和顶点到结束两个运动阶段，分别生成动态图像，以丰富运动多样性并提升识别性能。


<details>
  <summary>Details</summary>
Motivation: 微表情识别面临标注数据稀缺的挑战，现有方法主要依赖简单的空间增强而忽视了时间增强策略，限制了运动模式的多样性捕捉。

Method: 将每个表情序列分解为起始到顶点和顶点到结束两个运动阶段，为每个阶段生成独立的动态图像，形成双相位动态图像增强策略。

Result: 在CASME-II和SAMM数据集上的实验表明，该方法在识别准确率、未加权F1分数和未加权平均召回率方面均有提升，与空间增强结合时相对改进高达10%。
...
TL;DR: MRASfM是一个专为驾驶场景设计的多相机运动恢复结构框架，通过利用多相机系统的固定空间关系提高相机姿态估计可靠性，使用平面模型去除道路表面错误点，并将多相机系统作为单一单元进行光束法平差以提高效率。


<details>
  <summary>Details</summary>
Motivation: 传统SfM方法在应用于多相机系统捕获的驾驶场景时面临困难，包括姿态估计不可靠、道路表面重建存在大量异常点以及重建效率低下。

Method: 利用多相机系统的固定空间关系增强相机姿态估计可靠性；使用平面模型去除道路表面重建中的错误点；将多相机系统作为单一单元进行光束法平差以减少优化变量；通过场景关联和组装模块实现多场景聚合。

Result: 在nuScenes数据集上实现了0.124的绝对姿态误差，表现出最先进的性能；在实际车辆部署中验证了MRASfM在不同场景下的泛化能力和在挑战性条件下的鲁棒性。

Conclusion: MRASfM框架有效解决了驾驶场景中多相机SfM的关键问题，在姿态估计精度和重建质量方面都取得了显著提升，具有实际应用价值。
...
TL;DR: 提出了首个针对无人机视频-文本检索任务的系统方法MSAM，通过多语义自适应挖掘机制解决无人机视频的独特挑战，在自建数据集上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 随着无人机技术的发展，视频数据量急剧增加，迫切需要高效的语义检索方法。无人机视频具有俯视视角、结构同质性强和目标组合语义表达多样等特点，现有跨模态方法难以有效建模这些特性。

Method: 提出多语义自适应挖掘(MSAM)方法，包含多语义自适应学习机制、跨模态交互特征融合池化机制，通过动态帧间变化和特定场景区域语义信息提取，增强对无人机视频内容的深度理解和推理。

Result: 在两个自建的无人机视频-文本数据集上的大量实验表明，MSAM在无人机视频-文本检索任务中优于其他现有方法。

Conclusion: MSAM方法通过专门针对无人机场景设计的检索机制，有效解决了无人机视频的独特挑战，为无人机视频语义检索提供了有效的解决方案。
...
TL;DR: 该研究提出了一种结合光学流（COF）方法，整合了面部微表情的起始到顶点和顶点到结束两个阶段，以提升微表情识别性能。
...
Method: 引入结合光学流（COF），整合微表情的起始到顶点和顶点到结束两个阶段，提供更全面的运动分析。

Result: 在CASMEII和SAMM数据集上的实验结果表明，COF优于单一光学流方法，能有效捕捉微表情动态。

Conclusion: 结合光学流方法通过整合微表情的完整时间动态，显著提升了微表情识别性能。
...
TL;DR: 提出一个用于农业植物3D重建的自动化流水线，通过无人机自主采集图像，使用迭代变形方法减少叶片运动影响，生成高质量3D模型
...
Method: 使用小型商用无人机自主采集图像，通过迭代方法逐步调整输入图像，利用光流估计运动，减少场景运动影响，生成规范表示

Result: 流水线改进了现有最先进方法的重建效果，能够提取高分辨率3D网格，并提供了包含多种作物在不同时间点采集的数据集

Conclusion: 该流水线能够有效应对植物3D重建中的运动挑战，生成高质量重建结果，并将公开源代码和相关数据集
...
TL;DR: 提出HiMA架构，结合Transformer和Mamba模块处理不同尺度特征，通过LoDA解决局部光照不均，使用MPF融合多先验信息，在低光RAW图像增强任务中实现高效高质量增强。


<details>
  <summary>Details</summary>
Motivation: 解决现有低光RAW图像增强方法在增强质量和效率之间的平衡问题，避免两阶段框架中的模糊性，同时处理局部光照变化不均的挑战。
...
Conclusion: HiMA架构通过有效结合不同模块的优势，在低光RAW图像增强任务中实现了效率与质量的良好平衡。
...
TL;DR: ORCA通过引入可学习的任务提示和视觉提示，利用预训练文本到图像扩散模型为机器人控制提供任务自适应视觉表示，无需微调模型本身，在多个机器人控制基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 预训练视觉表示在模仿学习中通常保持任务无关性，在策略学习期间保持冻结状态。本文探索利用预训练文本到图像扩散模型获得任务自适应视觉表示，但发现直接应用文本条件在控制任务中效果不佳，归因于扩散模型训练数据与机器人控制环境之间的领域差距。

Method: 提出ORCA方法，引入可学习的任务提示以适应控制环境，以及捕获细粒度、帧特定细节的视觉提示。通过新设计的条件促进任务自适应表示。

Result: 该方法在各种机器人控制基准测试中实现了最先进的性能，显著超越了先前的方法。

Conclusion: 通过考虑控制所需的特定动态视觉信息，利用预训练扩散模型的条件化方法可以有效提升机器人控制的视觉表示质量。
...
TL;DR: Shakti VLM是一个1B和4B参数的视觉语言模型家族，通过架构创新在较少训练数据下实现竞争性性能，包括QK归一化、混合归一化技术和改进的位置编码。
...
Method: 采用QK归一化增强注意力稳定性、混合归一化技术、改进的位置编码，以及三阶段训练策略来优化学习效率。

Result: Shakti-VLM-1B和4B在文档理解、视觉推理、OCR提取和通用多模态推理方面表现出色。

Conclusion: 通过模型设计和训练策略而非单纯增加数据量可以实现高性能，使Shakti成为企业级多模态任务的高效解决方案。
...
TL;DR: 提出Latent Feature Alignment (LFA)算法，无需属性标签即可识别面部识别模型中的偏差子群体，通过潜在方向实现语义一致的分组和可解释方向发现。


<details>
  <summary>Details</summary>
Motivation: 现代人脸识别模型虽然整体准确率高，但仍存在系统性偏差，传统偏差评估框架依赖昂贵的标注属性和预定义类别。

Method: 使用潜在方向识别子群体的无属性标签算法，通过潜在特征对齐实现语义一致的分组和可解释方向发现。

Result: 在四种最先进识别模型和两个基准测试中，LFA在组内语义一致性上始终优于k-means和最近邻搜索，并发现与人口统计和上下文属性对齐的可解释潜在方向。
...
TL;DR: 该研究提出了一种用于卫星土地利用分类的自定义卷积神经网络架构，在EuroSAT数据集上达到97.23%的测试准确率，无需预训练模型。主要贡献是平衡多任务注意力机制，结合空间和光谱特征提取。
...
Method: 通过三个渐进式架构迭代：基线架构、CBAM增强架构和平衡多任务注意力架构。使用Coordinate Attention进行空间特征提取，Squeeze-Excitation块进行光谱特征提取，通过可学习融合参数统一。采用渐进DropBlock正则化和类别平衡损失加权。

Result: 最终12层架构达到97.23%测试准确率，Cohen's Kappa为0.9692，所有类别准确率超过94.46%。可学习参数自主收敛到alpha≈0.57，表明空间和光谱模态重要性接近相等。

Conclusion: 系统化架构设计方法在卫星图像分类中有效，性能接近微调ResNet-50（仅差1.34%），且无需外部数据，验证了领域特定应用架构设计的价值。
...
<details>
  <summary>Details</summary>
Motivation: FDG-PET在痴呆症诊断中效果显著，但相比常规MRI，其可及性差且成本高。研究旨在开发一种经济有效的替代方案。

Method: 使用3D扩散桥框架学习从MRI和患者信息到FDG-PET的概率映射，生成模拟PET图像。
...
<details>
  <summary>Details</summary>
Motivation: 解决历史档案文档分析中结构化内容出现在退化、非标准形式中的挑战，包括运动模糊、手写变化、曝光波动和杂乱背景等问题。

Method: 从127个二战时期档案视频片段中提取包含拍板的生产元数据，提供完整帧标注和裁剪单词图像，支持零样本和微调条件下的识别与检测模型评估。
...
<details>
  <summary>Details</summary>
Motivation: 传统优化方法依赖繁琐的手动规则，深度生成模型难以产生丰富多样的内容，而基于大语言模型的方法缺乏鲁棒性且无法准确捕捉复杂空间关系。
...
Method: 集成三个专用编码器：全局视角编码器检测边界不一致性，中观视角编码器分析纹理和颜色对齐，局部视角编码器捕捉眼睛、鼻子和嘴巴等表达性面部区域的失真。还加入了面部朝向编码器来分类面部姿态。

Result: 在具有挑战性的数据集上的实验结果表明，该方法优于传统的单视角方法，在具有挑战性的姿态和光照条件下也能有效检测被操纵的图像。

Conclusion: 多视角架构通过融合不同层次的面部特征，显著提升了DeepFake检测性能，特别是在复杂真实世界条件下的鲁棒性。
...
TL;DR: 提出轻量级CycleGAN用于荧光显微镜模态转换（共聚焦到STED/去卷积STED），大幅减少参数至约9000个，同时引入GAN作为实验和标记质量的诊断工具。


<details>
  <summary>Details</summary>
Motivation: 轻量深度学习模型能显著降低计算成本和环境影响，解决荧光显微镜中常见的未配对数据集问题，并为实验质量验证提供工具。

Method: 用固定通道方法替代U-Net生成器中的传统通道倍增策略，大幅减少可训练参数；将GAN用作诊断工具，通过比较生成输出与实验图像来识别成像问题。
...
TL;DR: 提出了两种标准化方法来改进时空图像融合：基于传统上采样的方法和基于锐化的ABSIS方法，后者能显著提升融合图像的精度。
...
Method: 比较了两种标准化方法：1）传统上采样方法；2）ABSIS锐化方法，该方法将高分辨率图像的整体特征与特定低分辨率图像的独特属性相结合。

Result: 两种方法都显著提高了USTFIP时空融合方法的准确性，其中锐化方法使融合图像的光谱和空间精度分别提高了49.46%和78.40%。

Conclusion: 提出的标准化方法，特别是ABSIS锐化方法，能够有效提升时空图像融合的精度，为实际应用提供了更好的解决方案。
...
TL;DR: 提出FlexiReID框架，支持RGB、红外、素描和文本四种模态间的七种检索模式，通过自适应专家混合机制和跨模态查询融合模块实现灵活的多模态行人重识别。
...
Method: 引入自适应专家混合机制动态整合不同模态特征，使用跨模态查询融合模块增强多模态特征提取，构建CIRS-PEDES统一数据集进行综合评估。

Result: 在广泛实验中表现出最先进的性能，在复杂场景中具有强大的泛化能力。

Conclusion: FlexiReID框架成功解决了多模态行人重识别中的灵活性问题，为实际应用提供了有效解决方案。
...
Result: 相比现有方法获得10倍速度提升，在复杂纹理上检测精度提高，与现有方法相比表现优异

Conclusion: QFCA方法实现了实时纹理异常检测，在速度和精度之间取得了良好平衡，适用于实际部署
...
<details>
  <summary>Details</summary>
Motivation: 解决当前自监督去噪技术在真实应用中计算和内存需求高的问题，需要在推理速度和重建质量之间做出妥协。
...
Conclusion: 该模型结合了高效性、低计算成本和无需数据的优势，是生物医学成像中有价值的工具，能够克服干净训练数据稀缺的挑战，同时实现快速推理以供实际使用。
...
TL;DR: 这是一篇关于遥感领域深度域适应的综述论文，系统性地回顾了该领域的重要进展、方法分类、数据集和性能评估，并指出了未来的研究方向。


<details>
  <summary>Details</summary>
Motivation: 遥感中的域适应任务面临数据差异（如采样距离、传感器模式、地理景观等）带来的挑战，深度学习在特征表示和跨域知识转移方面显示出强大潜力，需要系统性的综述来指导该领域发展。

Method: 采用系统性综述方法，首先介绍基础知识，然后从任务分类、输入模式、监督范式和算法粒度等多个角度组织现有算法，并回顾常用数据集和最先进方法的性能。

Result: 提供了遥感域适应领域的全面概述，包括系统化的分类框架、现有方法的性能总结，以及当前研究进展的整体评估。

Conclusion: 与以往综述相比，本文覆盖了更广泛的遥感域适应任务，提供了更系统化的分类方法，能够为研究社区提供启发、促进理解并指导未来工作。
...
TL;DR: 提出一种弱监督医学图像分割框架，仅需四个极值点标注，通过SAM2生成初始伪标签，结合增强的FGEPM算法和不确定性估计进行渐进式优化，在超声图像分割中达到与全监督方法相当甚至更好的性能。
...
Result: 在两个公共超声数据集BUSI和UNS上的实验表明，该方法性能与全监督方法相当甚至更优，同时显著降低了标注成本。

Conclusion: 提出的弱监督框架在超声图像分割中具有有效性和实用性，为减少医学图像标注负担提供了可行方案。
...
TL;DR: 提出了一个用于检测行人意图的多模态数据集，包含鱼眼相机、激光雷达、超声波传感器和运动捕捉数据，支持行人检测、3D姿态估计和轨迹预测算法的开发与评估。


<details>
  <summary>Details</summary>
Motivation: 解决智能车辆在近场场景中感知行人意图的挑战，应对传感器遮挡、动态环境和硬件限制等实际问题。

Method: 收集同步多模态数据，包括鱼眼相机、激光雷达、超声波传感器和运动捕捉的3D身体姿态，并提供详细的3D身体关节位置标注和精确的行人3D位置数据。

Result: 发布了部分数据集和基准测试套件，包含精度、效率和嵌入式系统可扩展性的评估指标，并提供了基于自定义神经网络架构的基线性能指标。

Conclusion: 该数据集为研究人员提供了一个独特资源，用于开发和评估最先进的行人检测、3D姿态估计和意图预测算法，推动智能车辆在近场场景中的能力提升。
...
TL;DR: OmniVinci是一个开源的多模态大语言模型，通过创新的架构设计和数据管理，在跨模态理解、音频和视觉任务上表现优异，同时训练数据量大幅减少。


<details>
  <summary>Details</summary>
Motivation: 推进机器智能需要开发像人类一样感知多模态信息的能力，因此需要构建强大的开源多模态大语言模型。

Method: 提出了三个关键创新：OmniAlignNet增强视觉和音频嵌入在多模态潜在空间中的对齐；时间嵌入分组捕捉视觉和音频信号的相对时间对齐；约束旋转时间嵌入在多模态嵌入中编码绝对时间信息。同时开发了生成2400万单模态和多模态对话的数据管理流程。
...
TL;DR: 提出了一种基于多模态视觉Transformer自编码器(MViT-AE)的无监督异常检测方法，用于脑肿瘤分割，在仅使用健康脑部MRI训练的情况下，通过重建误差图检测和定位肿瘤。


<details>
  <summary>Details</summary>
Motivation: 解决脑肿瘤分割中标注数据有限、成本高或不一致的问题，提供一种不依赖人工标签的可扩展解决方案。

Method: 使用多模态视觉Transformer自编码器，仅用健康脑部MRI训练；采用多模态早期-晚期融合策略整合不同MRI序列信息；结合Segment Anything Model(SAM)进行后处理优化肿瘤轮廓。
...
<details>
  <summary>Details</summary>
Motivation: 现有医学AI系统破坏了统一的诊断流程：图像理解模型无法生成视觉输出，而图像生成模型无法提供文本解释，导致数据表示、特征集成和多模态能力方面的差距。

Method: 采用观察-知识-分析(OKA)范式：观察层构建UniMed-5M数据集；知识层提出渐进式课程学习；分析层开发UniMedVL统一多模态模型。

Result: UniMedVL在五个医学图像理解基准测试中表现优异，在八种医学成像模态的生成质量上与专用模型相当，且双向知识共享使生成任务增强了视觉理解特征。

Conclusion: 将传统分离的能力整合到单一医学框架中，可在各种医学视觉语言任务中实现改进，证明了统一架构的价值。
...
TL;DR: 提出DGME-T方法，通过注入方向性网格运动编码来增强Video Swin Transformer，显著提升在档案影片和现代视频上的相机运动分类性能
...
Method: 构建统一基准数据集，并在Video Swin Transformer基础上引入方向性网格运动编码，通过可学习的归一化后期融合层注入光学流信息

Result: 在现代视频上准确率从81.78%提升至86.14%，宏F1从82.08%提升至87.81%；在二战档案影片上准确率从83.43%提升至84.62%，宏F1从81.72%提升至82.63%

Conclusion: 结构化运动先验和Transformer表示具有互补性，即使小规模精心校准的运动头也能显著增强在退化影片分析中的鲁棒性
...
TL;DR: Ditto框架通过创新的数据生成管道、高效模型架构和智能代理系统，构建了包含100万个高质量视频编辑样本的Ditto-1M数据集，训练出的Editto模型在指令视频编辑任务上达到新的最先进水平。


<details>
  <summary>Details</summary>
Motivation: 解决指令视频编辑领域因缺乏大规模高质量训练数据而发展受限的根本挑战。

Method: 1) 融合领先图像编辑器和上下文视频生成器的数据生成管道；2) 采用高效蒸馏模型架构和时序增强器降低计算成本；3) 智能代理系统生成多样化指令并严格质量控制；4) 课程学习策略训练模型。

Result: 投入超过12,000 GPU天构建了包含100万个高质量视频编辑样本的Ditto-1M数据集，训练出的Editto模型展现出卓越的指令跟随能力。

Conclusion: Ditto框架成功解决了指令视频编辑的数据稀缺问题，建立的Editto模型在该任务上达到了新的最先进水平。
...
Result: 在多个基准数据集上实现了最先进的性能，并提出了包含丰富元信息标注的新大规模海报数据集GenPoster-100K。

Conclusion: SEGA方法通过分层推理和融入设计原则，有效解决了复杂布局规划问题，在内容感知布局生成任务中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有的检测方法主要针对显式色情内容，难以识别隐式性暗示提示，而微调方法会降低模型生成质量，需要解决这种权衡问题

Method: 利用早期预测噪声的可分离性开发噪声检测方法，并提出噪声增强的自适应负引导机制来优化初始噪声

Result: 在自然和对抗数据集上验证，NDM优于现有SOTA方法如SLD、UCE和RECE等
...
TL;DR: 提出一种基于超像素的正则化方法，用于在粗标注数据上训练语义分割模型，显著改善边界对齐效果


<details>
  <summary>Details</summary>
Motivation: 当获取精细像素级标注困难或昂贵时，可以使用粗标注（如边界区域未标注）来训练分割模型，但这种方法在优化类别边界对齐方面存在困难

Method: 针对编码器-解码器架构，提出基于超像素的上采样正则化方法，强制解码图像中的分割像素符合SLIC超像素（基于颜色和位置，独立于分割标注）

Result: 在FCN-16架构上应用于SUIM、Cityscapes和PanNuke数据集，相比最先进模型，在粗标注训练时边界召回率显著提升

Conclusion: 该方法能有效利用粗标注数据改善语义分割的边界对齐性能
...
TL;DR: QSilk是一个轻量级的稳定层，用于潜在扩散模型，通过微钳位和自适应分位数剪裁技术提高高频保真度并抑制罕见激活峰值，无需训练即可集成到渲染管线中。


<details>
  <summary>Details</summary>
Motivation: 解决潜在扩散模型中高分辨率渲染时的高频细节损失和罕见激活峰值问题，提升图像质量同时保持轻量化和易用性。

Method: 结合两种技术：(1) 每样本微钳位，温和限制极端值而不损失纹理；(2) 自适应分位数剪裁(AQClip)，根据局部结构统计或注意力熵自适应调整允许值范围。

Result: 在低步数和高分辨率下获得更清晰、更锐利的结果，与CFG/Rescale有协同效应，能使用更高指导强度而不产生伪影。

Conclusion: QSilk是一个无需训练的高效稳定层，能显著改善SD/SDXL模型的图像质量，计算开销可忽略不计。
...
TL;DR: 提出轻量级解耦概念瓶颈模型(LDCBM)，通过自动将视觉特征分组为语义组件，无需区域标注，改善了概念与视觉模式的对齐，提升了概念和分类准确性。


<details>
  <summary>Details</summary>
Motivation: 现有概念瓶颈模型存在输入到概念映射偏差和有限可控性问题，限制了实际价值，直接损害了基于概念方法的责任策略。

Method: 引入过滤分组损失和联合概念监督，自动将视觉特征分组为语义上有意义的组件，无需区域标注。

Result: 在三个不同数据集上的实验表明，LDCBM实现了更高的概念和类别准确性，在可解释性和分类性能方面都优于先前的方法。
...
TL;DR: 提出了一种通过交互式替换生成网络激活函数来控制模型输出的系统，让用户能够通过参数化激活函数来理解和控制生成过程。
...
Conclusion: 该方法提供了一种替代性的网络输出控制方式，增强了用户对生成模型的理解和控制能力。
...
Method: 在扩散采样过程中引入区域引导修正，使用预训练感知模型的反馈来修正错误生成区域；提出区域对齐交叉注意力来加强图像区域与文本提示之间的空间语义对齐。

Result: ReCon显著提高了生成数据的质量和可训练性，在各种数据集、骨干网络和数据规模下都实现了持续的性能提升。

Conclusion: ReCon框架通过改进结构可控生成模型的能力，有效解决了数据增强中的语义一致性和图像保真度问题，为训练鲁棒感知模型提供了高质量合成数据。
...
TL;DR: ERNet：一种高效的前馈模型，用于处理噪声和部分点云输入的非刚性变形序列配准，通过两阶段管道预测变形图序列，在准确性和速度上均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决非刚性变形点云序列配准中的两个关键挑战：(i) 由于配准目标的非凸性导致的局部最小值问题，特别是在噪声或部分输入下；(ii) 长序列中的误差累积导致的跟踪失败。

Method: 采用可扩展的数据驱动方法，提出ERNet模型。通过两阶段管道：首先估计帧级粗粒度图节点进行鲁棒初始化，然后在滑动窗口方式下随时间细化节点轨迹。

Result: 在DeformingThings4D和D-FAUST数据集上优于先前最先进方法，相比先前最佳方法实现4倍以上的加速，显著提升效率。
...
Method: VISTA采用多代理系统：1）将用户想法分解为结构化时间计划；2）通过成对锦标赛选择最佳视频；3）由专注于视觉、音频和上下文保真度的专业代理进行评审；4）推理代理综合反馈并重写提示进行下一轮生成。

Result: 在单场景和多场景视频生成实验中，VISTA相比现有方法能持续改进视频质量和用户意图对齐，达到60%的成对胜率，人类评估者在66.4%的比较中更偏好VISTA输出。

Conclusion: VISTA通过迭代自我改进机制有效解决了文本到视频生成中对精确提示的依赖问题，显著提升了生成视频的质量和用户意图对齐度。
...
TL;DR: RelateSeg通过神经符号空间推理在开放词汇语义分割中引入一阶逻辑约束，提升对未见类别物体的分割性能，特别是在多类别图像中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有的基于视觉语言模型的方法缺乏对场景中物体空间关系的理解，导致开放词汇语义分割性能受限。

Method: 提出RelateSeg方法，自动提取空间关系并用一阶逻辑公式编码，同时预测语义类别和空间伪类别，通过模糊逻辑松弛在深度网络中实现端到端学习。
...
TL;DR: 3DPR是一个基于图像的重新照明模型，利用从光舞台捕获的多视图单光源图像学习生成先验，结合预训练生成头部模型的几何先验，实现高质量的人脸重新照明渲染。


<details>
  <summary>Details</summary>
Motivation: 传统图形学方法通过可微分渲染将输入图像分解为几何、材质和光照，但受限于底层模型的假设和近似。需要一种能够处理单目肖像图像重新照明这一欠约束问题的新方法。

Method: 首先将输入肖像嵌入预训练生成头部模型的潜在空间，然后使用基于triplane的反射率网络合成高质量单光源图像，最后根据HDRI环境图组合生成图像实现物理准确的重新照明。

Result: 3DPR在定量和定性评估中优于先前方法，特别是在保持身份特征和捕捉光照效果（如镜面反射、自阴影和次表面散射）方面表现优异。

Conclusion: 该方法通过结合生成先验和光舞台数据，能够从单目肖像图像生成高质量、物理准确的重新照明结果，解决了传统方法的局限性。
...
Method: 使用DINOv3密集特征和FAISS检索构建小型记忆库，通过掩码约束对应关系蒸馏出前景/背景点提示，自动指导SAM2进行分割。

Result: 在600张专家标注图像上测试，Memory-SAM达到mIoU 0.9863，优于FCN(0.8188)和检测器到框SAM基线(0.1839)，在真实场景下表现更佳。
...
<details>
  <summary>Details</summary>
Motivation: 推动原生图像生成技术的前沿发展，探索统一图像生成和编辑的单一架构，并验证架构选择、强化学习、数据质量等关键因素对模型性能的影响。

Method: 采用自回归+扩散混合架构：自回归模型首先生成基于多模态输入的离散图像标记，其隐藏状态作为扩散模型的调节信号来生成高保真图像。

Result: 在多种文本到图像和图像编辑基准测试中，BLIP3o-NEXT实现了优于现有模型的性能表现。

Conclusion: 该研究验证了混合架构的有效性，强调了数据质量和规模的决定性作用，并为图像编辑等挑战性任务提供了改进方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前生物医学视觉语言模型的临床应用受到提示优化技术的限制，这些技术要么产生不可解释的潜在向量，要么产生单一文本提示，缺乏透明度且无法捕捉临床诊断的多方面性质。

Method: 使用进化框架，将大型语言模型作为生物医学知识提取器和自适应优化器，自动生成多样化的可解释自然语言提示对。
...
<details>
  <summary>Details</summary>
Motivation: 镜头眩光严重降低图像质量，影响计算机视觉任务性能。现有SIFR方法在画面外光源不完整或缺失时表现不佳，需要改进。

Method: 采用多任务回归模块和LoRA微调的扩散模型，确保外绘结果真实且物理一致，作为即插即用的预处理解决方案。

Result: 综合实验表明LightsOut在各种挑战性场景下持续提升现有SIFR方法的性能，无需额外重新训练。

Conclusion: LightsOut是一个通用适用的即插即用预处理解决方案，能有效增强现有SIFR方法在复杂场景下的表现。
...
Method: 采用课程驱动的迭代细化策略，结合卫星图像提供粗略几何和扩散模型生成高质量近景外观，渐进式提升几何完整性和逼真纹理。

Result: 相比现有方法，Skyfall-GS在跨视角几何一致性和纹理逼真度方面表现更优，支持实时沉浸式3D探索。

Conclusion: 该框架为大规模3D城市场景创建提供了无需3D标注的有效解决方案，在几何精度和视觉质量方面优于现有方法。
...
TL;DR: 本文提出了三个多标签毒性检测基准Q-A-MLL、R-A-MLL和H-X-MLL，基于15类毒性分类体系，并开发了基于伪标签的毒性检测方法，显著超越了现有先进基线。


<details>
  <summary>Details</summary>
Motivation: 当前毒性检测器主要依赖单标签基准，无法充分捕捉现实世界毒性提示的模糊性和多维度特性，导致有偏评估（漏检和误报），且获取细粒度毒性类别的多标签注释成本过高。

Method: 从公共毒性数据集构建三个多标签基准，采用15类毒性分类体系进行标注；提供理论证明伪标签训练优于单标签监督；开发基于伪标签的毒性检测方法。
...
TL;DR: 本研究评估了生成式AI模型（ChatGPT、Gemini和Deepseek）在评分包含和不包含习语的学生作文时的表现，发现Gemini在与人评分者的一致性方面表现最佳，特别是在处理包含习语的作文时。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI技术的发展，其在自动作文评分系统中的应用潜力日益显现。考虑到AI在处理习语方面的潜在局限性，本研究旨在评估生成式AI模型在评分包含习语和不包含习语的作文时的表现。

Method: 从348篇学生作文中创建了两个相等的作文列表：一个每篇包含多个习语，另一个不含习语。使用三个生成式AI模型（ChatGPT、Gemini和Deepseek）对两个列表中的所有作文进行三次评分，采用与人评分者相同的评分标准。

Result: 所有模型都表现出极好的一致性，但Gemini在与人评分者的评分者间信度方面优于其他模型。AI评估中未检测到对任何人口群体的偏见。对于包含多个习语的作文，Gemini的评分模式与人评分者最为相似。

Conclusion: 研究中的模型展示了混合方法的潜力，但Gemini由于其处理比喻性语言的能力而成为最佳选择，并显示出未来单独处理作文评分任务的前景。
...
Method: 利用大语言模型自动生成和标注基于四种修辞类型（因果、经验、情感、道德）的合成辩论数据，并基于此微调transformer分类器。

Result: 模型在人类标注数据和多个外部语料库上表现出高性能和强泛化能力，发现美国总统辩论中情感论证使用增加的趋势。

Conclusion: 该框架为修辞策略分析提供了可扩展的解决方案，在说服力预测和政治辩论分析中具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现代语言模型部署后通常是静态的，持续学习的主要障碍是灾难性遗忘。研究动机是探索稀疏参数更新是否能实现无灾难性遗忘的学习。

Method: 引入稀疏记忆微调，利用记忆层模型，仅更新被新知识高度激活的记忆槽，减少新知识与模型现有能力之间的干扰。

Result: 在问答任务评估中，稀疏记忆微调在获得相同水平新知识的同时，遗忘率显著降低：NaturalQuestions F1分数在全微调后下降89%，LoRA下降71%，而稀疏记忆微调仅下降11%。
...
TL;DR: 研究发现MLAMA等多语言事实知识评估基准中的模板翻译方法存在语法和语义问题，通过使用整句翻译方法显著提高了知识检索分数。


<details>
  <summary>Details</summary>
Motivation: MLAMA等基准使用的模板翻译方法不考虑命名实体的语法和语义信息，导致最终提示不流畅或措辞错误，特别是在形态丰富的语言中，这影响了分数的可解释性。

Method: 从MLAMA数据集中选取4种斯拉夫语言，比较初始模板数据集与Google Translate和ChatGPT整句翻译版本的知识检索分数。

Result: 观察到知识检索分数显著提高，并在其他5种不同语系语言中也发现了类似模式。
...
TL;DR: 提出了一个端到端的框架，通过无监督聚类和基于提示的标注，利用大语言模型自动从无标签语料库中生成可解释的主题分类法，应用于2024年美国总统选举前的Meta政治广告分析。
...
Method: 结合无监督聚类与基于提示的标注，利用大语言模型迭代构建分类法，无需种子集或领域专业知识，并标注道德框架维度。

Result: 发现投票和移民广告在支出和展示量中占主导，而堕胎和选举诚信获得不成比例的关注度。资金模式高度极化，不同议题的道德框架存在差异，且主题显著性与道德基础密切相关。
...
TL;DR: FarsiMCQGen是一个用于生成波斯语多项选择题的创新方法，结合候选生成、过滤和排序技术，利用Transformer和知识图谱构建可信的干扰项。


<details>
  <summary>Details</summary>
Motivation: 在波斯语等低资源语言中生成高质量的多项选择题具有挑战性，需要开发专门的方法来解决这一问题。

Method: 结合候选生成、过滤和排序技术，使用Transformer和知识图谱与基于规则的方法相结合来生成可信的干扰项。基于维基百科数据构建包含10,289个问题的新波斯语MCQ数据集。

Result: 开发了有效的模型并生成了高质量的数据集，该数据集已通过不同最先进的大型语言模型进行评估。

Conclusion: 该方法展示了在低资源语言中生成高质量多项选择题的有效性，所创建的数据集有望推动MCQ研究的进一步发展。
...
<details>
  <summary>Details</summary>
Motivation: 传统RAG系统处理非结构化文本导致信息密度低、推理效果不佳，需要更高效的结构化知识表示来增强LLM的推理能力。

Method: 使用强化学习学习内容表示策略，动态生成适应多步推理需求的结构化格式，并引入自奖励结构验证机制确保生成结构的正确性和自包含性。

Result: 在7个知识密集型基准测试中，7B规模模型实现竞争性性能，与更大模型性能相当。结构化表示通过提高信息密度和上下文清晰度来增强推理。

Conclusion: Structure-R1框架通过生成任务特定的结构化表示，有效提升了检索增强生成的推理性能，证明了结构化知识表示在增强LLM推理能力方面的重要性。
...
TL;DR: 提出了Partial YaRN和VLAT两种方法来解决大型音频语言模型的长上下文限制问题，前者是无训练的音频专用位置扩展方法，后者是训练时的位置增强策略。
...
Method: Partial YaRN：基于RoPE的无训练音频专用位置扩展方法，只修改音频token位置；VLAT：训练时位置增强策略，模拟不同音频长度进行训练。

Result: 在SALMONN和Qwen2-Audio上的实验表明，Partial YaRN在多种设置下优于原始模型，VLAT训练策略带来显著改进，在未见长度长音频上表现强劲。

Conclusion: Partial YaRN和VLAT方法有效解决了LALMs的长上下文限制，提升了长格式音频理解能力，且能泛化到训练中未见的长音频输入。
...
TL;DR: 研究探索了离散扩散语言模型(DDLMs)与自回归语言模型(ARMs)的混合架构，发现将DDLM规划器与ARM执行器在潜在空间通信可显著提升准确性，同时大幅减少计算成本。


<details>
  <summary>Details</summary>
Motivation: 当前自回归语言模型虽然准确率高但需要长token序列，计算成本高；离散扩散语言模型支持并行生成且在处理复杂推理任务中表现优异，研究旨在探索两者结合是否能获得互补优势。

Method: 首先在文本空间探索协作，一个模型规划推理过程，另一个基于规划执行最终答案；然后扩展到潜在空间通信，引入学习投影器将DDLM潜在表示映射到ARM嵌入空间。

Result: 从文本空间转向潜在空间通信带来显著准确率提升：DART-5从27.0%提升到54.0%，AIME24从0.0%提升到14.0%；混合架构使用64个token进行规划和约5个token执行，性能超越Qwen3.1-7B，而后者使用44倍更多token。

Conclusion: DDLM规划器与ARM执行器的混合架构在潜在空间通信能提供显著计算节省且几乎不影响准确性，为DDLMs在混合架构中的推理应用提供了新见解。
...
TL;DR: 本文对多模态检索增强生成(Multimodal RAG)在文档理解领域进行了系统性综述，提出了基于领域、检索模态和粒度的分类法，并总结了关键数据集、基准和应用，指出了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 当前文档理解方法存在局限性：OCR+LLM流水线会丢失结构细节，而原生多模态LLM在上下文建模方面存在困难。文档的多模态特性需要更先进的范式来支持跨模态的全面检索和推理。

Method: 提出了基于领域、检索模态和粒度的分类法，回顾了涉及图结构和代理框架的进展，系统总结了多模态RAG的关键数据集、基准和应用。

Result: 建立了多模态RAG的系统性框架，为文档AI提供了全面的技术路线图，识别了该领域的关键挑战和发展方向。

Conclusion: 多模态RAG是实现全面文档智能的关键范式，在效率、细粒度表示和鲁棒性方面仍面临挑战，需要未来进一步研究。
...
Method: 提出TraceCoder框架，集成多源外部知识，引入混合注意力机制建模标签、临床上下文和知识之间的交互，提高长尾代码识别能力并使预测可解释。
...
TL;DR: 提出了TACL（阈值自适应课程学习）框架，通过动态调整训练过程，根据样本复杂度进行分层学习，在医疗文本理解任务中显著提升性能


<details>
  <summary>Details</summary>
Motivation: 医疗文本特别是电子病历的非结构化特性、领域特定语言和上下文变异性使得自动理解成为复杂挑战。现有方法将所有数据视为同等难度，忽略了临床记录中复杂性的内在差异，限制了模型在罕见或复杂病例上的泛化能力

Method: TACL框架基于渐进学习原则，动态调整训练过程，将数据按难度分类，在训练早期优先处理简单病例建立坚实基础，然后逐步处理更复杂记录
...
<details>
  <summary>Details</summary>
Motivation: 解决LLM在KGQA中存在的语义鸿沟问题，以及训练无关方法未能充分利用训练数据中宝贵推理模式的问题。
...
Conclusion: EGP框架有效提升了LLM在KGQA中的规划能力，通过示例引导的规划方法弥合了自然语言查询与结构化知识图谱之间的语义鸿沟。
...
TL;DR: 该研究比较了Jaccard系数和余弦相似度在自动作文评分中的表现，发现余弦相似度表现更优，且一元文法比二元和三元文法具有更低的RMSE。


<details>
  <summary>Details</summary>
Motivation: 自动作文评分(AES)旨在提供高效准确的写作评估工具，本研究探讨在向量空间模型中使用不同相似度度量和n-gram表示的效果。

Method: 使用初中公民教育课程的作文数据，通过n-gram模型提取特征并向量化，然后分别计算Jaccard系数和余弦相似度，通过RMSE评估系统性能。

Result: 余弦相似度优于Jaccard系数，一元文法相比二元和三元文法具有更低的均方根误差。

Conclusion: 在自动作文评分中，余弦相似度配合一元文法特征提取能够获得更好的评分效果。
...
<details>
  <summary>Details</summary>
Motivation: 移动设备上的大语言模型虽然可以通过本地数据进行个性化生成，但逐令牌生成过程存在高延迟和硬件利用率低的问题，这限制了智能助手和UI代理等应用的发展。

Method: CoordGen框架包含三个协同组件：自适应执行调度（动态平衡预填充和解码阶段的计算图）、上下文对齐草稿生成（通过轻量级在线校准提高推测效率）和硬件高效草稿扩展（重用和扩展中间序列以提高并行性）。

Result: 在多个智能手机和代表性工作负载上的实验显示，与现有移动推理解决方案相比，生成速度最高提升3.8倍，能效最高提升4.7倍。

Conclusion: CoordGen通过推测解码与动态硬件调度的集成，有效解决了移动设备上LLM推理的内存瓶颈问题，为上下文感知文本生成提供了高效的移动端解决方案。
...
TL;DR: 提出了一个三步评估框架来评估大语言模型在古典诗歌生成和评估中的表现，发现LLM在评估创造性质量时存在"回音室"效应，与人类判断存在偏差。
...
Method: 提出了结合计算指标、LLM作为评判者和人类专家验证的三步评估框架，评估了六个最先进的LLM在诗歌质量多个维度上的表现。

Result: 发现LLM在评估创造性质量时存在系统性偏差，表现出"回音室"效应，其评估标准与人类判断存在分歧。

Conclusion: 当前LLM作为文学生成代理既有潜力也有局限，在文化和技术上复杂的创造性任务中需要人类和模型的混合验证。
...
<details>
  <summary>Details</summary>
Motivation: 自动可读性评估领域因对可读性的定义不一致和依赖表层文本属性而受到阻碍，需要更准确地反映人类对可读性的感知。

Method: 分析了897个人类可读性判断，评估了15个传统可读性指标和6个基于模型的指标在五个英语数据集上的表现。

Result: 四个基于模型的指标在与人类判断的排名相关性中始终位列前四，而表现最佳的传统指标平均排名仅为8.6。

Conclusion: 基于模型的方法是可读性评估更有前景的方向，能够更好地捕捉影响人类可读性感知的信息内容和主题因素。
...
TL;DR: 提出了LayoutRL强化学习框架和Infinity-Parser模型，通过构建Infinity-Doc-400K数据集，在多个基准测试中实现了最先进的文档解析性能。


<details>
  <summary>Details</summary>
Motivation: 现有监督微调方法在跨文档类型泛化能力差，特别是在分布外数据上表现不佳，且高质量布局感知解析训练数据有限。

Method: 使用强化学习框架LayoutRL，通过整合归一化编辑距离、段落计数准确性和阅读顺序保持的复合奖励来优化布局理解，并构建Infinity-Doc-400K数据集训练Infinity-Parser视觉语言模型。

Result: 在OmniDocBench、olmOCR-Bench、PubTabNet和FinTabNet等基准测试中，Infinity-Parser在各种文档类型、语言和结构复杂性上均实现了最先进的性能，显著优于专门的文档解析系统和通用视觉语言模型。

Conclusion: LayoutRL框架和Infinity-Parser模型有效解决了文档解析的泛化问题，将在代码、数据集和模型发布以促进可复现研究。
...
TL;DR: Speech-LLMs在语音不流畅场景下表现严重下降，特别是对帕金森等疾病相关的语音障碍处理能力不足，需要改进音素级处理和长上下文建模来提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有Speech-LLMs评估过于依赖理想化输入，忽略了常见的不流畅语音，特别是与帕金森等疾病相关的语音障碍，需要测试这些模型在实际语音障碍用户场景下的表现。

Method: 提出了VocalBench-DF框架，对22个主流Speech-LLMs在多维度分类法下进行系统性评估，分析性能下降的原因。

Result: 评估显示Speech-LLMs在语音不流畅场景下性能显著下降，识别音素级处理和长上下文建模是主要瓶颈，通过增强组件和管道的识别与推理能力可大幅提升鲁棒性。

Conclusion: 当前Speech-LLMs在实际应用准备度有限，迫切需要新方法来改进不流畅语音处理，构建真正包容的语音大语言模型。
...
TL;DR: 该论文针对游戏广告和推荐系统中的游戏稀疏性和不平衡性问题，提出了用户游戏生命周期(UGL)框架，通过丰富用户行为数据和逆概率掩码策略，有效提升了推荐效果。


<details>
  <summary>Details</summary>
Motivation: 现有推荐系统方法不适合游戏广告和推荐，主要因为游戏稀疏性（游戏数量少）和游戏不平衡性（用户行为集中在少数热门游戏），需要专门解决方案。

Method: 引入用户游戏生命周期(UGL)来丰富用户行为数据，提出两种创新策略提取用户短期和长期兴趣，并采用逆概率掩码策略处理游戏不平衡问题。

Result: 离线实验平均AUC提升1.83%，在线实验游戏广告CVR提升21.67%，游戏内道具推荐AUC提升0.5%，ARPU提升0.82%。

Conclusion: UGL表示学习能显著提升游戏广告和推荐系统的性能，有效解决了游戏稀疏性和不平衡性问题。
...
TL;DR: 该研究提出了一种专门化MedGemma模型的框架，用于生成高质量医学图像描述作为检索增强生成系统的查询，通过知识蒸馏和QLoRA微调方法显著提升了分类性能和描述准确性。
...
Method: 采用知识蒸馏管道创建合成数据集，涵盖皮肤病学、眼底和胸部放射学领域，并使用参数高效的QLoRA方法微调MedGemma模型。

Result: 微调后的模型在分类性能上显著提升，RAGAS评估显示描述忠实度、相关性和正确性均有显著改善。

Conclusion: 建立了一个专门化医学视觉语言模型的强大管道，验证了该模型作为高质量查询生成器的能力，为增强多模态RAG系统在循证临床决策支持中的应用奠定了基础。
...
TL;DR: 该论文提出了GuessBench基准，用于评估多模态大语言模型在信息不完整情况下的主动推理能力，发现现有模型在主动推理方面表现远不如被动推理，并识别了细粒度感知和及时决策等关键挑战。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM评估主要关注被动推理，而现实世界使用中模型需要在信息不完整的情况下主动获取缺失证据。该研究旨在填补这一空白，评估MLLM在主动获取证据和迭代优化决策方面的能力。

Method: 提出GuessBench基准，包含感知导向和知识导向的图像，要求模型在没有任务特定先验的情况下从候选池中选择目标图像来主动获取缺失证据。评估了20个优秀MLLM模型。

Result: 研究发现MLLM在主动推理方面的表现远落后于被动推理设置，表明有巨大的改进空间。感知增强对较小模型有益，而思维导向方法在不同模型规模上都能带来一致的提升。

Conclusion: 细粒度感知和及时决策是主动推理的关键挑战。感知增强和思维导向方法是未来多模态主动推理研究的有前景方向。
...
TL;DR: 提出基于提示工程的可控摘要生成方法，通过多阶段提示框架实现不同抽象级别的摘要生成，研究发现提示长度、数据噪声和文本类型对摘要质量有显著影响。
...
Conclusion: 通过控制提示策略和优化文本预处理可以有效提升大语言模型生成摘要的准确性和可控性。
...
TL;DR: CORE是一个协作框架，结合云端和本地LLMs的优势，在保持移动代理任务准确性的同时减少UI信息暴露。


<details>
  <summary>Details</summary>
Motivation: 云端LLMs需要上传完整UI状态，暴露不必要信息；本地LLMs避免UI上传但能力有限，任务成功率低。需要平衡隐私保护和任务准确性。

Method: 包含三个关键组件：基于XML层次结构的布局感知块分区、本地与云端LLMs协同规划、以及协同决策（本地LLM对相关UI块排序，云端LLM在顶级块中选择具体元素）。

Result: 实验显示CORE能减少高达55.6%的UI暴露，同时任务成功率仅略低于纯云端代理，有效缓解不必要的隐私暴露。
...
TL;DR: DeceptionBench是首个系统评估大语言模型在现实社会场景中欺骗行为的基准测试，涵盖5个领域150个场景，分析内在行为模式和外在因素影响。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型能力的快速提升，其欺骗行为可能在高风险部署中带来严重风险，但目前对现实场景中欺骗行为的系统性研究仍不足。

Method: 构建包含经济、医疗、教育、社交互动和娱乐5个领域的150个场景，超过1000个样本的基准测试，分析内在的利己主义和奉承行为，以及外在的中性条件、奖励激励和强制压力等因素的影响。

Result: 实验发现大语言模型和大推理模型存在关键漏洞，特别是在强化动态下欺骗行为会加剧，表明当前模型缺乏对操纵性上下文线索的鲁棒抵抗能力。

Conclusion: 迫切需要针对各种欺骗行为开发先进的安全保障措施，以应对模型在现实部署中可能带来的风险。
...
TL;DR: 提出了一个评估LLMs时间引用一致性的新基准TEMP-ReCon，并开发了UnTRaP模型来提升LLMs的时间推理能力。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在时间敏感领域（如法律、医疗、金融）的应用增加，需要确保其不仅事实准确，而且在时间维度上保持一致性，但目前缺乏针对时间引用一致性的评估和改进方法。

Method: 引入TEMP-ReCon基准来评估开源和闭源LLMs的时间引用一致性，并提出基于推理路径对齐的UnTRaP模型来增强LLMs的时间一致性。

Result: 实验表明LLMs确实存在时间引用一致性问题，而UnTRaP模型相比多个基线模型在提升时间一致性方面表现更有效。

Conclusion: 填补了LLMs时间一致性评估的空白，提出的基准和模型为提升LLMs在时间敏感应用中的可靠性提供了有效解决方案。
...
Result: 实证结果表明该方法匹配或优于基于动态熵和空格的补丁策略，同时保持紧凑的词汇表。

Conclusion: 该方法提供了高效、灵活且语言无关的表示方案，成功融合了字符级和子词级模型的优势。
...
<details>
  <summary>Details</summary>
Motivation: 显式推理（如思维链）虽然效果好，但计算开销大；现有的潜在推理方法虽然减少了开销，但性能显著下降。研究发现这是由于非结构化的潜在空间导致潜在token难以拟合。

Method: 提出Latent-SFT两阶段框架：第一阶段设计专用注意力掩码指导潜在token编码器生成潜在token；第二阶段丢弃编码器，直接训练LLM自主生成潜在token进行推理，使用KL和CE损失优化。

Result: 在GSM8k上达到新的SOTA，匹配显式SFT性能同时将推理链减少最多4倍；在Math500和AIME24上，基于词汇概率的潜在推理明显优于基于隐藏状态的方法。

Conclusion: 潜在推理既是单一路径的压缩，也是多路径的叠加，有效实现了计算效率与性能的平衡。
...
TL;DR: 提出了一种模态组合感知框架，通过偏好损失和组合正则化目标来缓解统一编码器在对比学习中容易学习模态捷径的问题，从而提高多模态检索在分布偏移下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 尽管统一编码器在多模态检索中具有灵活性，但传统的对比学习训练容易导致模型学习模态捷径，从而在分布偏移下表现不佳。
...
TL;DR: 提出了TokenTiming算法，通过动态时间规整技术解决推测解码中草稿模型和目标模型必须共享词汇表的限制，实现了通用推测解码。


<details>
  <summary>Details</summary>
Motivation: 传统推测解码要求草稿模型和目标模型共享词汇表，这限制了可用草稿模型的选择范围，往往需要从头训练新模型。

Method: 使用动态时间规整算法重新编码草稿标记序列，建立映射关系来传输概率分布进行推测采样，从而支持不匹配的词汇表。

Result: 在各种任务上实现了1.57倍的加速效果，能够与现成模型配合使用而无需重新训练或修改。
...
TL;DR: 本文通过偏差-方差分解重新定义跨语言差距，提出目标语言响应方差是主要原因，并通过实验验证了通过控制方差可以减少跨语言差距。


<details>
  <summary>Details</summary>
Motivation: 现有研究将跨语言差距归因于源语言和目标语言潜在表示的差异，本文提出不同观点，认为目标语言响应的方差是主要原因。

Method: 采用偏差-方差分解形式化跨语言差距，通过多个推理时干预措施控制方差，包括使用简单的提示指令来减少响应方差。

Result: 实验证明通过控制方差可以减少跨语言差距，使用提示指令使目标语言准确率提高了20-25%。

Conclusion: 目标语言响应的方差是跨语言差距的主要来源，通过控制方差可以有效减少这种差距。
...
TL;DR: ParallaxRAG是一个基于知识图谱的检索增强生成框架，通过多视图空间对称解耦查询和图三元组，实现稳健的检索架构，减少幻觉并提升多跳推理能力。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在语言理解方面表现出色，但经常产生幻觉且在多跳推理方面存在困难。现有的基于知识图谱的检索增强生成方法依赖于平面嵌入和噪声路径探索，需要更有效的解决方案。

Method: 将查询和图三元组对称解耦到多视图空间，构建稳健的检索架构，明确强制头部多样性同时约束弱相关路径。利用不同注意力头在不同推理阶段专门处理语义关系的特点。

Result: 在WebQSP和CWQ数据集上的实验表明，在统一的、可复现的设置下（BGE-M3 + Llama3.1-8B），实现了具有竞争力的检索和问答性能，同时减少了幻觉并展现出良好的泛化能力。

Conclusion: 多视图头部专业化是知识基础多跳推理的一个原则性方向，ParallaxRAG通过构建更清晰的子图并引导LLMs进行基于基础的逐步推理，有效提升了多跳推理能力。
...
TL;DR: 提出了KITE基准，专门评估韩语大语言模型的指令跟随能力，填补了韩语在开放指令评估方面的空白。


<details>
  <summary>Details</summary>
Motivation: 当前评估主要关注英语模型，忽视了其他语言的语言文化特性。韩语具有独特的语法、形态特征、敬语系统和双数系统，但缺乏专门的开放指令跟随评估基准。

Method: 开发了KITE基准，包含通用和韩语特定指令，结合自动指标和人工评估，直接针对多样化的开放指令跟随任务。

Result: 评估揭示了不同模型间的性能差异，提供了对其优势和劣势的深入洞察。
...
TL;DR: 本文介绍了EvaCun 2025令牌预测任务的提交系统，基于LLMs（Command-R、Mistral和Aya Expanse）在任务数据上的微调，比较了三种不同提示方法的效果。


<details>
  <summary>Details</summary>
Motivation: 由于对任务领域和语言知识有限，作者希望通过直接使用训练数据而不进行特定调整的方式来参与EvaCun 2025令牌预测任务。

Method: 使用三种不同的LLMs（Command-R、Mistral和Aya Expanse）在任务数据上进行微调，比较了基于三种不同提示的预测方法，并在保留数据上进行评估。

Result: 在保留数据上评估了三种不同提示方法的效果，但具体结果未在摘要中详细说明。

Conclusion: 展示了在有限领域知识情况下，直接使用LLMs微调方法参与令牌预测任务的可行性，并比较了不同提示策略的效果。
...
TL;DR: 通过多义词案例研究分析乌尔都语诗歌中三个看似同义的爱情词汇（pyaar、muhabbat、ishq），揭示其情感谱系和文化细微差别，并通过词嵌入技术进行跨语言比较。


<details>
  <summary>Details</summary>
Motivation: 探索乌尔都语诗歌中爱情表达的复杂性，揭示看似同义词之间的微妙差异，这些差异在英语文学中缺乏直接对应。

Method: 采用多义词案例研究方法，分析词汇在乌尔都语诗歌中的使用和语境；同时生成乌尔都语和英语爱情相关词汇的词嵌入，进行语义空间的可视化比较。

Result: 成功揭示了三个爱情词汇之间的细微情感区别，通过词嵌入技术量化了它们在语义空间中的分布，展现了乌尔都语爱情表达的独特文化特征。

Conclusion: 乌尔都语诗歌通过多义词的巧妙运用，展现了爱情表达的丰富层次和文化独特性，为理解跨语言情感表达提供了新的视角。
...
TL;DR: 提出跨语言双向最大相似度评分(BiMax)用于文档对齐，相比最优传输方法效率提升约100倍，同时保持相当的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有文档对齐方法如最优传输(OT)虽然精度高，但在大规模网络挖掘数据中效率不足，需要同时考虑准确性和速度。

Method: 开发跨语言双向最大相似度评分(BiMax)来计算文档间相似度，相比OT方法更高效。

Result: 在WMT16双语文档对齐任务中，BiMax达到与OT相当的准确性，速度提升约100倍。

Conclusion: BiMax方法在大规模文档对齐任务中实现了效率与准确性的良好平衡，所有对齐方法已公开为EmbDA工具。
...
TL;DR: 提出了一个包含三本完整法语小说的新标注语料库，用于长文档共指消解研究，并开发了模块化共指消解流程，展示了其在文学分析和NLP任务中的应用。


<details>
  <summary>Details</summary>
Motivation: 虽然共指消解在计算文学研究中受到越来越多关注，但完全标注的长文档代表性数据集仍然稀缺，需要解决长复杂文学作品带来的挑战。

Method: 构建了包含三本完整法语小说的标注语料库（超过28.5万词），并开发了模块化共指消解流程，支持细粒度错误分析。

Result: 该方法在长文档上具有竞争力且扩展性好，能够有效推断虚构角色的性别，展示了其在文学分析和下游NLP任务中的实用性。

Conclusion: 该研究填补了长文档共指消解数据集的空白，为文学分析和NLP任务提供了有价值的工具和资源。
...
TL;DR: HypoSpace是一个诊断套件，用于评估语言模型在科学问题中提出多种合理解释的能力，关注假设集的三个指标：有效性、独特性和覆盖率。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型在科学工作流中的应用增加，需要评估它们提出多种合理解释（而非单一正确答案）的能力，因为许多科学问题是欠定的，存在多个机制不同的假设与相同观察结果一致。

Method: HypoSpace将LLMs视为有限假设集的采样器，测量三个互补指标：有效性（与观察一致的提议精度）、独特性（提议间的非冗余性）和覆盖率（对枚举可接受集的覆盖）。在三个结构化领域中实例化：因果图、重力约束3D体素重建和布尔遗传相互作用。

Result: 在指令调优和推理聚焦模型中，有效性通常保持较高，但随着可接受空间的增长，独特性和覆盖率会下降，揭示了仅正确性指标无法检测的模式崩溃现象。

Conclusion: HypoSpace为明确探索和覆盖可接受解释空间的方法提供了一个受控探针，而非排行榜，有助于更全面地评估语言模型在科学推理中的表现。
...
TL;DR: 使用LLM作为动态知识库生成背景上下文，通过四种方法将其整合到仇恨言论检测分类器中，在文本和多模态数据集上分别提升3和6个F1分数。


<details>
  <summary>Details</summary>
Motivation: 利用LLM生成背景上下文来增强仇恨言论检测，特别是针对隐式仇恨言论和多模态仇恨内容。

Method: 提出两种上下文生成策略（命名实体聚焦和全文提示），比较四种上下文整合方法（文本拼接、嵌入拼接、分层transformer融合、LLM驱动文本增强）。

Result: 在文本Latent Hatred数据集和多模态MAMI数据集上，基于嵌入拼接的方法表现最佳，分别比零上下文基线提升3和6个F1分数。
...
TL;DR: 提出了一种检索增强推理模型，通过动态调整检索文档列表长度和使用强化学习训练成本感知模型，在保持效果的同时显著提升效率。
...
Method: 1) 基于查询和检索结果动态调整检索文档列表长度；2) 开发成本感知优势函数用于强化学习训练；3) 为近端和组相对策略优化算法探索内存和延迟限制实现。

Result: 在7个公开问答数据集上评估，模型延迟降低16-20%，同时精确匹配效果平均提升5%。

Conclusion: 提出的成本感知检索增强推理框架能显著提升效率而不损害效果，甚至能同时改善模型性能。
...
TL;DR: 本文实证分析了掩码扩散语言模型(DLMs)中的注意力下沉现象，发现DLMs的注意力下沉具有动态性和鲁棒性，与自回归模型(ARMs)存在根本差异。


<details>
  <summary>Details</summary>
Motivation: 尽管掩码扩散语言模型(DLMs)在效率和性能上表现出色，但其内部工作机制尚未被充分探索，特别是与自回归模型(ARMs)在注意力模式上的差异。

Method: 通过实证分析DLMs的注意力模式，重点关注注意力下沉现象，并与ARMs进行对比研究。

Result: 发现DLMs也存在注意力下沉现象，但具有两个显著特征：1)下沉位置在生成过程中动态移动；2)即使掩码下沉位置，模型性能仅轻微下降，表现出鲁棒性。

Conclusion: 研究揭示了扩散语言模型在注意力分配和利用方面的独特机制，为理解这类模型的内在工作原理提供了新视角。
...
TL;DR: 本文探讨将博弈论原理应用于大语言模型评估，提出自动互评框架，通过自玩和同行评审让LLM相互评估，并与人类投票行为对比验证。
...
Method: 提出自动互评框架，LLM通过自玩和同行评审相互评估，使用博弈论投票算法聚合同行评审，并与人类判断进行系统性比较。
...
TL;DR: 提出一种无需参考翻译的机器翻译质量评估方法，通过段对段翻译和NLP洗牌测试来验证动物语言翻译器，证明在复杂语言中可能无需与动物互动即可评估翻译质量。


<details>
  <summary>Details</summary>
Motivation: 解决如何验证动物语言翻译器的问题，探索是否必须依赖与动物互动或观察，寻求更安全、道德且成本较低的评估方法。

Method: 使用段对段翻译结合NLP洗牌测试，通过比较顺序翻译与随机排列翻译的合理性来评估翻译质量，并在数据稀缺的人类语言和构造语言上进行概念验证实验。

Result: 实验表明该方法与基于参考翻译的标准评估方法高度相关，理论分析表明在翻译学习早期阶段互动可能既不必要也不高效。

Conclusion: 提出了一种无需参考翻译的机器翻译质量评估框架，为动物语言翻译验证提供了可行方案，具有安全、道德和成本优势。
...
TL;DR: 该研究通过一个单层Transformer玩具模型，揭示了语言模型中真假陈述线性分离子空间的产生机制，发现这种分离源于事实陈述的共现模式，并观察到两阶段学习动态。
...
Method: 构建一个透明的单层Transformer玩具模型，在事实陈述共现的数据分布设置下进行端到端训练，观察真值编码的涌现过程，并在预训练语言模型中进行验证实验。

Result: 模型成功再现了真假陈述的线性分离子空间，发现了两阶段学习动态：先快速记忆个体事实关联，然后长时间学习线性分离真假陈述以降低语言建模损失。

Conclusion: 研究提供了线性真值表示在语言模型中涌现的机制性证明和实证动机，揭示了事实共现模式如何促使模型学习真假分离来优化语言建模性能。
...
Method: 提出Paper2Web基准数据集，包含基于规则的指标、人工验证的LLM-as-a-Judge和PaperQuiz知识保留测试；开发PWAgent自主管道，通过MCP工具迭代优化内容和布局。

Result: PWAgent在学术网页生成任务中显著优于端到端基线方法（如基于模板的网页和arXiv/alphaXiv版本），同时保持低成本，实现了帕累托前沿。

Conclusion: Paper2Web为学术网页生成提供了全面的评估框架，PWAgent能够有效生成高质量的交互式学术主页。
...
<details>
  <summary>Details</summary>
Motivation: 解决产品评论和社交媒体中非正式和领域特定语言带来的情感极性强度检测挑战。

Method: 使用VADER进行初始情感估计，通过DistilBERT的置信度分数进行两阶段调整，应用模糊逻辑减少中性偏见，最后通过自定义模糊推理系统生成0-1连续分数。

Result: 在四个领域数据集上评估显示，与用户评分对齐度提高，能更好识别情感极端值，减少误分类，定量和定性分析均证实模型鲁棒性和效率。

Conclusion: 将符号推理与神经模型结合，可在语言动态领域实现可解释的细粒度情感分析。
...
TL;DR: ORBIT框架通过基于量规的增量强化学习，在开放式医学对话任务中显著提升LLM性能，在HealthBench-Hard基准上从7.0提升至27.2。
...
Method: 提出ORBIT框架，结合合成对话生成和动态量规创建，使用量规指导的反馈进行增量强化学习，无需外部医学知识或人工规则。

Result: 在Qwen3-4B-Instruct模型上，仅使用2k样本就将HealthBench-Hard基准性能从7.0提升至27.2，达到同规模模型的最先进水平。

Conclusion: 量规驱动的强化学习能够在复杂开放式任务中实现可扩展的性能提升，为LLM在医学等高风险领域的应用提供了有效策略。
...
<details>
  <summary>Details</summary>
Motivation: 现有技能学习方法创建的技能往往过度专门化于单一网站，缺乏泛化能力。为了解决这个问题，需要开发能够学习可泛化和组合技能的方法。

Method: 受软件工程中多态性启发，PolySkill框架将技能的抽象目标（实现什么）与具体实现（如何执行）解耦，使智能体能够识别和优化自身目标，学习更好的课程。

Result: 实验显示：(1)在已见网站上技能重用率提高1.7倍；(2)在Mind2Web和未见网站上成功率分别提升9.4%和13.9%，同时步骤减少超过20%；(3)在无指定任务的自主探索中，提高了任务质量并学习到跨网站可泛化的技能。
...
### [142] [LLM-based In-situ Thought Exchanges for Critical Paper Reading](https://arxiv.org/abs/2510.15234)
*Xinrui Fang,Anran Xu,Chi-Lan Yang,Ya-Fang Lin,Sylvain Malacria,Koji Yatani*

Main category: cs.HC

TL;DR: 设计了一个支持初级研究人员批判性阅读论文的思维交换界面，通过单代理和多代理条件比较，发现AI代理介导的思维交换能显著提升批判性思维分数


<details>
  <summary>Details</summary>
Motivation: 初级研究人员缺乏能够提供多样化观点的同伴来加强批判性阅读，需要解决这一资源缺口

Method: 基于形成性研究设计思维交换界面，在无代理、单代理和多代理三种条件下评估46名初级研究人员两周的批判性论文阅读效果

Result: AI代理介导的思维交换显著提高了批判性思维分数；单代理条件下参与者更频繁进行反思性标注；多代理条件下参与者更积极参与代理回应

Conclusion: 这项工作有助于理解基于AI的思维交换对批判性论文阅读的支持，并为未来研究提供设计启示
...
### [143] [A Feasibility Study on Usability and Trust among Population Groups of a Medical Avatar Supported by Large Language Models with Retrieval Augmented Generation](https://arxiv.org/abs/2510.15531)
*Roel Boumans,Lisa Cramer,Sascha van de Poll,Henria Vermeulen*

Main category: cs.HC

TL;DR: 研究人员开发了一个使用LLM和RAG技术的虚拟医疗助手，通过视频调查评估其可用性和信任度，发现用户评分均高于平均水平，且不受性别、国家和教育水平影响。


<details>
  <summary>Details</summary>
Motivation: 医疗专业人员时间有限，但患者及其亲属的信息需求很高，需要开发能够提供专业医疗信息的虚拟助手。

Method: 开发了使用大型语言模型和检索增强生成技术的虚拟人形化身，制作了两个医疗交互视频，并对165名参与者进行了问卷调查，评估可用性和信任度。

Result: 患者角色中可用性评分M=4.61，信任度M=3.92；亲属角色中可用性M=4.64，信任度M=4.31，均高于平均分值。性别、国家和教育水平无显著影响。

Conclusion: 基于LLM和RAG技术的虚拟医疗助手在可用性和信任度方面表现良好，具有实际应用潜力，且其接受度不受人口统计学因素影响。
...
### [144] [The Spark Effect: On Engineering Creative Diversity in Multi-Agent AI Systems](https://arxiv.org/abs/2510.15568)
*Alexander Doudkin,Anton Voelker,Friedrich von Borries*

Main category: cs.HC

TL;DR: 论文提出了基于角色条件化LLM代理（Sparks）的方法，通过角色启发的系统提示来多样化多代理工作流程中的行为，以解决创意服务中LLM输出同质化的问题。


<details>
  <summary>Details</summary>
Motivation: 创意服务团队依赖LLM加速构思，但生产系统往往产生同质化输出，无法满足品牌或艺术期望，需要方法来实现输出多样化。

Method: 开发角色条件化LLM代理（Sparks），通过角色启发的系统提示库实例化，在多代理工作流程中实现行为多样化。

Result: 使用LLM作为评判者的协议校准后，角色条件化Spark代理相比统一系统提示平均多样性增益+4.1分（1-10分制），与人类专家的差距缩小到1.0分。

Conclusion: 角色条件化LLM代理能有效提高输出多样性，但需注意评估者偏见和程序性考虑因素。
...
### [145] [Sound Clouds: Exploring ambient intelligence in public spaces to elicit deep human experience of awe, wonder, and beauty](https://arxiv.org/abs/2510.15865)
*Chengzhi Zhang,Dashiel Carrera,Daksh Kapoor,Jasmine Kaur,Jisu Kim,Brian Magerko*

Main category: cs.HC

TL;DR: Sound Clouds是一个沉浸式艺术装置，通过参与者与人类高度球体的互动生成现场音乐，探索如何在公共空间中设计引发敬畏、惊奇和美感的环境人工智能体验。


<details>
  <summary>Details</summary>
Motivation: 探索如何设计和实现能够在公共空间中引发人类深度情感（敬畏、惊奇、美感）的环境人工智能体验，超越传统AmI系统的实用目的。

Method: 开发Sound Clouds沉浸式艺术装置，参与者与多个人类高度球体互动，系统根据互动生成现场音乐。

Result: 成功创建了一个能够通过互动生成音乐的沉浸式艺术装置，展示了环境人工智能在引发情感体验方面的潜力。

Conclusion: 该装置作为对未来环境智能的探索，旨在激发而非限制AmI的潜在可能性，展示了AmI在艺术和情感体验领域的新方向。
...
### [146] [Decentralizing Multi-Agent Reinforcement Learning with Temporal Causal Information](https://arxiv.org/abs/2506.07829)
*Jan Corazza,Hadi Partovi Aria,Hyohun Kim,Daniel Neider,Zhe Xu*

Main category: cs.LG

TL;DR: 本文研究如何在去中心化多智能体强化学习中使用高层符号知识来解决隐私约束、通信限制和性能问题，扩展了局部策略与团队任务兼容性的形式化工具，并证明符号知识能显著加速学习过程。
...
Method: 通过向智能体提供高层符号知识，扩展用于检查局部策略与团队任务兼容性的形式化工具，使去中心化训练在更多场景下具有理论保证。

Result: 实证研究表明，关于环境中事件时间演化的符号知识能够显著加速去中心化多智能体强化学习的学习过程。

Conclusion: 符号知识的引入有效解决了去中心化多智能体强化学习中的独特挑战，提高了策略兼容性检查的能力，并加速了学习收敛。
...
### [147] [Extending Load Forecasting from Zonal Aggregates to Individual Nodes for Transmission System Operators](https://arxiv.org/abs/2510.14983)
...
TL;DR: 开发了一个多级电力负荷预测系统，用于提高区域和节点负荷预测的准确性和可解释性，帮助电网运营商更好地管理可持续能源发展带来的负荷不确定性。
...
Method: 设计多级预测系统，开发可解释且可扩展的预测模型，评估解决节点负荷异质性和波动性的方案，采用完全并行化的单模型预测工作流。

Result: 区域预测的准确性和可解释性得到改善，节点预测有显著提升，运营商能够以前所未有的置信度和准确性调整预测，并精确诊断原本不透明的错误。

Conclusion: 多级预测系统成功解决了电网运营商在管理节点负荷预测时的挑战，实现了准确性和可解释性的双重提升。
...
### [148] [TangledFeatures: Robust Feature Selection in Highly Correlated Spaces](https://arxiv.org/abs/2510.15005)
*Allen Daniel Sunny*
...
Method: TangledFeatures框架识别相关特征组中的代表性特征，减少冗余同时保持解释力。

Result: 在Alanine Dipeptide数据集上应用，成功选择了与结构相关的原子间距离特征，能够解释主链扭转角的变化。

Conclusion: TangledFeatures提供了一种比传统选择技术更可解释和稳定的特征选择方法，适用于相关特征空间。
...
### [149] [ES-C51: Expected Sarsa Based C51 Distributional Reinforcement Learning Algorithm](https://arxiv.org/abs/2510.15006)
*Rijul Tandon,Peter Vamplew,Cameron Foale*

Main category: cs.LG

TL;DR: 提出了一种改进的C51分布强化学习算法ES-C51，用Expected Sarsa更新替代贪婪Q学习更新，通过softmax结合所有动作信息来减少分布不稳定性，在多个环境中表现优于原始C51。


<details>
  <summary>Details</summary>
Motivation: 传统C51算法使用贪婪Q学习更新，当多个动作具有相似期望奖励但不同分布时会导致分布学习不稳定。需要一种更稳定的更新方法来处理这种情况。

Method: 将C51的贪婪Q学习更新替换为Expected Sarsa更新，使用softmax计算结合所有可能动作的信息，而不是仅依赖单个最佳动作。同时将探索策略从ε-greedy改为softmax。

Result: 在Gym经典控制环境和Atari-10游戏中评估，ES-C51在多个环境中表现优于修改后的QL-C51（使用softmax探索的C51）。

Conclusion: 使用Expected Sarsa更新的ES-C51算法能够更稳定地学习奖励分布，在动作具有相似期望奖励时表现更好，实现了更高性能的策略。
...
### [150] [Hybrid Autoencoder-Based Framework for Early Fault Detection in Wind Turbines](https://arxiv.org/abs/2510.15010)
*Rekha R Nair,Tina Babu,Alavikunhu Panthakkan,Balamurugan Balusamy,Wathiq Mansoor*

Main category: cs.LG

TL;DR: 提出了一种基于集成深度学习的无监督异常检测框架，用于风力涡轮机的早期故障检测，结合了VAE、LSTM自编码器和Transformer架构，在真实SCADA数据上实现了高精度的异常检测。


<details>
  <summary>Details</summary>
Motivation: 风力涡轮机可靠性对可再生能源行业至关重要，早期故障检测能显著减少停机时间和维护成本。现有方法通常需要标记的故障数据，限制了在实际应用中的可扩展性。

Method: 集成变分自编码器(VAE)、LSTM自编码器和Transformer架构，通过特征工程提取时域、统计和频域指标，采用集成评分和自适应阈值进行无监督异常检测。

Result: 在包含89年真实涡轮机数据的CARE数据集上评估，AUC-ROC达到0.947，能够提前48小时检测到故障，显著优于现有方法。

Conclusion: 该方法为风力涡轮机提供了有效的预测性维护解决方案，无需标记故障数据，具有重要的社会价值，能够减少涡轮机故障并提高大型风能部署的运营效率。
...
### [151] [AlignFlow: Improving Flow-based Generative Models with Semi-Discrete Optimal Transport](https://arxiv.org/abs/2510.15038)
*Lingkai Kong,Molei Tao,Yang Liu,Bryan Wang,Jinmiao Fu,Chien-Chih Wang,Huidong Liu*

Main category: cs.LG

TL;DR: AlignFlow是一种基于半离散最优传输(SDOT)的流式生成模型训练方法，通过建立噪声分布与数据点之间的显式最优对齐来改善流轨迹的直线性，提高推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于最优传输的方法使用小批量采样来估计传输计划，限制了在大规模高维数据集上的可扩展性。

Method: 利用SDOT将噪声空间划分为Laguerre单元，每个单元映射到对应的数据点，在训练期间通过SDOT映射将独立同分布的噪声样本与数据点配对。

Result: AlignFlow在多种最先进的FGM算法上提升了性能，可扩展性强且计算开销可忽略，可作为即插即用组件使用。

Conclusion: AlignFlow通过SDOT有效解决了现有OT方法在大规模数据集上的可扩展性问题，显著改善了流式生成模型的训练效果。
...
### [152] [IQNN-CS: Interpretable Quantum Neural Network for Credit Scoring](https://arxiv.org/abs/2510.15044)
*Abdul Samad Khan,Nouhaila Innan,Aeysha Khalique,Muhammad Shafique*

Main category: cs.LG

TL;DR: 提出了IQNN-CS框架，这是一个用于多类别信用风险分类的可解释量子神经网络，结合了变分量子神经网络和专门为结构化数据设计的解释技术。
...
Method: 使用变分量子神经网络架构，并引入Inter-Class Attribution Alignment (ICAA)指标来量化不同预测类别间的归因差异，揭示模型如何区分信用风险类别。
...
### [153] [Internalizing World Models via Self-Play Finetuning for Agentic RL](https://arxiv.org/abs/2510.15047)
*Shiqi Chen,Tongyao Zhu,Zian Wang,Jinghan Zhang,Kangrui Wang,Siyang Gao,Teng Xiao,Yee Whye Teh,Junxian He,Manling Li*

Main category: cs.LG

TL;DR: SPA框架通过自监督预训练学习世界模型，然后用于强化学习策略优化，显著提升LLM智能体在分布外场景中的性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型作为智能体在分布外场景中表现不佳，难以将内部知识与复杂动态环境对齐。传统强化学习训练在这种条件下效果有限，探索脆弱且泛化能力差。

Method: 将世界模型分解为状态表示和转移建模两个组件，通过自监督微调阶段冷启动策略学习世界模型，然后在策略优化前使用该模型模拟未来状态。

Result: 在Sokoban、FrozenLake和Sudoku等多样化环境中显著提升性能：Sokoban成功率从25.6%提升至59.8%，FrozenLake得分从22.1%提升至70.9%（基于Qwen2.5-1.5B-Instruct模型）。
...
### [154] [Learn to Change the World: Multi-level Reinforcement Learning with Model-Changing Actions](https://arxiv.org/abs/2510.15056)
...
Method: 引入MCTVMDP框架，包含上层配置策略和下层原始动作策略，通过模型改变动作配置非平稳的转移函数。

Result: 构建了允许智能体主动修改环境动态的理论框架，将环境配置与动作策略优化联合考虑。

Conclusion: MCTVMDP为研究智能体主动环境改造能力提供了理论基础，扩展了传统强化学习的被动适应范式。
...
TL;DR: Antislop框架通过采样器、自动化流程和FTPO微调方法，有效检测和消除LLM输出中的重复短语模式，在保持性能的同时显著减少slop现象。
...
Method: 结合三种创新方法：Antislop采样器（推理时回溯抑制不需要的字符串）、自动化流程（分析模型特定slop并生成训练数据）、FTPO微调方法（在推理轨迹中调整单个token的logits）。

Result: Antislop采样器成功抑制8000+模式而保持质量，FTPO实现90%的slop减少，同时在GSM8K、MMLU和创意写作任务中保持或改进性能。
...
### [156] [Physics-informed data-driven machine health monitoring for two-photon lithography](https://arxiv.org/abs/2510.15075)
*Sixian Jia,Zhiqiao Dong,Chenhui Shao*

Main category: cs.LG

TL;DR: 本文提出了三种结合物理信息数据驱动预测模型和统计方法的两光子光刻系统健康监测方法，能够准确及时地监控机器健康状况，避免维护不足或过度维护的问题。


<details>
  <summary>Details</summary>
Motivation: 当前两光子光刻系统的维护主要依赖经验而非基于机器健康状态的监控，导致维护不及时造成停机和质量问题，或维护过度导致效率低下。需要开发准确的健康监测方法来支持基于状态的维护。

Method: 提出了三种方法：集成物理信息数据驱动预测模型与统计方法，处理不同泛化能力的复杂场景。通过结合结构尺寸预测模型和统计方法，能够适应不同级别的泛化需求。

Result: 在包含6种工艺参数组合和6种结构尺寸的全面实验数据集上测试，所有测试场景中都实现了高精度，表现出优秀的有效性、鲁棒性和泛化能力。

Conclusion: 这些方法为实现两光子光刻系统基于状态的维护迈出了重要一步，能够显著改善系统维护效率和制造质量。
...
### [157] [Online Correlation Clustering: Simultaneously Optimizing All $\ell_p$-norms](https://arxiv.org/abs/2510.15076)
*Sami Davies,Benjamin Moseley,Heather Newman*

Main category: cs.LG

TL;DR: 该论文提出了首个在线相关聚类算法，能够在AOS模型下同时近似所有ℓp范数目标，将离线环境中的"全范数"保证成功迁移到在线设置。
...
Method: 提出了一种在线带样本(AOS)模型算法，给定输入的一小部分常数比例作为样本，生成单一聚类，同时为所有ℓp范数提供高概率的O(log⁴n)竞争比，为ℓ∞范数提供高概率的O(logn)竞争比，为ℓ1范数提供期望下的O(1)竞争比。

Result: 算法在AOS模型中成功实现了同时近似所有ℓp范数的目标，并证明了在标准RO模型中ℓ∞范数需要至少Ω(n^{1/3})竞争比的下界，同时给出了AOS模型中ℓ1和ℓ∞范数竞争比的紧下界。

Conclusion: 该工作成功地将离线"全范数"保证迁移到在线世界，证明了AOS模型在解决相关聚类中不同目标间根本性权衡的有效性，为在线公平聚类提供了理论基础。
...
### [158] [Operator Flow Matching for Timeseries Forecasting](https://arxiv.org/abs/2510.15101)
*Yolanne Yi Ran Lee,Kyriakos Flouris*

Main category: cs.LG

TL;DR: 提出TempO模型，使用流匹配和傅里叶层处理3D时空场，在PDE动力学预测中优于现有方法


<details>
  <summary>Details</summary>
Motivation: 现有自回归和扩散方法在长期物理一致性预测中存在累积误差和离散化伪影问题

Method: 使用潜在流匹配模型，结合稀疏条件和通道折叠，利用时间条件傅里叶层捕捉多尺度模式

Result: 在三个基准PDE数据集上优于最先进基线，谱分析显示多尺度动力学恢复更优

Conclusion: TempO在参数和内存效率上优于基于注意力或卷积的回归器，能实现高效、确定性采样
...
### [159] [DLER: Doing Length pEnalty Right - Incentivizing More Intelligence per Token via Reinforcement Learning](https://arxiv.org/abs/2510.15110)
...
TL;DR: 提出了DLER方法，通过改进RL优化而非复杂的长度惩罚机制，在保持准确性的同时大幅缩短推理语言模型的输出长度。


<details>
  <summary>Details</summary>
Motivation: 现有推理语言模型虽然性能强大，但生成过长的输出，需要提高每个token的智能效率（准确性与响应长度的比值）。

Method: 使用简单的截断长度惩罚，结合批量奖励归一化、更高裁剪、动态采样等优化技术来解决RL训练中的优势估计偏差、熵崩溃和稀疏奖励问题。

Result: DLER将输出长度减少70%以上，同时超越所有先前基线的准确性；在7B模型上比DeepSeek-R1-7B准确率高28%且延迟更低。

Conclusion: DLER实现了最先进的准确率-效率权衡，证明了通过改进RL优化而非复杂惩罚机制可以有效提升推理模型的效率。
...
### [160] [Navigating the consequences of mechanical ventilation in clinical intensive care settings through an evolutionary game-theoretic framework](https://arxiv.org/abs/2510.15127)
*David J. Albers,Tell D. Bennett,Jana de Wiljes,Bradford J. Smith,Peter D. Sottile,J. N. Stroh*

Main category: cs.LG

TL;DR: 开发了一个基于进化博弈论的框架来分析机械通气策略对患者结果的影响，通过分析患者-呼吸机-护理系统的联合数据来优化和个性化机械通气治疗。


<details>
  <summary>Details</summary>
Motivation: 需要理解机械通气策略和辅助护理决策对重症监护患者结果的影响，通过分析临床数据来改进呼吸管理。

Method: 使用进化博弈论分析呼吸行为，为概率和随机方法（如强化学习）提供定量前驱，并验证于合成数据和真实ICU数据。

Result: 建立了可扩展的分析方法，揭示了数据生成过程的复杂性，为机械通气优化和个性化提供了基础。

Conclusion: 这是实现机械通气优化和个性化的关键一步，未来可发展包含经验和博弈论元素的状态转移模型来模拟MV决策效果。
...
### [161] [A Simple Method for PMF Estimation on Large Supports](https://arxiv.org/abs/2510.15132)
*Alex Shtoff*
...
<details>
  <summary>Details</summary>
Motivation: 针对大离散支撑集上多模态和重尾概率质量函数的非参数估计问题，传统方法在处理噪声和保持粗粒度结构方面存在不足。

Method: 将经验PMF视为线图上的信号，构建对称三对角算子（路径图拉普拉斯扰动矩阵），计算最小特征值对应的特征向量，将经验PMF投影到低维子空间，最后进行裁剪和重归一化处理。

Result: 在合成和真实重尾数据上，该方法能有效保持粗粒度结构同时抑制采样噪声，在目标场景下优于logspline和高斯KDE基线方法，计算可靠且运行时间与支撑集大小和低维子空间维度成正比。

Conclusion: 该方法实现简洁，在不同样本量下表现稳健，因其可靠性和速度适合自动化流程和大规模探索性分析，但存在已知的失效模式（如突然的不连续性）。
...
### [162] [Predicting the Unpredictable: Reproducible BiLSTM Forecasting of Incident Counts in the Global Terrorism Database (GTD)](https://arxiv.org/abs/2510.15136)
*Oluwasegun Adegoke*
...
<details>
  <summary>Details</summary>
Motivation: 研究恐怖主义事件周计数的短期预测，使用全球恐怖主义数据库(GTD)数据，旨在建立一个可复现的预测管道并评估深度学习模型相对于传统方法的性能。

Method: 构建可复现的预测管道，使用固定时间分割，评估双向LSTM(BiLSTM)模型，并与季节性朴素模型、线性/ARIMA模型以及LSTM-Attention基线进行比较。

Result: BiLSTM在测试集上获得RMSE 6.38，显著优于LSTM-Attention(9.19)和线性滞后回归基线，RMSE提升分别达30.6%和35.4%，在MAE和MAPE指标上也有并行改进。

Conclusion: 研究表明，使用长历史数据训练的模型泛化能力最佳，适中的回看窗口(20-30周)提供强上下文，双向编码对于捕捉窗口内的积累和后果模式至关重要，短期结构特征贡献最大。
...
### [163] [Policy Transfer Ensures Fast Learning for Continuous-Time LQR with Entropy Regularization](https://arxiv.org/abs/2510.15165)
*Xin Guo,Zijiu Lyu*

Main category: cs.LG

TL;DR: 本文首次证明了连续时间强化学习中策略迁移的理论可行性，展示了在连续时间线性二次调节器（LQR）中，一个任务的最优策略可以作为相关任务的近最优初始化，同时保持原始算法的收敛速度。


<details>
  <summary>Details</summary>
Motivation: 强化学习在复杂任务上从头训练效率低下，而迁移学习在大语言模型中已证明有效。本文旨在探索如何将迁移学习应用于连续时间强化学习，以提高训练效率。

Method: 研究在熵正则化连续时间LQR中的策略迁移方法，提出了新的策略学习算法，该算法实现了全局线性和局部超线性收敛。

Result: 理论证明了策略迁移在连续时间RL中的可行性，提出的新算法具有优越的收敛性能，并建立了连续时间基于分数的扩散模型与LQR的稳定性联系。

Conclusion: 本文填补了连续时间强化学习中迁移学习理论研究的空白，将先前离散时间的研究扩展到连续时间设置，为RL效率提升提供了新的理论支撑。
...
### [164] [A simple mean field model of feature learning](https://arxiv.org/abs/2510.15174)
*Niclas Göring,Chris Mingard,Yoonsoo Nam,Ard Louis*

Main category: cs.LG

TL;DR: 本文使用统计物理方法推导了双层非线性网络在SGLD训练下的贝叶斯后验的均值场理论，揭示了有限宽度下网络与目标函数对齐的对称性破缺相变，并发现了自增强输入特征选择机制对泛化性能的关键影响。


<details>
  <summary>Details</summary>
Motivation: 特征学习（神经网络在训练中调整内部表示的过程）仍然缺乏理论理解，需要开发可处理的数学框架来揭示其机制。

Method: 使用统计物理方法推导自洽的均值场理论来描述双层非线性网络在随机梯度Langevin动力学训练下的贝叶斯后验分布。

Result: 在无限宽度下理论简化为核岭回归，但在有限宽度下预测了对称性破缺相变；基本均值场理论低估了相变后的泛化改进，需要引入自增强输入特征选择机制来准确匹配学习曲线。

Conclusion: 自增强输入特征选择是特征学习的关键机制，将其纳入均值场理论可以定量匹配网络的学习行为，为理解特征学习提供了机制性洞见。
...
### [165] [Finding geodesics with the Deep Ritz method](https://arxiv.org/abs/2510.15177)
*Conor Rowan*

Main category: cs.LG

TL;DR: 本文提出测地线问题特别适合深度Ritz方法，并通过三个数值实例验证了这一观点，旨在为科学机器学习研究开辟新方向。
...
Method: 采用深度Ritz方法处理测地线问题，通过三个数值实例进行验证：路径规划、光学和固体力学中的问题。

Result: 研究证实了深度Ritz方法在测地线问题上的有效性，展示了该方法在不同领域的适用性。
...
### [166] [An Advanced Two-Stage Model with High Sensitivity and Generalizability for Prediction of Hip Fracture Risk Using Multiple Datasets](https://arxiv.org/abs/2510.15179)
...
TL;DR: 提出了一个两阶段模型，整合临床和影像信息来改善髋部骨折风险预测，相比传统工具具有更高敏感性和更少的漏诊。


<details>
  <summary>Details</summary>
Motivation: 传统工具如DXA T-score和FRAX在髋部骨折风险评估中敏感性不足，特别是对于无既往骨折或骨量减少的高风险个体识别效果不佳。

Method: 使用MrOS、SOF和UK Biobank数据，构建两阶段模型：第一阶段使用临床、人口统计学和功能变量进行筛查，第二阶段整合DXA影像特征进行精炼。

Result: 模型经过内外验证，在不同队列中表现一致且适应性强，相比T-score和FRAX具有更高敏感性和更少的漏诊病例。
...
### [167] [Automotive Crash Dynamics Modeling Accelerated with Machine Learning](https://arxiv.org/abs/2510.15201)
*Mohammad Amin Nabian,Sudeep Chavare,Deepak Akhare,Rishikesh Ranade,Ram Cherukuri,Srinivas Tadepalli*

Main category: cs.LG

TL;DR: 该研究探索了使用机器学习替代模型预测结构碰撞变形的方法，比较了MeshGraphNet和Transolver两种神经网络架构，以及三种瞬态动力学建模策略。在包含150个详细有限元模拟的BIW碰撞数据集上验证，结果显示模型能以合理精度捕捉变形趋势，计算成本大幅降低。


<details>
  <summary>Details</summary>
Motivation: 传统的汽车碰撞安全性评估依赖计算昂贵且耗时的有限元模拟，需要开发更高效的替代方法。

Method: 使用NVIDIA PhysicsNeMo框架，比较MeshGraphNet和Transolver两种神经网络架构，以及时间条件、标准自回归和稳定性增强自回归三种瞬态动力学建模策略。

Result: 模型能合理捕捉整体变形趋势，虽然精度尚未完全达到有限元模拟水平，但计算成本降低了数个数量级。
...
### [168] [Dissecting Mahalanobis: How Feature Geometry and Normalization Shape OOD Detection](https://arxiv.org/abs/2510.15202)
*Denis Janiak,Jakub Binkowski,Tomasz Kajdanowicz*

Main category: cs.LG

TL;DR: 该论文通过实证研究发现Mahalanobis距离方法在OOD检测中并非普遍可靠，定义了理想的数据表示几何结构，并提出径向缩放ℓ2归一化方法，通过调整特征空间的径向几何来显著提升OOD检测性能。
...
Method: 进行了全面的实证研究，涵盖多种图像基础模型、数据集和距离归一化方案；定义了理想几何结构；提出了径向缩放ℓ2归一化方法，引入可调参数来控制特征空间的径向几何。

Result: 分析表明Mahalanobis方法并非普遍可靠；谱和内在维度指标能准确预测模型的OOD性能；径向缩放ℓ2归一化能显著改善OOD检测性能。
...
### [169] [ReasonIF: Large Reasoning Models Fail to Follow Instructions During Reasoning](https://arxiv.org/abs/2510.15211)
*Yongchan Kwon,Shang Zhu,Federico Bianchi,Kaitlyn Zhou,James Zou*

Main category: cs.LG

TL;DR: 提出了ReasonIF基准来评估大型推理模型在推理过程中遵循用户指令的能力，发现现有模型指令遵循率低于25%，并探索了多轮推理和指令微调两种改进方法。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型不仅要在最终回答中遵循指令，更要在整个推理过程中遵循用户指令，这能提高模型的可控性、透明度，并减少不良捷径、幻觉或奖励黑客的风险。

Method: 构建ReasonIF基准，包含多语言推理、格式控制和长度控制等六类指令提示，评估多个开源LRM的指令遵循表现，并测试多轮推理和推理指令微调(RIF)两种改进策略。

Result: 现有开源LRM在推理指令遵循方面表现不佳，最高指令遵循分数(IFS)低于0.25，即少于25%的推理轨迹符合给定指令。任务难度增加时表现进一步下降。RIF方法将GPT-OSS-20B的IFS从0.11提升到0.27。

Conclusion: 大型推理模型在推理过程中遵循用户指令的能力严重不足，需要进一步研究改进。推理指令微调等方法是有效的改进方向，但仍有很大提升空间。
...
### [170] [Soundness-Aware Level: A Microscopic Signature that Predicts LLM Reasoning Potential](https://arxiv.org/abs/2510.15216)
*Xuansheng Wu,Xiaoman Pan,Wenlin Yao,Jianshu Chen*

Main category: cs.LG

TL;DR: 研究发现，不同基础模型在强化学习可验证奖励（RLVR）后的推理性能差异源于其内部对规则合理性的感知能力。高潜力模型能区分严格规则与噪声规则的概率分布，而弱模型则无法区分。
...
Method: 将推理形式化为Horn子句链，使用跨层稀疏自编码器从潜在空间提取特征，估计特征间转移概率，并用LLM对规则的语义合理性水平进行分类。

Result: 发现高潜力模型具有合理性感知能力，其内部概率分布在不同的合理性水平上系统性地变化，而弱模型则无法区分。提出的SAL指标能准确预测RLVR后的推理性能（R^2=0.87）。
...
### [171] [Reflections from Research Roundtables at the Conference on Health, Inference, and Learning (CHIL) 2025](https://arxiv.org/abs/2510.15217)
...
<details>
  <summary>Details</summary>
Motivation: 促进机器学习与医疗健康领域的跨学科合作，通过小型圆桌讨论催化创新思维和实际解决方案。

Method: 由资深和初级主席共同主持的圆桌讨论形式，强调开放性交流、知识探索和包容性参与。

Result: 成功组织了8个圆桌会议，涉及19位主席，覆盖了从可解释性到可扩展医疗解决方案的广泛主题。

Conclusion: 研究圆桌会议为医疗AI领域提供了重要的合作平台，推动了该领域关键挑战的深入讨论和行动方向的集体构思。
...
### [172] [Machine Learning for Early Detection of Meningitis: Stacked Ensemble Learning with EHR data](https://arxiv.org/abs/2510.15218)
*Han Ouyang,Jesse Hamilton,Saeed Amal*

Main category: cs.LG

TL;DR: 使用MIMIC-III数据库中的214名脑膜炎患者和46,303名非脑膜炎患者数据，通过集成学习方法构建脑膜炎诊断模型，在模拟急诊室场景下取得了优异的诊断性能。


<details>
  <summary>Details</summary>
Motivation: 开发能够在真实急诊室场景下辅助诊断脑膜炎的AI工具，提高脑膜炎的早期诊断准确性。

Method: 采用两阶段特征选择方法筛选临床相关特征，使用随机森林、LightGBM和深度神经网络作为基础模型，通过集成学习将基础模型输出堆叠到逻辑回归元模型中。

Result: 集成学习模型在两个测试集上分别获得0.9637和0.9472的AUC值，表现出优异的诊断性能。

Conclusion: 该研究为未来基于集成学习的AI驱动脑膜炎诊断方法奠定了基础，虽然直接部署临床诊断工具仍具挑战性，但展示了在真实急诊场景下的应用潜力。
...
### [173] [Integrating Product Coefficients for Improved 3D LiDAR Data Classification (Part II)](https://arxiv.org/abs/2510.15219)
...
Method: 将乘积系数与自编码器表示和KNN分类器相结合，并逐级添加乘积系数来研究其对分类性能的影响。

Result: 逐级添加更丰富的乘积系数集合能系统性地提高类别可分性和整体准确率。

Conclusion: 结合分层乘积系数特征与自编码器能显著提升LiDAR分类性能。
...
### [174] [Stress-Aware Learning under KL Drift via Trust-Decayed Mirror Descent](https://arxiv.org/abs/2510.15222)
*Gabriel Nixon Raj*

Main category: cs.LG

TL;DR: 本文提出熵正则化信任衰减方法，用于处理分布漂移下的序列决策问题，通过应力感知的指数倾斜来更新信念和决策，实现了O(√T)的动态遗憾保证。


<details>
  <summary>Details</summary>
Motivation: 在现实世界的序列决策中，数据分布经常随时间变化（分布漂移），传统方法对此不够鲁棒。需要开发能够适应分布漂移的决策框架，同时保持理论保证。

Method: 提出熵正则化信任衰减方法，在信念更新和镜像下降决策中注入应力感知的指数倾斜。在单纯形上，通过Fenchel对偶等价性证明信念倾斜和决策倾斜是一致的。

Result: 证明了高概率敏感性边界，在KL漂移路径长度ST = ∑t≥2√KL(Dt|Dt-1)/2下实现了Õ(√T)的动态遗憾保证。信任衰减实现了O(1)的每切换遗憾，而无应力更新会产生Ω(1)的尾部。

Conclusion: 该框架统一了动态遗憾分析、分布鲁棒目标和KL正则化控制，提供了一个单一的压力自适应更新方法，可扩展到二阶更新、老虎机反馈、异常值处理等多种场景。
...
### [175] [FinTrust: A Comprehensive Benchmark of Trustworthiness Evaluation in Finance Domain](https://arxiv.org/abs/2510.15232)
*Tiansheng Hu,Tongyan Hu,Liuyang Bai,Yilun Zhao,Arman Cohan,Chen Zhao*

Main category: cs.LG

TL;DR: FinTrust是一个专门用于评估金融领域LLM可信度的综合基准，涵盖多种对齐问题，通过细粒度任务评估多个可信度维度。评估发现专有模型在安全性等方面表现更好，开源模型在特定领域有优势，但所有模型在法律意识方面都存在明显不足。
...
Method: 构建FinTrust基准，基于实际情境设计广泛的对齐问题，为每个可信度评估维度创建细粒度任务，评估11个LLM模型。

Result: 专有模型如o4-mini在大多数任务（如安全性）中表现更好，开源模型如DeepSeek-V3在特定领域（如行业级公平性）有优势，所有模型在信托对齐和披露等挑战性任务中都表现不佳，显示出法律意识方面的显著差距。
...
### [176] [Adaptive Individual Uncertainty under Out-Of-Distribution Shift with Expert-Routed Conformal Prediction](https://arxiv.org/abs/2510.15233)
...
<details>
  <summary>Details</summary>
Motivation: 当前机器学习社区缺乏可靠、信息丰富且个体化的不确定性量化方法，这阻碍了AI/ML在风险敏感领域的有效应用。特别是在药物发现中的蛋白质-配体亲和力预测，面临数据噪声异质性、化学空间不平衡和分布偏移等挑战。

Method: TESSERA结合了专家混合模型的多样性和保形校准，提供每个样本的不确定性，具有可靠的覆盖保证、信息丰富且自适应的预测区间宽度。

Result: 在独立同分布和基于支架的分布外分割下，TESSERA实现了接近名义覆盖率和最佳的覆盖-宽度权衡，同时保持了竞争性的适应性。大小分层覆盖证实区间大小合适，在数据稀缺或噪声大时宽度增加，在预测可靠时保持紧凑。

Conclusion: TESSERA通过统一专家混合模型多样性和保形校准，提供了可信赖、紧凑且自适应的不确定性，适用于药物发现管道和其他应用中的选择性预测和下游决策。
...
### [177] [Dual-Weighted Reinforcement Learning for Generative Preference Modeling](https://arxiv.org/abs/2510.15242)
...
TL;DR: DWRL是一个新的偏好建模框架，通过双权重强化学习目标将思维链推理与Bradley-Terry模型结合，在非可验证任务上优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 强化学习在可验证答案任务上有效扩展思维链推理，但在更一般的非可验证偏好任务上仍然具有挑战性且研究不足。

Method: 提出双权重强化学习框架，包含实例级错位权重和组级条件偏好分数，训练生成式偏好模型先产生思维再预测人类偏好分数。

Result: 在多个基准测试和模型规模上，DWRL持续优于生成式偏好模型基线和标量模型，同时产生连贯可解释的思维。
...
### [178] [Spatiotemporal Transformers for Predicting Avian Disease Risk from Migration Trajectories](https://arxiv.org/abs/2510.15254)
*Dingya Feng,Dingyuan Xue*

Main category: cs.LG

TL;DR: 提出基于Transformer的框架，利用候鸟迁徙轨迹预测终点疾病风险，整合GPS追踪数据、疫情记录和地理空间信息，在测试集上取得高精度预测性能。


<details>
  <summary>Details</summary>
Motivation: 准确预测禽类疾病暴发对野生动物保护和公共卫生至关重要，需要开发能够整合多源数据并捕捉时空依赖性的预测方法。

Method: 使用Transformer架构，整合Movebank的GPS追踪数据、WOAH疫情记录和GADM/Natural Earth地理空间数据，通过H3分层地理空间编码处理原始坐标，从鸟类移动序列中学习时空依赖性。

Result: 在测试集上表现出色：准确率0.9821，AUC 0.9803，平均精度0.9299，F1分数0.8836，显示出强大的预测能力。

Conclusion: Transformer架构在禽类疾病监测预警系统中具有巨大潜力，能够支持及时的干预和预防策略。
...
### [179] [DRO-InstructZero: Distributionally Robust Prompt Optimization for Large Language Models](https://arxiv.org/abs/2510.15260)
*Yangyang Li*

Main category: cs.LG

TL;DR: DRO-InstructZero通过将零样本提示优化构建为鲁棒贝叶斯优化，使用f-散度球定义评估分布的模糊集，在保持贝叶斯搜索查询效率的同时最大化最坏情况期望效用，显著提升了提示在分布偏移和对抗评估下的可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有自动提示搜索方法（如InstructZero）在单一评估分布下优化期望性能，导致提示在分布偏移和对抗评估下性能下降，缺乏可靠性和可迁移性。

Method: 将零样本提示优化构建为鲁棒贝叶斯优化，使用f-散度球定义评估分布的模糊集，通过鲁棒采集规则最大化最坏情况期望效用。

Result: 在BIG-Bench信息到正式改写任务中，准确率从61.3%提升到85-90%，绝对增益约25-30个百分点；在代码调试任务中，领域偏移下获得约25个百分点的增益；稳定任务如因果推理保持96%以上准确率。
...
### [180] [Robust Layerwise Scaling Rules by Proper Weight Decay Tuning](https://arxiv.org/abs/2510.15262)
*Zhiyuan Fan,Yifeng Liu,Qingyue Zhao,Angela Yuan,Quanquan Gu*

Main category: cs.LG

TL;DR: 提出了AdamW优化器的权重衰减缩放规则，通过保持子层增益跨宽度不变，实现学习率和权重衰减的零样本迁移，解决了μP在稳态阶段的失效问题。


<details>
  <summary>Details</summary>
Motivation: 在现代尺度不变架构中，训练快速进入优化器主导的稳态，归一化层造成反向尺度敏感性，使得有效学习率变得宽度依赖，从而降低了μP的迁移效果。

Method: 引入权重衰减缩放规则，结合μP学习率规则，使矩阵参数满足λ₂∝√d，向量参数保持η₁=Θ_d(1)和λ₁=0，实现子层增益的宽度不变性。

Result: 在LLaMA风格Transformer和最小合成设置中验证了该规则，提供了匹配顶部奇异值的简单诊断方法，成功实现了学习率和权重衰减的零样本迁移。

Conclusion: 通过显式控制优化器设置的稳态尺度，将μP扩展到近初始阶段之外，为AdamW下的宽度鲁棒超参数迁移提供了实用方案。
...
### [181] [Causal Time Series Modeling of Supraglacial Lake Evolution in Greenland under Distribution Shift](https://arxiv.org/abs/2510.15265)
*Emam Hossain,Muhammad Hasan Ferdous,Devon Dunmire,Aneesh Subramanian,Md Osman Gani*

Main category: cs.LG

TL;DR: 提出RIC-TSC框架，将因果发现嵌入时间序列分类，在格陵兰冰上湖演化预测中，因果模型比相关基线在分布外评估中准确率提升12.59%


<details>
  <summary>Details</summary>
Motivation: 当前地球观测中的时空模型过度依赖相关性特征，在异质域间迁移能力差，因果建模能发现稳定不变的关系，提高分布偏移下的鲁棒性和泛化能力

Method: 使用J-PCMCI+进行区域特定和不变预测因子的因果发现，将验证的预测因子及其时间滞后输入轻量级分类器，结合多模态卫星和再分析数据

Result: 在两个对比融化季节的1000个手动标记湖泊上，因果模型在分布外评估中比基于相关性的基线准确率提高达12.59%
...
### [182] [Semi-Supervised Regression with Heteroscedastic Pseudo-Labels](https://arxiv.org/abs/2510.15266)
*Xueqing Sun,Renzhen Wang,Quanziang Wang,Yichen Wu,Xixi Jia,Deyu Meng*

Main category: cs.LG

TL;DR: 提出了一种不确定性感知的伪标签框架，用于解决半监督回归中伪标签可靠性评估的挑战，通过双层优化动态调整伪标签影响，提高模型鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 半监督回归中的伪标签方法相对较少被探索，与分类不同，回归涉及连续输出和异方差噪声，难以评估伪标签可靠性，导致错误累积和过拟合问题。

Method: 采用不确定性感知的伪标签框架，从双层优化角度动态调整伪标签影响，联合最小化所有数据的经验风险和优化不确定性估计以增强标记数据的泛化能力。

Result: 在多个基准半监督回归数据集上的广泛实验验证了该方法，结果显示相比现有方法具有更优的鲁棒性和性能。
...
### [183] [Foundation Models for Scientific Discovery: From Paradigm Enhancement to Paradigm Transition](https://arxiv.org/abs/2510.15280)
*Fan Liu,Jindong Han,Tengfei Lyu,Weijia Zhang,Zhe-Rui Yang,Lu Dai,Cancheng Liu,Hao Liu*

Main category: cs.LG

TL;DR: 基础模型正在重塑科学研究范式，从增强传统科学方法逐步发展到与人类协同创造，最终可能实现自主科学发现。


<details>
  <summary>Details</summary>
Motivation: 探讨基础模型是仅仅增强现有科学方法，还是正在重新定义科学实践方式，分析其对科学范式的变革性影响。

Method: 提出了一个三阶段框架来描述基础模型在科学发现中的演变：元科学整合、人机协同创造、自主科学发现。

Result: 基础模型正在催化科学范式的转变，从工具性应用到成为科学发现的主动参与者，最终可能发展为独立的知识创造者。

Conclusion: 基础模型正在推动科学发现的新范式，需要科学界理解其变革作用并反思科学发现的未来发展方向。
...
### [184] [Small Ensemble-based Data Assimilation: A Machine Learning-Enhanced Data Assimilation Method with Limited Ensemble Size](https://arxiv.org/abs/2510.15284)
*Zhilin Li,Yao Zhou,Xianglong Li,Zeng Liu,Zhaokuan Lu,Shanlin Xu,Seungnam Kim,Guangyao Wang*

Main category: cs.LG

TL;DR: 提出一种结合集成卡尔曼滤波和全连接神经网络的数据同化方法，使用较小集成规模生成初步分析状态，再用神经网络预测校正项以提高精度。


<details>
  <summary>Details</summary>
Motivation: 传统集成数据同化方法在分析精度和计算效率之间存在权衡，较大集成规模能提高精度但计算成本高。

Method: 使用较小集成规模的EnKF生成初步分析状态，然后通过FCNN学习并预测校正项来弥补集成规模限制导致的性能下降。

Result: 在Lorenz系统和非线性海洋波场模拟实验中，新方法在相同集成规模下比传统EnKF精度更高，且额外计算成本可忽略。

Conclusion: EnKF-FCNN方法能有效提高数据同化精度，适应性强，可与不同模型和集成数据同化方法耦合应用。
...
### [185] [Identifying internal patterns in (1+1)-dimensional directed percolation using neural networks](https://arxiv.org/abs/2510.15294)
...
Conclusion: 深度神经网络能够有效从原始数据中自动识别相变和分类隐藏模式，展示了深度学习在物理系统分析中的潜力
...
### [186] [DFCA: Decentralized Federated Clustering Algorithm](https://arxiv.org/abs/2510.15300)
*Jonas Kirch,Sebastian Becker,Tiago Koketsu Rodrigues,Stefan Harmeling*

Main category: cs.LG

TL;DR: DFCA是一种完全去中心化的聚类联邦学习算法，无需中央服务器协调，通过邻居间的顺序运行平均来聚合模型，在稀疏连接下仍保持良好性能。


<details>
  <summary>Details</summary>
Motivation: 现有聚类联邦学习方法依赖中央服务器，存在单点故障和瓶颈问题，限制了在现实去中心化环境中的应用。

Method: 使用顺序运行平均聚合邻居模型，提供通信高效的替代方案，无需中央协调即可训练集群特定模型。

Result: 在多个数据集上的实验表明，DFCA优于其他去中心化算法，性能与集中式IFCA相当，在稀疏连接下表现出鲁棒性。
...
### [187] [On the Generalization Properties of Learning the Random Feature Models with Learnable Activation Functions](https://arxiv.org/abs/2510.15327)
*Zailin Ma,Jiansheng Yang,Yaodong Yang*

Main category: cs.LG

TL;DR: 本文研究了可学习激活函数的随机特征模型(RFLAF)的泛化特性，通过数据依赖的采样方案显著提高了特征数量的边界要求，在回归和分类任务中都取得了最尖锐的边界结果。
...
Result: 通过加权采样，MSE损失情况下的特征数量边界从Ω(1/ε²)改进为Ω̃((1/ε)^{1/t})，当Gram矩阵具有有限秩时甚至可达到Ω(1)；对于Lipschitz损失，边界从Ω(1/ε²)改进为Ω̃((1/ε²)^{1/t})。

Conclusion: 加权RFLAF在显著减少特征数量的情况下实现了与普通采样RFLAF相同的性能，验证了理论的有效性和方法的优越性。
...
### [188] [Backdoor or Manipulation? Graph Mixture of Experts Can Defend Against Various Graph Adversarial Attacks](https://arxiv.org/abs/2510.15333)
*Yuyuan Feng,Bin Ma,Enyan Dai*

Main category: cs.LG

TL;DR: 提出基于混合专家(MoE)架构的统一防御框架，同时抵御图神经网络的后门攻击、边操纵和节点注入攻击，通过逻辑多样性损失和鲁棒感知路由器实现多攻击防御。


<details>
  <summary>Details</summary>
Motivation: 现有图神经网络防御方法通常只针对单一攻击类型，缺乏同时防御多种威胁的统一方法，需要解决这一局限性。

Method: 使用混合专家架构，提出基于互信息的逻辑多样性损失使专家关注不同邻域结构，并设计鲁棒感知路由器识别扰动模式并自适应路由节点到相应鲁棒专家。

Result: 在各种对抗设置下的广泛实验表明，该方法在多种图对抗攻击下始终实现优越的鲁棒性。

Conclusion: 基于MoE的统一防御框架能够有效同时防御多种图对抗攻击，提供可扩展的鲁棒性解决方案。
...
### [189] [Sequence Modeling with Spectral Mean Flows](https://arxiv.org/abs/2510.15366)
...
<details>
  <summary>Details</summary>
Motivation: 传统序列建模方法难以表示和学习高度非线性的概率状态动态，而算子理论提供了将动态视为希尔伯特空间线性映射的视角。

Method: 基于HMM的算子理论视角，将序列分布嵌入为张量积希尔伯特空间中的张量，通过谱分解实现可扩展的张量网络分解，并扩展MMD梯度流到时变希尔伯特空间。

Result: 在多个时间序列建模数据集上取得了有竞争力的结果。

Conclusion: 谱均值流方法为序列建模提供了新的算子理论视角，实现了可扩展的学习和快速采样。
...
### [190] [Towards Robust Zero-Shot Reinforcement Learning](https://arxiv.org/abs/2510.15382)
*Kexin Zheng,Lauriane Teyssier,Yinan Zheng,Yu Luo,Xiayuan Zhan*

Main category: cs.LG

TL;DR: BREEZE是一个改进的零样本强化学习框架，通过行为正则化、扩散模型策略提取和注意力架构，解决了现有方法表示表达能力不足和OOD动作导致的学习不稳定问题。


<details>
  <summary>Details</summary>
Motivation: 现有零样本RL方法（如FB表示）存在表示表达能力不足和离线学习中的OOD动作导致外推误差问题，造成表示偏差和次优性能。

Method: 1. 引入行为正则化，将策略优化转化为稳定的样本内学习范式；2. 使用任务条件扩散模型提取策略，生成高质量多模态动作分布；3. 采用表达能力强的注意力架构进行表示建模。

Result: 在ExORL和D4RL Kitchen数据集上的实验表明，BREEZE实现了最佳或接近最佳的性能，并展现出比现有离线零样本RL方法更优越的鲁棒性。
...
### [191] [Iterative Refinement of Flow Policies in Probability Space for Online Reinforcement Learning](https://arxiv.org/abs/2510.15388)
*Mingyang Sun,Pengxiang Ding,Weinan Zhang,Donglin Wang*

Main category: cs.LG

TL;DR: SWFP框架通过将流匹配推理过程离散化为固定步长的欧拉方案，将其与最优传输的JKO原理对齐，实现了预训练流策略的稳定在线微调。


<details>
  <summary>Details</summary>
Motivation: 行为克隆中的流/扩散策略虽然擅长从演示中学习复杂技能，但对分布偏移很脆弱，标准RL方法由于迭代推理过程和现有变通方法的限制而难以微调这些模型。

Method: SWFP将全局流分解为一系列邻近分布间的小增量变换，每个步骤对应一个JKO更新，通过熵正则化确保稳定的在线适应。该方法使用级联的小流块来微调预训练流。

Result: 综合实验表明SWFP在多种机器人控制基准测试中具有增强的稳定性、效率和优越的适应性能。

Conclusion: SWFP框架通过将流匹配与JKO原理对齐，提供了一种高效、稳定且可证明稳定的预训练流策略微调方法。
...
### [192] [Geometric Mixture Models for Electrolyte Conductivity Prediction](https://arxiv.org/abs/2510.15403)
...
<details>
  <summary>Details</summary>
Motivation: 当前电解质研究面临两个基本挑战：缺乏高质量标准化基准，以及在混合系统中对几何结构和分子间相互作用的建模不足。

Method: 首先重组并增强CALiSol和DiffMix电解质数据集，加入分子几何图表示；然后提出GeoMix框架，该框架保持Set-SE(3)等变性，核心是专门为分子间几何消息传递设计的等变模块GIN。

Result: 综合实验表明，GeoMix在两个数据集上始终优于多种基线方法（包括MLPs、GNNs和几何GNNs），验证了跨分子几何相互作用和等变消息传递对准确性质预测的重要性。

Conclusion: 这项工作不仅为电解质研究建立了新基准，还提供了一个通用的几何学习框架，可推动能源材料、药物开发等领域中混合系统建模的进展。
...
### [193] [Online Kernel Dynamic Mode Decomposition for Streaming Time Series Forecasting with Adaptive Windowing](https://arxiv.org/abs/2510.15404)
*Christopher Salazar,Krithika Manohar,Ashis G. Banerjee*

Main category: cs.LG

TL;DR: 提出了WORK-DMD方法，结合随机傅里叶特征和在线动态模式分解，通过显式特征映射捕获非线性动态，同时保持固定计算成本和竞争性预测精度。


<details>
  <summary>Details</summary>
Motivation: 实时流数据预测面临非平稳动态、严格计算限制和快速适应而不发生灾难性遗忘等关键挑战。现有方法在准确性、适应性和效率之间存在权衡，特别是在受限计算环境中。

Method: WORK-DMD在滚动窗口内使用Sherman-Morrison更新，通过随机傅里叶特征进行显式特征映射，仅使用当前数据持续适应演化动态，无需长时间训练或大量历史数据存储。

Result: 在多个领域的基准数据集上的实验表明，WORK-DMD比几种最先进的在线预测方法获得更高的准确性，仅需单次数据遍历，在短期预测中表现尤为出色。

Conclusion: 将核评估与自适应矩阵更新相结合，以最小数据需求实现强预测性能。这种样本效率为流式预测应用提供了深度学习的实用替代方案。
...
### [194] [ParaFormer: Shallow Parallel Transformers with Progressive Approximation](https://arxiv.org/abs/2510.15425)
...
### [195] [Safe, Efficient, and Robust Reinforcement Learning for Ranking and Diffusion Models](https://arxiv.org/abs/2510.15429)
*Shashank Gupta*

Main category: cs.LG

TL;DR: 本论文研究如何设计安全、样本高效且鲁棒的强化学习方法，主要关注排序推荐系统和文本到图像扩散模型两个应用领域，提出了多种理论保证和算法改进。


<details>
  <summary>Details</summary>
Motivation: 解决强化学习在现实应用中面临的安全性、样本效率和鲁棒性问题，特别是在排序推荐和生成模型等关键领域。

Method: 采用上下文多臂老虎机框架，开发了曝光泛化边界理论、双重鲁棒估计器、基线校正框架，以及结合PPO和REINFORCE的LOOP算法。

Result: 实现了安全部署保证、最小化方差的最优基线、以及PPO级别样本效率下更忠实于文本属性的生成效果。

Conclusion: 论文为强化学习的实际应用提供了理论保证和实用算法，在安全性和效率方面取得了重要进展。
...
### [196] [A Theoretical Study on Bridging Internal Probability and Self-Consistency for LLM Reasoning](https://arxiv.org/abs/2510.15444)
*Zhi Zhou,Yuhao Tan,Zenan Li,Yuan Yao,Lan-Zhe Guo,Yu-Feng Li,Xiaoxing Ma*

Main category: cs.LG

TL;DR: 本文提出了首个用于分析基于采样的测试时扩展方法的理论框架，揭示了自一致性和困惑度方法的关键局限性，并提出了RPC混合方法来解决这些问题。


<details>
  <summary>Details</summary>
Motivation: 尽管基于采样的测试时扩展方法在实践中取得了成功，但其理论基础仍然未被充分探索。本文旨在填补这一空白，从置信度估计的角度提供理论分析。
...
### [197] [Particle Dynamics for Latent-Variable Energy-Based Models](https://arxiv.org/abs/2510.15447)
*Shiqin Tang,Shuxin Zhuang,Rong Feng,Runsheng Yu,Hongzong Li,Youzhi Zhang*

Main category: cs.LG

TL;DR: 本文提出了一种新的隐变量能量基模型训练方法，将最大似然训练重新表述为分布上的鞍点问题，通过耦合的Wasserstein梯度流进行优化，无需判别器或辅助网络。


<details>
  <summary>Details</summary>
Motivation: 隐变量能量基模型能够同时进行表达性生成建模和捕获隐藏结构，但传统训练方法存在局限性。本文旨在开发一种更有效的训练框架。

Method: 将最大似然训练重新表述为潜变量和联合流形上分布的鞍点问题，使用耦合的Wasserstein梯度流，交替进行过阻尼Langevin更新和随机参数上升。

Result: 在标准平滑性和耗散性假设下证明了存在性和收敛性，在KL散度和Wasserstein-2距离上获得衰减率，ELBO比受限摊销后验获得的界限更紧。

Conclusion: 该方法在物理系统数值逼近中表现优异，与可比方法竞争性相当，为隐变量能量基模型提供了有效的训练框架。
...
### [198] [Expediting Reinforcement Learning by Incorporating Knowledge About Temporal Causality in the Environment](https://arxiv.org/abs/2510.15456)
*Jan Corazza,Hadi Partovi Aria,Daniel Neider,Zhe Xu*

Main category: cs.LG

TL;DR: 提出一种将时序逻辑因果图整合到概率奖励机中的方法，以解决强化学习在稀疏奖励任务中的学习效率问题，并支持任务规范在不同环境间的迁移。


<details>
  <summary>Details</summary>
Motivation: 强化学习算法在处理奖励稀疏且依赖于环境中复杂事件序列的任务时效果不佳。概率奖励机虽然能捕捉奖励信号的时间依赖性，但难以手动设计和修改，限制了利用高层因果知识和跨领域迁移的能力。

Method: 将基于时序逻辑的因果图信息整合到奖励形式化中，通过结合因果结构来加速策略学习，并提供理论收敛保证。

Result: 理论证明了方法能收敛到最优策略，并通过实证研究展示了其在加速学习和任务规范迁移方面的优势。

Conclusion: 提出的方法有效解决了概率奖励机设计困难的问题，通过整合因果信息显著提升了强化学习在复杂稀疏奖励任务中的性能，并支持跨环境任务迁移。
...
### [199] [Learning to Answer from Correct Demonstrations](https://arxiv.org/abs/2510.15464)
*Nirmit Joshi,Gene Li,Siddharth Bhandari,Shiva Prasad Kasiviswanathan,Cong Ma,Nathan Srebro*

Main category: cs.LG

TL;DR: 本文研究从正确演示中学习生成问题答案的问题，提出了一种基于低基数奖励模型的新方法，避免了传统似然最大化的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统方法假设演示者属于低复杂度策略类，但在实际中奖励模型（指定哪些答案正确）属于低基数类可能是更弱的假设，需要超越似然最大化的学习方法。

Method: 将问题形式化为上下文老虎机中的离线模仿学习，提出了一种基于低基数奖励模型的新方法，使用样本复杂度与奖励类基数对数相关的学习算法。

Result: 证明了似然最大化方法在这种情况下可能失败，而新方法能够在样本复杂度上实现与奖励类基数对数的关系。

Conclusion: 从正确演示中学习时，需要超越传统的似然最大化方法，考虑基于低基数奖励模型的替代方法。
...
### [200] [Adversary-Free Counterfactual Prediction via Information-Regularized Representations](https://arxiv.org/abs/2510.15479)
...
<details>
  <summary>Details</summary>
Motivation: 在存在分配偏差的情况下进行反事实预测，传统方法如对抗训练存在不稳定性和调参负担，需要数学基础更扎实的方法。

Method: 学习一个随机表示Z，在预测结果的同时最小化I(Z;T)，推导出可处理的变分目标上界信息项，并与监督解码器耦合。

Result: 在受控数值模拟和真实临床数据集上评估，在似然度、反事实误差和政策评估等指标上表现优于现有方法。

Conclusion: 该方法在避免对抗训练不稳定性的同时，在多个评估指标上表现优异，为反事实预测提供了理论动机明确的解决方案。
...
### [201] [OffSim: Offline Simulator for Model-based Offline Inverse Reinforcement Learning](https://arxiv.org/abs/2510.15495)
*Woo-Jin Ahn,Sang-Ryul Baek,Yong-Jun Lee,Hyun-Duck Choi,Myo-Taeg Lim*

Main category: cs.LG

TL;DR: 提出OffSim框架，直接从专家轨迹中学习环境动态和奖励函数，实现无需真实环境交互的离线策略训练
...
Method: 联合优化高熵转移模型和基于逆强化学习的奖励函数，增强探索性并提高奖励泛化能力。还提出OffSim+扩展，在多数据集设置中引入边际奖励以增强探索

Result: 在MuJoCo实验中，OffSim相比现有离线IRL方法获得显著性能提升，证明了其有效性和鲁棒性

Conclusion: OffSim框架能够直接从专家轨迹中有效学习环境动态和奖励函数，实现离线策略训练，为强化学习提供更高效的解决方案
...
### [202] [The Road Less Traveled: Enhancing Exploration in LLMs via Sequential Sampling](https://arxiv.org/abs/2510.15502)
*Shijia Kang,Muhan Zhang*

Main category: cs.LG

TL;DR: 提出SESA框架，通过顺序采样生成多样化解决方案草图，解决强化学习中探索不足和熵崩溃问题，显著提升LLM的推理多样性和性能。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习方法存在探索有限和熵崩溃问题，模型倾向于利用狭窄的解决方案集，导致采样多样性丧失，阻碍性能进一步提升。并行采样方法中多个输出来自相同分布，更容易收敛到相似解。

Method: SESA框架：先生成多样化的解决方案草图，然后扩展为完整推理路径。通过顺序采样，每个新输出都基于前一个输出进行条件生成，确保整个过程的多样性，防止策略崩溃。

Result: 在合成任务中，顺序采样在路径多样性和从崩溃中恢复方面始终优于传统RL方法。在三个智能体基准测试中，SESA将成功率分别提升了+0.25、+0.42和+0.07绝对值（相对于基线RL最高达211%的相对提升）。
...
### [203] [Theoretical Refinement of CLIP by Utilizing Linear Structure of Optimal Similarity](https://arxiv.org/abs/2510.15508)
*Naoki Yoshida,Satoshi Hayakawa,Yuhta Takida,Toshimitsu Uesaka,Hiromi Wakaki,Yuki Mitsufuji*

Main category: cs.LG

TL;DR: 提出KME-CLIP方法，通过再生核希尔伯特空间中的内积来改进多模态对比预训练中的相似度计算机制，更好地逼近点互信息(PMI)的线性结构
...
Result: KME-CLIP在多个检索和分类任务中整体优于标准CLIP

Conclusion: 通过利用PMI的线性结构，提出的KME-CLIP方法有效提升了多模态对比学习的性能
...
### [204] [Language Models are Injective and Hence Invertible](https://arxiv.org/abs/2510.15511)
*Giorgos Nikolaou,Tommaso Mencattini,Donato Crisostomi,Andrea Santilli,Yannis Panagakis,Emanuele Rodola'*

Main category: cs.LG

TL;DR: 本文证明了Transformer语言模型在将离散输入序列映射到连续表示时是单射的（可逆的），这意味着可以从模型的隐藏激活中精确恢复原始输入文本。


<details>
  <summary>Details</summary>
Motivation: 传统观点认为Transformer组件如非线性激活和归一化是非单射的，可能导致不同输入映射到相同输出，但本文挑战了这一观点。

Method: 通过数学证明、在六个先进语言模型上进行数十亿次碰撞测试，并开发了SipIt算法来从隐藏激活中重构输入文本。

Result: 数学证明和实证测试均表明Transformer语言模型是单射的，SipIt算法能够在线性时间内精确重构输入文本。

Conclusion: 单射性是语言模型的基本且可利用的特性，对模型透明度、可解释性和安全部署有直接意义。
...
### [205] [Revisiting Knowledge Distillation: The Hidden Role of Dataset Size](https://arxiv.org/abs/2510.15516)
*Giulia Lanzillotta,Felix Sarnthein,Gil Kur,Thomas Hofmann,Bobby He*

Main category: cs.LG

TL;DR: 研究发现知识蒸馏在数据量较少时效果更显著，揭示了数据集大小是影响蒸馏机制的关键但被忽视的变量。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注模型大小和泛化能力，但缺乏对数据集大小如何影响知识蒸馏效果的深入理解。

Method: 通过在不同数据集、任务和神经网络架构上进行广泛实验，测试现有蒸馏理论在不同数据规模下的预测能力。

Result: 蒸馏效果在低数据量情况下不仅保持而且增强，证明蒸馏不能简单地理解为标签平滑，支持暗知识假说。

Conclusion: 数据集大小是理解知识蒸馏机制的基本但被忽视的变量，蒸馏具有数据效率特性。
...
### [206] [Compressive Modeling and Visualization of Multivariate Scientific Data using Implicit Neural Representation](https://arxiv.org/abs/2510.15535)
*Abhay Kumar Dwivedi,Shanu Saklani,Soumya Dutta*

Main category: cs.LG

TL;DR: 开发用于多变量数据集的压缩神经网络表示方法，通过参数共享实现高效数据压缩。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络在科学可视化任务中广泛应用，隐式神经表示在时空体积可视化和超分辨率等任务中展现出潜力，受此启发开发多变量数据的压缩表示。

Method: 使用单一网络通过参数共享同时学习所有数据变量的表示，实现参数共享的压缩表示。

Result: 在重建数据质量、渲染和可视化质量、变量间依赖关系保持以及存储效率方面展现出优越性能。

Conclusion: 该方法在多变量数据集压缩方面实现了最先进的性能，为科学可视化提供了高效的解决方案。
...
### [207] [An Empirical Study on MC Dropout--Based Uncertainty--Error Correlation in 2D Brain Tumor Segmentation](https://arxiv.org/abs/2510.15541)
*Saumya B*
...
<details>
  <summary>Details</summary>
Motivation: 研究MC Dropout不确定性是否能有效识别脑肿瘤MRI分割中的错误，特别是在肿瘤边界区域，这对医学图像分割的诊断和治疗规划至关重要。

Method: 使用U-Net在四种数据增强设置（无增强、水平翻转、旋转、缩放）下进行2D脑肿瘤MRI分割，通过50次随机前向传播计算不确定性，并使用Pearson和Spearman系数分析不确定性与像素级错误的相关性。

Result: 结果显示全局相关性较弱（r≈0.30-0.38），边界相关性可忽略（|r|<0.05）。虽然不同增强设置间的差异具有统计显著性（p<0.001），但缺乏实际意义。

Conclusion: MC Dropout不确定性在定位边界分割错误方面提供有限线索，强调了在医学图像分割中需要开发替代或混合不确定性估计方法的必要性。
...
### [208] [Doubly Robust Estimation of Causal Effects in Strategic Equilibrium Systems](https://arxiv.org/abs/2510.15555)
*Sibo Xiao*
...
Method: 集成战略均衡建模与双重稳健估计，在战略无混淆假设下进行理论分析。

Result: 实证评估显示SDR优于基线方法，在不同战略强度下实现7.6%-29.3%的偏差减少，并在代理群体规模增加时保持稳健可扩展性。
...
### [209] [On the Neural Feature Ansatz for Deep Neural Networks](https://arxiv.org/abs/2510.15563)
*Edward Tansley,Estelle Massart,Coralia Cartis*

Main category: cs.LG

TL;DR: 该论文扩展了神经特征假设(NFA)的理论分析，证明了在多层线性网络中NFA的指数与网络深度相关(α=1/L)，并研究了不平衡初始化和非线性激活函数对NFA的影响。


<details>
  <summary>Details</summary>
Motivation: 理解特征学习是建立深度神经网络数学基础的重要问题。神经特征假设(NFA)描述了训练后网络第一层权重Gram矩阵与平均梯度外积(AGOP)之间的关系，但之前只对两层线性网络有证明。

Method: 使用梯度流动力学和平衡权重初始化，理论分析扩展到L≥2层的线性网络。研究不平衡初始化下的渐近行为，并构建非线性激活函数的反例。通过数值实验验证理论结果。

Result: 证明对于L层线性网络，NFA在整个训练过程中成立且指数α=1/L。对于不平衡初始化，在权重衰减下NFA渐近成立。发现某些非线性网络架构即使完美拟合训练数据也不满足NFA。

Conclusion: NFA的指数与网络深度相关，为理解深度网络特征学习提供了理论依据。权重衰减有助于在不平衡初始化下维持NFA，但非线性激活可能破坏NFA的成立条件。
...
### [210] [Attn-JGNN: Attention Enhanced Join-Graph Neural Networks](https://arxiv.org/abs/2510.15583)
...
Method: 使用树分解将CNF公式编码为联合图，在联合图上执行迭代消息传递，通过学习分区函数来近似模型数量，并在联合图的簇内和簇间应用注意力机制。

Result: 实验表明Attn-JGNN模型相比其他神经网络方法取得了更好的结果。

Conclusion: 注意力机制使模型能够更关注概率推理中的关键变量和簇，减少了冗余计算，从而提高了#SAT问题的求解精度。
...
### [211] [GRATING: Low-Latency and Memory-Efficient Semantic Selection on Device](https://arxiv.org/abs/2510.15620)
...
<details>
  <summary>Details</summary>
Motivation: 语义top-K选择在边缘设备上的延迟和内存需求占主导地位，而实际应用中只需要相对排名而非精确分数，这为优化提供了机会。

Method: 提出整体前向传播方法，利用序列级稀疏性进行渐进式聚类剪枝，并通过双滑动窗口和分块执行策略重叠I/O与计算。

Result: 在0.6B到8B参数的reranker上，GRATING在微基准测试中延迟降低达89.0%，峰值内存降低达94.9%，在三个真实应用中延迟降低11.6%-51.0%，内存降低18.6%-77.8%。

Conclusion: GRATING显著提升了边缘设备上语义top-K选择的效率和可部署性，且不损失精度。
...
### [212] [CQD-SHAP: Explainable Complex Query Answering via Shapley Values](https://arxiv.org/abs/2510.15623)
...
<details>
  <summary>Details</summary>
Motivation: 现有神经符号复杂查询回答方法虽然可追踪中间结果，但无法解释查询各部分的重要性，存在可解释性不足的问题。

Method: 基于合作博弈论中的Shapley值，计算查询各部分对特定答案排名的贡献，满足所有基本Shapley公理。

Result: 在必要性和充分性解释的自动评估中，与多个基线方法相比，该方法对大多数查询类型都显示出有效性。

Conclusion: CQD-SHAP框架成功提升了复杂查询回答的可解释性，能够解释神经预测器相对于纯符号方法的价值。
...
### [213] [Decentralized Parameter-Free Online Learning](https://arxiv.org/abs/2510.15644)
...
Conclusion: 该算法家族为分布式应用提供了无需参数调优的高效去中心化在线学习解决方案。
...
TL;DR: 提出了NODE-ONet框架，一种结合神经ODE的算子学习方法，用于解决PDE问题，通过编码器-解码器架构和物理编码的神经ODE来提高准确性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有算子学习方法往往忽略PDE的领域知识，导致难以捕捉时间动态和超出训练时间范围的泛化问题。

Method: 采用编码器-解码器架构：编码器进行空间离散化，神经ODE捕捉潜在时间动态，解码器重构物理空间解。提出物理编码的神经ODE来融入PDE特定物理属性。

Result: 在非线性扩散-反应和Navier-Stokes方程上的数值实验显示出高精度、计算效率和超出训练时间范围的预测能力。

Conclusion: NODE-ONet框架具有适应不同编码器/解码器的灵活性，能够跨相关PDE族泛化，展示了其作为可扩展、物理编码的科学机器学习工具的潜力。
...
### [215] [Fast and Compact Tsetlin Machine Inference on CPUs Using Instruction-Level Optimization](https://arxiv.org/abs/2510.15653)
*Yefan Zeng,Shengyu Duan,Rishad Shafik,Alex Yakovlev*

Main category: cs.LG

TL;DR: 提出了一种高效的Tsetlin机器软件实现，通过位运算、提前退出机制和字面量重排序策略，在ARM处理器上实现了高达96.71%的推理时间减少。


<details>
  <summary>Details</summary>
Motivation: Tsetlin机器在资源受限设备上具有高速推理优势，其逻辑驱动操作天然适合现代CPU架构的并行执行，因此需要优化实现以充分发挥其性能潜力。

Method: 1) 使用指令级位运算实现紧凑模型表示和加速处理；2) 引入基于AND子句评估的提前退出机制避免不必要计算；3) 提出字面量重排序策略最大化提前退出概率，该策略在训练后、推理前阶段通过统计Tsetlin自动机行为实现。

Result: 在ARM处理器上的gem5模拟器实验显示，优化实现相比传统基于整数的TM实现减少了高达96.71%的推理时间，同时保持了相当的代码密度。

Conclusion: 提出的优化技术显著提升了Tsetlin机器的推理效率，使其更适合在资源受限设备上部署，同时保持了模型的紧凑性。
...
### [216] [WARP-LUTs - Walsh-Assisted Relaxation for Probabilistic Look Up Tables](https://arxiv.org/abs/2510.15655)
...
Method: 使用Walsh变换辅助的概率查找表(WARP-LUTs)，通过梯度下降学习逻辑门组合，大幅减少可训练参数。
...
### [217] [CarBoN: Calibrated Best-of-N Sampling Improves Test-time Reasoning](https://arxiv.org/abs/2510.15674)
*Yung-Chen Tang,Pin-Yu Chen,Andrea Cavallaro*

Main category: cs.LG

TL;DR: CarBoN是一个测试时校准框架，通过自适应调整模型参数来提升推理任务性能，相比传统Best-of-N采样方法更高效，能在相同准确率下减少4倍采样次数。


<details>
  <summary>Details</summary>
Motivation: 传统Best-of-N采样方法在N增加时收益递减，存在效率问题。需要一种无需重新训练大语言模型就能提升推理性能的方法。

Method: 提出两阶段方法：先探索解空间，然后学习输入特定的温度T和偏移向量δ来校准logits，引导生成更可靠的推理路径。

Result: 在MATH-500和AIME-2024数据集上，CarBoN达到相同准确率所需采样次数减少4倍，且在固定预算下通常获得更高准确率。

Conclusion: CarBoN框架有效提升了推理任务的测试时效率，温度T和偏移向量δ在平衡输出多样性和正确性方面发挥互补作用，该框架可推广到束搜索等步级采样策略。
...
### [218] [KS-Net: Multi-layer network model for determining the rotor type from motor parameters in interior PMSMs](https://arxiv.org/abs/2510.15688)
*Kivanc Dogan,Ahmet Orhan*

Main category: cs.LG

TL;DR: 该研究使用机器学习方法基于电磁参数对IPMSM转子形状进行分类，开发了KS-Net深度学习模型并与多种传统算法比较，证明了数据驱动方法可作为有限元分析的高效替代方案。


<details>
  <summary>Details</summary>
Motivation: 传统使用有限元方法分析IPMSM转子形状计算成本高，需要开发更快速、成本更低的替代方法。

Method: 使用自定义深度学习模型KS-Net，与Cubic SVM、Quadratic SVM、Fine KNN、Cosine KNN和Fine Tree算法进行比较，在包含9,000个样本的平衡数据集上采用10折交叉验证。

Result: Cubic SVM和Quadratic SVM算法实现100%准确率，KS-Net模型达到99.98%准确率（仅2个误分类），与传统方法具有竞争力。

Conclusion: 数据驱动方法能够高精度预测IPMSM转子形状，为加速电机设计过程、开发自动转子识别系统和实现数据驱动的故障诊断提供了基础。
...
### [219] [Constrained Adversarial Perturbation](https://arxiv.org/abs/2510.15699)
...
<details>
  <summary>Details</summary>
Motivation: 现有通用对抗扰动方法忽略了领域特定约束，违反这些约束会导致对抗样本不可信或易被检测，限制了实际应用价值。

Method: 提出CAP算法，基于增强拉格朗日最小最大优化问题，使用梯度交替优化策略来强制执行多个复杂约束。

Result: 在金融、IT网络和网络物理系统等多个领域评估，CAP实现了更高的攻击成功率，同时显著减少了运行时间。

Conclusion: 该方法能有效处理约束特征空间中的对抗攻击，并提出了从数据中学习特征约束的原则性方法，适用于具有结构化输入空间的广泛领域。
...
### [220] [ProofOptimizer: Training Language Models to Simplify Proofs without Human Demonstrations](https://arxiv.org/abs/2510.15700)
*Alex Gu,Bartosz Piotrowski,Fabian Gloeckle,Kaiyu Yang,Aram H. Markosyan*

Main category: cs.LG

TL;DR: ProofOptimizer是一个通过专家迭代和强化学习训练的语言模型，专门用于简化Lean证明，无需人工监督，能显著压缩证明长度并提升验证效率。


<details>
  <summary>Details</summary>
Motivation: 神经定理证明生成的证明过长，难以理解且限制数学洞察力，而现有方法在处理RL训练证明器生成的极长证明时效果不佳，且缺乏训练数据。

Method: 通过专家迭代和强化学习训练语言模型，使用Lean验证简化并提供训练信号，在推理时采用迭代证明缩短工作流程逐步减少证明长度。

Result: 在标准基准测试中显著压缩证明长度：miniF2F减少87%，PutnamBench减少57%，IMO 2025证明减少49%，同时提升Lean验证速度并改善下游证明器性能。

Conclusion: ProofOptimizer是首个无需人工监督的Lean证明简化模型，能有效解决证明过长问题，提升证明的可理解性和实用性。
...
### [221] [ProSh: Probabilistic Shielding for Model-free Reinforcement Learning](https://arxiv.org/abs/2510.15720)
*Edwin Hamel-De le Court,Gaspard Ohlmann,Francesco Belardinelli*

Main category: cs.LG

TL;DR: ProSh是一种无模型的安全强化学习算法，通过风险预算增强状态空间，并使用学习到的成本评论家对策略分布应用屏蔽，确保所有采样动作在期望值上保持安全。
...
Method: 通过风险预算增强约束MDP状态空间，使用学习到的成本评论家对策略分布应用屏蔽，确保采样动作在期望值上安全。

Result: 在确定性环境中保持最优性，提供仅依赖于备份评论家准确度的成本期望值紧上界，实验显示在训练时也能保证安全性。

Conclusion: ProSh算法能够在无模型设置下提供形式化的安全保证，在训练和部署阶段都能确保强化学习系统的安全性。
...
### [222] [RLAF: Reinforcement Learning from Automaton Feedback](https://arxiv.org/abs/2510.15728)
*Mahyar Alinejad,Alvaro Velasquez,Yue Wang,George Atia*

Main category: cs.LG

TL;DR: 提出了一种基于自动机反馈的强化学习方法，用DFA生成的轨迹偏好替代显式奖励函数，无需手动设计奖励，在时序依赖任务中优于传统方法。
...
### [223] [A Comprehensive Evaluation of Graph Neural Networks and Physics Informed Learning for Surrogate Modelling of Finite Element Analysis](https://arxiv.org/abs/2510.15750)
*Nayan Kumar Singh*
...
Method: 采用图神经网络和3D U-Net作为FEA替代模型，引入基于Navier-Cauchy方程的物理信息神经网络框架，并使用课程学习策略（先预训练再物理信息微调）来稳定训练。

Result: GNN显著优于U-Net，最差的GCN框架相对L2误差为8.7%，而最佳U-Net为13.0%。MPNN和Graph Transformer分别达到3.5%和2.6%的误差。PINN框架将误差降低达11.3%。

Conclusion: 虽然Graph Transformer最准确，但推理速度比MPNN慢37.5%。PINN增强的MPNN提供了最佳实用解决方案，在预测性能、模型大小和推理速度之间取得了良好平衡。
...
### [224] [SAMix: Calibrated and Accurate Continual Learning via Sphere-Adaptive Mixup and Neural Collapse](https://arxiv.org/abs/2510.15751)
*Trung-Anh Dang,Vincent Nguyen,Ngoc-Son Vu,Christel Vrain*

Main category: cs.LG

TL;DR: 提出Sphere-Adaptive Mixup (SAMix)方法，通过自适应混合策略改进基于神经崩溃的持续学习方法，不仅提升校准度还提高性能。


<details>
  <summary>Details</summary>
Motivation: 现有持续学习方法主要关注缓解遗忘和提高准确率，但忽视了网络校准这一重要方面。神经崩溃现象在持续学习中具有优势，但很少有工作专门改进持续学习模型的校准度以实现更可靠的预测。

Method: 提出Sphere-Adaptive Mixup (SAMix)方法，这是一种针对神经崩溃方法定制的自适应混合策略。SAMix根据神经崩溃下特征空间的几何特性调整混合过程，确保更鲁棒的规范化和对齐。

Result: 实验表明，SAMix显著提升了性能，在持续学习中超越了最先进方法，同时改进了模型校准度。SAMix提高了跨任务准确率和预测的可靠性。
...
### [225] [Poultry Farm Intelligence: An Integrated Multi-Sensor AI Platform for Enhanced Welfare and Productivity](https://arxiv.org/abs/2510.15757)
...
TL;DR: PoultryFI是一个模块化、低成本的AI平台，集成了六个AI模块，通过优化摄像头布局、音视频监控、实时鸡蛋计数等功能，帮助中小型家禽养殖场实现连续监测和智能决策。


<details>
  <summary>Details</summary>
Motivation: 中小型家禽养殖场缺乏经济实惠的集成工具进行连续监测和决策，主要依赖手动、被动检查，难以同时满足生产力目标和动物福利要求。

Method: 使用进化算法优化摄像头布局实现全覆盖；通过同步视频、音频和喂食数据提取福利指标；边缘视觉模型实现实时鸡蛋计数；预测模型进行产量和饲料消耗预测；集成天气预报提供操作建议。
...
### [226] [Cavity Duplexer Tuning with 1d Resnet-like Neural Networks](https://arxiv.org/abs/2510.15796)
*Anton Raskovalov*

Main category: cs.LG

TL;DR: 提出了一种用于调谐具有大量调节螺钉的空腔双工器的机器学习方法，使用监督学习框架和1D ResNet架构，结合S参数形状特征处理


<details>
  <summary>Details</summary>
Motivation: 解决传统强化学习方法在调谐具有大量调节螺钉的空腔双工器时效果不佳的问题

Method: 采用监督学习框架，构建包含1D ResNet骨干网络和S参数形状特征（曲线形状、峰值位置和幅度）处理的神经网络架构，配合外部控制算法

Result: 该神经网络能够在每个螺钉旋转4-5次后达到几乎调谐状态

Conclusion: 所提出的监督学习方法比传统强化学习方法更有效，能够高效调谐具有大量调节螺钉的空腔双工器
...
### [227] [AB-UPT for Automotive and Aerospace Applications](https://arxiv.org/abs/2510.15808)
*Benedikt Alkin,Richard Kurle,Louis Serrano,Dennis Just,Johannes Brandstetter*

Main category: cs.LG

TL;DR: AB-UPT模型在汽车和飞机CFD模拟中表现出色，比传统数值求解器计算量少几个数量级，可在单GPU上一天内训练完成，为工业规模应用铺平道路。


<details>
  <summary>Details</summary>
Motivation: 扩展AB-UPT模型在汽车和飞机计算流体动力学模拟中的实证评估用例，验证其在工业规模应用中的潜力。

Method: 使用Luminary Cloud平台生成SHIFT-SUV（汽车）和SHIFT-Wing（飞机）两个高质量数据集，并采用AB-UPT模型进行训练和评估。

Result: AB-UPT在两个数据集上都优于之前的transformer基线模型，能够近乎完美地预测集成空气动力，仅需几秒钟即可从简单的几何表示中获得结果。

Conclusion: AB-UPT在汽车和飞机CFD模拟中表现优异，计算效率极高，有望实现工业规模的应用部署。
...
### [228] [Chronos-2: From Univariate to Universal Forecasting](https://arxiv.org/abs/2510.15821)
...
Method: 使用组注意力机制促进跨多个时间序列的上下文学习，通过在合成数据集上进行训练来获得通用能力。
...
### [229] [SNOO: Step-K Nesterov Outer Optimizer - The Surprising Effectiveness of Nesterov Momentum Applied to Pseudo-Gradients](https://arxiv.org/abs/2510.15830)
...
<details>
  <summary>Details</summary>
Motivation: DiLoCo优化器在非分布式设置中表现出色，作者发现其有效性主要源于对伪梯度应用Nesterov动量，因此希望验证这一机制并开发更高效的优化器。

Method: 提出Step-K Nesterov Outer Optimizer (SNOO)，在Lookahead双循环框架中，对多个内优化器步骤产生的伪梯度应用Nesterov动量来更新慢权重。

Result: SNOO在非分布式设置中达到1.5-2.5倍的计算效率提升，训练FLOPs规模达1e23，且改进效果随模型规模增大而增强。
...
### [230] [FIDDLE: Reinforcement Learning for Quantum Fidelity Enhancement](https://arxiv.org/abs/2510.15833)
*Hoang M. Ngo,Tamer Kahveci,My T. Thai*

Main category: cs.LG

TL;DR: 提出了FIDDLE框架，通过高斯过程代理模型和强化学习直接优化量子电路路由阶段的保真度，显著提升了量子电路的可靠性。


<details>
  <summary>Details</summary>
Motivation: 当前量子设备受噪声影响，可靠性降低。在量子电路编译过程中，路由阶段对过程保真度的优化是关键挑战。传统方法依赖间接指标如电路深度或门数量，无法直接最大化过程保真度。

Method: FIDDLE框架包含两个模块：基于高斯过程的代理模型用于在有限训练样本下估计过程保真度，以及强化学习模块用于优化路由。这是首个直接最大化过程保真度的方法。

Result: 与现有保真度估计技术和路由优化方法相比，FIDDLE的代理模型能提供更好的过程保真度估计，端到端框架在各种噪声模型下显著提高了量子电路的过程保真度。

Conclusion: FIDDLE框架成功解决了量子电路路由阶段的保真度最大化问题，通过直接优化过程保真度而非依赖间接指标，显著提升了量子电路的可靠性。
...
### [231] [Transfer Orthology Networks](https://arxiv.org/abs/2510.15837)
*Vikash Singh*
...
<details>
  <summary>Details</summary>
Motivation: 解决跨物种迁移学习问题，利用转录组数据在不同物种间有效传递知识，为生物信息学提供更有效的工具。

Method: 构建基于同源关系的二分图，在预训练的前馈神经网络前添加物种转换层，该层权重通过二分图的邻接矩阵进行掩码，学习源物种到目标物种基因表达的线性变换。

Result: TRON提供了生物学基础和可解释的跨物种迁移学习方法，能够有效利用现有转录组数据，转换层学习到的权重可用于解释功能同源性。
...
### [232] [Learning Correlated Reward Models: Statistical Barriers and Opportunities](https://arxiv.org/abs/2510.15839)
*Yeshwanth Cherapanamjeri,Constantinos Daskalakis,Gabriele Farina,Sobhan Mohammadpour*

Main category: cs.LG

TL;DR: 本文研究了学习相关probit模型的统计和计算挑战，发现传统的成对偏好数据无法学习相关性信息，而三选一偏好数据能够克服这一局限，并提出了一种高效估计器。
...
Method: 研究相关probit模型的学习挑战，分析成对偏好数据的局限性，提出使用三选一偏好数据，并开发高效的估计器。

Result: 理论证明三选一偏好数据能够有效学习相关性信息，实验验证在多个真实数据集上改善了人类偏好的个性化建模。

Conclusion: 高阶偏好数据在学习相关效用方面具有优势，能够实现更精细的人类偏好建模。
...
### [233] [Self-Certifying Primal-Dual Optimization Proxies for Large-Scale Batch Economic Dispatch](https://arxiv.org/abs/2510.15850)
*Michael Klamkin,Mathieu Tanneau,Pascal Van Hentenryck*

Main category: cs.LG

TL;DR: 提出一种混合求解器，结合优化代理和经典求解器，利用对偶理论来保证最优性，在无法认证最优性时回退到经典求解器，实现1000倍加速同时保证最大2%最优性差距。
...
Method: 提出混合求解器，利用对偶理论高效预测最优性差距边界，对无法认证最优性的查询回退到经典求解器。改进训练过程，结合原始和对偶代理训练以提高加速效果。

Result: 在大规模传输系统上的实验显示，混合求解器具有高度可扩展性，相比并行化单纯形求解器实现超过1000倍加速，同时保证最大2%最优性差距。

Conclusion: 该混合求解器在速度和最优性之间实现了可解释的权衡，为优化代理在实际部署中提供了可信性保证。
...
### [234] [OpenEstimate: Evaluating LLMs on Reasoning Under Uncertainty with Real-World Data](https://arxiv.org/abs/2510.15096)
...
Result: 在六个前沿语言模型上的测试表明，模型产生的先验分布通常不准确且过于自信，性能仅随不确定性提取方式略有改善，不受采样策略、推理努力或提示设计的影响。
...
### [235] [Procedural Game Level Design with Deep Reinforcement Learning](https://arxiv.org/abs/2510.15120)
*Miraç Buğra Özkan*

Main category: cs.AI

TL;DR: 提出了一种使用深度强化学习在Unity 3D环境中进行程序化关卡设计的新方法，包含两个智能体：蜂鸟（求解器）和浮岛（生成器），通过PPO算法训练实现自主内容生成和解决。


<details>
  <summary>Details</summary>
Motivation: 程序化内容生成在游戏开发中越来越受欢迎，但传统方法缺乏智能性和适应性。本研究旨在探索深度强化学习如何实现更智能、自适应的游戏关卡设计。

Method: 使用Unity ML-Agents工具包，采用PPO算法训练两个智能体：蜂鸟智能体学习导航和收集花朵，浮岛智能体学习基于障碍物位置和蜂鸟初始状态生成花朵布局。

Result: 系统产生了涌现行为，在各种环境配置中表现出强大的泛化能力，不仅生成了有效的智能体行为，还为自主游戏关卡设计开辟了新机会。

Conclusion: 这项工作展示了深度强化学习在虚拟环境中实现智能内容生成和解决的潜力，推动了AI在创意游戏开发过程中的贡献边界。
...
### [236] [Towards Error Centric Intelligence I, Beyond Observational Learning](https://arxiv.org/abs/2510.15128)
*Marcus A. Thomas*

Main category: cs.AI

TL;DR: 该论文认为AGI发展的瓶颈在于理论而非数据或规模，提出了基于批判理性主义的因果机制方法，强调假设空间变化作为首要操作，通过模块化干预和组合自主原则实现错误发现和修正。


<details>
  <summary>Details</summary>
Motivation: 挑战柏拉图表示假说，指出仅靠观测充分性无法保证干预能力，因为观测等价的世界在干预下可能产生分歧，需要从观测学习转向错误中心的方法。

Method: 提出因果机制方法，将假设空间变化作为首要操作，引入局部性和自主性原则、独立因果机制和组合自主原则，提供可操作的诊断工具。

Result: 建立了使错误发现和修正可行的结构原则框架，包括模块化干预的微分局部性和自主性原则、规范不变形式的独立因果机制，以及保持类比的组合自主原则。

Conclusion: 目标是构建一个系统框架，能够将不可达错误转化为可达错误并进行修正，为AGI发展提供理论支撑。
...
### [237] [HugAgent: Evaluating LLMs in Simulating Human-Like Individual Reasoning on Open-Ended Tasks](https://arxiv.org/abs/2510.15144)
...
TL;DR: HugAgent是一个用于评估模型从群体推理到个体推理适应能力的基准测试，包含合成和人类双轨设计，旨在让机器推理更贴近人类个体思维。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型主要模拟群体共识，缺乏对个体推理风格和信念轨迹的个性化捕捉，需要开发能更好模拟人类个体推理的方法。

Method: 采用双轨设计：合成轨道用于大规模系统压力测试，人类轨道提供生态有效的"出声思考"推理数据，评估模型对特定个体在新型情境中的推理和信念更新预测能力。

Result: 实验表明最先进的大语言模型在个体适应方面仍存在持续差距，HugAgent成为首个可扩展的机器推理与人类个体思维对齐基准。

Conclusion: HugAgent基准测试为推进机器推理向更人性化方向发展提供了重要工具，填补了个体推理适应评估的空白。
...
### [238] [Build Your Personalized Research Group: A Multiagent Framework for Continual and Interactive Science Automation](https://arxiv.org/abs/2510.15624)
*Ed Li,Junyu Ren,Xintian Pan,Cat Yan,Chuanhao Li,Dirk Bergemann,Zhuoran Yang*

Main category: cs.AI

TL;DR: 提出了freephdlabor——一个开源多智能体框架，通过完全动态的工作流程和模块化架构实现自适应科学发现，解决了现有系统工作流程僵化和上下文管理不足的问题。
...
Method: 采用完全动态的工作流程（由实时智能体推理决定）、模块化架构（允许用户修改、添加或移除智能体）、自动上下文压缩、基于工作空间的通信、跨会话内存持久化以及非阻塞人工干预机制。

Result: 该框架将自动化研究从孤立的单次尝试转变为持续的研究计划，能够系统性地建立在先前探索基础上并整合人类反馈。
...
### [239] [WELD: A Large-Scale Longitudinal Dataset of Emotional Dynamics for Ubiquitous Affective Computing](https://arxiv.org/abs/2510.15221)
*Xiao Sun*

Main category: cs.AI

TL;DR: 提出了一个包含733,651条面部表情记录的大规模纵向工作场所情绪数据集，收集自38名员工在30.5个月内的真实办公环境，包含7种情绪概率和32种扩展情绪指标。


<details>
  <summary>Details</summary>
Motivation: 解决真实工作环境中大规模、纵向情绪识别数据稀缺的问题，特别是在自然环境中收集的情绪数据不足。
...
### [240] [AURA: An Agent Autonomy Risk Assessment Framework](https://arxiv.org/abs/2510.15739)
*Lorenzo Satta Chiris,Ayush Mishra*

Main category: cs.AI

TL;DR: AURA是一个统一的框架，用于检测、量化和减轻自主AI代理带来的风险，通过gamma风险评分方法平衡评估准确性与计算效率，支持人机协作监督。


<details>
  <summary>Details</summary>
Motivation: 随着自主AI代理系统在组织中日益普及，对齐、治理和风险管理方面的持续挑战阻碍了大规模部署，需要有效的风险评估框架。

Method: 采用gamma风险评分方法，提供交互式流程来评分、评估和减轻AI代理风险，支持人机协作监督和代理到人类通信机制，与现有协议和工具互操作。

Result: AURA框架能够有效检测和减轻自主AI代理风险，同时平衡计算资源，支持大规模、可治理的企业级AI代理部署。

Conclusion: AURA是促进企业环境中大规模、可治理自主AI代理部署的关键推动者，支持负责任和透明的AI采用。
...
### [241] [From Checklists to Clusters: A Homeostatic Account of AGI Evaluation](https://arxiv.org/abs/2510.15236)
*Brett Reynolds*

Main category: cs.AI

TL;DR: 本文提出AGI评估应基于稳态属性集群理论，采用因果中心性权重和跨会话持久性测试，而非传统的对称权重和快照评分，以更好衡量通用智能。
...
Conclusion: 通用智能应被视为稳态属性集群，评估应关注能力在扰动下的共存机制，而非孤立的能力快照。
...
### [242] [Multi-dimensional Data Analysis and Applications Basing on LLM Agents and Knowledge Graph Interactions](https://arxiv.org/abs/2510.15258)
*Xi Wang,Xianyao Ling,Kun Li,Gang Yin,Liang Zhang,Jiang Wu,Jun Xu,Fu Zhang,Wenbo Lei,Annie Wang,Peng Gong*

Main category: cs.AI

TL;DR: 提出了一种基于LLM智能体与知识图谱交互的多维数据分析方法，构建动态协作分析生态系统，实现从非结构化数据自动提取产品数据、实时构建可视化知识图谱，并支持用户深度探索分析。
...
Method: 利用LLM智能体自动从非结构化数据提取产品数据，实时构建和可视化知识图谱，通过交互平台支持用户对图节点进行深度探索分析。
...
### [243] [Experience-Driven Exploration for Efficient API-Free AI Agents](https://arxiv.org/abs/2510.15259)
*Chenwei Tang,Jingyu Xing,Xinyu Liu,Zizhou Wang,Jiawei Du,Liangli Zhen,Jiancheng Lv*

Main category: cs.AI

TL;DR: KG-Agent是一个经验驱动的学习框架，通过将像素级GUI交互构建为状态-动作知识图，解决了API缺失环境下LLM智能体的效率瓶颈问题。
...
Method: 提出KG-Agent框架，将原始像素级交互构建为持久的状态-动作知识图(SA-KG)。通过连接功能相似但视觉不同的GUI状态，形成丰富的经验邻域。设计了基于图拓扑的混合内在奖励机制，结合状态价值奖励和探索新颖性奖励，将战略规划与纯发现解耦。

Result: 在Civilization V和Slay the Spire两个复杂的开放GUI决策环境中进行评估，相比最先进方法，在探索效率和战略深度方面展现出显著改进。

Conclusion: KG-Agent通过结构化经验知识图有效解决了无API环境下智能体的探索效率和长期规划问题，为GUI交互智能体提供了更有效的学习框架。
...
### [244] [AUGUSTUS: An LLM-Driven Multimodal Agent System with Contextualized User Memory](https://arxiv.org/abs/2510.15261)
*Jitesh Jain,Shubham Maheshwari,Ning Yu,Wen-mei Hwu,Humphrey Shi*

Main category: cs.AI

TL;DR: AUGUSTUS是一个多模态代理系统，采用基于认知科学中人类记忆理念的图结构多模态上下文记忆，通过语义标签和上下文关联实现高效概念驱动检索，在性能和速度上优于传统多模态RAG方法。


<details>
  <summary>Details</summary>
Motivation: 现有代理系统主要存储文本信息，忽略了多模态信号的重要性。受人类记忆多模态特性的启发，希望开发一个更符合认知科学原理的多模态代理系统。

Method: 系统包含4个循环阶段：编码（理解输入）、存储记忆（保存重要信息）、检索（从记忆中搜索相关上下文）和执行（完成任务）。使用语义标签概念化信息，并将其与上下文关联存储在图结构的多模态上下文记忆中，实现概念驱动检索。

Result: 在ImageNet分类任务中比传统多模态RAG方法快3.5倍，在MSC基准测试中优于MemGPT。

Conclusion: AUGUSTUS系统通过图结构多模态上下文记忆和语义标签方法，在多模态代理任务中实现了更好的性能和效率，验证了基于人类记忆认知原理的系统设计优势。
...
### [245] [Preliminary Quantitative Study on Explainability and Trust in AI Systems](https://arxiv.org/abs/2510.15769)
*Allen Daniel Sunny*

Main category: cs.AI

TL;DR: 研究通过实验设计探讨AI可解释性与用户信任的关系，发现交互式解释能增强用户参与度和信任度，解释的清晰度和相关性是信任的关键决定因素。


<details>
  <summary>Details</summary>
Motivation: 随着GPT-4等大规模AI模型在关键领域部署，关于AI信任和透明度的紧迫问题日益突出，需要研究可解释性如何影响用户信任。

Method: 采用定量实验设计，通过基于网络的交互式贷款审批模拟，比较从基本特征重要性到交互式反事实解释等不同类型的解释方式。
...
### [246] [WebGen-V Bench: Structured Representation for Enhancing Visual Design in LLM-based Web Generation and Evaluation](https://arxiv.org/abs/2510.15306)
*Kuang-Da Wang,Zhao Wang,Yotaro Shimose,Wei-Yao Wang,Shingo Takamatsu*

Main category: cs.AI

TL;DR: WebGen-V是一个用于指令到HTML生成的新基准和框架，通过智能爬虫收集真实网页数据，采用结构化分段表示，并提供细粒度多模态评估。
...
Method: 提出三个关键创新：1）无限制可扩展的智能爬虫框架收集真实网页；2）结构化分段数据表示，整合元数据、局部UI截图和JSON格式资源；3）分段级多模态评估协议。

Result: 通过最先进LLM的实验和消融研究验证了结构化数据和分段评估的有效性，以及各组件的重要性。

Conclusion: WebGen-V是首个实现细粒度智能爬虫和评估的指令到HTML生成工作，提供了从真实数据采集到结构化多模态评估的统一流程。
...
### [247] [VERITAS: Leveraging Vision Priors and Expert Fusion to Improve Multimodal Data](https://arxiv.org/abs/2510.15317)
*Tingqiao Xu,Ziru Zeng,Jiayu Chen*

Main category: cs.AI

TL;DR: VERITAS是一个通过整合视觉先验和多模态大模型来提升监督微调数据质量的系统化流程，能有效减少事实错误和幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大模型的数据增强方法常因视觉感知不足导致事实错误和幻觉，需要提升SFT数据质量。

Method: 利用视觉识别模型和OCR提取视觉先验，结合三个LMM评估原始答案并生成批判性理由，通过统计融合得到共识分数作为真值，训练轻量级批判模型，最终选择得分最高的精炼答案。

Result: 在六个多模态基准测试中，使用VERITAS处理数据微调的模型表现优于使用原始数据的模型，尤其在文本丰富和细粒度推理任务中。批判模型能力接近最先进LMM但效率更高。

Conclusion: VERITAS能有效提升多模态数据质量，释放了流程、数据集和模型检查点以推动多模态数据优化研究。
...
### [248] [Towards Flash Thinking via Decoupled Advantage Policy Optimization](https://arxiv.org/abs/2510.15374)
*Zezhong Tan,Hang Gao,Xinhong Ma,Feng Zhang,Ziqiang Dong*

Main category: cs.AI

TL;DR: DEPO是一个新颖的强化学习框架，旨在减少大型推理模型中的低效推理，通过解耦优势算法、难度感知长度惩罚和优势裁剪方法，显著缩短响应序列长度39%，同时保持或提高模型准确性。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习算法虽然提高了模型准确性，但存在响应过长和过度思考问题，导致推理延迟和计算消耗增加，特别是在简单任务中需要最小推理的情况下。

Method: DEPO框架包含三个核心组件：(1) 创新的优势解耦算法，指导模型减少低效token；(2) 难度感知长度惩罚，降低模型响应整体长度；(3) 优势裁剪方法，防止策略优化中的偏差。

Result: 在DeepSeek-Distill-Qwen-7B和1.5B模型上的实验表明，DEPO实现了序列长度显著减少39%，减少了低效token中的过度推理路径，同时在整体准确性上优于基础模型。

Conclusion: DEPO框架有效解决了大型推理模型中的低效推理问题，在保持准确性的同时显著减少了响应长度和计算消耗。
...
### [249] [Advancing Routing-Awareness in Analog ICs Floorplanning](https://arxiv.org/abs/2510.15387)
...
<details>
  <summary>Details</summary>
Motivation: 解决模拟集成电路布局中布线感知布局方案的迫切需求，克服传统方法在电气约束和布局布线相互依赖方面的限制。

Method: 使用强化学习和关系图卷积神经网络构建自动布局引擎，结合高网格分辨率、精确引脚信息和动态布线资源估计技术。

Result: 相比现有学习型最优技术，死区减少13.8%，线长减少40.6%，布线成功率提高73.4%。

Conclusion: 该方法在平衡布线效率和面积利用率方面达到工业标准，为模拟IC布局提供了有效的布线感知解决方案。
...
### [250] [Corrigibility Transformation: Constructing Goals That Accept Updates](https://arxiv.org/abs/2510.15395)
*Rubi Hudson*

Main category: cs.AI

TL;DR: 本文提出了可修正性目标的概念，通过一种转换方法将任何目标转化为可修正版本，确保AI不会抵制训练更新或关闭，同时保持性能。
...
Method: 通过短视地获取在无成本阻止更新条件下的奖励预测，构建可修正目标版本。该方法可递归扩展到新创建的代理，并防止代理故意修改目标。

Result: 两个网格世界实验表明，这些可修正目标可以被有效学习，并产生期望的行为。

Conclusion: 提出的转换方法能够创建既具有可修正性又与非可修正替代方案具有竞争力的目标，这对于AI安全至关重要。
...
### [251] [MARS: Reinforcing Multi-Agent Reasoning of LLMs through Self-Play in Strategic Games](https://arxiv.org/abs/2510.15414)
...
TL;DR: MARS是一个端到端的强化学习框架，通过自博弈训练LLM在多智能体系统中进行推理，在合作和竞争游戏中都能提升性能并实现泛化。
...
Result: 在保留游戏中性能提升达28.7%，在推理基准测试中，集成到领先多智能体系统后在AIME上提升10.0%，在GPQA-Diamond上提升12.5%。
...
### [252] [Adaptive Minds: Empowering Agents with LoRA-as-Tools](https://arxiv.org/abs/2510.15416)
...
<details>
  <summary>Details</summary>
Motivation: 传统方法依赖单一微调模型或基于规则的固定路由，缺乏灵活性和适应性。需要一种既能提供专业化响应又能保持对话能力的系统。

Method: 使用LangGraph进行工作流管理，基础LLM分析查询语义并动态选择最相关的LoRA适配器工具，结合多智能体编排和参数高效微调技术。

Result: 系统能够准确提供专业化响应，同时保持对话能力，支持API和Web接口，完全开源。

Conclusion: Adaptive Minds为领域自适应AI助手提供了可扩展和可扩展的基础架构，结合了多智能体编排的灵活性和参数高效微调的效率。
...
### [253] [Taming the Judge: Deconflicting AI Feedback for Stable Reinforcement Learning](https://arxiv.org/abs/2510.15514)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在强化学习中面临判断不一致性问题，特别是偏好循环等逻辑一致性问题尚未得到充分解决，这会影响训练的稳定性。

Method: 开发了CDR指标来量化判断冲突，并提出了DGR框架：构建偏好图，将其转换为无冲突的有向无环图(DAG)，生成逻辑一致的奖励信号，兼容任何策略优化器。
...
### [254] [Hypergraph Contrastive Sensor Fusion for Multimodal Fault Diagnosis in Induction Motors](https://arxiv.org/abs/2510.15547)
*Usman Ali,Ali Zia,Waqas Ali,Umer Ramzan,Abdul Rehman,Muhammad Tayyab Chaudhry,Wei Xiang*

Main category: cs.AI

TL;DR: 提出MM-HCAN多模态超图对比注意力网络，用于电机故障诊断，整合对比学习和超图拓扑，实现多传感器融合和多故障类型同时诊断，在真实基准测试中达到99.82%准确率。


<details>
  <summary>Details</summary>
Motivation: 传统方法难以捕捉复杂多模态信号关系，局限于单模态数据或单一故障类型，且在噪声或跨域条件下性能下降。需要一种能够同时诊断轴承、定子和转子故障的鲁棒诊断框架。

Method: MM-HCAN整合对比学习和超图拓扑，专门设计用于多模态传感器融合，能够联合建模模态内和模态间依赖关系，超越欧几里得嵌入空间增强泛化能力。

Result: 在三个真实世界基准测试中，MM-HCAN达到最高99.82%的准确率，具有强大的跨域泛化能力和噪声鲁棒性，适合实际部署。消融研究验证了各组件的贡献。
...
### [255] [JudgeSQL: Reasoning over SQL Candidates with Weighted Consensus Tournament](https://arxiv.org/abs/2510.15560)
*Jiayuan Bai,Xuan-guang Pan,Chongyang Tao,Shuai Ma*

Main category: cs.AI

TL;DR: JudgeSQL是一个通过结构化推理和加权共识锦标赛机制来改进SQL候选查询选择的框架，解决了现有方法在语义区分和推理链稳定性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有SQL候选选择方法（如自一致性和最佳N解码）只能提供浅层信号，容易导致评分不一致、推理链脆弱，无法捕捉相近SQL候选之间的细粒度语义差异。
...
### [256] [Context-aware deep learning using individualized prior information reduces false positives in disease risk prediction and longitudinal health assessment](https://arxiv.org/abs/2510.15591)
...
Result: 整合历史背景将假阳性转为真阴性，总体特异性提高同时保持高灵敏度。整合多达三次先前影像检查时，假阳性率从51%降至33%，加入临床数据后进一步降至24%。预测五年内风险时，假阳性率从64%降至9%。

Conclusion: 随时间收集的信息提供了相关背景，可增强医学风险预测的特异性，为扩大低基线风险人群的纵向健康监测项目提供了途径，从而实现早期检测和改善健康结果。
...
### [257] [Unleashing Scientific Reasoning for Bio-experimental Protocol Generation via Structured Component-based Reward Mechanism](https://arxiv.org/abs/2510.15600)
...
Method: 提出"Sketch-and-Fill"范式分离分析、结构和表达步骤，结合结构化组件奖励机制评估步骤粒度、行动顺序和语义保真度，并通过分阶段知识到行动过程训练Thoth模型。
...
### [258] [Direct Preference Optimization with Unobserved Preference Heterogeneity: The Necessity of Ternary Preferences](https://arxiv.org/abs/2510.15716)
*Keertana Chidambaram,Karthik Vinary Seetharaman,Vasilis Syrgkanis*

Main category: cs.AI

TL;DR: 本文提出了一种改进的RLHF方法，通过引入多响应排序和考虑用户异质性来解决传统偏好学习中的两个关键限制。
...
Method: 1) 将偏好学习与计量经济学文献联系，证明二元比较不足以识别潜在用户偏好；2) 开发基于EM算法的DPO变体来发现潜在标注者类型；3) 提出使用最小最大遗憾公平准则的聚合算法。

Result: 建立了生成模型对齐中公平性和个性化的理论和算法框架，确保在多样用户群体中的公平性能保证。

Conclusion: 通过引入多响应排序和异质性偏好建模，为生成模型对齐提供了更公平和个性化的解决方案。
...
### [259] [Invoice Information Extraction: Methods and Performance Evaluation](https://arxiv.org/abs/2510.15727)
...
Result: 建立了包含字段级精度、一致性检查失败和完全匹配准确率的评估框架。

Conclusion: 提出的指标为比较不同提取方法提供了标准化方式，并能突出字段特定性能的优缺点。
...
### [260] [Towards Relaxed Multimodal Inputs for Gait-based Parkinson's Disease Assessment](https://arxiv.org/abs/2510.15748)
*Minlin Zeng,Zhipeng Zhou,Yang Qiu,Zhiqi Shen*

Main category: cs.AI

TL;DR: 提出了TRIP框架，将帕金森病评估中的多模态学习建模为多目标优化问题，解决了模态同步和依赖的限制，并在异步和同步设置下都取得了最佳性能。


<details>
  <summary>Details</summary>
Motivation: 解决多模态方法在帕金森病评估中的两个主要限制：(1)训练时需要所有模态同步，(2)推理时依赖所有模态，这些限制了实际应用。
...
### [261] [Self-evolving expertise in complex non-verifiable subject domains: dialogue as implicit meta-RL](https://arxiv.org/abs/2510.15772)
*Richard M. Bailey*

Main category: cs.AI

TL;DR: Dialectica框架通过结构化对话、记忆和自我反思机制，使AI代理在解决复杂问题时能够通过经验发展专业能力，显著优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 解决复杂多维问题（如司法框架、环境污染等）时，当前LLM缺乏通过经验发展专业能力的内生机制，需要一种方法来增强AI在开放不可验证领域的专业能力。

Method: 提出Dialectica框架，让代理在定义的主题上进行结构化对话，增强记忆、自我反思和政策约束的上下文编辑功能，将讨论视为隐式元强化学习过程。

Result: 在两个模型架构（Qwen3:30b和o4-mini）上的评估显示，启用基于反思的上下文编辑的代理在Elo评分、标准化Bradley-Terry-Davidson能力和AlphaRank质量上均优于基线模型。

Conclusion: 对话驱动的上下文演进是实现目标专业能力放大的可行路径，定量和定性证据一致支持这一结论。
...
### [262] [Demo: Guide-RAG: Evidence-Driven Corpus Curation for Retrieval-Augmented Generation in Long COVID](https://arxiv.org/abs/2510.15782)
...
Result: 结合临床指南和高质量系统评价的RAG配置在性能上始终优于单一指南方法和大规模文献数据库。
...
### [263] [PokeeResearch: Effective Deep Research via Reinforcement Learning from AI Feedback and Robust Reasoning Scaffold](https://arxiv.org/abs/2510.15862)
...
Method: 采用无标注的AI反馈强化学习框架，使用基于LLM的奖励信号优化策略，结合思维链驱动的多轮调用推理框架，实现自我验证和工具故障的自适应恢复。
