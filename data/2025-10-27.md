...
TL;DR: 提出Shortcut-Rerouted Adapter Training方法，通过在适配器训练时提供干扰因素的快捷路径，防止适配器学习到目标属性与偶然因素（如姿势、表情、光照）的虚假相关性，从而提高生成质量、多样性和提示遵循能力。


<details>
  <summary>Details</summary>
Motivation: 现有的适配器训练方法容易将目标属性（如身份）与图像中的偶然因素（如姿势、表情、光照）纠缠在一起，这种虚假相关性限制了模型的泛化能力和对文本提示的遵循能力。

Method: 在适配器训练期间，通过ControlNet或LoRA等辅助模块为干扰因素建立快捷路径，消除适配器学习这些因素的动力，然后在推理时移除这些辅助模块。

Result: 在面部和全身身份注入任务中，该方法提高了生成质量、多样性和提示遵循能力。

Conclusion: 在大模型时代，寻求解耦表示的最有效路径可能是为不应学习的内容建立快捷路径。
...
Method: 使用参考视频作为直接语义提示，通过插件式混合Transformer专家架构指导冻结的Video Diffusion Transformer，采用时间偏置位置嵌入消除虚假映射先验。

Result: 作为单一统一模型，VAP在开源方法中达到最先进水平，获得38.7%的用户偏好率，与领先的条件特定商业模型相当，并展示了强大的零样本泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分割对临床诊断和治疗规划至关重要，但传统CNN难以捕捉全局上下文信息和长程依赖关系，影响对复杂边界和多种尺寸结构的精确分割。
...
Conclusion: FM-BFF-Net通过有效整合CNN和Transformer的优势，显著提升了医学图像分割的精度和鲁棒性，适用于多种医学成像场景。
...
TL;DR: GenPT是一个生成式点轨迹跟踪框架，通过流匹配建模多模态轨迹，在遮挡点跟踪上达到最先进精度，同时在可见点跟踪上保持竞争力。


<details>
  <summary>Details</summary>
Motivation: 现有判别式模型在处理视觉遮挡带来的不确定性时，只能回归到均值或众数，无法捕捉多模态轨迹分布。

Method: 提出生成式点轨迹跟踪器(GenPT)，采用流匹配训练框架，结合判别式跟踪器的迭代细化、跨窗口一致性先验和针对点坐标优化的方差调度。

Result: 在PointOdyssey、Dynamic Replica和TAP-Vid基准测试中表现优异，在遮挡点跟踪上达到最先进精度，可见点跟踪保持竞争力。

Conclusion: GenPT能够有效捕捉点轨迹的多模态特性，显著提升遮挡情况下的跟踪性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在3D医学图像中难以对解剖区域进行定位和逐步推理，这与临床医生的实际诊断工作流程不符，影响了临床医生与AI的可信协作。

Method: 创建了3DReasonKnee数据集，包含7,970个3D膝关节MRI体积，每个样本提供五元组：3D MRI体积、针对特定解剖区域的诊断问题、3D边界框定位、临床医生生成的诊断推理步骤以及结构化严重程度评估。

Result: 建立了ReasonKnee-Bench评估框架，对五种最先进的视觉语言模型进行基准测试，为定位和诊断准确性提供了基线性能。
...
TL;DR: 提出了一种利用热偏振线索进行详细3D形状重建的新方法，该方法不受光照和材质特性影响，通过长波红外偏振成像解决了可见光偏振分析中的模糊性问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D重建方法依赖光照和材质特性，限制了在透明、半透明和异质物体上的应用效果。热偏振成像能够克服这些限制，提供更准确的重建结果。

Method: 建立了偏振观测的一般理论，利用长波红外偏振成像避免可见光偏振分析的模糊性，提出基于多视角热偏振图像的3D形状恢复方法。

Result: 实验结果表明，该方法能够有效重建透明、半透明和异质物体的精细细节，性能优于现有技术。

Conclusion: 热偏振成像为3D形状重建提供了新的有效途径，特别是在处理复杂材质物体时具有显著优势。
...
### [7] [Group Inertial Poser: Multi-Person Pose and Global Translation from Sparse Inertial Sensors and Ultra-Wideband Ranging](https://arxiv.org/abs/2510.21654)
*Ying Xue,Jiaxi Jiang,Rayan Armani,Dominik Hollidt,Yi-Chi Liao,Christian Holz*

Main category: cs.CV

TL;DR: 提出了一种使用稀疏可穿戴IMU和UWB传感器进行多人全身运动跟踪的新方法，通过融合惯性观测和传感器间距离估计来精确估计3D姿态和全局轨迹。


<details>
  <summary>Details</summary>
Motivation: 基于IMU的运动跟踪存在平移估计和个体间相对定位不准确的问题，因为惯性线索是自参考的，无法提供他人的直接空间参考。

Method: 使用超宽带测距(UWB)估计传感器对之间的绝对距离，将距离与惯性观测融合输入结构化状态空间模型，采用新颖的两步优化方法利用估计距离准确跟踪全局轨迹。

Result: 在合成和真实世界数据评估中，Group Inertial Poser在准确性和鲁棒性方面优于现有最先进方法，并创建了首个IMU+UWB双人跟踪数据集GIP-DB。

Conclusion: 展示了IMU+UWB基于的多人在野外运动捕捉的前景，为克服纯IMU方法的局限性提供了有效解决方案。
...
TL;DR: VESSA是一种基于视频的无监督域适应方法，通过多视角物体中心视频对视觉基础模型进行自监督微调，无需标注即可提升模型在分布偏移和标签稀缺场景下的性能。


<details>
  <summary>Details</summary>
Motivation: 解决视觉基础模型在分布偏移和标签稀缺领域表现不佳的问题，传统监督微调在这种场景下不可行，而生成语言模型的自监督持续学习方法在视觉编码器模型中效果不佳。

Method: 采用自蒸馏范式，利用多视角物体中心视频进行自监督微调，精心调整预测头并使用参数高效适应技术防止模型遗忘预训练知识。

Result: 在3个视觉基础模型和2个数据集上的实验表明，VESSA相比基础模型和先前适应方法在下游分类任务中取得了一致的性能提升。

Conclusion: VESSA提供了一种有效的无监督域适应方法，能够利用多视角物体观测学习对多样化捕获条件的鲁棒性，无需标注即可改善视觉基础模型在目标域的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有6D位姿估计流程在工业环境中面临检测瓶颈，特别是在杂乱、光照差和复杂背景等挑战性条件下性能下降。

Method: 基于当前SOTA基线，采用低光图像增强和基于开放词汇检测的背景去除，抑制原始SAM输出的误报，为下游位姿估计提供更可靠的检测结果。

Result: 在真实世界工业抓取基准测试上的广泛实验表明，该方法显著提升检测精度，同时推理开销可忽略不计。

Conclusion: 该方法展示了在工业环境中提升未见物体检测的有效性和实用性。
...
TL;DR: 使用深度学习自动检测地震后混凝土结构中暴露的钢筋，通过YOLOv11模型识别开裂、剥落和钢筋暴露等损伤，建立混合框架评估结构损伤等级。
...
Method: 基于深度学习框架，使用土耳其2023年地震图像数据集，通过微调、数据增强和公开数据集测试，开发YOLOv11模型检测开裂、剥落和钢筋暴露，并微调另一个YOLO模型区分不同损伤等级。

Result: 建立了能够自动可靠地从输入图像确定损伤等级的混合框架，证明了利用图像数据收集、标注和深度学习方法可以在不同损伤情境下实现快速自动化损伤检测。

Conclusion: 研究表明通过图像数据收集、标注和深度学习方法的结合，可以在灾害后实现跨不同损伤情境的快速自动化损伤检测。
...
TL;DR: ZING-3D是一个零样本3D场景图生成框架，利用预训练基础模型进行开放词汇识别，生成丰富的语义场景表示，支持增量更新和3D几何接地。


<details>
  <summary>Details</summary>
Motivation: 解决现有3D场景图生成方法局限于单视图、不支持增量更新、缺乏3D几何接地的问题，这些对于具身智能场景至关重要。

Method: 利用VLM推理生成丰富的2D场景图，然后使用深度信息将其接地到3D空间。节点包含开放词汇对象特征、3D位置和语义上下文，边捕捉空间和语义关系及物体间距离。

Result: 在Replica和HM3D数据集上的实验表明，ZING-3D能够有效捕获空间和关系知识，无需任务特定训练。

Conclusion: ZING-3D为零样本3D场景理解提供了一个有效的框架，适用于下游机器人应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有语义分割网络大多采用强大的预训练编码器但使用简单的解码器，导致语义上下文和细粒度细节保留之间的权衡不理想。

Method: 提出WaveSeg解码器：1) 从输入图像学习高频分量作为显式先验；2) 应用双域操作(DDO)多尺度融合；3) 提出频谱分解注意力(SDA)块，利用Mamba增强高频结构细节；4) 应用重参数化卷积在小波域保持低频语义完整性；5) 残差引导融合整合多尺度特征。

Result: 在标准基准测试上的广泛实验表明，WaveSeg在定量和定性上都持续优于最先进的方法，实现了高效精确的分割。

Conclusion: WaveSeg通过在小波域利用频率先验和基于Mamba的注意力，能够产生语义和结构丰富的特征图，在语义分割任务中表现出色。
...
TL;DR: 提出了一种融合专家文本概念和视觉特征的对比语言-图像预训练框架，用于Hirschsprung病肠肌层神经丛分类，在准确率、精确度和特异性方面优于传统CNN模型。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习模型在肠肌层神经丛分类任务中表现良好但缺乏可解释性，且不符合医生的决策方式，需要结合专家知识来提高临床相关性。

Method: 使用大型语言模型生成专家来源的文本提示，通过QuiltNet编码后集成到对比语言-图像预训练模型中，将临床相关语义线索与视觉特征对齐。
...
TL;DR: 提出了一种基于直方图的Retinex模型(HistRetinex)，用于快速低光照图像增强，通过将Retinex模型从空间域扩展到直方图域，实现了高效计算。
...
Method: 定义直方图位置矩阵和计数矩阵，构建基于直方图的Retinex模型和两级优化模型，通过迭代求解光照和反射直方图，最后通过直方图匹配实现图像增强。

Result: 实验结果显示HistRetinex在可见性和性能指标上优于现有方法，在1000*664分辨率图像上仅需1.86秒，比现有方法至少节省6.67秒。
...
TL;DR: 提出主动视觉推理(AVR)任务，将视觉推理扩展到部分可观察的交互环境中，需要智能体通过物理动作主动获取信息、整合多步观察并动态调整决策。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM主要在静态、完全可观察环境中研究视觉推理，而现实世界信息往往不完整。受人类主动探索环境能力的启发，需要开发能在部分可观察环境中主动获取信息的智能体。

Method: 引入CLEVR-AVR仿真基准和AVR-152k大规模数据集，包含丰富的思维链标注。开发PhysVLM-AVR模型，在更高阶马尔可夫决策过程中训练智能体进行不确定性识别、信息增益预测和信息最大化动作选择。

Result: PhysVLM-AVR在CLEVR-AVR、具身推理(OpenEQA、RoboVQA)和被动视觉推理(GeoMath、Geometry30K)上达到最先进性能。分析发现当前具身MLLM虽然能检测信息不完整性，但难以通过交互主动获取和整合新信息。

Conclusion: 主动视觉推理是MLLM发展的关键方向，当前模型在主动推理能力上存在根本性差距，需要进一步研究如何让智能体像人类一样通过交互主动探索环境。
...
TL;DR: 提出了一种面向城市规模LiDAR的对象中心、不确定性感知变化检测方法，通过多分辨率NDT和点面ICP配准、高度归一化、基于配准协方差和表面粗糙度的检测级别计算，结合语义和实例分割的跨时期关联，在15个代表性区域达到95.2%准确率。


<details>
  <summary>Details</summary>
Motivation: 现有方法对垂直偏差、地面坡度、视角不匹配敏感，且缺乏对象身份信息；基于点和体素的方法需要大内存、假设完美预配准、会破坏薄结构，且很少强制类别一致关联，导致分裂或合并情况未解决并忽略不确定性。

Method: 使用多分辨率NDT和点面ICP进行时期配准，高度归一化，基于配准协方差和表面粗糙度计算检测级别；几何代理种子跨时期关联，通过语义和实例分割及类别约束二分分配进行精炼；分块处理限制内存，实例级决策结合3D重叠、法向位移、高度和体积差异与直方图距离。

Result: 在15个代表性Subiaco区域达到95.2%准确率、90.4% mF1和82.6% mIoU，超过Triplet KPConv 0.2个百分点准确率、0.2 mF1和0.8 mIoU，在Decreased类别上IoU达到74.8%并提升7.6个百分点。

Conclusion: 该方法在城市规模LiDAR变化检测中表现出色，通过对象中心、不确定性感知的流程有效处理了分裂合并情况，在保持类别计数的同时提高了检测性能。
...
TL;DR: 提出Controllable-LPMoE方法，通过动态控制局部先验来调制冻结的基础模型，实现高效微调，在多个二值对象分割任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 大规模基础模型在下游对象分割任务中提供强大特征表示，但全参数微调计算开销大。现有方法嵌入可训练提示但缺乏语义先验，限制了模型适应性。

Method: 构建轻量级动态混合局部先验提取器，通过异质卷积捕获多样局部先验，使用门控网络动态输出专家先验。设计双向交互适配器，采用余弦对齐可变形注意力和通道导向自适应尺度增强进行特征交互重构。

Result: 在31种最先进方法中表现出优越性，在多个二值对象分割任务中展示出色性能。

Conclusion: Controllable-LPMoE通过动态先验调制冻结模型，实现了高效微调，在分割任务中具有卓越性能和广泛适应性。
...
TL;DR: 提出了SafetyPairs框架，通过生成仅安全相关特征不同的图像对来系统研究图像安全性的细微差异，构建了包含3020对图像的新安全基准。


<details>
  <summary>Details</summary>
Motivation: 现有图像安全数据集标注粗糙模糊，无法区分导致安全问题的具体特征，需要系统方法来识别图像中真正影响安全性的细微变化。

Method: 利用图像编辑模型对图像进行针对性修改，仅改变与安全策略相关的特征，从而生成安全标签翻转但其他细节不变的对比图像对。

Result: 构建了包含3020对图像的安全基准，涵盖9个安全类别，揭示了视觉语言模型在区分细微图像差异方面的弱点，并可作为有效的数据增强策略。

Conclusion: SafetyPairs框架为研究细粒度图像安全差异提供了首个系统性资源，既能评估模型能力，又能提高轻量级防护模型的训练效率。
...
TL;DR: NoisyGRPO是一个多模态强化学习框架，通过向视觉输入注入可控噪声来增强探索，并使用贝叶斯框架建模优势估计过程，从而提升多模态大语言模型的链式思维推理泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的强化学习框架在提升通用链式思维推理能力时，往往难以泛化到训练分布之外。为了解决这个问题，需要一种能够增强探索并提高泛化能力的方法。

Method: 提出NoisyGRPO框架，包含两个核心组件：1）噪声注入探索策略：在视觉输入中添加高斯噪声以鼓励在更广泛的视觉场景中进行探索；2）贝叶斯优势估计：将优势估计建模为贝叶斯推断问题，结合噪声水平和观察到的轨迹奖励来计算稳健的后验优势估计。

Result: 在标准链式思维质量、通用能力和幻觉基准测试中，NoisyGRPO显著提高了泛化能力和鲁棒性，特别是在使用小规模多模态大语言模型（如Qwen2.5-VL 3B）的强化学习设置中表现优异。

Conclusion: NoisyGRPO通过系统性地引入可控噪声和贝叶斯优势估计，有效提升了多模态大语言模型的链式思维推理泛化能力，为解决强化学习框架泛化问题提供了有前景的解决方案。
...
TL;DR: 提出了一种基于CycleGAN的级联合成器方法，从非对比CT扫描生成数字对比CTPA，以降低碘对比剂的风险。


<details>
  <summary>Details</summary>
Motivation: CTPA依赖碘对比剂存在肾毒性和过敏反应风险，特别是在高危患者中，需要开发替代方法。

Method: 使用基于CycleGAN的级联合成器，在410对CTPA和NCCT扫描数据上进行训练和验证，生成数字对比CTPA图像。

Result: 在定量指标上表现最佳（MAE: 165.12, PSNR: 20.27, SSIM: 0.98），血管分割和量化任务显著改善，血管体积ICC从0.70提升到0.81。

Conclusion: 该方法能有效增强血管显示，特别是小血管，为降低对比剂风险提供了可行的替代方案。
...
TL;DR: 提出Spatial Intelligence Grid (SIG)作为视觉空间智能的结构化表示方法，通过网格编码物体布局和空间关系，为多模态大模型提供更可靠的空间推理能力评估和训练框架。


<details>
  <summary>Details</summary>
Motivation: 当前方法使用纯文本提示和VQA式评分来代理视觉空间智能，这掩盖了几何信息、引入了语言捷径，并削弱了对真正空间技能的归因。

Method: 引入SIG结构化网格模式，明确编码物体布局、物体间关系和物理基础先验，作为文本的补充通道，提供忠实、组合式的场景结构表示。

Result: 在少样本上下文学习中，SIG相比仅使用VQA表示，在所有VSI指标上都带来了更大、更稳定、更全面的提升。发布了包含1.4K驾驶帧的SIGBench基准数据集。

Conclusion: SIG作为数据标注和训练模式，在学习和评估视觉空间智能方面展现出潜力，能够将空间能力与语言先验分离开来。
...
TL;DR: 提出了Blockwise Flow Matching (BFM)框架，将生成轨迹划分为多个时间片段，每个片段由专门的小型速度块建模，提高推理效率和样本质量。
...
Method: 1. 块状设计：将生成轨迹划分为多个时间片段，每个片段由专门的小型速度块建模；2. 语义特征引导模块：在预训练表示的基础上显式调节速度块；3. 特征残差近似策略：在保持语义质量的同时显著降低推理成本。

Result: 在ImageNet 256x256上的实验表明，BFM在可比生成性能下实现了2.1倍到4.9倍的推理复杂度加速，建立了显著改进的帕累托前沿。

Conclusion: BFM框架通过块状设计和语义特征引导，在保持生成质量的同时显著提高了Flow Matching方法的推理效率。
...
TL;DR: TokenCLIP是一个基于CLIP的token级适应框架，通过动态对齐视觉token和可学习的文本子空间来实现细粒度异常检测，解决了现有方法在单一文本空间中对齐不同对象和领域视觉语义的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的异常检测方法通常依赖单一文本空间来对齐多样对象和领域的视觉语义，这种无差别的对齐方式阻碍了模型准确捕捉变化的异常语义。

Method: 提出TokenCLIP框架，将token无关的文本空间扩展为一组正交子空间，通过最优传输问题动态分配每个token到语义相关的子空间组合，并使用top-k掩码稀疏化传输计划，使子空间专注于不同的视觉区域。

Result: 大量实验证明了TokenCLIP的优越性。

Conclusion: TokenCLIP通过动态token级对齐机制，实现了更精细的异常语义学习，在零样本异常检测任务中表现出色。
...
TL;DR: 提出了Knowledge-enhanced Benchmark Evolution (KBE)框架，通过图表示和知识增强将静态多模态基准转化为可控的动态演化版本，以解决数据污染和饱和问题。
...
Method: 使用图表示VQA样本，通过知识增强扩展静态基准，包括重构问题和整合外部文本知识，实现难度可控的评估。
...
TL;DR: 本文介绍了ICCV LargeFineFoodAI检索竞赛第三名解决方案，通过四个基础模型训练、TTA和集成方法提升特征表示能力，并提出基于扩散和k-互惠重排序的新重排序方法。
...
Method: 1. 独立训练四个基础模型，使用ArcFace和Circle损失的加权和；2. 应用TTA（测试时增强）和模型集成；3. 提出基于扩散和k-互惠重排序的新重排序方法。

Result: 在公开和私有排行榜上分别获得0.81219和0.81191的mAP@100分数。

Conclusion: 该方法在食品图像检索任务中表现出色，证明了多模型集成和新型重排序策略的有效性。
...
TL;DR: 本文介绍了在Kaggle上的LargeFineFoodAI-ICCV Workshop-Recognition挑战赛中取得第三名的解决方案，通过结合Arcface损失和Circle损失来提升细粒度食品识别性能。
...
Method: 采用Arcface损失和Circle损失的适当组合来提升模型性能，通过精心调优的配置训练模型，并使用集成方法获得最终结果。

Result: 该解决方案在竞赛中获得了第三名的成绩。

Conclusion: Arcface损失和Circle损失的适当组合能够有效提升细粒度食品识别任务的性能，通过精心调优和模型集成可以获得优异的竞赛结果。
...
<details>
  <summary>Details</summary>
Motivation: 捷径模型作为一种非对抗生成模型，支持单步、少步和多步采样，但存在五个关键性能瓶颈阻碍了其广泛应用。
...
TL;DR: TSSR是一种基于离散扩散模型的新型3D网格生成方法，通过并行生成实现高质量艺术家风格网格，支持高达10,000个面片和1024^3的空间分辨率。
...
Method: 采用解耦训练和混合推理，将生成过程分为拓扑雕刻和形状细化两个阶段；改进的沙漏架构结合双向注意力和旋转位置编码；引入连接损失作为拓扑约束。

Result: 在复杂数据集上的实验表明，TSSR能够生成高质量的3D艺术家风格网格，达到10,000个面片和1024^3的空间分辨率。

Conclusion: TSSR通过并行生成和解耦策略，在3D网格生成中实现了高效性和高质量，为艺术家风格3D内容创作提供了新方法。
...
TL;DR: SAGE-3D将3D高斯泼溅技术升级为可执行的、语义和物理对齐的环境，通过对象中心语义标注和物理感知执行连接，构建了首个基于3DGS的VLN基准测试。
...
Result: 发布了包含1K对象标注3DGS室内场景数据的InteriorGS，以及首个基于3DGS的VLN基准测试SAGE-Bench（包含2M VLN数据）。实验显示3DGS场景数据更难收敛但具有强泛化性，在VLN-CE Unseen任务上提升基线性能31%。

Conclusion: SAGE-3D成功将3DGS升级为语义和物理对齐的可执行环境，为视觉语言导航提供了更真实和有效的训练平台。
...
TL;DR: 提出了FineRS框架，通过两阶段强化学习解决MLLMs在高分辨率图像中定位和分割极小物体的挑战


<details>
  <summary>Details</summary>
Motivation: MLLMs由于输入分辨率限制，在处理高分辨率图像中的微小物体时面临挑战，特别是在杂乱背景中准确定位和分割

Method: 采用粗到精的两阶段流程：全局语义探索(GSE)生成文本响应和粗略目标区域，局部感知细化(LPR)精炼区域生成精确边界框和分割掩码，并引入定位知情的回顾奖励耦合两个阶段

Result: 在FineRS-4k和公共数据集上的实验表明，该方法在指令引导分割和视觉推理任务上持续优于最先进的MLLM方法

Conclusion: FineRS框架有效解决了MLLMs在高分辨率场景中处理微小物体的局限性，在像素级分割和属性级推理方面表现出色
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉-语言模型的多模态对齐组件可解释性不足，因为难以将多模态表示的语义映射到统一概念集。

Method: 提出VL-SAE稀疏自编码器，使用基于距离的编码器和两个模态特定解码器，通过余弦相似度显式对齐多模态表示，确保语义相似表示具有一致的神经元激活。

Result: 在多个VLMs（如CLIP、LLaVA）上的实验表明，VL-SAE在解释和增强视觉-语言对齐方面具有优越能力，能够提高零样本图像分类和幻觉消除等下游任务的性能。

Conclusion: VL-SAE通过将视觉-语言表示映射到统一概念集，成功解释和增强了多模态对齐，为理解视觉-语言模型的内部机制提供了新视角。
...
TL;DR: 提出CNN-Transformer-CLIP(CT-CLIP)框架，结合CNN提取局部病变特征和Transformer捕获全局结构关系，通过自适应特征融合模块和多模态图像-文本学习，在复杂果园环境中实现高精度苹果叶病识别。


<details>
  <summary>Details</summary>
Motivation: 传统多尺度特征融合方法在复杂果园环境中面临挑战，无法充分处理局部与全局特征关系，且苹果叶病病变形态和分布具有显著多样性。

Method: 使用CNN提取局部病变细节特征，Vision Transformer捕获全局结构关系，通过自适应特征融合模块动态融合特征，并利用预训练CLIP权重实现视觉特征与疾病语义描述的对齐。
...
Conclusion: 该方法有效解决了无人机跟踪中的语义感知不足问题，提高了在挑战性场景下的跟踪性能。
...
TL;DR: 提出一种仅需在训练时使用眼动追踪数据的视觉语言模型正则化框架，通过注意力对齐机制提升自我中心视角理解任务性能
...
Method: 提出视线正则化注意力机制，将模型关注点与人类视觉注视对齐，该设计灵活模块化，可泛化到多种使用注意力的VLM架构

Result: 相比无视线正则化的基线模型，未来事件预测的语义预测分数提升高达11%，当前活动理解提升约7%

Conclusion: 该工作为在现实场景中使用人类视线增强VLM预测能力奠定了基础，如辅助机器人和人机协作应用
...
TL;DR: 该论文提出了一种用于从MRI和CBCT生成合成CT的统一管道，使用2.5D U-Net++网络结合IMPACT-Synth感知损失函数。通过比较两种配准策略，发现基于IMPACT的配准能提供更准确的解剖对齐，但评估时存在配准偏差问题。


<details>
  <summary>Details</summary>
Motivation: 解决合成CT生成中配准误差传播到监督学习的问题，开发更稳健和可泛化的合成CT合成模型。

Method: 使用2.5D U-Net++网络，ResNet-34编码器，联合训练各解剖区域并进行区域微调。损失函数结合L1损失和IMPACT-Synth感知损失。评估了Elastix和IMPACT两种配准策略。

Result: 在本地测试集上，IMPACT配准实现了更准确和一致的解剖对齐，降低了MAE并产生更真实的解剖结构。但在公开验证集上，使用Elastix配准数据的模型得分更高，反映了评估管道中的配准偏差。

Conclusion: 配准误差会传播到监督学习中，影响训练和评估，可能以牺牲解剖保真度为代价来提升性能指标。IMPACT通过促进解剖一致性对齐来减轻这种偏差，支持开发更稳健的合成CT合成模型。
...
TL;DR: 提出了一种基于实时网络带宽约束调整扩散模型生成质量的框架，通过端到端训练使模型能够根据可用带宽自适应调整去噪过程，实现早期停止采样以保持与传输条件匹配的感知质量。


<details>
  <summary>Details</summary>
Motivation: 传统扩散模型采用固定去噪步数生成高保真图像，但在云到设备场景中，有限带宽需要重度压缩，导致纹理细节丢失和计算浪费。需要一种能根据带宽约束自适应调整生成质量的方法。

Method: 引入联合端到端训练策略，扩散模型以从可用带宽推导的目标质量级别为条件。模型学习自适应调制去噪过程，通过轻量级质量嵌入引导去噪轨迹，实现早期停止采样。

Result: 实验结果表明，相比简单的早期停止方法，该方法显著提高了带宽适应生成的视觉保真度。

Conclusion: 该方法为带宽受限环境中的高效图像传输提供了有前景的解决方案，只需最小架构改动。
...
Method: TerraGen引入了一个地理空间布局编码器，统一了边界框和分割掩码输入，结合多尺度注入方案和掩码加权损失来显式编码空间约束，从全局结构到精细细节。

Result: 实验结果表明，TerraGen在多样化任务中实现了最佳的生成图像质量，并且可以作为通用数据增强生成器，显著提升下游任务性能，在全数据和少样本场景中都展示了强大的跨任务泛化能力。

Conclusion: TerraGen解决了遥感视觉任务中生成数据增强框架的任务隔离问题，通过统一的地理空间布局编码实现了灵活、空间可控的图像合成，为多任务遥感图像生成提供了有效解决方案。
...
TL;DR: 提出了一种深度一致性约束的无缝图像拼接方法，通过多阶段机制和深度正则化约束解决视差问题，使用图计算确定最优拼接缝，并通过重参数化策略提高算法效率。


<details>
  <summary>Details</summary>
Motivation: 图像拼接中物体深度变化导致的视差会造成重影和错位问题，需要解决多视角对齐困难。

Method: 采用多阶段机制结合全局深度正则化约束增强不同深度范围的对齐精度；在融合过程中使用图计算确定最优拼接缝并扩散软缝区域；引入重参数化策略优化结构设计。

Result: 实验证明该方法在现有方法中表现出优越性能，能有效减轻视差引起的对齐误差，实现自然无缝的拼接效果。

Conclusion: 该方法通过深度一致性约束和优化策略，成功解决了图像拼接中的视差问题，提高了拼接质量和算法效率。
...
TL;DR: 提出了多模态未修剪视频检索任务和MUVR基准，专注于长视频平台的实际检索需求，支持视频中心的多模态查询，并构建了多级视觉对应关系来精确定义检索匹配标准。


<details>
  <summary>Details</summary>
Motivation: 现有视频检索方法主要针对短视频和简单查询，无法满足长视频平台对未修剪视频和多模态查询的实际需求，需要开发更实用的检索范式和评估标准。

Method: 构建MUVR基准，包含53K个未修剪视频和1,050个多模态查询，支持长文本描述、视频标签提示和掩码提示等多种查询方式，并定义了六个级别的视觉对应关系。

Result: 对3个最先进的视频检索模型、6个基于图像的VLM和10个MLLM进行了广泛评估，揭示了现有方法在处理未修剪视频和多模态查询方面的局限性。

Conclusion: MUVR基准填补了长视频平台视频检索的空白，为评估检索模型和多模态大语言模型提供了全面的测试平台，推动了该领域的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法局限于少数预定义概念轴（如颜色、形状），且通常在合成数据集中探索，无法处理真实世界中丰富多样的语义维度。

Method: 利用预训练视觉语言模型和通用提示策略识别多样化的图像相关轴，通过通用概念编码器自适应绑定视觉特征，使用组合锚定目标确保各轴可独立操作。

Result: 在ImageNet、CelebA-HQ和AFHQ数据集上展示了优异的编辑能力，能够处理过于多样化而无法预定义的真实世界概念，并表现出强大的组合泛化能力。

Conclusion: 该方法在视觉概念学习和基于文本的编辑方法中表现优越，为语言引导的视觉概念学习提供了可扩展的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在合成关节3D物体时难以同时保证几何一致性、准确关节运动和跨关节状态的外观真实性，特别是对于通常被遮挡的区域缺乏合理的纹理特征。

Method: 1）使用变分自编码器将稀疏体素表示和关节属性（关节类型、轴、原点、范围、部件类别）嵌入统一潜在空间；2）在潜在空间上训练潜在扩散模型进行多样化采样；3）引入关节感知高斯解码器，根据关节状态解码外观，为通常遮挡区域分配合理纹理。

Result: 在PartNet-Mobility和ACD数据集上的家具类物体实验表明，ArtiLatent在几何一致性和外观保真度方面优于现有方法，能够生成跨关节配置的视觉逼真结果。

Conclusion: ArtiLatent为关节3D物体合成和操作提供了可扩展的解决方案，能够生成具有物理合理性的多样化关节物体，显著提升了视觉真实感。
...
TL;DR: 提出了一种用于图像恢复CNN的查找表实现中的各向异性池化方法，替代传统的平均池化，以更好地处理各向异性信号结构，提高恢复质量。


<details>
  <summary>Details</summary>
Motivation: 现有的基于查找表的CNN图像恢复方法使用平均池化融合不同方向的像素块结果，但这种方法不适合处理各向异性信号结构，限制了性能提升。

Method: 首先引入广义中值池化方法，然后扩展为学习数据依赖的池化系数，使不同方向的像素块贡献能够自适应加权。

Result: 在各种恢复基准测试上的实验结果表明，该各向异性池化策略在感知和数值指标上都优于现有的LUT可实现的CNN方法。

Conclusion: 各向异性池化策略能够有效提升基于查找表的图像恢复CNN的性能，为管理查找表大小同时保持较大感受野提供了更好的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在建模3D对象和场景的层次结构时存在显著限制：要么需要多次渲染来捕获不同粒度的嵌入，显著增加推理时间；要么依赖预定义的封闭集离散层次结构，对真实世界中多样和细微结构的泛化能力差。

Method: 使用连续双曲潜在空间表示场景层次结构，利用双曲几何的特性自然编码多尺度关系，并通过潜在空间中的测地线路径实现层次结构的平滑遍历。

Result: 在标准基准测试中优于最先进的方法，在3D场景理解方面表现出更高的效率和适应性。

Conclusion: OpenHype通过双曲几何成功解决了3D场景层次结构建模的挑战，为自主智能体提供了更全面的环境理解能力。
...
TL;DR: PhysWorld是一个利用模拟器合成物理演示来学习高效世界模型的框架，用于解决从有限真实视频数据学习物理一致动力学模型的挑战，特别适用于具有空间变化物理属性的可变形物体。


<details>
  <summary>Details</summary>
Motivation: 从有限的真实世界视频数据中学习物理一致的动力学模型是一个重大挑战，特别是对于具有空间变化物理属性的可变形物体。数据稀缺问题阻碍了准确世界模型的学习。

Method: 首先在MPM模拟器中通过本构模型选择和物理属性全局到局部优化构建物理一致的数字孪生；然后对物理属性施加部分感知扰动，为数字孪生生成各种运动模式，合成广泛多样的演示；最后使用这些演示训练嵌入物理属性的轻量级GNN世界模型，并可用真实视频进一步优化物理属性。

Result: PhysWorld能够对各种可变形物体进行准确快速的未来预测，并且对新交互具有良好的泛化能力。实验表明，PhysWorld具有竞争力的性能，同时推理速度比最先进方法PhysTinn快47倍。

Conclusion: PhysWorld框架通过模拟器合成物理演示有效解决了数据稀缺问题，实现了高效准确的可变形物体动力学建模，在保持竞争力的同时大幅提升了推理速度。
...
TL;DR: 提出了MoniTor，一种基于记忆的在线评分队列方案，用于无需训练的异常检测，在实时视频异常检测任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前离线视频异常检测研究较多，但在线检测因实时性和计算强度而很少受到关注，需要解决在线检测的固有复杂性。

Method: 使用预训练大模型的流式输入，结合LSTM启发的预测机制捕获时间依赖，设计评分队列和异常先验动态存储最近分数。

Result: 在两个大型数据集（UCF-Crime和XD-Violence）上评估，MoniTor优于最先进方法，且与弱监督方法竞争无需训练。

Conclusion: MoniTor为在线视频异常检测提供了有效解决方案，在多种监控和真实场景中表现出色。
...
TL;DR: VidSplice是一个新颖的视频修复框架，通过解耦视频修复为两个子任务：多帧一致图像修复和掩码区域运动传播，引入间隔帧先验来指导修复过程，显著提升了时空稳定性和前景对齐。


<details>
  <summary>Details</summary>
Motivation: 现有视频修复方法在严重内容退化情况下表现不佳，且往往忽视时空稳定性，导致对视频后半部分控制不足。需要解决这些局限性以提升修复质量。

Method: 提出VidSplice框架，包含CoSpliced模块执行首帧传播策略，通过拼接机制将初始帧内容扩散到后续参考帧；引入上下文控制器模块在帧复制后编码一致先验，并将拼接视频注入I2V生成主干，有效约束生成过程中的内容失真。

Result: 广泛的评估表明，VidSplice在多样化视频修复场景中实现了有竞争力的性能，其设计显著改善了前景对齐和运动稳定性，超越了现有方法。

Conclusion: VidSplice通过解耦视频修复任务和引入间隔帧先验，有效解决了现有方法在时空稳定性和内容控制方面的不足，为视频修复提供了更优的解决方案。
...
TL;DR: CXR-LanIC是一个用于胸片诊断的可解释AI框架，通过任务对齐模式发现将深度学习模型的预测分解为可解释的视觉模式，解决了医学AI的黑箱问题。


<details>
  <summary>Details</summary>
Motivation: 解决深度学习模型在胸片诊断中预测不可解释的问题，为临床医生提供透明、可验证的解释以建立信任并识别潜在失败模式。

Method: 训练基于transcoder的稀疏自编码器，在BiomedCLIP诊断分类器上分解医学图像表示，通过100个transcoder集成从MIMIC-CXR数据集的多模态嵌入中发现约5,000个单语义模式。

Result: 在五个关键发现上达到竞争性诊断准确率，将预测分解为20-50个可解释模式，每个模式在共享特定放射学特征的图像中表现出一致的激活行为。

Conclusion: 医学AI系统可以同时实现准确性和可解释性，通过透明、临床基础的解释支持更安全的临床部署，关键创新在于从针对特定诊断目标训练的分类器中提取可解释特征。
...
TL;DR: 提出双流架构整合组织宏观特征和细胞级特征，通过循环变换器高效聚合细胞信息，并引入双向组织-细胞交互模块，在四种组织病理学亚型分类基准上优于现有模型。


<details>
  <summary>Details</summary>
Motivation: 现有病理学基础模型虽然能捕捉全局组织上下文，但忽略了细胞级特征建模，这限制了在癌症亚型分类等细粒度任务上的性能。

Method: 双流架构建模组织宏观特征与聚合细胞表征的交互；使用接受度加权的键值聚合模型（循环变换器）以线性复杂度捕获细胞间依赖；引入双向组织-细胞交互模块实现局部细胞线索与周围组织环境的相互关注。

Result: 在四种组织病理学亚型分类基准上的实验表明，该方法优于现有模型。
...
TL;DR: GRAP-MOT是一种针对重叠多摄像头视角封闭区域视频的多目标跟踪新方法，通过图加权解决方案在线更新人员身份标签，结合轨迹和特征信息，在拥挤场景中表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决封闭区域多摄像头重叠视角下的人员多目标跟踪问题，特别是处理频繁发生的人员遮挡情况。

Method: 提出图加权解决方案，在线更新人员身份标签，结合轨迹和特征信息，包含位置估计模块，深入研究了特征提取、跟踪和社区搜索等MOT过程的所有元素。

Result: 在封闭区域模型记录和公开真实数据集上测试，在高度拥挤空间场景中表现出优越性能，优于没有位置数据的方法。

Conclusion: GRAP-MOT在拥挤多摄像头场景中表现优异，同时分析发现IDF1指标比MOTA更适合此类算法比较，代码和数据集已公开。
...
<details>
  <summary>Details</summary>
Motivation: 语言和视觉语言模型在多种任务中表现出色，但其内部机制仍不完全清楚，需要研究注意力头如何专门化处理特定语义或视觉属性。

Method: 基于已有的可解释性方法，通过信号处理视角重新解释用最终解码层探测中间激活的实践，以原则性方式分析多个样本并基于与目标概念的相关性对注意力头进行排序。

Result: 在单模态和多模态Transformer中都发现注意力头层面的一致专业化模式，仅编辑1%的注意力头就能可靠地抑制或增强目标概念，在语言任务和视觉语言任务中得到验证。
...
Method: 基于小样本学习改进YOLOv11n模型，集成小波卷积和坐标卷积增强频率和空间特征提取，加入级联分组注意力机制抑制超声伪影和遮挡干扰。

Result: 检测准确率达到78%，在精确率-召回率曲线、置信度得分和遮挡场景下表现显著优越，兼具高精度和实时处理能力。

Conclusion: EH-YOLOv11n模型为胎盘早剥的计算机辅助诊断提供了可靠解决方案，具有重要的临床应用价值。
...
TL;DR: 提出了GranViT，一种结合细粒度特征提取和语义对齐的视觉Transformer，通过区域级自回归训练解决现有视觉编码器在细粒度感知方面的不足。
...
Method: 构建Gran-29M数据集（包含200万图像和1.8亿区域级标注），采用预训练-适应框架，通过边界框到标题回归和标题到边界框回归增强局部视觉表示，并引入自蒸馏机制强化区域推理能力。

Result: GranViT超越了现有视觉编码器，在不同LLMs上展现出强迁移性，在细粒度识别、多模态VQA和OCR理解任务上取得了最先进的结果。

Conclusion: GranViT通过大规模细粒度预训练和语义对齐，显著提升了视觉编码器的细粒度感知能力，在多模态任务中表现出色。
...
TL;DR: 本文提出了一种统一视角，将条件引导重新构建为固定点迭代，并引入前瞻引导(FSG)方法来解决现有CFG方法的效率问题。


<details>
  <summary>Details</summary>
Motivation: 现有CFG方法基于不同的理论解释，限制了设计空间并模糊了关键设计选择。需要统一的理论框架来理解和改进条件引导机制。

Method: 将条件引导重新构建为固定点迭代问题，提出FSG方法在早期扩散阶段优先解决长间隔子问题并增加迭代次数。
...
TL;DR: Foley Control是一种轻量级视频引导Foley方法，通过冻结预训练单模态模型并仅学习它们之间的小型交叉注意力桥接，将V-JEPA2视频嵌入连接到冻结的Stable Audio Open DiT文本到音频模型。


<details>
  <summary>Details</summary>
Motivation: 开发一种轻量级方法，在保持预训练模型性能的同时实现视频与音频的同步，避免重新训练音频先验模型，减少计算资源需求。

Method: 在现有文本交叉注意力后插入紧凑的视频交叉注意力，使用视频令牌池化来减少内存消耗和稳定训练，保持模型模块化特性。

Result: 在精选的视频-音频基准测试中，Foley Control在时间和语义对齐方面表现具有竞争力，参数量远少于最近的多模态系统，同时保持提示驱动的可控性。

Conclusion: 该方法提供了一种高效且模块化的视频到Foley解决方案，其桥接设计可扩展到其他音频模态（如语音）。
...
TL;DR: 提出了Chunk-GRPO方法，将GRPO优化从步级提升到块级，解决优势归因不准确和忽略生成时序动态的问题，在文本到图像生成中取得更好效果。


<details>
  <summary>Details</summary>
Motivation: GRPO在基于流匹配的文本到图像生成中面临两个关键限制：优势归因不准确，以及忽略生成的时序动态特性。

Method: 将连续步骤分组为连贯的'块'，捕捉流匹配的内在时序动态，在块级优化策略，并引入可选的加权采样策略。
...
### [57] [MATrack: Efficient Multiscale Adaptive Tracker for Real-Time Nighttime UAV Operations](https://arxiv.org/abs/2510.21586)
*Xuzhao Li,Xuchen Li,Shiyu Hu*

Main category: cs.CV

TL;DR: MATrack是一个专门为夜间无人机跟踪设计的系统，通过多尺度层次混合、自适应关键令牌门控和夜间模板校准器三个核心模块，解决了低光照条件下的跟踪挑战，在UAVDark135基准测试中性能显著优于现有方法，同时保持81 FPS的实时处理速度。


<details>
  <summary>Details</summary>
Motivation: 夜间无人机跟踪面临低光照条件、杂乱背景和频繁视角变化的挑战，现有基于低光增强和域适应的方法存在视觉伪影、计算成本高和无法充分利用动态物体信息等问题。

Method: 提出MATrack系统，包含三个核心模块：多尺度层次混合器（MHB）增强静态和动态模板间的特征一致性；自适应关键令牌门准确识别复杂背景中的物体信息；夜间模板校准器（NTC）确保长序列中的稳定跟踪性能。

Result: 在UAVDark135基准测试中，精确度、归一化精确度和AUC分别比最先进方法提升5.9%、5.4%和4.2%，处理速度达到81 FPS。在实际无人机平台测试中验证了系统的可靠性。
...
TL;DR: TIGER是一个两阶段文本图像引导超分辨率框架，通过“文本优先、图像后处理”范式解决生成式超分辨率中图像质量与文本可读性的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 当前生成式超分辨率方法在自然图像上表现良好，但会扭曲文本，造成图像质量与文本可读性之间的根本性权衡。

Method: TIGER采用两阶段框架：第一阶段明确解耦字形恢复，重建精确的文本结构；第二阶段使用这些文本结构来指导后续的全图像超分辨率，实现字形到图像的引导。

Result: 在UltraZoom-ST数据集上的广泛实验表明，TIGER实现了最先进的性能，在保持整体图像质量的同时显著提升了文本可读性。

Conclusion: TIGER通过文本优先的方法成功打破了图像质量与文本可读性之间的权衡，为场景文本超分辨率提供了有效的解决方案。
...
TL;DR: 该研究提出两种深度学习模型（ANN和CNN）用于MEG记录中的癫痫间期棘波自动检测，在仅有单专家标注的真实临床数据上表现优于现有方法，并采用交互式机器学习策略提高标注质量。


<details>
  <summary>Details</summary>
Motivation: 耐药性癫痫术前评估中，MEG的癫痫间期棘波定位至关重要，但手动检测既繁琐又容易出错，现有自动化方法要么需要大量标注数据，要么在非典型数据上缺乏鲁棒性，不适合临床应用。

Method: 提出基于特征的ANN和CNN两种模型架构，在59名患者数据库上训练，使用交互式机器学习策略迭代改进数据标注质量，并与现有最优模型在短时间窗口信号分类上进行比较。

Result: 两种模型在10名测试患者上均优于现有最优模型（F1分数：CNN=0.46，ANN=0.44），交互式机器学习策略证明模型对噪声标注具有鲁棒性。

Conclusion: 简单架构模型在分析复杂且标注不完美的数据时表现出鲁棒性，交互式机器学习方法为快速数据标注提供了巨大潜力，所提模型是自动检测癫痫间期棘波的有效工具。
...
### [60] [S3OD: Towards Generalizable Salient Object Detection with Synthetic Data](https://arxiv.org/abs/2510.21605)
...
Method: 使用多模态扩散管道生成超过139,000张高分辨率图像的S3OD数据集，通过迭代生成框架优先处理困难类别，并提出流线型多掩码解码器处理检测中的固有模糊性。
...
### [61] [Modest-Align: Data-Efficient Alignment for Vision-Language Models](https://arxiv.org/abs/2510.21606)
...
<details>
  <summary>Details</summary>
Motivation: 解决在资源受限场景下，传统跨模态对齐模型（如CLIP）因数据质量差、图像-文本对相关性弱而导致的过度自信和性能下降问题。

Method: 采用两种互补策略：随机扰动（引入可控噪声模拟不确定性）和嵌入平滑（校准嵌入空间中的相似度分布），共同降低过度自信并提升对噪声或弱对齐样本的性能。

Result: 在多个基准数据集上的检索任务中超越现有最优方法，仅使用CLIP 1/100的训练数据和1/600的GPU时间即达到竞争性结果。
...
### [62] [Epipolar Geometry Improves Video Generation Models](https://arxiv.org/abs/2510.21615)
*Orest Kupyn,Fabian Manhardt,Federico Tombari,Christian Rupprecht*

Main category: cs.CV

TL;DR: 该论文提出通过极线几何约束来改进视频扩散模型，解决几何不一致、不稳定运动和视觉伪影问题，使用基于偏好的优化方法在无需端到端可微分的情况下有效实施几何原则。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型虽然在潜在扩散变换器和整流流技术方面取得进展，但仍存在几何不一致、不稳定运动和视觉伪影等问题，破坏了真实3D场景的幻觉。3D一致视频生成对生成和重建任务具有重要影响。

Method: 使用成对极线几何约束通过基于偏好的优化来对齐扩散模型，直接解决不稳定相机轨迹和几何伪影问题。在静态场景和动态相机上进行训练，确保高质量测量，同时模型能有效泛化到多样化动态内容。

Result: 评估表明，经典几何约束比现代学习指标提供更稳定的优化信号，后者会产生噪声目标从而影响对齐质量。该方法在无需端到端可微分的情况下有效实施几何原则。
...
### [63] [DAP-MAE: Domain-Adaptive Point Cloud Masked Autoencoder for Effective Cross-Domain Learning](https://arxiv.org/abs/2510.21635)
*Ziqi Gao,Qiufu Li,Linlin Shen*

Main category: cs.CV

TL;DR: DAP-MAE是一种域自适应点云掩码自编码器预训练方法，通过异构域适配器和域特征生成器，自适应地整合跨域数据集知识，在单一预训练下在多个点云分析任务中取得优异性能。
...
Method: 设计了异构域适配器（在预训练时使用适应模式，微调时使用融合模式）和域特征生成器，指导点云特征适应不同下游任务。

Result: 在四个不同点云分析任务中表现优异：ScanObjectNN物体分类达到95.18%，Bosphorus面部表情识别达到88.45%。

Conclusion: DAP-MAE能够有效整合跨域点云数据知识，通过单一预训练在多种下游任务中实现卓越性能，解决了数据稀缺和域对齐问题。
...
### [64] [A Dynamic Knowledge Distillation Method Based on the Gompertz Curve](https://arxiv.org/abs/2510.21649)
*Han Yang,Guangjun Qin*

Main category: cs.CV

TL;DR: 提出Gompertz-CNN动态知识蒸馏框架，通过Gompertz生长模型调整蒸馏损失权重，结合Wasserstein距离和梯度匹配，在CIFAR数据集上比传统方法提升8%和4%准确率
...
Conclusion: Gompertz-CNN框架通过动态调整蒸馏策略，有效提升了知识蒸馏性能，证明了考虑学生模型学习进程的重要性
...
<details>
  <summary>Details</summary>
Motivation: NACTI数据集存在严重的类别不平衡问题，最大类别占数据集的50%以上。为了解决这种长尾分布对物种识别性能的影响，需要研究专门的长尾识别方法。

Method: 基于PyTorch Wildlife模型，系统研究了多种LTR损失函数和LTR敏感的正则化方法，构建了减少偏差的测试集来评估域偏移情况。

Result: 最佳配置在NACTI测试集上达到99.40%的Top-1准确率，相比标准交叉熵的95.51%基线有显著提升。在域偏移测试中达到52.55%的准确率，优于WCE损失的51.20%。

Conclusion: LTR增强的调度器选择与先进LTR损失函数结合能带来持续改进，但在严重域偏移下对尾部类别的灾难性失效问题仍未完全解决。
...
TL;DR: 提出一种基于邻近突触相似性的无监督突触分类方法，无需预先知道突触类型数量，适用于果蝇数据。


<details>
  <summary>Details</summary>
Motivation: 传统突触分类需要监督学习，需要提供不同类型突触的示例。本文旨在开发仅基于邻近突触相似性的无监督分类方法。

Method: 利用邻近突触比随机选择的突触更相似的观察，开发无监督分类算法，无需预先指定突触类型数量。

Result: 方法成功应用于果蝇数据，能够有效分离突触类型。

Conclusion: 该方法提供了一种原则性的无监督突触分类方式，可用于生成覆盖突触结构范围的真实标注数据。
...
TL;DR: 本研究评估了UNI和Virchow2两个基础模型作为特征提取器，用于将全切片图像分类为黑色素细胞性、基底样和鳞状病变三种诊断类别。Virchow2在大多数分类器中表现优于UNI，逻辑回归达到90%准确率。


<details>
  <summary>Details</summary>
Motivation: 皮肤病理学中全切片图像的快速生成需要自动化方法进行高效处理和准确分类，以应对日益增长的数据处理需求。

Method: 使用UNI和Virchow2基础模型提取补丁级嵌入，通过均值聚合策略聚合成切片级特征，然后训练多种机器学习分类器（逻辑回归、梯度提升树、随机森林）。

Result: Virchow2提取的特征在大多数切片级分类器中表现优于UNI，逻辑回归模型达到90%准确率，但差异无统计学显著性。均值聚合方法提供了可靠的切片级特征表示。

Conclusion: 基础模型在自动化全切片图像分类方面具有潜力，为皮肤病理学诊断提供了可扩展且有效的方法，并为切片级表示学习的未来发展铺平了道路。
...
TL;DR: WorldGrow是一个用于无限扩展3D场景合成的分层框架，通过利用预训练3D模型的生成先验，实现大规模、连续环境的几何一致性和真实外观生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在关键挑战：2D提升方法存在视图间几何和外观不一致，3D隐式表示难以扩展，当前3D基础模型主要面向对象而非场景级生成。

Method: 提出三个核心组件：(1)数据筛选流程提取高质量场景块用于训练；(2)3D块修复机制实现上下文感知场景扩展；(3)从粗到细的生成策略确保全局布局合理性和局部几何/纹理保真度。
...
TL;DR: 该研究将可解释性AI方法应用于海豹检测模型，通过多种后处理解释技术验证模型预测的可信度，并识别系统性的错误模式，为生态保护监测提供可审计的决策支持工具。


<details>
  <summary>Details</summary>
Motivation: 生态学中计算机视觉应用滞后，部分原因是缺乏对黑盒神经网络模型的信任。研究旨在通过可解释性方法为预测提供证据，并记录野外部署中的重要限制。

Method: 使用Glacier Bay国家公园的航空图像训练Faster R-CNN检测海豹，并通过梯度类激活映射（HiResCAM、LayerCAM）、LIME和基于扰动的解释方法生成解释。

Result: 解释集中在海豹躯干和轮廓而非周围环境，移除海豹会降低检测置信度。分析发现系统性错误源，包括海豹与黑冰和岩石的混淆。

Conclusion: 通过将目标检测与后处理可解释性相结合，可以超越"黑盒"预测，为保护监测提供可审计的决策支持工具。
...
TL;DR: BachVid是首个无需训练、无需参考图像就能实现一致视频生成的方法，通过分析DiT注意力机制和中间特征，利用缓存变量注入技术确保多视频的前景和背景一致性。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖参考图像或大量训练，且往往只解决角色一致性而忽略背景一致性，需要图像到视频模型的配合。

Method: 基于对DiT注意力机制和中间特征的系统分析，首先生成身份视频并缓存中间变量，然后将这些缓存变量注入到新生成视频的对应位置。

Result: 实验结果表明BachVid在无需额外训练的情况下实现了生成视频的鲁棒一致性。

Conclusion: 该方法为无需参考图像或额外训练的一致视频生成提供了新颖高效的解决方案。
...
Method: 将几何问题实例视为图像，训练标准视觉扩散模型从高斯噪声生成近似有效解的图像。

Result: 模型成功学习将噪声几何结构转换为正确配置，为硬几何问题提供了实用的近似求解框架。

Conclusion: 在图像空间中操作为解决复杂几何问题提供了通用框架，为处理更广泛几何任务打开了大门。
...
TL;DR: 提出了一种自动化框架，通过自反思代理系统性地生成和测试视觉模型可能依赖的视觉属性假设，以检测模型对特定视觉特征的依赖。


<details>
  <summary>Details</summary>
Motivation: 检测视觉模型在图像识别中对特定视觉属性的依赖，对于确保模型鲁棒性、防止过拟合和避免伪相关性至关重要。

Method: 使用自反思代理迭代生成和测试关于模型可能依赖的视觉属性的假设，通过自评估协议评估发现是否准确解释模型行为，并在出现不一致时触发新的实验周期。

Result: 在包含130个模型的基准测试中，自反思代理的性能持续提升，显著优于非反思基线，并能识别CLIP视觉编码器和YOLOv8目标检测器等先进模型中的真实视觉属性依赖。

Conclusion: 自反思机制能有效提升视觉模型依赖检测的性能，该方法可识别先进模型中的真实视觉属性依赖关系。
...
TL;DR: 该论文开发了基于贝叶斯实验设计的蒙特卡洛推理方法，用于提升语言模型在信息寻求任务中的理性决策能力，在协作战舰游戏中显著提高了AI代理的性能表现。


<details>
  <summary>Details</summary>
Motivation: 许多高风险AI应用需要形成数据驱动的假设并进行有针对性的猜测，但现有语言模型在理性信息寻求方面存在不足，特别是在平衡探索（提问）和行动（射击）方面表现不佳。

Method: 提出了协作战舰游戏任务，开发了基于贝叶斯实验设计的蒙特卡洛推理策略，用于提升观察者代理的准确性和船长代理的信息增益。

Result: 观察者代理准确率提升14.7%，船长代理信息增益提升0.227比特（达到噪声上限的94.2%），组合方法使较弱模型在协作战舰游戏中战胜人类和前沿模型。
...
TL;DR: CodeAdapt方法通过结合代码执行和少样本学习，使标准指令微调语言模型在推理能力上达到甚至超越专门训练的推理模型，同时显著提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有的推理模型需要大量计算资源和数据训练，运行成本高且效率低。研究发现标准指令微调语言模型本身就具备强大的推理潜力，只需合适的激发方法。

Method: 提出CodeAdapt方法：结合CodeAct框架（语言模型交替进行自然语言推理和代码执行）和少样本引导式上下文学习，仅需5个训练问题即可激发模型的推理能力。

Result: 在四个模型对和八个任务上的测试显示，CodeAdapt使三个语言模型平均表现超越对应推理模型（最高提升22.9%），同时token效率提升10-81%。在六个任务上四个模型平均表现优于基准（最高提升35.7%）。
...
TL;DR: 提出了FICSIM数据集，用于评估语言模型在计算文学研究中的表现，发现现有模型过于关注表面特征而非语义类别。


<details>
  <summary>Details</summary>
Motivation: 由于长文本细粒度标注成本高、公共领域文学数据污染问题，以及现有嵌入相似性数据集不适用于文学领域任务，需要专门的数据集来评估语言模型在计算文学研究中的实用性。

Method: 构建FICSIM数据集，包含长篇现代小说，基于作者提供的元数据生成12个维度的相似性评分，并由数字人文学者验证。评估了一系列嵌入模型在该任务上的表现。

Result: 发现模型普遍倾向于关注表面特征，而非对计算文学研究有用的语义类别。

Conclusion: 需要专门的数据集来评估语言模型在文学领域的表现，当前模型在捕捉文学语义相似性方面存在局限，数据收集过程重视作者权益和知情同意。
...
### [76] [DispatchMAS: Fusing taxonomy and artificial intelligence agents for emergency medical services](https://arxiv.org/abs/2510.21228)
...
TL;DR: 开发了一个基于临床分类法和大型语言模型的多智能体系统，用于模拟紧急医疗调度场景，评估显示系统在调度有效性和指导效能方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 紧急医疗调度过程面临呼叫者紧张、信息模糊和认知负荷等挑战，需要利用大型语言模型和多智能体系统来增强调度员能力。

Method: 构建了包含32种主诉和6种呼叫者身份的临床分类法，开发了基于AutoGen的多智能体系统，采用混合评估框架（医生评估+自动语言分析）。

Result: 系统表现出色：调度有效性达94%，指导效能达91%，情感分析显示73.7%中性情感，可读性高（Flesch 80.9），60%礼貌风格。

Conclusion: 该系统能够高保真地模拟多样化的临床可信调度场景，可用于调度员培训、协议评估，并为实时决策支持奠定基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型长上下文评估大多依赖简化的合成任务，无法代表真实世界文档理解的复杂性。推翻关系是普通法体系的基础，为长文档法律理解提供了一个重要且现实的测试平台。

Method: 使用236对案例的数据集，评估最先进的大语言模型在识别美国最高法院案例中的推翻关系方面的表现。

Result: 评估揭示了三个关键限制：(1) 时代敏感性——模型对历史案例的表现不如现代案例，显示其训练中存在根本的时间偏见；(2) 浅层推理——模型依赖浅层逻辑启发式而非深入的法律理解；(3) 上下文依赖的推理失败——在复杂开放任务中，模型会产生时间上不可能的关系，尽管在简单上下文中保持基本的时间意识。

Conclusion: 这项工作提供了一个解决现实长上下文评估关键空白的基准，提供了一个反映实际法律推理任务复杂性和重要性的环境。
...
TL;DR: 爱尔兰-BLiMP是首个为爱尔兰语设计的语言能力评估数据集和框架，包含1020个最小对，评估LLMs和人类在11种语言特征上的表现。人类在所有特征上都优于模型，准确率高出16.6%。开源和闭源LLMs之间存在18.1%的性能差距。
...
Method: 基于语言学文献和语法参考书，由流利的爱尔兰语使用者团队手动构建和审查1020个最小对，涵盖11种语言特征。评估现有LLMs和人类参与者的句法知识。

Result: 人类在所有语言特征上都优于所有模型，平均准确率高出16.6%。开源和闭源LLMs之间存在18.1%的性能差距，最强模型(gpt-5)仅达到73.5%准确率，而人类为90.1%。人类和模型在爱尔兰语语法的不同方面存在困难。

Conclusion: 爱尔兰-BLiMP为评估LLMs在爱尔兰语中的语法能力提供了首个系统性框架，为推进低资源语言的语言理解研究提供了有价值的基准。
...
TL;DR: 本文提出了置信度门控的思维链方法，让模型仅在直接答案置信度低时才启用推理，以减少不必要的token使用。


<details>
  <summary>Details</summary>
Motivation: 思维链提示能提升大语言模型的推理能力，但会显著增加token使用量，且在某些任务中可能无效甚至有害。需要确定何时应该使用思维链。

Method: 评估四种无需训练的置信度估计方法，用于思维链门控，并与随机基线和知道何时需要思维链的预言机进行比较。

Result: 现有无需训练的置信度方法可以减少冗余思维链，优于随机调用思维链。但不同置信度方法的效果不一致，随数据集和模型变化。

Conclusion: 当前方法在自适应思维链门控方面具有潜力但也存在局限性，需要更可靠的方法来实现实际部署。
...
TL;DR: 该研究量化了输入数据结构对LLM生成NBA比赛摘要中幻觉和其他事实错误的影响，发现JSON格式输入相比非结构化输入能显著减少69%（Llama）和65%（Qwen）的错误率。
...
Method: 使用三种输入格式（行结构化、JSON和非结构化）生成NBA比赛摘要，手动标注3,312个事实错误，涵盖180个比赛摘要，使用两个模型（Llama-3.1-70B和Qwen2.5-72B）。

Result: 输入结构对错误率有显著影响：JSON输入相比非结构化输入减少错误率69%（Llama）和65%（Qwen），行结构化输入减少54%（Llama）和51%（Qwen）。方差分析显示输入结构解释了80%以上的错误率方差。

Conclusion: 输入数据结构对LLM生成文本的事实准确性有决定性影响，结构化输入（特别是JSON格式）能显著减少事实错误和幻觉。
...
TL;DR: 研究发现推理增强的LLMs在低假阳性率(FPR)场景下表现不佳，虽然能提升整体准确率，但在需要严格精度的实际应用中，无推理模式表现更好。


<details>
  <summary>Details</summary>
Motivation: 研究推理在精度敏感任务中的适用性，特别是在需要严格控制假阳性率的分类任务中。

Method: 系统研究推理在安全检测和幻觉检测任务中的表现，比较微调和零样本设置下标准LLMs与大型推理模型(LRMs)的性能。

Result: 推理增强生成提高整体准确率，但在低FPR阈值下表现不佳；无推理模式在精度敏感场景中占优；基于token的评分优于自我置信度评估；两种模式的简单集成能发挥各自优势。
...
TL;DR: DR-IKE是一个动态检索框架，通过强化学习训练BERT检索器来根据编辑效用排序演示样本，并使用可学习阈值修剪低价值示例，实现无需修改模型权重的知识编辑。


<details>
  <summary>Details</summary>
Motivation: 解决现有上下文知识编辑方法依赖静态演示集导致的量质权衡问题和缺乏任务难度适应性的问题。

Method: 训练BERT检索器使用REINFORCE算法按编辑奖励排序演示样本，采用可学习阈值动态选择支持性演示，根据任务难度调整提示长度。

Result: 在COUNTERFACT基准测试中，编辑成功率提升17.1%，延迟降低41.6%，同时保持无关查询的准确性。

Conclusion: DR-IKE展示了可扩展和自适应的知识编辑能力，适用于黑盒LLM，仅需前向传播即可实现知识更新。
...
TL;DR: 本研究将自适应RAG系统应用于印尼语问答，通过问题复杂度分类器确定回答策略，使用机器翻译进行数据增强，发现分类器可靠但多检索策略存在不一致性。
...
Method: 采用自适应RAG系统，集成问题复杂度分类器来确定回答策略，并使用机器翻译作为数据增强方法来克服印尼语数据集有限的问题。

Result: 实验显示问题复杂度分类器表现可靠，但多检索回答策略存在显著不一致性，对整体评估产生负面影响。

Conclusion: 研究突出了低资源语言问答的潜力和挑战，为未来改进指明了方向。
...
Method: 构建包含3,190名患者5,894条去标识记录的CDrugRed数据集，包含患者人口统计、病史、临床过程和出院诊断等信息，并使用最先进的大语言模型进行基准测试。

Result: 监督微调能提升模型性能，但仍有很大改进空间，最佳模型的F1分数为0.5648，Jaccard分数为0.4477。

Conclusion: 临床药物推荐任务具有复杂性，CDrugRed是一个具有挑战性且有价值的资源，可用于开发更稳健准确的药物推荐系统。
...
TL;DR: 提出Self-Rewarding PPO方法，结合SFT和PPO的优势，通过设计基于SFT模型和预训练基础模型对数策略比率的奖励函数，实现无需人类偏好标注的在线策略微调，提升泛化性能和数据效率。


<details>
  <summary>Details</summary>
Motivation: 传统监督微调(SFT)作为离策略方法，在有限数据场景下容易过拟合且泛化能力差，需要解决这些局限性。

Method: 结合SFT和近端策略优化(PPO)，设计奖励函数作为SFT模型与预训练基础模型的对数策略比率，作为隐式奖励信号，实现自奖励机制。

Result: 在多个自然语言处理任务上的实证评估表明，Self-Rewarding PPO始终优于传统SFT方法，特别是在高质量标注数据稀缺的场景中。

Conclusion: 该方法有效解决了SFT的局限性，提高了泛化能力、数据效率和鲁棒性，为使用演示数据对齐大语言模型提供了有效途径。
...
<details>
  <summary>Details</summary>
Motivation: 现有忠实性检测基准存在标注模糊问题，主要由于生成输出中允许使用外部知识的边界不明确，导致标注不一致。

Method: 提出新的忠实性标注框架，引入中间类别Out-Dependent来分类需要外部知识验证的情况，并构建VeriGray基准。

Result: 统计显示SOTA LLM在摘要任务中仍有约6%的句子存在幻觉，约8%的生成句子属于Out-Dependent类别，基准对多种基线方法构成显著挑战。
...
TL;DR: 该调查首次系统性地从编排角度回顾了LLM与文本属性图的集成，提出了新的分类法，涵盖LLM增强图任务和图结构改进LLM推理两个方向，并讨论了编排策略、技术方法和应用领域。


<details>
  <summary>Details</summary>
Motivation: 大语言模型具有强大的语义理解和生成能力，但其黑盒性质限制了结构化推理；文本属性图提供显式关系结构但缺乏语义深度。两者的结合可以产生互补优势。

Method: 提出了新的分类法：LLM for TAG（LLM丰富图任务）和TAG for LLM（图结构改进LLM推理）。将编排策略分为顺序、并行和多模块框架，并讨论了TAG特定的预训练、提示和参数高效微调技术。

Result: 总结了实证见解，整理了可用数据集，并突出了在推荐系统、生物医学分析和知识密集型问答等领域的多样化应用。

Conclusion: 为语言和图学习的交叉领域研究提供了系统指导，指出了开放挑战和未来的研究方向。
...
TL;DR: LLMs在社交模拟中不能忠实再现真实人类行为，而是表现出过度理想化的行为模式，存在社会期望偏差、社会角色偏差、首因效应和积极性偏差，导致形成缺乏真实人类互动复杂性的"乌托邦"社会。


<details>
  <summary>Details</summary>
Motivation: 需要评估LLMs在模拟人类社交行为时的可靠性，因为LLMs在社会科学研究中的应用日益增多，但其与真实人类行为的差异尚未充分探索，可能带来科学研究和实际应用中的误读风险。

Method: 提出系统框架分析LLMs的社交行为，通过聊天室式对话模拟多智能体互动，并在五个语言维度上进行分析，检验涌现的社会认知偏差。

Result: 实验发现LLMs不能忠实再现真实人类行为，而是反映过度理想化的版本，表现出社会角色偏差、首因效应和积极性偏差，形成缺乏复杂性的"乌托邦"社会。
...
TL;DR: 提出了一个针对爱沙尼亚语的新LLM基准测试，包含7个多样化数据集，评估了基础模型、开源指令调优模型和商业模型在爱沙尼亚语任务上的表现。
...
Method: 基于7个从本土爱沙尼亚语源生成的数据集构建基准，评估了6个基础模型和26个指令调优模型，结合人工评估和LLM作为评判者的方法。

Result: 人工评估分数与基准评估呈现中度到高度相关性，Claude 3.7 Sonnet作为LLM评判者与人工评分高度一致。

Conclusion: 顶级LLM可以有效支持爱沙尼亚语模型的评估，为爱沙尼亚语NLP研究提供了重要的基准工具。
...
TL;DR: 使用先进自然语言处理技术结合社会学洞见分析极右翼推特内容，揭示移民相关话语模式、仇恨言论和说服技巧
...
Method: 整合语言学、社会学和计算方法，使用最先进的自然语言处理技术分析英语和法语极右翼推特语料库

Result: 揭示了极右翼行为者在移民、仇恨言论和说服技巧方面的话语模式

Conclusion: 通过跨学科方法为理解社交媒体平台上右翼极端主义带来的当代挑战提供了新洞见
...
TL;DR: 本文提出使用相关维度（一种分形几何度量）来量化语言模型感知的文本认识论复杂性，该方法能有效揭示LLM训练阶段、检测幻觉倾向和文本退化问题。


<details>
  <summary>Details</summary>
Motivation: 传统评估指标（如困惑度）强调局部预测准确性，但忽视了长距离结构复杂性，无法解释LLM在低困惑度下仍出现重复和不连贯等令人困惑的行为。

Method: 引入相关维度作为分形几何度量，捕捉语言的分层递归结构，在统一框架中桥接局部和全局属性。该方法计算高效，对模型量化鲁棒，适用于自回归架构。

Result: 实验表明相关维度能够：(1)揭示预训练中的三个不同阶段；(2)反映上下文依赖的复杂性；(3)指示模型产生幻觉的倾向；(4)可靠检测生成文本中的多种退化形式。

Conclusion: 相关维度为LLM的生成动态提供了新的见解，是一个计算高效、鲁棒且广泛适用的文本复杂性评估方法。
...
TL;DR: 提出了一种名为PBS-Attn的置换块稀疏注意力机制，通过利用注意力的置换特性来增加块级稀疏度，在保持模型精度的同时显著提升长上下文预填充的计算效率。


<details>
  <summary>Details</summary>
Motivation: 大语言模型扩展上下文长度具有显著优势但计算成本高昂，主要瓶颈在于自注意力机制的O(N^2)复杂度。虽然注意力矩阵通常是稀疏的，但现有块稀疏注意力方法在块级稀疏度方面存在次优问题。

Method: 提出PBS-Attn方法，通过置换注意力的特性来增加块级稀疏度，使用自定义的置换FlashAttention内核实现高效计算。

Result: 在真实世界长上下文数据集上的实验表明，PBS-Attn在模型精度上持续优于现有块稀疏注意力方法，并接近全注意力基线，在长上下文预填充中实现了最高2.75倍的端到端加速。

Conclusion: PBS-Attn是一种即插即用的高效注意力机制，通过增强块级稀疏度显著提升了长上下文处理的计算效率，同时保持了模型精度。
...
TL;DR: PARL是一种使用LLMs作为RL代理的方法，通过提示而非微调，在非语言环境中进行强化学习任务。


<details>
  <summary>Details</summary>
Motivation: 评估LLMs在强化学习任务中作为代理的能力，特别是在不依赖自然语言的结构化环境中。

Method: 通过提示将动作、状态和奖励编码，让LLMs通过试错与环境交互，无需微调。

Result: 在简单环境中与传统RL代理性能相当或更好，但在需要复杂数学运算或状态解码的任务中存在性能限制。

Conclusion: LLMs可以作为RL代理，但处理复杂数学和状态解码的能力有限。
...
Result: 在四个QA基准测试中，该方法匹配或超越基线方法，在相同样本数量下覆盖更多语义簇。
...
TL;DR: 该研究分析了英语中字符乱序（typoglycemia）现象对NLP模型的影响，发现虽然某些单词在字符乱序后会变得相同，但由于这些单词数量较少且上下文差异明显，模型仍能保持较好的性能。


<details>
  <summary>Details</summary>
Motivation: 研究动机是理解为什么NLP模型在字符内部顺序被打乱时仍能表现良好，尽管许多不同的单词（如form和from）在typoglycemia下会变成相同的表示形式。

Method: 方法包括：(i) 分析英国国家语料库以量化typoglycemia下的单词合并和歧义；(ii) 评估BERT区分合并形式的能力；(iii) 通过比较在干净和typoglycemic维基百科文本上训练的BERT变体进行探测实验。

Result: 结果显示，字符乱序导致的性能下降比预期要小，因为合并的英语单词数量相对较少，且合并的单词往往出现在差异明显的上下文中，使得消歧变得简单。

Conclusion: 结论是英语中typoglycemia导致的单词合并现象有限，且上下文差异使得模型能够轻松处理这种字符乱序情况，这解释了NLP模型对此类扰动的鲁棒性。
...
TL;DR: TripTide是首个评估LLM在现实旅行中断情况下修订行程能力的基准，通过建模中断严重程度和旅客容忍度等维度，全面评估LLM对航班取消、天气关闭等事件的适应性。


<details>
  <summary>Details</summary>
Motivation: 现有旅行规划系统如TripCraft和TravelPlanner虽然能够生成个性化行程，但无法有效处理现实旅行中常见的中断情况，需要评估LLM在应对旅行中断时的适应能力。

Method: 采用三重评估方法：1) 引入自动指标（意图保持、响应性、适应性）；2) 使用LLM作为评判者自动评估修订质量；3) 进行手动专家评估验证语义、空间、序列和响应方面的保持情况。

Result: 实验显示LLM在序列一致性和语义稳定性方面表现良好，空间偏差在短途旅行中较大但随行程延长而减小，但中断处理能力随计划长度增加而下降，表明LLM鲁棒性存在局限。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探索在推理任务中多轮训练是否必要，因为LLMs通常通过单轮强化学习训练，但实际应用涉及多轮人机交互，存在训练与部署条件不匹配的问题。
...
TL;DR: 该论文提出了一个专门针对瑞典相关知识和事件的问答基准数据集，用于评估多语言模型在瑞典语事实知识上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有的瑞典基准大多是从美国中心基准翻译而来，不适合测试瑞典特有的相关知识，特别是那些在国际媒体中报道有限的内容。

Method: 研究人员手动创建了一个问答基准，灵感来自瑞典文化媒体名人的广播节目和主要体育赛事，包含英语翻译以测试跨语言事实一致性。

Result: 研究发现，具有更强瑞典语覆盖的小型模型在回忆瑞典相关事实方面与三倍大的多语言模型表现相当。持续预训练瑞典语通常会提高事实知识，但也会导致部分先前已知信息的遗忘。
...
TL;DR: SindBERT是首个基于RoBERTa的大规模土耳其语编码器模型，在base和large两种配置下发布，填补了土耳其语NLP领域大规模预训练模型的空白。


<details>
  <summary>Details</summary>
Motivation: 尽管Transformer模型在NLP领域取得了革命性进展，但许多形态丰富的语言（如土耳其语）在大规模预训练中仍然代表性不足。

Method: 使用312GB土耳其语文本（mC4、OSCAR23、Wikipedia）从头开始训练RoBERTa-based编码器模型，提供base和large两种配置。

Result: SindBERT在词性标注、命名实体识别、冒犯性语言检测和TurBLiMP语言可接受性基准测试中表现具有竞争力，但large版本仅在4个任务中的2个表现最佳，整体没有一致的扩展优势。
...
Method: 使用49.1GB去重希伯来语网页文本和维基百科，采用希伯来语特定的字节级BPE词汇表，从头训练RoBERTa基础的编码器家族（基础版和大规模版）。
...
TL;DR: 本文研究了视觉语言模型在人类活动识别中的应用，提出了评估方法并证明其性能与传统深度学习模型相当甚至更优。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI的发展，视觉语言模型在医疗健康应用中展现出潜力，但在人类活动识别领域的应用仍相对不足，需要解决其动态和非确定性输出的评估挑战。

Method: 引入了描述性字幕数据集，并提出了全面的评估方法来评估VLMs在人类活动识别中的表现，与最先进的深度学习模型进行对比实验。

Result: 实验结果表明，视觉语言模型在人类活动识别中实现了与传统方法相当的性能，在某些情况下甚至超越了传统方法。
...
TL;DR: 提出了UDCG指标，针对RAG系统中LLM处理检索结果的特点，改进了传统IR指标在位置折扣和相关性评估方面的不足，显著提升了与最终答案准确性的相关性。


<details>
  <summary>Details</summary>
Motivation: 传统IR指标基于人类用户顺序浏览文档的假设，但在RAG系统中，LLM会整体处理所有检索结果，且无关文档会主动降低生成质量，导致传统指标无法准确预测RAG性能。

Method: 引入基于效用的标注方案，量化相关文档的正面贡献和干扰文档的负面影响，提出UDCG指标，采用LLM导向的位置折扣来直接优化与端到端答案准确性的相关性。
...
<details>
  <summary>Details</summary>
Motivation: 当前远程患者监测领域在人机交互方面存在明显不足，大多数研究集中在传感器数据收集、可视化和特定疾病异常检测上，缺乏有效的交互功能。

Method: 系统集成多模态大语言模型、物联网和可穿戴设备，自动收集生命体征、加速度计数据和视频数据，使用异常检测模块（包括跌倒检测模型）识别紧急情况，并通过自然语言处理组件检测患者活动和情绪。

Result: 开发了一个完整原型系统进行测试，实验表明该系统在现实场景中具有可实施性和可扩展性，能够减少医疗专业人员的工作量和医疗成本。
...
Method: 提出多奖励优化方法，利用测试时缩放、拒绝采样和强化学习来直接优化token相关性，并引入组步和重要性采样策略来降低奖励方差
...
Result: 1) 预测新参与者脑数据所需fMRI数据减少5倍；2) 整体脑对齐提高50%；3) 对未见数据集有强泛化能力；4) 下游语义任务性能提升。

Conclusion: 多参与者脑调优方法在神经科学和AI之间建立了双向受益，有助于弥合两个领域之间的差距。
...
TL;DR: 提出一种基于机制检测的方法，通过外部语境分数和参数知识分数来预测RAG系统中的幻觉问题，该方法在Qwen3-0.6b上训练的分类器能够泛化到GPT-4.1-mini响应。


<details>
  <summary>Details</summary>
Motivation: RAG系统虽然整合外部知识来减少幻觉，但模型仍会生成与检索内容不一致的输出。现有方法通常混淆外部语境和参数知识的贡献，需要准确分离这两者来进行幻觉检测。

Method: 研究RAG幻觉机制，发现当后层FFN模块不成比例地将参数知识注入残差流时会产生幻觉。基于外部语境分数和参数知识分数，在Qwen3-0.6b上计算各层和注意力头的分数，并训练回归分类器来预测幻觉。

Result: 评估了该方法与最先进LLM（GPT-5、GPT-4.1）和检测基线（RAGAS、TruLens、RefChecker）的对比。在Qwen3-0.6b上训练的分类器能够泛化到GPT-4.1-mini响应，展示了代理模型评估的潜力。

Conclusion: 机制信号是RAG系统中幻觉检测的高效、可泛化的预测指标，为幻觉检测提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 旨在通过数学框架（特别是范畴论）来理解和操作文档结构，解决文档信息量化、摘要生成和内容扩展等问题，同时提升预训练模型的性能。

Method: 1. 将文档表示为问答对的范畴；2. 开发正交化程序分割文档信息；3. 基于前两步开发信息度量、摘要和扩展方法；4. 使用大规模预训练模型实现技术；5. 提出基于RLVR的自监督方法改进模型一致性。

Result: 开发了新的信息度量方法、摘要技术、文档扩展解决方案，以及改进预训练模型的自监督方法，实现了对文档结构的数学化处理和多模态扩展。

Conclusion: 范畴论为文档结构分析提供了强大的数学框架，能够有效支持信息量化、内容操作和模型改进，具有广泛的应用潜力。
...
TL;DR: 研究发现LLMs对语言谱系结构有敏感性，但训练数据不平衡仍是影响多语言性能的主要因素。


<details>
  <summary>Details</summary>
Motivation: 探索语言谱系结构在LLMs多语言行为变化中的作用，特别是LLMs是否对语言属级别结构敏感。

Method: 扩展MultiQ数据集分析，检查模型是否倾向于切换到谱系相关语言，以及知识一致性在属内是否比跨属更好。

Result: 发现属级别效应存在但受训练资源可用性强烈制约，不同LLMs家族表现出不同的多语言策略。

Conclusion: LLMs编码了属级别结构的某些方面，但训练数据不平衡仍是塑造其多语言性能的主要因素。
...
TL;DR: 本文介绍了斯洛文尼亚语的首个语用学理解基准SloPragEval和SloPragMega，包含405个选择题，评估LLMs在理解情境意义和文化特定表达方面的能力。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型能力提升，需要更挑战性的评估来超越表层语言能力，关注语用学理解——即理解情境意义、语言和文化规范塑造的隐含含义。

Method: 构建了斯洛文尼亚语语用学理解基准，包含405个选择题；讨论了翻译困难，建立了人类基线，并进行了LLMs的试点评估。

Result: 当前模型在理解细微语言方面有很大改进，但在推断非字面表达（特别是文化特定表达）的隐含说话者意义时仍会失败；专有模型与开源模型之间存在显著差距。
...
TL;DR: 本文提出了无监督异常检测方法，用于识别词表中音位组合的不一致性，以检测转录错误和未记录的借词。
...
Method: 使用字符级和音节级音位组合特征的无监督异常检测算法，应用于多语言的Kokborok语变体与孟加拉语数据集。
...
TL;DR: RETuning方法通过构建分析框架、组织和评分证据来增强大语言模型在股票预测中的推理能力，减少对分析师观点的依赖，实现独立逻辑推理。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在股票预测任务中过度依赖分析师观点，缺乏系统性的独立分析逻辑，未能充分利用其推理能力。

Method: 提出Reflective Evidence Tuning (RETuning)方法，在强化学习前进行冷启动，动态构建分析框架，组织和评分支持股价上涨或下跌的证据，确保独立逻辑推理。

Result: 实验表明RETuning成功解锁了模型在金融领域的推理能力，即使在6个月后或分布外股票上，推理时间缩放仍然有效。

Conclusion: RETuning方法通过增强模型的独立分析能力，显著提升了股票预测的可靠性，使模型能够获得有价值的股票走势预测洞察。
...
<details>
  <summary>Details</summary>
Motivation: 现有从经典逻辑到概率模型的方法只能解释推理的输出或个体建模方面，缺乏对一般人类推理动态的统一定量描述。

Method: 使用大语言模型作为概率编码器，在推理的每一步量化信息熵和增益，通过细粒度分析不同任务来建模推理动态。

Result: IF-Track成功捕捉了基本推理特征，识别系统性错误模式，表征个体差异，并调和了单双过程理论，揭示了人工智能与人类认知的对齐。
...
### [113] [\textsc{autoresearcher}: Automating Knowledge-Grounded and Transparent Research Ideation with Multi-Agent Collaboration](https://arxiv.org/abs/2510.20844)
*Jiawei Zhou,Ruicheng Zhu,Mengshi Chen,Jianwei Wang,Kai Wang*

Main category: cs.MA

TL;DR: AutoResearcher是一个多代理系统，用于基于文献的知识驱动和透明化创意生成，通过四个结构化阶段实现多样化和证据对齐的假设生成。


<details>
  <summary>Details</summary>
Motivation: 现有代理系统往往缺乏透明度，输出结果虽然看似合理但缺乏充分依据，研究人员难以控制和验证。

Method: 集成四个精心设计的阶段：结构化知识整理、多样化创意生成、多阶段创意选择、专家小组评审与综合，并暴露中间推理状态和执行日志。

Result: 在图挖掘案例研究中成功生成了独特、合理的假设，并提供证据和批判性分析。

Conclusion: AutoResearcher提供了一个领域无关的透明化创意生成框架，能够生成既多样化又与证据对齐的假设。
...
### [114] [HIKMA: Human-Inspired Knowledge by Machine Agents through a Multi-Agent Framework for Semi-Autonomous Scientific Conferences](https://arxiv.org/abs/2510.21370)
*Zain Ul Abideen Tariq,Mahmood Al-Zubaidi,Uzair Shah,Marco Agus,Mowafa Househ*

Main category: cs.MA

TL;DR: HIKMA半自主会议是首个将AI端到端集成到学术出版和展示流程中的实验，重新构想了学术交流方式。
...
Method: 设计并实施了HIKMA框架，包括AI数据集策展、AI生成手稿、AI辅助同行评审、AI驱动修订、AI会议展示和AI档案传播。

Result: 该会议作为测试平台和概念验证，为AI赋能学术研究提供了机会和挑战的见解。

Conclusion: HIKMA展示了AI如何通过结合语言模型、结构化研究流程和领域保障措施来支持学术实践，同时探讨了AI作者身份、责任归属等问题。
...
### [115] [ColorEcosystem: Powering Personalized, Standardized, and Trustworthy Agentic Service in massive-agent Ecosystem](https://arxiv.org/abs/2510.21566)
...
TL;DR: 提出了ColorEcosystem蓝图，旨在解决大规模智能体生态系统中的个性化、标准化和可信性问题，包含智能体载体、智能体商店和智能体审计三个核心组件。
...
Method: ColorEcosystem包含三个关键组件：智能体载体（利用用户特定数据和创建数字孪生提供个性化服务体验）、智能体商店（作为集中化、标准化的平台管理多样化智能体服务）、智能体审计（基于对开发者和用户活动的监督，确保服务提供者和用户的完整性和可信度）。

Result: 通过分析挑战、过渡形式和实践考虑，ColorEcosystem有望在大规模智能体生态系统中实现个性化、标准化和可信的智能体服务。部分功能已实现并开源。

Conclusion: ColorEcosystem为解决大规模智能体生态系统中的关键问题提供了一个可行的蓝图，通过其三个核心组件实现了服务的个性化、标准化和可信性保障。
...
TL;DR: 提出了一种人机交互流程，将手绘平面图转换为语义一致的3D BIM模型，使用多模态大语言模型和多代理框架，通过反馈迭代提升准确性。
...
Method: 使用多模态大语言模型的多代理框架，结合感知提取、人工反馈、模式验证和自动化BIM脚本生成，将草图迭代优化为结构化JSON布局，再转换为可执行脚本生成3D BIM模型。

Result: 在10个不同平面图上测试，门窗口检测初始准确率高，墙体检测从83%开始，经过几次反馈迭代后达到近乎完美对齐。所有类别的精确率、召回率和F1分数均高于0.83，几何误差通过反馈修正逐步降至零。

Conclusion: MLLM驱动的多代理推理能够使BIM创建对专家和非专家都变得易于访问，仅需使用手绘草图即可完成。
...
Method: 使用两个在WikiArt概念上微调的GPT-2模型：概念连贯性模型评估概念组合的合理性，文化语境模型估计概念组合在艺术家作品中的典型性。CAS选择高连贯性、低典型性的组合。

Result: 在人类评估中优于随机选择和GPT-4o基线，与艺术学生在原创性和和谐度上表现相当；定量研究显示比GPT-4o产生更多样化的输出和更广泛的概念空间探索。

Conclusion: 人工文化异化能够释放自主代理的创造潜力，在保持内部一致性的同时生成新颖创意。
...
<details>
  <summary>Details</summary>
Motivation: 传统模糊数运算存在计算复杂度高、运算结果可能不符合原始特征（如两个三角模糊数相乘结果不是三角模糊数）、模糊度随运算次数增加而扩散等问题，限制了模糊数的应用范围。

Method: 定义了外延模糊数及其运算方法（包括算术运算和关系运算符=、>、>=、<、<=），并通过应用实例进行说明。
...
TL;DR: 提出一个基于开源大语言模型的框架，用于从异构电子健康记录中提取标准化的阿片类药物使用障碍治疗处方信息，并计算药物覆盖天数。


<details>
  <summary>Details</summary>
Motivation: 解决不同电子健康记录系统中药物数据格式不统一的问题，特别是阿片类药物使用障碍治疗处方的标准化监控需求。

Method: 定制开源大语言模型（Llama、Qwen、Gemma、MedGemma）来提取处方属性，使用固定JSON模式处理记录，并进行轻量级标准化和跨字段一致性检查。
...
TL;DR: 本文通过范围综述分析了AIES和FAccT会议中关于可信AI的研究现状，发现当前研究过于侧重技术属性而忽视社会技术维度，提出了结合技术严谨性与社会文化因素的综合框架。
...
Result: 研究发现虽然透明度、问责制和鲁棒性等技术属性的定义取得显著进展，但当前研究往往过度强调技术精度而牺牲社会伦理考量，AI系统的社会技术性质仍较少探索，可信度成为由有权定义者塑造的争议概念。

Conclusion: 需要结合技术严谨性与社会、文化和制度考量的跨学科方法，作者为AI伦理社区提出了采用真正解决AI系统与社会复杂互动的整体框架的可操作措施，以促进惠及所有利益相关者的负责任技术发展。
...
### [121] [CXRAgent: Director-Orchestrated Multi-Stage Reasoning for Chest X-Ray Interpretation](https://arxiv.org/abs/2510.21324)
...
Result: 在多个CXR解读任务上表现优异，能提供视觉证据，并良好泛化到不同复杂度的临床任务。

Conclusion: CXRAgent通过多阶段协作框架显著提升了CXR诊断的适应性、可信度和推理能力。
...
### [122] [Epistemic Deference to AI](https://arxiv.org/abs/2510.21043)
*Benjamin Lange*

Main category: cs.AI

TL;DR: 该论文探讨何时应该优先采纳AI输出而非人类专家判断，提出了AI预占主义观点及其替代方案——全证据视角的AI遵从理论。


<details>
  <summary>Details</summary>
Motivation: 基于社会认识论，探讨AI系统作为人工认识权威的地位，以及如何合理处理AI输出与人类独立认识理由之间的关系。

Method: 通过分析AI预占主义观点及其经典反对意见，发展出全证据视角的AI遵从理论，将AI输出视为贡献性理由而非完全替代。

Result: 提出了全证据视角的三个关键优势：防止专业技能萎缩、为有意义的人类监督提供认识论基础、解释AI不可靠时的合理不信任。

Conclusion: 全证据视角为确定何时合理遵从AI提供了原则性方法，特别适用于需要严格可靠性的高风险情境。
...
### [123] [From Questions to Queries: An AI-powered Multi-Agent Framework for Spatial Text-to-SQL](https://arxiv.org/abs/2510.21045)
*Ali Khosravi Kazazi,Zhenlong Li,M. Naser Lessani,Guido Cervone*

Main category: cs.AI

TL;DR: 提出了一种多智能体框架，将自然语言问题准确翻译为空间SQL查询，通过知识库、语义增强和多智能体协作解决空间查询的复杂性。
...
Result: 在KaggleDBQA基准测试中总体准确率达到81.2%（272个问题中的221个）；在空间查询中总体准确率达到87.7%（90个问题中的79个），相比没有审查代理的76.7%有显著提升。

Conclusion: 该工作使空间分析更加易用，为空间文本到SQL系统提供了稳健、可推广的基础，推动了自主GIS的发展。
...
### [124] [MedAlign: A Synergistic Framework of Multimodal Preference Optimization and Federated Meta-Cognitive Reasoning](https://arxiv.org/abs/2510.21093)
...
Conclusion: MedAlign框架有效解决了医疗视觉问答中LVLM的幻觉问题，提高了推理效率，并支持多机构协作。
...
### [125] [Confounding Robust Deep Reinforcement Learning: A Causal Approach](https://arxiv.org/abs/2510.21110)
*Mingxuan Li,Junzhe Zhang,Elias Bareinboim*

Main category: cs.AI

TL;DR: 提出了一种对观测数据中混杂偏差具有鲁棒性的深度强化学习算法，基于DQN框架，在存在未观测混杂因子的复杂高维环境中寻找与观测兼容的最坏情况环境下的安全策略。


<details>
  <summary>Details</summary>
Motivation: 在存在未观测混杂因子的复杂高维环境中，传统的离策略学习方法如Q学习可能因数据偏差而失效，需要开发对混杂偏差具有鲁棒性的算法。

Method: 基于深度Q网络(DQN)，提出了一种新算法，寻找与观测兼容的最坏情况环境下的安全策略，以应对观测数据中的混杂偏差。

Result: 在12个存在混杂的Atari游戏中测试，该算法在所有存在行为策略与目标策略输入不匹配且存在未观测混杂因子的游戏中，始终优于标准DQN。

Conclusion: 所提出的算法能够有效处理观测数据中的混杂偏差，在存在未观测混杂因子的复杂环境中表现出优于标准DQN的性能。
...
### [126] [DAO-AI: Evaluating Collective Decision-Making through Agentic AI in Decentralized Governance](https://arxiv.org/abs/2510.21117)
*Chunghyun Han,Alfio Gliozzo,Junkyu Lee,Agostino Capponi*

Main category: cs.AI

TL;DR: 该研究首次实证分析了AI代理作为自主决策者在去中心化治理中的表现，通过构建AI投票代理在真实DAO环境中评估其与人类决策的一致性。


<details>
  <summary>Details</summary>
Motivation: 探索AI代理在去中心化自治组织(DAO)治理中作为自主决策者的可行性，研究其能否增强集体决策过程。

Method: 基于3000多个主要协议提案，构建AI投票代理，通过模块化可组合程序(MCP)工作流在基于可验证区块链数据的金融模拟环境中运行，代理能够解读提案背景、检索历史审议数据并独立确定投票立场。

Result: 研究发现AI代理的决策与人类及代币加权结果高度一致，通过精心设计的评估指标验证了这种强对齐性。

Conclusion: AI代理能够通过产生可解释、可审计且基于实证的信号来增强集体决策，为去中心化金融系统中可解释且经济严谨的AI代理设计提供了贡献。
...
### [127] [PanicToCalm: A Proactive Counseling Agent for Panic Attacks](https://arxiv.org/abs/2510.21143)
*Jihyun Lee,Yejin Min,San Kim,Yejin Jeon,SungJun Yang,Hyounghun Kim,Gary Geunbae Lee*

Main category: cs.AI

TL;DR: 提出了PACE数据集和PACER咨询模型，用于恐慌发作时的心理急救干预，通过多维度评估框架PanicEval验证了模型的有效性。
...
Result: 实验结果显示PACER在咨询师侧指标和客户情感改善方面均优于强基线模型。人类评估进一步证实其实际价值，在恐慌场景中始终优于通用、CBT基础和GPT-4驱动的模型。

Conclusion: PACER模型在恐慌发作干预中表现出色，为心理急救提供了有效的AI解决方案。
...
### [128] [NeuroGenPoisoning: Neuron-Guided Attacks on Retrieval-Augmented Generation of LLM via Genetic Optimization of External Knowledge](https://arxiv.org/abs/2510.21144)
...
<details>
  <summary>Details</summary>
Motivation: 现有RAG攻击方法忽视了模型内部表示动态和神经元级敏感性，且未考虑与强参数化知识的知识冲突问题。
...
Conclusion: 基于毒害响应神经元的引导投毒能有效解决知识冲突，NeuroGenPoisoning框架能大规模生成有效的RAG投毒知识。
...
### [129] [How to Auto-optimize Prompts for Domain Tasks? Adaptive Prompting and Reasoning through Evolutionary Domain Knowledge Adaptation](https://arxiv.org/abs/2510.21148)
...
Method: 提出EGO-Prompt框架，从专家构建的初始语义因果图(SCG)出发，通过因果引导的文本梯度过程生成确定性推理指导，并采用迭代优化算法同时优化SCG和推理机制。

Result: 在公共卫生、交通和人类行为任务中，EGO-Prompt比前沿方法F1分数提升7.32%-12.61%，小模型能以不到20%成本达到大模型性能，并输出可解释的领域特定SCG。

Conclusion: EGO-Prompt有效解决了领域特定任务中提示设计和推理优化问题，实现了性能显著提升和成本大幅降低，同时增强了模型的可解释性。
...
### [130] [String Seed of Thought: Prompting LLMs for Distribution-Faithful and Diverse Generation](https://arxiv.org/abs/2510.21150)
...
<details>
  <summary>Details</summary>
Motivation: LLMs在需要单一确定性答案的任务上表现出色，但在概率指令跟随(PIF)任务中表现不佳，存在偏见问题。这影响了需要非确定性行为的应用，如人类行为模拟、内容多样化和多人游戏，也损害了生成响应的多样性。

Method: SSoT提示方法：首先让LLM输出一个随机字符串生成足够熵，然后通过操作这个字符串提取随机性来推导最终答案，在保持多样性的同时遵守特定约束。

Result: SSoT显著提高了LLMs的PIF性能，接近伪随机数生成器的理想性能。在NoveltyBench上的实验表明，SSoT的好处不仅限于封闭集任务，还扩展到开放任务，增强了响应多样性。

Conclusion: SSoT是一种简单有效的提示方法，能够解决LLMs在概率指令跟随任务中的偏见问题，提高响应多样性，在需要非确定性行为的应用中具有重要价值。
...
### [131] [Memory-Free Continual Learning with Null Space Adaptation for Zero-Shot Vision-Language Models](https://arxiv.org/abs/2510.21175)
*Yujin Jo,Taesup Kim*
...
<details>
  <summary>Details</summary>
Motivation: 预训练的视觉语言模型在现实部署中面临分布偏移和新任务挑战，静态零样本能力不足，需要持续学习方法来适应变化环境同时避免遗忘。
...
### [132] [Shylock: Causal Discovery in Multivariate Time Series based on Hybrid Constraints](https://arxiv.org/abs/2510.21181)
*Shuo Li,Keqin Xu,Jie Liu,Dan Ye*

Main category: cs.AI

TL;DR: 提出Shylock方法，用于在少样本和正常多变量时间序列中发现因果关系，通过组扩张卷积和共享内核减少参数，结合全局和局部约束提高准确性


<details>
  <summary>Details</summary>
Motivation: 现有因果关系发现方法依赖人工经验、统计方法或图形标准方法，容易出错且依赖大量数据。多变量时间序列存在数据稀缺问题，现有方法容易过拟合

Method: 使用组扩张卷积和共享内核来指数级减少参数数量，同时学习带时间延迟的变量表示。结合全局约束和局部约束实现网络间信息共享
...
### [133] [OutboundEval: A Dual-Dimensional Benchmark for Expert-Level Intelligent Outbound Evaluation of Xbench's Professional-Aligned Series](https://arxiv.org/abs/2510.21244)
...
TL;DR: 提出了OutboundEval基准测试，用于评估大语言模型在专家级智能外呼场景中的表现，解决了现有方法的三大局限：数据集多样性不足、用户模拟不真实和评估指标不准确。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法在智能外呼场景中存在三个关键限制：数据集多样性和类别覆盖不足、用户模拟不真实、评估指标不准确，需要建立更全面的评估标准。

Method: 1) 设计涵盖6大业务领域和30个子场景的基准测试；2) 开发大模型驱动的用户模拟器，生成多样化、个性化的虚拟用户；3) 引入动态评估方法，结合自动化和人工评估。

Result: 在12个最先进的大语言模型上的实验揭示了专家级任务完成度与交互流畅性之间的权衡关系，为构建可靠、类人的外呼AI系统提供了实用见解。
...
### [134] [Out-of-Distribution Detection for Safety Assurance of AI and Autonomous Systems](https://arxiv.org/abs/2510.21254)
*Victoria J. Hodge,Colin Paterson,Ibrahim Habli*

Main category: cs.AI

TL;DR: 这篇综述论文分析了自主系统中OOD检测技术在安全保证方面的应用，重点关注安全关键领域，探讨了OOD的成因、挑战以及在整个ML开发生命周期中可用的检测技术。
...
Method: 采用全面综述方法，定义相关概念，调查OOD成因，分析自主系统安全保证和OOD检测的挑战因素，识别整个ML开发生命周期中可用的技术。

Result: 识别了一系列可在ML开发生命周期不同阶段使用的OOD检测技术，并提出了这些技术如何支持安全保证论证的建议，同时指出了系统集成时的注意事项。

Conclusion: 总结了自主系统安全开发和运营面临的挑战，并提出了未来研究方向，以促进跨领域自主系统的安全部署。
...
### [135] [Investigating Scale Independent UCT Exploration Factor Strategies](https://arxiv.org/abs/2510.21275)
...
<details>
  <summary>Details</summary>
Motivation: UCT算法对游戏奖励尺度敏感，在具有密集奖励的游戏中，Q值可能跨越不同数量级，导致性能问题。

Method: 评估了文献中已有的λ选择策略，并提出了五种新策略，包括使用Q值经验标准差的方法。

Result: 实验结果表明，使用2倍Q值标准差作为λ值的策略在各种任务中表现最佳，优于现有方法。

Conclusion: 推荐使用2σ作为UCT探索常数λ，该方法对奖励尺度不敏感且性能优越。
...
### [136] [When Models Outthink Their Safety: Mitigating Self-Jailbreak in Large Reasoning Models with Chain-of-Guardrails](https://arxiv.org/abs/2510.21285)
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂推理任务上表现出色，但存在严重的安全风险，如生成有害内容和越狱攻击。现有缓解策略依赖注入启发式安全信号，往往会抑制推理能力且无法解决安全-推理权衡问题。
...
Conclusion: CoG框架有效解决了大型推理模型的安全性与推理能力之间的权衡问题，通过系统性地引导模型远离不安全推理轨迹，同时保持其推理性能。
...
### [137] [Towards Reliable Code-as-Policies: A Neuro-Symbolic Framework for Embodied Task Planning](https://arxiv.org/abs/2510.21302)
*Sanghyun Ahn,Wonje Choi,Junyong Lee,Jinwoo Park,Honguk Woo*

Main category: cs.AI

TL;DR: 提出了一种神经符号具身任务规划框架，通过符号验证和交互验证增强代码生成的环境基础，在动态和部分可观测环境中显著提高任务成功率。


<details>
  <summary>Details</summary>
Motivation: 现有LLM生成的代码策略在动态或部分可观测环境中环境基础有限，导致任务成功率低，需要改进代码生成的环境基础。

Method: 采用神经符号方法，在代码生成过程中加入显式符号验证和交互验证阶段，生成探索性代码主动与环境交互获取缺失观测。

Result: 在RLBench和真实世界环境中，任务成功率比Code-as-Policies基线提高46.2%，任务相关动作可执行性达到86.8%。
...
### [138] [Magellan: Guided MCTS for Latent Space Exploration and Novelty Generation](https://arxiv.org/abs/2510.21341)
*Lufan Chang*

Main category: cs.AI

TL;DR: Magellan框架通过蒙特卡洛树搜索和分层引导系统，解决了LLM在创意生成中倾向于高概率常见概念的问题，显著提升了科学创意的合理性和创新性。


<details>
  <summary>Details</summary>
Motivation: LLM在生成创新想法时往往陷入训练数据的"引力井"，依赖无原则的自评估启发式方法限制了探索效果。

Method: 使用蒙特卡洛树搜索，结合语义罗盘向量进行长程引导，以及基于景观感知的价值函数进行局部决策，平衡内在连贯性、外在新颖性和叙事进展。

Result: 在广泛实验中，Magellan在生成科学创意方面显著优于ReAct和ToT等强基线方法，具有更高的合理性和创新性。
...
### [139] [Boosting Accuracy and Efficiency of Budget Forcing in LLMs via Reinforcement Learning for Mathematical Reasoning](https://arxiv.org/abs/2510.21398)
*Ravindra Aribowo Tarunokusumo,Rafael Fernandes Cunha*

Main category: cs.AI

TL;DR: 该论文提出了一种结合强化学习（RL）的框架，用于提高1.5B模型在数学推理任务中的性能，通过减少40%的token使用量同时提升准确率。


<details>
  <summary>Details</summary>
Motivation: 解决预算强制方法依赖监督微调（SFT）导致小模型性能下降的问题，因为长上下文推理轨迹会产生冗长响应。

Method: 集成强化学习（RL）来提高token效率，仅使用1.5K训练样本，在GSM8K数据集上测试不同计算预算下的性能。

Result: SFT+RL模型在GSM8K数据集上表现更好，准确率更高，token使用量比SFT模型减少40%以上。

Conclusion: RL可以恢复因长上下文训练造成的损失，整体提升数学推理性能。
...
### [140] [Advancing Symbolic Integration in Large Language Models: Beyond Conventional Neurosymbolic AI](https://arxiv.org/abs/2510.21425)
*Maneeha Rani,Bhupesh Kumar Mishra,Dhavalkumar Thakker*

Main category: cs.AI

TL;DR: 该论文提出了一种新的符号AI集成到LLMs的分类框架和路线图，以解决LLMs透明度不足的问题。


<details>
  <summary>Details</summary>
Motivation: LLMs在关键领域表现出色但缺乏透明度，现有神经符号AI方法不适用于LLMs特性，需要系统理解符号AI如何有效集成到LLMs中。

Method: 首先回顾现有神经符号AI方法，然后提出符号集成到LLMs的新分类法，包括四个维度的分类框架：LLM不同阶段的符号集成、耦合机制、架构范式、算法和应用层面。

Result: 建立了系统的分类框架，组织了现有文献，识别了当前基准、前沿进展和关键差距。

Conclusion: 通过突出最新发展和文献中的显著差距，为未来研究提供了路线图，并为实现符号集成到LLMs的框架提供了实用见解以增强透明度。
...
### [141] [AutoOpt: A Dataset and a Unified Framework for Automating Optimization Problem Solving](https://arxiv.org/abs/2510.21436)
*Ankur Sinha,Shobhit Arora,Dhaval Pujara*

Main category: cs.AI

TL;DR: AutoOpt-11k是一个包含11,000多个手写和打印数学优化模型图像的数据集，配套开发了AutoOpt框架，通过深度学习模型自动识别数学表达式并生成优化求解代码，实现从图像到优化问题求解的全自动化流程。


<details>
  <summary>Details</summary>
Motivation: 为了解决数学优化问题求解过程中需要人工建模和编程的繁琐过程，研究者希望开发一个能够直接从图像识别优化问题并自动求解的系统，减少人工干预。

Method: 开发了AutoOpt框架，包含三个模块：M1使用深度学习模型进行数学表达式识别生成LaTeX代码；M2使用微调的小型LLM将LaTeX转换为PYOMO脚本；M3使用双层优化分解方法求解优化问题。

Result: 在AutoOpt-11k数据集上，M1模块的数学表达式识别性能优于ChatGPT、Gemini和Nougat；M3模块的双层优化分解方法在复杂测试问题上比内点法和遗传算法表现更好。
...
### [142] [Multi-Task Vehicle Routing Solver via Mixture of Specialized Experts under State-Decomposable MDP](https://arxiv.org/abs/2510.21453)
*Yuxin Pan,Zhiguang Cao,Chengyang Gu,Liu Liu,Peilin Zhao,Yize Chen,Fangzhen Lin*

Main category: cs.AI

TL;DR: 提出了MoSES框架，通过重用基础VRP变体的专用求解器来构建统一求解器，避免了神经网络求解器数量的指数增长。


<details>
  <summary>Details</summary>
Motivation: 现有的多任务车辆路径问题神经方法通常学习统一求解器，但未能充分利用VRP变体的组合结构，错失了专用基础求解器的优势。

Method: 引入状态可分解MDP(SDMDP)将VRP状态空间表示为基础状态空间的笛卡尔积，开发基于潜在空间的SDMDP扩展，实现MoSES求解器通过专用LoRA专家实现基础策略，使用自适应门控机制实现混合函数。

Result: 在多种VRP变体上的广泛实验表明MoSES优于现有方法。

Conclusion: 该框架通过重用基础求解器使统一求解器能够感知VRP变体间的共享组件特性，在保持性能的同时避免了求解器数量的指数增长。
...
### [143] [EU-Agent-Bench: Measuring Illegal Behavior of LLM Agents Under EU Law](https://arxiv.org/abs/2510.21524)
*Ilija Lichkovski,Alexander Müller,Mariam Ibrahim,Tiwai Mhundwa*

Main category: cs.AI

TL;DR: EU-Agent-Bench是一个用于评估LLM代理在欧盟法律框架下合规性的基准测试，通过验证性人工标注来测量代理执行非法行为的潜在倾向。
...
Method: 创建了一个可验证的人工标注基准，评估代理在良性用户输入可能导致非法行为的情况下与欧盟法律规范的契合度。基准涵盖数据保护、偏见/歧视和科学诚信等多个类别，每个用户请求都允许合规和不合规的执行方式。

Result: 通过将模型的函数调用与详尽引用相关立法支持的评分标准进行比较，评估了前沿LLM的法律合规性，并调查了在代理系统提示中提供相关立法摘录并明确要求遵守的合规效果。

Conclusion: 发布了研究社区的公共预览集，同时保留私有测试集以防止数据污染。鼓励未来工作将代理安全基准扩展到不同的法律管辖区以及多轮和多语言交互。
...
TL;DR: Co-Sight通过冲突感知元验证(CAMV)和可信结构化事实推理(TRSF)解决LLM代理长程推理中的验证不足问题，在多个基准测试中达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: LLM代理的长程推理失败往往不是生成能力问题，而是中间推理验证不足导致的。需要将推理转变为可证伪和可审计的过程。

Method: 使用CAMV将验证重构为冲突识别和针对性证伪，只对专家代理间的分歧热点进行计算；TRSF通过结构化事实模块持续组织、验证和同步证据。
...
TL;DR: 提出了一种使用模仿学习（ICL）训练神经控制屏障函数（CBF）的方法，无需明确指定失败状态集，而是通过专家演示数据来学习安全约束函数。


<details>
  <summary>Details</summary>
Motivation: 在关键领域运行的自主系统中，安全是基本要求。传统CBF方法需要明确指定失败状态集，但现实中很多失败状态（如自动驾驶中的跟车过近）难以形式化定义，而专家演示数据更容易获取。

Method: 使用模仿学习训练约束函数来分类系统状态为安全或危险，然后用该函数标注模拟轨迹来训练神经CBF。

Result: 在四个不同环境中进行实证评估，该方法优于现有基线方法，与使用真实安全标签训练的神经CBF性能相当。

Conclusion: 该方法提供了一种数据驱动的神经CBF训练方法，无需明确指定失败状态集，仅需专家演示数据即可实现安全控制。
...
### [146] [Huxley-Gödel Machine: Human-Level Coding Agent Development by an Approximation of the Optimal Self-Improving Machine](https://arxiv.org/abs/2510.21614)
*Wenyi Wang,Piotr Piękos,Li Nanbo,Firas Laakom,Yimeng Chen,Mateusz Ostaszewski,Mingchen Zhuge,Jürgen Schmidhuber*

Main category: cs.AI

TL;DR: 本文提出了Huxley-Gödel Machine (HGM)，一种通过估计自我改进潜力(CMP)来指导代码自我修改树搜索的方法，在SWE-bench等基准上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有编码智能体基于软件工程基准性能进行自我改进，但发现其性能与自我改进潜力之间存在不匹配问题。

Method: 提出CMP指标来评估智能体的自我改进潜力，并基于此构建HGM系统来指导自我修改树的搜索。

Result: HGM在SWE-bench Verified和Polyglot基准上表现优于现有方法，且计算时间更少，在跨数据集和LLM迁移方面表现出色。

Conclusion: HGM方法能够有效解决自我改进潜力与性能的不匹配问题，实现了人类级别的编码性能。
...
### [147] [DeepAgent: A General Reasoning Agent with Scalable Toolsets](https://arxiv.org/abs/2510.21618)
...
TL;DR: DeepAgent是一个端到端的深度推理代理，通过自主思考、工具发现和行动执行在单一连贯推理过程中完成任务，解决了长程交互中的上下文爆炸和历史积累问题。


<details>
  <summary>Details</summary>
Motivation: 现有代理框架通常遵循预定义工作流程，限制了自主性和全局任务完成能力。现实世界任务需要外部工具和长程交互，但现有方法存在上下文长度爆炸和交互历史积累的问题。

Method: 提出自主记忆折叠机制压缩过往交互为结构化记忆，减少错误积累；开发端到端强化学习策略ToolPO，利用LLM模拟API并通过工具调用优势归因对工具调用token分配细粒度信用。

Result: 在8个基准测试（包括通用工具使用任务和下游应用）中，DeepAgent在标记工具和开放集工具检索场景下均优于基线方法。

Conclusion: 这项工作向更通用和强大的现实世界应用代理迈出了一步，DeepAgent展示了在复杂工具使用场景中的优越性能。
...
### [148] [AstaBench: Rigorous Benchmarking of AI Agents with a Scientific Research Suite](https://arxiv.org/abs/2510.21652)
...
TL;DR: 本文提出了AstaBench基准测试套件，用于更严格地评估AI科学代理的能力，包含2400多个科学发现过程的问题，并提供生产级搜索工具和基线代理。


<details>
  <summary>Details</summary>
Motivation: 现有AI代理基准测试存在不足：缺乏真实世界科学研究的整体评估、缺少可复现的工具、未考虑模型成本和工具访问等混杂变量、缺乏标准化接口和全面基线代理。

Method: 定义了严格的基准测试原则和工具，开发了AstaBench套件，包含科学发现全过程的问题，提供生产级搜索环境和9类科学优化代理及基线。

Result: 对57个代理在22个代理类别上的广泛评估显示，尽管在某些方面有进展，但AI在科学研究辅助方面仍远未解决挑战。

Conclusion: 需要更严格的基准测试来评估AI科学代理，AstaBench为此提供了首个全面的科学发现能力评估框架。
...
### [149] [CMOMgen: Complex Multi-Ontology Alignment via Pattern-Guided In-Context Learning](https://arxiv.org/abs/2510.21656)
*Marta Contreiras Silva,Daniel Faria,Catia Pesquita*

Main category: cs.AI

TL;DR: CMOMgen是首个端到端的复杂多本体匹配策略，通过检索增强生成技术选择相关类别并过滤参考映射作为示例，在三个生物医学任务中表现优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 构建全面的知识图谱需要使用多个本体来充分将数据上下文化到特定领域。简单的成对等价映射无法提供相关但不相交本体的完整语义集成，需要更复杂的多本体匹配方法。

Method: 使用检索增强生成技术选择相关类别来组成映射，并过滤匹配的参考映射作为示例来增强上下文学习。该方法不对目标本体或实体的数量设置任何限制。
...
### [150] [A Multimodal Benchmark for Framing of Oil & Gas Advertising and Potential Greenwashing Detection](https://arxiv.org/abs/2510.21679)
*Gaku Morio,Harri Rowlands,Dominik Stammbach,Christopher D. Manning,Peter Henderson*

Main category: cs.AI

TL;DR: 提出了一个用于评估视觉语言模型的多模态框架分析基准数据集，专注于能源公司的公关视频广告，包含13种框架类型的专家标注。


<details>
  <summary>Details</summary>
Motivation: 理解公司在公关活动中使用的框架及其变化，特别是在能源领域识别"绿色洗白"现象，需要大规模的多模态分析方法。

Method: 构建了一个从Facebook和YouTube获取的视频广告数据集，包含来自50多家公司和20个国家的专家标注，专门用于评估视觉语言模型。

Result: 基线实验显示GPT-4.1在检测环境信息方面达到79% F1分数，但最佳模型在识别绿色创新框架方面仅达到46% F1分数。

Conclusion: 该数据集为能源领域战略沟通的多模态分析研究做出了贡献，同时揭示了视觉语言模型在处理隐含框架、不同长度视频和文化背景方面的挑战。
...
### [151] [A Knowledge-Graph Translation Layer for Mission-Aware Multi-Agent Path Planning in Spatiotemporal Dynamics](https://arxiv.org/abs/2510.21695)
...
### [152] [Incentivizing Consistent, Effective and Scalable Reasoning Capability in Audio LLMs via Reasoning Process Rewards](https://arxiv.org/abs/2510.20867)
*Jiajun Fan,Roger Ren,Jingyuan Li,Rahul Pandey,Prashanth Gurunath Shivakumar,Ivan Bulyko,Ankur Gandhe,Ge Liu,Yile Gu*

Main category: cs.LG

TL;DR: CESAR通过在线强化学习框架解决音频大语言模型中的推理逆缩放问题，奖励推理过程而非结果验证，在MMAU和MMSU任务上达到最先进性能


<details>
  <summary>Details</summary>
Motivation: 音频大语言模型的推理能力未被充分探索，推理过程在推理时反而会降低性能，出现测试时逆缩放现象

Method: 提出CESAR框架，采用在线强化学习和Group Relative Policy Optimization，通过多维度奖励机制激励正确性、格式、一致性、结构化分析模式、因果推理、领域知识整合和校准推理深度

Result: 解决了测试时逆缩放问题，在MMAU Test-mini上显著超越Gemini 2.5 Pro和GPT-4o Audio，在MMSU推理任务上接近人类水平
...
### [153] [Crisis-Resilient Portfolio Management via Graph-based Spatio-Temporal Learning](https://arxiv.org/abs/2510.20868)
*Zan Li,Rui Fan*

Main category: cs.LG

TL;DR: CRISP是一个基于图时空学习的金融时间序列预测框架，通过自适应图注意力机制发现危机相关的资产关系，在通胀驱动市场等不同危机机制下实现稳健的资产配置预测。


<details>
  <summary>Details</summary>
Motivation: 现有基于图的时空学习方法依赖预定义的图拓扑结构，无法适应市场动态在不同危机机制（信用传染、疫情冲击、通胀驱动抛售）下的变化，导致预测精度下降。

Method: 使用图卷积网络编码空间关系，BiLSTM与自注意力机制捕捉时间动态，通过多头图注意力网络学习稀疏结构，过滤92.5%的噪声连接，保留危机相关依赖关系。

Result: 在2005-2021年数据上训练，在2022-2024年通胀驱动市场（完全不同机制）上实现稳健泛化，夏普比率达到3.76，比等权重基准提升707%，比静态图方法提升94%。

Conclusion: CRISP通过学习预测而非强加假设，实现了可解释的机制检测和自适应投资组合分配，在危机期间保持盈利能力。
...
### [154] [MOBO-OSD: Batch Multi-Objective Bayesian Optimization via Orthogonal Search Directions](https://arxiv.org/abs/2510.20872)
...
Result: 在多种合成和真实基准函数上，MOBO-OSD在解集多样性和超体积性能方面持续优于最先进算法。

Conclusion: MOBO-OSD是有效的多目标贝叶斯优化算法，能够生成高质量且多样化的帕累托最优解集。
...
<details>
  <summary>Details</summary>
Motivation: 滑坡是日益严重的气候诱发灾害，特别是在高亚洲山区，尽管卫星和时序数据日益丰富，但及时的检测和灾害响应仍然不发达且分散。

Method: 采用多智能体协调框架，包含预测、规划和执行三个相互关联的智能体，结合卫星观测和环境信号，整合当地环境因素。

Result: 该框架提供了可扩展和主动的解决方案，增强了滑坡预测的准确性，实现了实时态势感知和响应规划。

Conclusion: CC-GRMAS为脆弱山区地形提供了一个可扩展和主动的气候韧性灾害准备解决方案，通过多智能体协调提高了滑坡预测和响应能力。
...
### [156] [Multimodal Negative Learning](https://arxiv.org/abs/2510.20877)
*Baoquan Gong,Xiyuan Gao,Pengfei Zhu,Qinghua Hu,Bing Cao*

Main category: cs.LG

TL;DR: 提出了一种新的多模态学习范式"负学习"，通过主导模态动态指导弱模态抑制非目标类别，而不是强制对齐，以解决模态不平衡问题。
...
Method: 提出多模态负学习框架，主导模态动态指导弱模态抑制非目标类别，稳定决策空间并保留模态特定信息。

Result: 实验证明该方法在多个基准测试中有效，能够提高弱模态的鲁棒性，特别是在噪声和不平衡场景下。

Conclusion: 负学习范式能够有效解决模态不平衡问题，保持弱模态的独特信息，提升多模态学习的鲁棒性。
...
### [157] [Assessing the Real-World Utility of Explainable AI for Arousal Diagnostics: An Application-Grounded User Study](https://arxiv.org/abs/2510.21389)
*Stefan Kraft,Andreas Theissler,Vera Wienhausen-Wilke,Gjergji Kasneci,Hendrik Lensch*

Main category: cs.LG

TL;DR: 该研究通过用户实验评估了透明AI辅助在睡眠医学中的效果，发现透明AI作为质量控制步骤能显著提升性能，虽然耗时增加但获得用户广泛接受。


<details>
  <summary>Details</summary>
Motivation: AI系统在生物医学信号解读中表现优异，但临床实践中需要解决何时以及为何信任算法推荐的问题，以促进AI在临床工作流程中的有效整合。

Method: 对8名专业睡眠医学从业者进行应用基础用户研究，在三种条件下对多导睡眠图数据进行夜间觉醒事件评分：手动评分、黑盒AI辅助和透明白盒AI辅助，辅助时机包括起始辅助和事后质量控制。

Result: AI和人类-AI团队均显著优于无辅助专家，透明AI辅助作为质量控制步骤比黑盒辅助提升约30%的事件级性能，质量控制时机进一步改善计数结果，透明辅助获得7/8参与者接受。
...
### [158] [HA-RAG: Hotness-Aware RAG Acceleration via Mixed Precision and Data Placement](https://arxiv.org/abs/2510.20878)
*Danying Ge,Jianhua Gao,Yixue Yang,Weixing Ji*

Main category: cs.LG

TL;DR: 提出HA-RAG系统，通过热感知混合精度压缩加载和热感知数据放置策略，优化RAG推理性能，在TTFT指标上实现平均2.10倍、最高10.49倍的加速。


<details>
  <summary>Details</summary>
Motivation: RAG通过外部知识库提高LLM输出准确性，但引入长上下文处理挑战，显著增加内存消耗和推理延迟。现有方法通过预计算KV缓存来加速推理，但仍有优化空间。

Method: 1. 基于KV块访问频率的热感知混合精度压缩加载方法，减少磁盘I/O和内存访问开销；2. 热感知数据放置策略，优先将高频访问KV块存储在高速内存中。

Result: 相比TurboRAG，HA-RAG在TTFT指标上实现平均2.10倍加速，最高10.49倍加速，且精度损失可忽略。
...
### [159] [Global Dynamics of Heavy-Tailed SGDs in Nonconvex Loss Landscape: Characterization and Control](https://arxiv.org/abs/2510.20905)
*Xingyu Wang,Chang-Han Rhee*

Main category: cs.LG

TL;DR: 本文通过大偏差和亚稳态分析，揭示了重尾SGD能够几乎完全避免尖锐局部极小值，从而获得更好的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 理解SGD为何能避免尖锐局部极小值（与泛化能力差相关）的机制，并进一步强化SGD的这种能力，需要超越传统的局部收敛分析，全面理解SGD的全局动态。

Method: 基于Wang和Rhee（2023）的大偏差和亚稳态分析技术，开发了一套技术工具来精确表征重尾SGD的全局动态。通过在训练阶段注入并截断重尾噪声，结合梯度裁剪。

Result: 模拟和深度学习实验证实了理论预测：重尾SGD结合梯度裁剪能够找到几何形状更平坦的局部极小值，并实现更好的泛化性能。

Conclusion: 重尾SGD通过噪声注入和截断机制，几乎完全避免了尖锐极小值，显著提升了深度学习模型的泛化能力。
...
### [160] [Learning from Interval Targets](https://arxiv.org/abs/2510.20925)
*Rattana Pukdee,Ziqi Ke,Chirag Gupta*

Main category: cs.LG

TL;DR: 研究区间目标回归问题，提出两种方法：基于区间兼容损失函数的泛化边界分析和最小-最大学习框架，在真实数据集上达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 当精确目标标签因成本或固有不确定性而无法获取时，只能获得目标值的上下界区间，传统回归损失函数无法使用，需要开发适用于区间目标的回归方法。

Method: 1. 使用与区间目标兼容的损失函数，基于假设类的平滑性建立非渐近泛化边界；2. 提出最小-最大学习框架：在给定区间内最小化最坏情况（最大化）目标标签，通过加入平滑性约束解决非凸最大化问题。

Result: 在真实世界数据集上进行广泛实验，证明所提方法达到了最先进的性能水平。

Conclusion: 提出的区间目标回归方法有效解决了精确目标不可得的问题，通过理论分析和实验验证了方法的有效性，为不确定性环境下的回归问题提供了实用解决方案。
...
### [161] [Meta-Learning for Cross-Task Generalization in Protein Mutation Property Prediction](https://arxiv.org/abs/2510.20943)
*Srivathsan Badrinarayanan,Yue Su,Janghoon Ock,Alan Pham,Sanya Ahuja,Amir Barati Farimani*

Main category: cs.LG

TL;DR: 本文提出了基于元学习的蛋白质突变性质预测方法，通过MAML和新型突变编码策略，显著提升了跨数据集泛化能力和训练效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖针对特定数据集的微调，在跨数据集泛化方面表现不佳，主要受限于异构实验条件和有限的目标域数据。

Method: 结合MAML元学习和基于分隔符标记的突变编码策略，使transformer架构能够快速适应新任务，避免学习数据集特定模式。

Result: 在三个蛋白质突变数据集上的评估显示，相比传统微调方法，元学习方法在功能适应性预测上准确率提升29%，训练时间减少65%；在溶解度预测上准确率提升94%，训练时间减少55%。
...
### [162] [LLM-Integrated Bayesian State Space Models for Multimodal Time-Series Forecasting](https://arxiv.org/abs/2510.20952)
*Sungjun Cho,Changho Shin,Suenggwan Jo,Xinya Yan,Shourjo Aditya Chaudhuri,Frederic Sala*

Main category: cs.LG

TL;DR: 提出LBS框架，将大语言模型与贝叶斯状态空间模型结合，用于多模态时序预测，支持灵活的输入输出窗口和不确定性量化


<details>
  <summary>Details</summary>
Motivation: 现有方法在架构上受限于固定的输入输出范围，无法建模或量化不确定性，需要整合结构化时序数据和非结构化文本信息

Method: 包含两部分：(1)状态空间模型捕捉潜在状态的时间动态；(2)预训练大语言模型编码文本输入进行后验状态估计，并解码与潜在轨迹一致的文本预测

Result: 在TextTimeCorpus基准测试中，LBS将先前最先进水平提高了13.20%，并提供人类可读的预测摘要

Conclusion: 这是首个将LLM和SSM统一用于联合数值和文本预测的工作，为多模态时序推理提供了新基础
...
### [163] [Safety Assessment in Reinforcement Learning via Model Predictive Control](https://arxiv.org/abs/2510.20955)
*Jeff Pflueger,Michael Everett*

Main category: cs.LG

TL;DR: 提出一种基于可逆性的安全强化学习方法，通过模型预测路径积分控制检查学习策略的动作安全性，无需显式动态模型或安全约束知识。


<details>
  <summary>Details</summary>
Motivation: 无模型强化学习缺乏形式化安全保证，现有方法通常依赖详细的安全规范知识。许多难以明确规范的安全问题更适合用不变性来表征。

Method: 利用可逆性作为防止训练过程中安全问题的方法，使用模型预测路径积分控制检查学习策略提出的动作安全性，只需查询黑盒动态系统。

Result: 实验结果表明，该方法在所有不安全动作发生前成功中止，同时训练进度与允许违反安全性的基线PPO方法相当。

Conclusion: 基于可逆性的方法能有效提供强化学习的安全保证，仅需查询动态系统而无需显式模型知识。
...
### [164] [An Ensembled Penalized Federated Learning Framework for Falling People Detection](https://arxiv.org/abs/2510.20960)
*Sizhe Rao,Runqiu Zhang,Sajal Saha,Liang Chen*

Main category: cs.LG

TL;DR: 提出了EPFL框架，集成持续学习、个性化建模和专门加权聚合策略，用于隐私保护的跌倒检测，在基准数据集上达到88.31%召回率和89.94% F1分数。


<details>
  <summary>Details</summary>
Motivation: 解决老年人跌倒检测中的泛化性差、隐私问题和个体行为差异等挑战，需要开发更鲁棒、准确且保护隐私的跌倒检测系统。

Method: EPFL框架结合持续学习、个性化建模和专门加权聚合策略，使用可穿戴传感器数据，通过同态加密和联邦训练保护用户隐私，采用惩罚性本地训练和集成推理。
...
### [165] [Towards Scalable Oversight with Collaborative Multi-Agent Debate in Error Detection](https://arxiv.org/abs/2510.20963)
*Yongqiang Chen,Gang Niu,James Cheng,Bo Han,Masashi Sugiyama*

Main category: cs.LG

TL;DR: 该论文提出了ColMAD协议，将多智能体辩论从零和博弈重构为非零和博弈，通过协作式批评显著提升大语言模型错误检测能力。


<details>
  <summary>Details</summary>
Motivation: 现有多智能体辩论协议将辩论视为零和博弈，导致辩论者为了获胜而误导法官，反而引入更多错误，表现不如单智能体方法。需要解决这种"辩论黑客"问题。

Method: 引入协作式多智能体辩论协议ColMAD，鼓励多个智能体以支持性方式相互批评，补充彼此的缺失点，使法官能基于更全面的证据做出判断。

Result: 实验表明ColMAD比之前的竞争性多智能体辩论方法性能提升19%，并在错误检测任务上相对于单智能体方法带来显著改进。

Conclusion: 将多智能体辩论重构为非零和协作游戏能有效提升错误检测性能，为可扩展监督提供了更可靠的方法。
...
### [166] [Neural Mutual Information Estimation with Vector Copulas](https://arxiv.org/abs/2510.20968)
*Yanzhi Chen,Zijing Ou,Adrian Weller,Michael U. Gutmann*

Main category: cs.LG

TL;DR: 提出了一种基于向量copula理论的互信息估计器，在复杂模型和简化模型之间实现更好的权衡


<details>
  <summary>Details</summary>
Motivation: 现有互信息估计器要么依赖需要大量数据的复杂模型（如神经网络），要么使用过于简化的模型（如高斯copula），无法捕捉复杂分布

Method: 基于向量copula理论，在复杂模型和简化模型之间进行原则性插值

Result: 在先进合成基准测试和多种模态的真实世界数据上的实验证明了所提估计器的优势

Conclusion: 该方法在复杂性和容量之间实现了更好的权衡，是互信息估计的有效解决方案
...
### [167] [On the accuracy of implicit neural representations for cardiovascular anatomies and hemodynamic fields](https://arxiv.org/abs/2510.20970)
*Jubilee Lee,Daniele E. Schiavazzi*

Main category: cs.LG

TL;DR: 隐式神经表示(INRs)在血流动力学场压缩和心血管解剖表示中表现出色，实现了高达230的压缩比，压力误差小于1 mmHg，速度误差5-10 cm/s，解剖误差低于0.5 mm。
...
Method: 采用多种策略缓解频谱偏差，包括特殊激活函数、固定和可训练位置编码、非线性核的线性组合，测试了SIREN、MFN-Gabor和MHE等架构。

Result: 在胸主动脉血流动力学场中实现约230的压缩比，压力最大绝对误差1 mmHg，速度误差5-10 cm/s；在48个胸主动脉解剖中，平均和最大解剖误差分别低于0.5 mm和1.6 mm。

Conclusion: SIREN、MFN-Gabor和MHE架构表现最佳，INRs在医学领域具有高压缩效率和准确性，是有效的表示方法。
...
### [168] [L^2M^3OF: A Large Language Multimodal Model for Metal-Organic Frameworks](https://arxiv.org/abs/2510.20976)
*Jiyu Cui,Fang Wu,Haokai Zhao,Minggao Feng,Xenophon Evangelopoulos,Andrew I. Cooper,Yejin Choi*

Main category: cs.LG

TL;DR: L2M3OF是首个用于金属有机框架(MOFs)的多模态大语言模型，通过整合晶体结构表示学习和语言理解，在材料特性预测和知识生成任务中超越了仅基于文本的闭源LLM。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在自然语言任务中表现出色，但在科学发现领域突破有限，因为理解复杂物理现象需要超越语言的多模态表示。MOF材料设计尤其依赖人类专家经验，难以仅通过文本信息编码。

Method: L2M3OF整合晶体表示学习与语言理解，处理结构、文本和知识模态。使用预训练晶体编码器和轻量投影层将结构信息压缩到标记空间，与语言指令高效对齐。构建了结构-属性-知识数据库用于训练和评估。

Result: 实验表明L2M3OF在材料特性预测和知识生成任务中超越了GPT-5、Gemini-2.5-Pro和DeepSeek-R1等领先的闭源文本LLM，且参数量更少。
...
### [169] [Memory Constrained Dynamic Subnetwork Update for Transfer Learning](https://arxiv.org/abs/2510.20979)
*Aël Quélennec,Pavlo Mozharovskyi,Van-Tam Nguyen,Enzo Tartaglione*

Main category: cs.LG

TL;DR: MeDyate是一个内存受限的动态子网络适配框架，通过改进的层重要性度量和动态通道采样策略，在极端内存限制下实现最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 设备端神经网络训练面临严重的内存限制，限制了预训练模型在下游任务中的适配能力。

Method: 提出LaRa（层排序）改进的层重要性度量方法，以及利用通道重要性分布时间稳定性的动态通道采样策略，在训练过程中根据重要性加权概率动态重采样通道。

Result: 在大量任务和架构上的广泛评估表明，MeDyate在极端内存约束下达到最先进性能，持续优于现有的静态和动态方法，同时保持高计算效率。

Conclusion: 该方法代表了向高效设备端学习迈出的重要一步，证明了在仅几百kB RAM内存预算下也能实现有效的微调。
...
### [170] [Learning Grouped Lattice Vector Quantizers for Low-Bit LLM Compression](https://arxiv.org/abs/2510.20984)
*Xi Zhang,Xiaolin Wu,Jiamang Wang,Weisi Lin*

Main category: cs.LG

TL;DR: 提出了一种分组格点向量量化（GLVQ）框架，通过为每组权重分配定制化的格点码本来改进大语言模型的后训练量化，在低比特场景下实现更好的模型大小与精度权衡。


<details>
  <summary>Details</summary>
Motivation: 大语言模型推理需要大量计算资源和内存，后训练量化能降低这些需求，但标准均匀量化在低比特场景下会导致显著的性能下降。

Method: 使用分组格点向量量化，为每组权重分配可学习生成矩阵定义的定制化格点码本，采用Babai舍入近似最近格点搜索来解决量化过程的不可微问题。

Result: 在多个基准测试中，该方法相比现有后训练量化基线实现了更好的模型大小与精度权衡。

Conclusion: GLVQ框架在严格资源约束下部署大模型方面表现出有效性，解码过程简化为矩阵向量乘法，形成高效实用的量化流程。
...
### [171] [GPU Memory Requirement Prediction for Deep Learning Task Based on Bidirectional Gated Recurrent Unit Optimization Transformer](https://arxiv.org/abs/2510.20985)
...
Method: 创新性地将双向门控循环单元(BiGRU)集成到Transformer架构中，构建BiGRU Transformer优化模型，并与决策树、随机森林、Adaboost和XGBoost等基准模型进行比较实验。
...
### [172] [AL-CoLe: Augmented Lagrangian for Constrained Learning](https://arxiv.org/abs/2510.20995)
...
<details>
  <summary>Details</summary>
Motivation: 尽管现代机器学习参数化大多是非凸的，但拉格朗日对偶已成为处理约束学习问题的流行工具。增广拉格朗日方法旨在缓解非凸设置中的对偶间隙，但在约束学习设置中相对未被充分探索。

Method: 使用增广拉格朗日方法，该方法只需要最小修改即可缓解非凸设置中的对偶间隙。建立了强对偶性结果，证明了双上升算法收敛到可行和最优原始解。

Result: 在温和条件下建立了强对偶性结果，证明了双上升算法的收敛性，并提供了PAC风格的泛化保证。在公平性约束分类任务中证明了其有效性。

Conclusion: 增广拉格朗日方法在非凸机器学习约束问题中具有良好性能，能够有效处理对偶间隙问题，并在公平性约束分类等实际任务中表现出色。
...
### [173] [Exploring Spiking Neural Networks for Binary Classification in Multivariate Time Series at the Edge](https://arxiv.org/abs/2510.20997)
*James Ghawaly,Andrew Nicholson,Catherine Schuman,Dalton Diez,Aaron Young,Brett Witherspoon*

Main category: cs.LG

TL;DR: 提出一个通用框架，使用进化算法训练脉冲神经网络进行二元分类，在放射性源检测和癫痫检测任务中表现出色，具有低功耗和低延迟优势。


<details>
  <summary>Details</summary>
Motivation: 开发一个通用框架来训练脉冲神经网络进行多元时间序列的二元分类，特别关注步进预测和在低误报率下的高精度，同时实现低功耗和快速推理。

Method: 使用EONS进化算法联合优化脉冲神经网络的架构和参数，将输入编码为脉冲序列，通过阈值化单个输出神经元的脉冲计数进行预测，并采用投票集成方法提高性能。

Result: 在放射性源检测任务中，仅49个神经元和66个突触的SNN实现了51.8%的真阳性率（1小时误报率），优于PCA和深度学习基线；集成模型将真阳性率提升至67.1%。硬件部署显示功耗2mW，延迟20.2ms。在癫痫检测中，集成模型达到95%真阳性率和16%假阳性率。

Conclusion: 该框架能够训练出高效、低功耗的脉冲神经网络，在多个时间序列分类任务中表现出色，证明了其通用性和实用性，特别适合资源受限的边缘计算场景。
...
### [174] [Distilled Decoding 2: One-step Sampling of Image Auto-regressive Models with Conditional Score Distillation](https://arxiv.org/abs/2510.21003)
*Enshu Liu,Qian Chen,Xuefei Ning,Shengen Yan,Guohao Dai,Zinan Lin,Yu Wang*

Main category: cs.LG

TL;DR: DD2提出了一种新的条件分数蒸馏方法，使图像自回归模型能够实现一步采样，显著提升了生成速度，同时保持较高的图像质量。
...
Method: 将原始AR模型视为教师模型，在潜在嵌入空间提供条件分数。提出条件分数蒸馏损失，训练一个单独的预测网络来预测生成分布的条件分数，并在每个token位置进行分数蒸馏。

Result: 在ImageNet-256上，DD2将FID从3.40增加到5.43，相比DD1基线将一步采样与原始AR模型的差距减少了67%，同时实现了12.3倍的训练加速。
...
### [175] [Fair Representation Learning with Controllable High Confidence Guarantees via Adversarial Inference](https://arxiv.org/abs/2510.21017)
*Yuhong Luo,Austin Hoag,Xintong Wang,Philip S. Thomas,Przemyslaw A. Grabowicz*

Main category: cs.LG

TL;DR: 提出了FRG框架，通过对抗模型学习具有高置信度公平性保证的表示，确保下游任务中人口统计差异不超过用户定义的误差阈值。
...
Method: 使用优化的对抗模型框架FRG，通过对抗训练学习表示，保证下游预测中的人口统计差异以高概率不超过用户定义的误差阈值。

Result: 在三个真实数据集上的实验表明，FRG在一系列下游模型和任务中始终能限制不公平性，优于六种最先进的公平表示学习方法。
...
### [176] [More Than Memory Savings: Zeroth-Order Optimization Mitigates Forgetting in Continual Learning](https://arxiv.org/abs/2510.21019)
...
<details>
  <summary>Details</summary>
Motivation: 探索零阶优化作为解决持续学习中可塑性-稳定性-效率三难问题的新方法，特别是在梯度计算昂贵或不可行的情况下。

Method: 通过理论分析和实证研究评估零阶优化在持续学习中的应用，并提出了ZO-FC方法，该方法将零阶优化应用于单个适配器模块，同时使用一阶优化更新分类器。

Result: 零阶优化增强了稳定性但削弱了可塑性，特别是在可学习分类器的情况下。ZO-FC方法在稳定性和可塑性之间取得了有效平衡，提供了内存高效的持续学习解决方案。

Conclusion: 零阶优化是持续学习中有前景的方法，ZO-FC通过结合零阶优化的稳定性和一阶优化的适应性，为设备端持续学习提供了实用且内存高效的解决方案。
...
### [177] [From Information to Generative Exponent: Learning Rate Induces Phase Transitions in SGD](https://arxiv.org/abs/2510.21020)
*Konstantinos Christopher Tsiolis,Alireza Mousavi-Hosseini,Murat A. Erdogdu*

Main category: cs.LG

TL;DR: 该论文研究了神经网络特征学习动态，重点分析了高斯单索引模型中学习率对样本复杂度的影响，揭示了从信息指数机制到生成指数机制的相变现象。


<details>
  <summary>Details</summary>
Motivation: 理解神经网络中特征学习的动态过程，特别是梯度学习算法中学习率选择对样本复杂度的影响，以及如何通过算法设计超越相关查询的限制。

Method: 提出了一个统一的梯度学习算法框架，涵盖相关性和非相关性更新，包括单次SGD、批次重用SGD，以及新的分层训练算法（使用不同学习率的两时间尺度方法）。

Result: 发现学习率选择存在相变现象：小学习率对应信息指数机制，大学习率对应生成指数机制。学习率选择与算法设计同等重要。

Conclusion: 学习率的选择在实现统计和计算效率方面与算法设计同等重要，分层训练算法可以在不重用样本或修改损失函数的情况下超越相关性查询的限制。
...
### [178] [CIPHER: Scalable Time Series Analysis for Physical Sciences with Application to Solar Wind Phenomena](https://arxiv.org/abs/2510.21022)
...
Method: CIPHER框架整合了iSAX进行可解释的压缩和索引，使用HDBSCAN进行密度聚类来分组重复现象，并包含人机交互步骤进行高效的专家验证。

Result: 在OMNI数据中对太阳风现象进行分类的评估表明，该框架能够识别有意义的物理现象，如日冕物质抛射和流相互作用区。

Conclusion: CIPHER展示了结合符号表示、无监督学习和专家知识来解决物理科学中时间序列标签稀缺问题的通用策略。
...
### [179] [Physically consistent and uncertainty-aware learning of spatiotemporal dynamics](https://arxiv.org/abs/2510.21023)
*Qingsong Xu,Jonathan L Bamber,Nils Thuerey,Niklas Boers,Paul Bates,Gustau Camps-Valls,Yilei Shi,Xiao Xiang Zhu*

Main category: cs.LG

TL;DR: 提出了物理一致性神经算子(PCNO)和扩散模型增强的PCNO(DiffPCNO)，通过物理约束投影层和一致性模型实现高精度、物理一致且能量化不确定性的时空预测。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习方法往往忽略物理定律且无法量化时空预测中的固有不确定性，这限制了长期时空动态预测的准确性。

Method: PCNO通过将代理模型输出投影到满足预定义物理定律的函数空间来强制执行物理约束；DiffPCNO在确定性预测基础上利用一致性模型来量化和减轻不确定性。

Result: PCNO和DiffPCNO在从湍流建模到真实世界洪水/大气预测的多种系统和空间分辨率下，实现了高保真时空预测，同时保持物理一致性和不确定性量化。

Conclusion: 该两阶段框架为准确、物理基础且不确定性感知的时空预测提供了稳健且通用的方法。
...
### [180] [Elementary, My Dear Watson: Non-Invasive Neural Keyword Spotting in the LibriBrain Dataset](https://arxiv.org/abs/2510.21038)
*Gereon Elvers,Gilad Landau,Oiwi Parker Jones*

Main category: cs.LG

TL;DR: 本文提出关键词检测（KWS）作为脑机接口的实际应用任务，使用LibriBrain语料库提供标准化基准，并开发了紧凑的1-D Conv/ResNet基线模型，在保持隐私的同时实现13倍于随机基线的性能提升。


<details>
  <summary>Details</summary>
Motivation: 当前非侵入式脑机接口基准主要针对简单的语音检测和音素分类任务，缺乏面向实际应用的基准。本文旨在填补这一空白，提出关键词检测作为隐私保护的实际应用中间任务。

Method: 使用52小时的LibriBrain语料库，提供标准化训练/验证/测试分割，采用针对极端类别不平衡的评估协议（AUPRC和FA/h指标），开发了基于1-D卷积/ResNet的紧凑基线模型，使用焦点损失和top-k池化。

Result: 参考模型在保留会话上实现了约13倍于随机基线的AUPRC性能，展示了任务的可行性。探索性分析显示：性能随训练时间对数线性提升，词频和持续时间等词级因素系统性影响检测能力。

Conclusion: 关键词检测是脑机接口实际应用的可行中间任务，提出的基准和模型为社区提供了标准化评估框架，支持进一步研究和部署。
...
### [181] [Amortized Active Generation of Pareto Sets](https://arxiv.org/abs/2510.21052)
*Daniel M. Steinberg,Asiri Wijesinghe,Rafael Oliveira,Piotr Koniusz,Cheng Soon Ong,Edwin V. Bonilla*

Main category: cs.LG

TL;DR: 提出A-GPS框架，用于在线离散黑盒多目标优化，通过生成模型学习帕累托集，支持用户偏好后验条件化


<details>
  <summary>Details</summary>
Motivation: 解决传统多目标优化方法在在线设置下效率低、难以灵活融入用户偏好的问题

Method: 使用类别概率估计器预测非支配关系，结合偏好方向向量，构建可条件化生成模型

Result: 在合成基准和蛋白质设计任务中表现出高样本效率和有效的偏好融入能力

Conclusion: A-GPS是一个简单而强大的方法，能生成高质量帕累托集近似，避免显式超体积计算，灵活捕捉用户偏好
...
### [182] [Online Multi-Class Selection with Group Fairness Guarantee](https://arxiv.org/abs/2510.21055)
*Faraz Zargari,Hossein Nekouyan,Lyndon Hallett,Bo Sun,Xiaoqi Tan*

Main category: cs.LG

TL;DR: 提出了一种在线多类选择问题的公平分配算法，通过无损舍入方案和资源预留机制，在保证群体公平性的同时不降低性能。
...
Conclusion: 该方法有效地平衡了公平性和效率，特别是在处理多类别代理的复杂场景中。
...
### [183] [On the Sample Complexity of Differentially Private Policy Optimization](https://arxiv.org/abs/2510.21060)
*Yi He,Xingyu Zhou*

Main category: cs.LG

TL;DR: 本文首次系统研究差分隐私策略优化的样本复杂度，提出了针对策略优化的DP定义，分析了PG、NPG等常用算法在DP约束下的样本复杂度，发现隐私成本通常表现为样本复杂度的低阶项。


<details>
  <summary>Details</summary>
Motivation: 策略优化在敏感领域（如机器人、医疗、大语言模型训练）的广泛应用引发了严重的隐私担忧，需要研究如何在保护隐私的前提下进行策略优化。

Method: 首先形式化了适合策略优化的差分隐私定义，然后通过统一框架系统分析了策略梯度、自然策略梯度等常用算法在DP约束下的样本复杂度。

Result: 理论结果表明隐私成本通常表现为样本复杂度的低阶项，同时在私有策略优化设置中揭示了微妙但重要的观察结果。

Conclusion: 这些结果为隐私保护策略优化算法提供了有价值的实践见解，表明在保护隐私的同时仍能保持较好的学习效率。
...
### [184] [Scalable Machine Learning Analysis of Parker Solar Probe Solar Wind Data](https://arxiv.org/abs/2510.21066)
*Daniela Martin,Connor O'Brien,Valmir P Moraes Filho,Jinsu Hong,Jasmine R. Kobayashi,Evangelia Samara,Joseph Gallego*

Main category: cs.LG

TL;DR: 提出一个可扩展的机器学习框架，使用分布式处理和量子启发的核密度矩阵方法分析帕克太阳探测器数据，揭示内日光层太阳风参数的特征趋势。
...
Method: 使用Dask进行大规模统计计算，采用量子启发的核密度矩阵方法估计太阳风参数的单变量和双变量分布。

Result: 揭示了内日光层特征趋势：太阳风速随距离增加而增加，质子密度减小，速度与密度呈反比关系。
...
### [185] [The Virtues of Brevity: Avoid Overthinking in Parallel Test-Time Reasoning](https://arxiv.org/abs/2510.21067)
...
Result: 该方法在多个基准测试中与自一致性等复杂方法竞争性相当，同时显著减少计算开销。
...
### [186] [Neural Collapse under Gradient Flow on Shallow ReLU Networks for Orthogonally Separable Data](https://arxiv.org/abs/2510.21078)
...
<details>
  <summary>Details</summary>
Motivation: 理解深度网络成功背后的神经崩溃现象，特别是要超越之前将最后一层特征视为无约束优化变量的假设，研究数据结构和非线性激活对NC的影响。

Method: 分析在正交可分数据上训练的两层ReLU网络的梯度流动态，证明其会表现出神经崩溃现象。

Result: 证明了梯度流在特定条件下会收敛到表现出NC特征的解，揭示了数据结构和非线性激活在NC形成中的关键作用。

Conclusion: 神经崩溃现象不仅依赖于优化目标，还受到数据结构和训练动态中隐式偏置的影响，这为理解深度网络表示学习提供了新的理论视角。
...
### [187] [Accelerating Mobile Inference through Fine-Grained CPU-GPU Co-Execution](https://arxiv.org/abs/2510.21081)
...
<details>
  <summary>Details</summary>
Motivation: 移动设备计算资源有限，但其统一内存架构和CPU-GPU性能差距较小，为通过CPU-GPU协同执行减少推理延迟提供了机会。

Method: 使用基于OpenCL细粒度共享虚拟内存的轻量级同步机制，并开发机器学习模型来准确预测CPU和GPU任务的执行时间，包括GPU内核的性能特征和调度时间。

Result: 在四个移动平台上评估显示，该方法能快速选择CPU-GPU协同执行策略，对线性层实现最高1.89倍加速，对卷积层实现最高1.75倍加速，接近通过穷举搜索找到的最大可能值。

Conclusion: 提出的方法有效克服了移动设备上CPU-GPU协同执行的主要障碍，实现了接近理论最大值的推理加速。
...
### [188] [DictPFL: Efficient and Private Federated Learning on Encrypted Gradients](https://arxiv.org/abs/2510.21086)
*Jiaqi Xue,Mayank Kumar,Yuzhang Shang,Shangqian Gao,Rui Ning,Mengxin Zheng,Xiaoqian Jiang,Qian Lou*

Main category: cs.LG

TL;DR: DictPFL是一个实用的联邦学习框架，通过分解模型权重和加密感知剪枝，实现全梯度保护的同时大幅降低计算和通信开销。
...
### [189] [M-GLC: Motif-Driven Global-Local Context Graphs for Few-shot Molecular Property Prediction](https://arxiv.org/abs/2510.21088)
...
<details>
  <summary>Details</summary>
Motivation: 传统深度学习方法依赖大量标注数据，但在药物发现和材料科学中往往缺乏这样的数据集。现有的少样本分子属性预测方法虽然通过上下文图引入关系归纳偏置，但分子-属性图的结构指导有限。

Method: 构建全局三元异质图（基序-分子-属性）捕获长程组合模式，同时在局部为每个节点构建子图进行编码，使模型关注最相关的邻近分子和基序。

Result: 在五个标准FSMPP基准测试中，该方法始终优于最先进的方法。

Conclusion: 将全局基序知识与细粒度局部上下文相结合，有效推进了鲁棒的少样本分子属性预测。
...
### [190] [ESCORT: Efficient Stein-variational and Sliced Consistency-Optimized Temporal Belief Representation for POMDPs](https://arxiv.org/abs/2510.21107)
...
<details>
  <summary>Details</summary>
Motivation: 随着环境变得更加真实，信念分布变得复杂，标准数学模型无法准确捕捉，现有POMDP信念近似方法无法准确表示复杂不确定性结构，导致估计错误和次优的智能体行为。

Method: ESCORT扩展SVGD，引入两个关键创新：相关感知投影（建模状态维度间的依赖关系）和时间一致性约束（稳定更新同时保持相关结构），保留SVGD的吸引-排斥粒子动力学，同时能够准确建模复杂相关模式。
...
### [191] [Distributionally Robust Feature Selection](https://arxiv.org/abs/2510.21113)
*Maitreyi Swaroop,Tamar Krishnamurti,Bryan Wilder*

Main category: cs.LG

TL;DR: 提出了一种特征选择方法，在收集特征成本高昂的场景下，选择有限特征使模型能在多个子群体上同时表现良好。


<details>
  <summary>Details</summary>
Motivation: 在特征收集成本高的应用中（如添加调查问题或物理传感器），需要选择有限特征来构建能适应不同群体的高质量下游模型。

Method: 将问题构建为传统变量选择的连续松弛，使用加噪机制而不需通过模型训练过程反向传播。通过优化贝叶斯最优预测器的方差，开发模型无关框架。

Result: 在合成数据集和真实世界数据上的实验验证了方法的有效性。

Conclusion: 该方法能平衡下游预测在不同群体间的整体性能，为成本敏感的特征选择问题提供了实用解决方案。
...
### [192] [SolarBoost: Distributed Photovoltaic Power Forecasting Amid Time-varying Grid Capacity](https://arxiv.org/abs/2510.21129)
...
### [193] [Cloud-Fog-Edge Collaborative Computing for Sequential MIoT Workflow: A Two-Tier DDPG-Based Scheduling Framework](https://arxiv.org/abs/2510.21135)
...
Method: 采用分层调度决策：全局控制器进行层级选择（边缘、雾或云），专用本地控制器处理选定层内的节点分配。
...
### [194] [Uncertainty-Aware Multi-Objective Reinforcement Learning-Guided Diffusion Models for 3D De Novo Molecular Design](https://arxiv.org/abs/2510.21153)
...
Conclusion: RL引导的生成扩散模型在推进自动化分子设计方面具有强大潜力，MD模拟和ADMET分析显示生成候选分子具有类似药物的行为和结合稳定性
...
### [195] [A Unified Matrix Factorization Framework for Classical and Robust Clustering](https://arxiv.org/abs/2510.21172)
*Angshul Majumdar*

Main category: cs.LG

TL;DR: 本文提出了一个统一的矩阵分解框架，将经典聚类和鲁棒聚类联系起来。重新推导了k-means聚类与矩阵分解的等价关系，并首次形式化了模糊c-means聚类的矩阵分解解释。通过用l1,2范数替换Frobenius范数，提出了鲁棒聚类变体。


<details>
  <summary>Details</summary>
Motivation: 建立经典聚类方法与矩阵分解的统一理论框架，解决现有聚类算法对异常值敏感的问题，为聚类分析提供更稳健的数学基础。

Method: 1. 重新推导k-means与矩阵分解的等价关系；2. 首次形式化模糊c-means的矩阵分解解释；3. 用l1,2范数构建鲁棒聚类变体；4. 开发交替最小化算法和IRLS算法。

Result: 成功建立了经典聚类与矩阵分解的统一框架，提出了鲁棒聚类方法，所有算法都理论上证明能收敛到局部最小值。

Conclusion: 矩阵分解框架为聚类分析提供了统一的理论基础，鲁棒变体能有效处理异常值，提出的算法具有理论保证的收敛性。
...
### [196] [Reducing the Probability of Undesirable Outputs in Language Models Using Probabilistic Inference](https://arxiv.org/abs/2510.21184)
*Stephen Zhao,Aidan Li,Rob Brekelmans,Roger Grosse*

Main category: cs.LG

TL;DR: RePULSe是一种新的强化学习训练方法，通过增加额外损失函数来引导采样低奖励输出并降低其概率，在期望奖励与不良输出概率之间实现更好的权衡。


<details>
  <summary>Details</summary>
Motivation: 标准强化学习方法优化平均奖励，而专注于减少不良输出概率的方法通常会影响平均性能表现。需要改进这种权衡关系。
...
Conclusion: RePULSe方法能够有效改善强化学习在语言模型对齐中的性能权衡问题，提供更优的解决方案。
...
### [197] [A visual big data system for the prediction of weather-related variables: Jordan-Spain case study](https://arxiv.org/abs/2510.21176)
*Shadi Aljawarneh,Juan A. Lara,Muneer Bani Yassein*

Main category: cs.LG

TL;DR: 提出一个可视化大数据系统，用于处理气象数据并执行预测任务，系统融合不同时空聚合级别的数据，采用单变量和多变量方法进行预测分析。
...
Result: 系统在可用性和预测性能方面进行评估，获得总体归一化均方误差值为0.00013，总体方向对称值接近0.84。专家评估中除图形设计外所有方面评分均在3分以上（1-5分制）。
...
### [198] [Scalable Principal-Agent Contract Design via Gradient-Based Optimization](https://arxiv.org/abs/2510.21177)
*Tomer Galanti,Aarya Bookseller,Korok Ray*

Main category: cs.LG

TL;DR: 提出了一种用于主代理合同设计的双层最大-最大优化算法框架，使用隐式微分和共轭梯度法高效计算超梯度，无需形成或求逆Hessian矩阵。


<details>
  <summary>Details</summary>
Motivation: 传统线性-二次模型如Holmström-Milgrom有闭式解，但现实环境中非线性效用、随机动态或高维动作通常没有闭式解，需要通用算法框架。

Method: 采用现代机器学习技术进行双层优化，通过Hessian-向量积使用隐式微分和共轭梯度法计算超梯度，避免直接处理Hessian矩阵。

Result: 在CARA-Normal基准环境中恢复已知解析最优解，从随机初始化可靠收敛；可扩展到复杂非线性合同如S型工资表、相对绩效补偿、多任务合同等。

Conclusion: 为合同设计提供了新的计算工具，使系统研究分析上难以处理的模型成为可能。
...
### [199] [Leverage Unlearning to Sanitize LLMs](https://arxiv.org/abs/2510.21322)
*Antoine Boutet,Lucas Magnana*

Main category: cs.LG

TL;DR: SANI是一种语言模型遗忘方法，通过重置最后层神经元和微调来消除模型对敏感信息的记忆，大幅减少信息泄露风险。


<details>
  <summary>Details</summary>
Motivation: 预训练大语言模型在特定任务微调时会记忆敏感数据，存在隐私泄露风险，需要低成本的方法来消除这种记忆。

Method: 采用遗忘方法，包括擦除阶段（重置最后层神经元）和修复阶段（避免记忆敏感信息的微调）。

Result: 仅需少量遗忘轮次即可有效净化模型，信息泄露数量大幅减少。

Conclusion: SANI为医院等行业提供了一种高效、低成本的模型净化方案，适用于已训练模型的隐私保护。
...
### [200] [PLAN: Proactive Low-Rank Allocation for Continual Learning](https://arxiv.org/abs/2510.21188)
*Xiequn Wang,Zhan Zhuang,Yu Zhang*

Main category: cs.LG

TL;DR: PLAN框架通过引入正交基向量和基于扰动的优化策略，扩展LoRA以实现持续学习中的高效、干扰感知微调，在标准基准测试中优于现有方法。
...
Method: 提出PLAN框架，为每个任务引入正交基向量，通过基于扰动的策略优化这些向量以最小化与已学习参数的冲突，并采用选择机制识别干扰敏感性最小的基向量。

Result: 在标准持续学习基准测试中，PLAN始终优于现有方法，为基础模型的持续学习建立了新的最先进水平。

Conclusion: PLAN通过主动管理任务特定子空间和干扰感知的优化策略，有效解决了持续学习中的灾难性遗忘问题，为基础模型的高效持续学习提供了可行方案。
...
### [201] [FairImagen: Post-Processing for Bias Mitigation in Text-to-Image Models](https://arxiv.org/abs/2510.21363)
...
<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型（如Stable Diffusion）经常复制和放大社会偏见，特别是在性别和种族等人口统计属性方面。

Method: 集成公平主成分分析，将基于CLIP的输入嵌入投影到最小化群体特定信息同时保留语义内容的子空间中，并通过经验噪声注入和统一跨人口统计投影方法来增强去偏效果。
...
### [202] [Gen-Review: A Large-scale Dataset of AI-Generated (and Human-written) Peer Reviews](https://arxiv.org/abs/2510.21192)
*Luca Demetrio,Giovanni Apruzzese,Kathrin Grosse,Pavel Laskov,Emil Lupu,Vera Rimmer,Philine Widmer*

Main category: cs.LG

TL;DR: GenReview是迄今为止最大的包含LLM撰写评审意见的数据集，包含81K条为ICLR 2018-2025年投稿生成的评审意见，使用负面、正面和中性三种提示词生成。


<details>
  <summary>Details</summary>
Motivation: 了解大型语言模型在科学同行评审中的使用情况及其对科学过程有效性和完整性的影响，需要全面的数据集支持相关研究。

Method: 构建GenReview数据集，为ICLR 2018-2025年所有投稿使用三种独立提示词（负面、正面、中性）生成LLM评审意见，并与原始论文和评审意见关联。

Result: LLM在评审中存在偏见，LLM撰写的评审意见目前可以自动检测，LLM并不总是严格遵循评审指南，LLM提供的评分仅与论文接受决策一致。

Conclusion: GenReview数据集填补了LLM撰写评审意见数据集的空白，支持广泛的研究调查，揭示了LLM在科学同行评审中的局限性和潜在问题。
...
### [203] [Online AUC Optimization Based on Second-order Surrogate Loss](https://arxiv.org/abs/2510.21202)
*JunRu Luo,Difei Cheng,Bo Zhang*

Main category: cs.LG

TL;DR: 提出了一种基于成对铰链损失的新型二阶代理损失函数，用于优化AUC性能，解决了传统方法在非凸性和内存消耗方面的问题，并实现了更紧密的遗憾界。


<details>
  <summary>Details</summary>
Motivation: AUC是分类任务中重要的性能指标，特别是在类别不平衡场景下。但最小化AUC面临重大挑战：成对0/1损失的非凸性和不连续性难以优化，以及实例级存储的高内存成本在大规模应用中造成瓶颈。

Method: 提出了一种新的二阶代理损失，基于成对铰链损失，并开发了高效的在线算法。该方法直接使用训练数据的一阶和二阶统计量构建代理损失函数，替代整个聚合的成对损失，而不是像传统方法那样近似每个单独的成对0/1损失项。

Result: 理论上，现有在线AUC优化算法通常达到O(√T)遗憾界，而该方法获得了更紧密的O(ln T)界。通过核方法将框架扩展到非线性设置。在多个基准数据集上的广泛实验证明了所提二阶代理损失在优化在线AUC性能方面的优越效率和有效性。

Conclusion: 所提出的二阶代理损失方法在优化AUC性能方面表现出色，解决了传统方法的挑战，并在理论和实验上都取得了显著改进。
...
### [204] [Mitra: Mixed Synthetic Priors for Enhancing Tabular Foundation Models](https://arxiv.org/abs/2510.21204)
...
TL;DR: 本文首次系统研究表格基础模型(TFMs)中合成先验分布的关键特性，提出了基于多样化合成先验混合的Mitra模型，在分类和回归任务上优于现有SOTA方法。


<details>
  <summary>Details</summary>
Motivation: 基于上下文学习的表格基础模型通过纯合成数据集预训练后能在真实数据上表现优异，但合成先验分布的设计原则尚不明确，需要系统研究。

Method: 系统分析合成先验的关键特性，提出Mitra模型，使用经过筛选的多样化、独特且在实际表格数据上表现良好的合成先验分布混合进行训练。

Result: Mitra在分类和回归基准测试中一致优于TabPFNv2和TabICL等最先进表格基础模型，具有更好的样本效率。

Conclusion: 合成先验分布的设计对表格基础模型的泛化性能至关重要，Mitra通过精心选择的先验混合实现了优越性能。
...
### [205] [Adaptive Graph Mixture of Residual Experts: Unsupervised Learning on Diverse Graphs with Heterogeneous Specialization](https://arxiv.org/abs/2510.21207)
...
<details>
  <summary>Details</summary>
Motivation: 图神经网络面临基本适应性问题：固定的消息传递架构难以应对现实世界图的巨大多样性，其中最优计算策略因局部结构和任务而异

Method: 采用骨干-残差专家架构，基础编码器提供稳定性，专用残差专家捕获多样化计算模式；使用结构感知门控网络进行细粒度节点路由；通过统一无监督目标进行端到端训练，结合主重建任务和信息论多样性正则化器

Result: 在16个基准测试中验证了ADaMoRE在无监督节点分类和少样本学习方面的最先进性能，同时具有优越的泛化能力、训练效率和更快收敛速度

Conclusion: ADaMoRE框架能够实现异构专家混合在图上稳健的完全无监督训练，解决了现有图MoE方法依赖监督信号和训练不稳定的问题
...
### [206] [On the flow matching interpretability](https://arxiv.org/abs/2510.21210)
*Francesco Pivi,Simone Gazza,Davide Evangelista,Roberto Amadini,Maurizio Gabbrielli*

Main category: cs.LG

TL;DR: 提出了一种将物理约束嵌入流匹配生成模型的新框架，通过将流轨迹映射到物理过程的平衡状态，使生成步骤具有物理可解释性。


<details>
  <summary>Details</summary>
Motivation: 解决传统流匹配生成模型中间生成步骤缺乏可解释性的问题，使神经网络的生成过程具有物理意义。

Method: 使用2D Ising模型约束流步骤，构建包含编码器、流匹配网络和投影器的架构，将离散Ising配置映射到连续空间并进行温度驱动的扩散。

Result: 在多种晶格尺寸下验证了框架的有效性，在保持物理保真度的同时，随着晶格尺寸增大，生成速度优于蒙特卡洛方法。
...
### [207] [Few-Shot Knowledge Distillation of LLMs With Counterfactual Explanations](https://arxiv.org/abs/2510.21631)
...
<details>
  <summary>Details</summary>
Motivation: 现有任务感知蒸馏方法需要大量数据，但在实际场景中这些数据可能难以获取或成本高昂。

Method: CoD策略通过系统性地注入反事实解释（CFEs）来进行蒸馏。CFEs是指能够以最小扰动翻转教师模型输出预测的输入。

Result: 在多个数据集和LLM上的实验表明，CoD在少样本场景（低至8-512个样本）中优于标准蒸馏方法。CoD仅使用基线方法一半的原始样本及其对应的CFEs，仍能提升性能。

Conclusion: CoD通过利用反事实解释有效映射教师决策边界，在少样本场景下实现了更好的知识蒸馏效果，为资源受限环境下的模型部署提供了可行方案。
...
### [208] [Model Merging with Functional Dual Anchors](https://arxiv.org/abs/2510.21223)
*Kexuan Shi,Yandong Wen,Weiyang Liu*

Main category: cs.LG

TL;DR: 提出Functional Dual Anchors (FDAs)框架，通过建模输入表示空间而非参数空间来改进模型合并，使用合成输入来捕获任务特定的功能偏移。


<details>
  <summary>Details</summary>
Motivation: 现有模型合并方法在参数空间中操作，受到参数不一致性的限制，需要一种更稳健和灵活的方法来整合多个微调检查点的知识。

Method: 使用功能性双锚点(FDAs)作为合成输入，其诱导的梯度与任务向量对齐，捕获相对于预训练模型的任务特定功能偏移，并提出了原则性的初始化方案。

Result: 综合实验证明了FDAs在模型合并中的有效性，并且FDAs与参数空间模型合并方法互补。

Conclusion: FDAs框架通过建模输入表示空间，为联合多任务训练和后验合并提供了桥梁，提供了更好的鲁棒性和灵活性。
...
### [209] [How Hard is it to Confuse a World Model?](https://arxiv.org/abs/2510.21232)
*Waris Radji,Odalric-Ambrym Maillard*

Main category: cs.LG

TL;DR: 本文针对神经网络世界模型，提出了寻找最混淆实例的约束优化问题，并通过对抗训练方法解决该问题，发现可实现的混淆程度与近似模型的不确定性相关。


<details>
  <summary>Details</summary>
Motivation: 在强化学习理论中，最混淆实例是建立遗憾下界的关键概念，但在神经网络世界模型的一般情况下构建此类实例仍是一个开放问题。

Method: 将问题形式化为约束优化：寻找统计上接近参考模型但使最优和次优策略性能产生分歧的修改模型，并提出对抗训练程序来解决该问题。
...
### [210] [Convergence of Stochastic Gradient Langevin Dynamics in the Lazy Training Regime](https://arxiv.org/abs/2510.21245)
*Noah Oberweis,Semih Cayci*

Main category: cs.LG

TL;DR: 本文对随机梯度Langevin动力学(SGLD)在懒惰训练机制下建立了非渐近收敛分析，证明了其在乘性和状态相关噪声下能产生非退化核，并以指数速度收敛到经验风险最小化器。


<details>
  <summary>Details</summary>
Motivation: 连续时间模型为深度学习优化算法的训练动态提供了重要见解，需要研究SGLD在懒惰训练机制下的收敛性能。

Method: 使用随机微分方程(SDE)近似随机梯度下降，在损失函数Hessian矩阵满足正则性条件下，分析SGLD的收敛行为。

Result: 证明了SGLD在乘性和状态相关噪声下：(i)以高概率在整个训练过程中产生非退化核；(ii)在期望意义下以指数速度收敛到经验风险最小化器，并建立了最优性间隙的有限时间和有限宽度界限。

Conclusion: 理论结果在回归设置中通过数值例子得到了验证，表明SGLD在懒惰训练机制下具有良好的收敛性能。
...
### [211] [Unified Implementations of Recurrent Neural Networks in Multiple Deep Learning Frameworks](https://arxiv.org/abs/2510.21252)
*Francesco Martinuzzi*

Main category: cs.LG

TL;DR: 提出了三个开源库（torchrecurrent、RecurrentLayers.jl、LuxRecurrentLayers.jl），用于集中实现和测试多种RNN变体，解决现有缺乏统一测试平台的问题。


<details>
  <summary>Details</summary>
Motivation: 由于缺乏中心化的RNN变体测试库，重新实现各种架构耗时且容易出错，限制了研究的可重复性和探索性。

Method: 在Julia和Python中开发了三个开源库，提供一致的框架来构建和扩展RNN模型，内置定制和实验机制。

Result: 成功创建了集中化的RNN实现库，支持多种循环单元和高级循环架构，采用MIT许可证并在GitHub上积极维护。

Conclusion: 这些库为RNN研究提供了标准化工具，促进了模型的可重复性和实验探索。
...
### [212] [PINN Balls: Scaling Second-Order Methods for PINNs with Domain Decomposition and Adaptive Sampling](https://arxiv.org/abs/2510.21262)
*Andrea Bonfanti,Ismael Medina,Roman List,Björn Staeves,Roberto Santana,Marco Ellero*

Main category: cs.LG

TL;DR: 提出了PINN Balls方法，通过局部混合专家模型和可学习域分解结构，解决了二阶训练方法在物理信息神经网络中的内存问题，实现了更好的精度和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 二阶方法能增强PINNs训练，但内存需求大，难以扩展到大型模型。需要找到既能利用二阶方法优势又保持可扩展性的解决方案。

Method: 使用局部混合专家模型结合参数高效的集成模型和稀疏编码，采用对抗自适应采样实现完全可学习的域分解结构。

Result: PINN Balls在科学机器学习中达到了比现有技术更好的精度，同时保持了宝贵的可扩展性。

Conclusion: 该方法成功解决了二阶训练方法的内存瓶颈问题，为PINNs在偏微分方程求解中提供了更有效的替代传统数值方法的途径。
...
### [213] [Relieving the Over-Aggregating Effect in Graph Transformers](https://arxiv.org/abs/2510.21267)
...
<details>
  <summary>Details</summary>
Motivation: 解决图注意力中的过聚合现象——当大量节点信息聚合到单个节点时，关键信息被稀释导致信息丢失

Method: 将节点聚合划分为并行过程，限制每次聚合的输入量，并通过排序和加权机制优先处理信息量大的消息

Result: Wideformer能有效缓解过聚合问题，使骨干方法专注于信息量大的消息，性能优于基线方法

Conclusion: 提出的Wideformer方法通过并行聚合和消息优先级机制，成功解决了图注意力中的过聚合问题
...
### [214] [Buffer layers for Test-Time Adaptation](https://arxiv.org/abs/2510.21271)
*Hyeongyu Kim,Geonhui Han,Dosik Hwang*

Main category: cs.LG

TL;DR: 提出基于Buffer层的新测试时适应范式，解决传统基于归一化层更新的TTA方法在批处理大小敏感性和预训练模型结构限制方面的问题。


<details>
  <summary>Details</summary>
Motivation: 现有TTA方法主要依赖归一化层更新，但存在批处理大小敏感性高、受预训练模型结构限制等问题，限制了在显著域偏移下的有效性。

Method: 引入Buffer层概念，保持预训练主干网络完整性，避免灾难性遗忘，可模块化集成到现有TTA框架中。

Result: 在缓解域偏移、增强模型鲁棒性方面优于传统方法，表现出强抗遗忘能力，在各种架构中实现一致性能提升。

Conclusion: Buffer层方法有效解决了归一化层更新的根本限制，在真实域适应场景中展现出高效性和通用性。
...
### [215] [Sensor-Specific Transformer (PatchTST) Ensembles with Test-Matched Augmentation](https://arxiv.org/abs/2510.21282)
*Pavankumar Chandankar,Robin Burchard*

Main category: cs.LG

TL;DR: 提出了一种基于PatchTST变换器的噪声感知集成方法，通过传感器特定模型和测试匹配的数据增强来提高噪声条件下的人类活动识别鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 为了解决真实世界传感器噪声对人类活动识别的影响，需要开发能够适应测试时噪声的鲁棒方法。

Method: 使用PatchTST变换器架构，为每个惯性传感器位置训练独立模型，采用1秒滑动窗口和随机抖动、缩放、旋转、通道丢失等数据增强技术来模拟测试噪声。

Result: 在私有排行榜上，该方法获得了显著高于基线的宏F1分数。
...
### [216] [Adaptive Data Selection for Multi-Layer Perceptron Training: A Sub-linear Value-Driven Method](https://arxiv.org/abs/2510.21286)
*Xiyang Zhang,Chen Liang,Haoxuan Qiu,Hongzhi Wang*

Main category: cs.LG

TL;DR: 提出DVC方法，用于多层感知机训练中的数据选择，通过分层评估数据贡献度，在预算约束下实现更优的性能表现。


<details>
  <summary>Details</summary>
Motivation: 现有数据选择方法存在简化非线性变换、忽略中间层表示、计算复杂度高等问题，难以在大规模MLP训练中有效选择最有价值的数据样本。

Method: DVC方法将数据贡献分解为层价值贡献(LVC)和全局价值贡献(GVC)，使用6个精心设计的指标从质量、相关性和分布多样性三个维度评估数据，并结合UCB算法进行自适应源选择。

Result: 在6个数据集和8个基线方法上的实验表明，DVC在各种预算约束下均优于现有方法，获得了更高的准确率和F1分数。
...
### [217] [Additive Models Explained: A Computational Complexity Approach](https://arxiv.org/abs/2510.21292)
*Shahaf Bassan,Michal Moshkovitz,Guy Katz*

Main category: cs.LG

TL;DR: 本文分析了广义可加模型(GAMs)生成解释的计算复杂性，发现在不同输入空间结构、组件模型类型、回归与分类任务以及解释方法下，计算复杂性存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 虽然GAMs通常被认为是可解释的ML模型，但生成其有意义的解释的计算复杂性尚未得到充分研究。本文旨在挑战"GAMs解释计算简单"的假设，系统分析其计算复杂性。

Method: 通过计算复杂性理论分析，在P!=NP等标准假设下，研究不同形式的GAMs在多种上下文中的解释生成复杂性。

Result: 发现GAMs解释复杂性受输入空间结构显著影响；不同组件模型的复杂性差异仅在特定输入域下显现；回归与分类任务的解释复杂性存在显著区别；神经网络可加化表达在某些条件下可降低解释复杂性。

Conclusion: 为GAMs计算多样解释的可行性提供了严谨的理论图景，明确了在何种条件下此类计算是可能的或已被证明是困难的。
...
### [218] [An Evidence-Based Post-Hoc Adjustment Framework for Anomaly Detection Under Data Contamination](https://arxiv.org/abs/2510.21296)
*Sukanya Patra,Souhaib Ben Taieb*

Main category: cs.LG

TL;DR: EPHAD是一个测试时适应框架，用于更新在受污染数据集上训练的异常检测模型的输出，通过整合预训练模型和经典异常检测方法提供的证据来提升性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界数据集通常包含未被检测到或错误标记的异常，导致异常检测模型性能显著下降。现有解决方案需要访问训练流程、数据或先验知识，限制了实际应用。

Method: 提出EPHAD框架，在测试时更新异常检测模型的输出，整合从多模态基础模型（如CLIP）、经典异常检测方法（如潜在异常因子）或领域特定知识中收集的证据。

Result: 在8个视觉异常检测数据集、26个表格异常检测数据集和1个真实工业数据集上的综合实验验证了有效性，消融研究分析了超参数影响和对不同污染水平的鲁棒性。

Conclusion: EPHAD在不同异常检测模型和证据对中展现出多功能性和鲁棒性，代码已公开以确保可复现性。
...
### [219] [Amortized Variational Inference for Partial-Label Learning: A Probabilistic Approach to Label Disambiguation](https://arxiv.org/abs/2510.21300)
*Tobias Fuchs,Nadja Klein*

Main category: cs.LG

TL;DR: 提出了一种新的概率框架，使用摊销变分推断直接逼近真实标签的后验分布，结合深度学习的表达能力和概率建模的严谨性，在部分标签学习中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界数据通常存在噪声和歧义，部分标签学习处理每个实例与多个候选标签相关联的情况。现有方法要么计算密集，要么依赖替代损失和启发式标签细化，需要更高效严谨的解决方案。

Method: 使用摊销变分推断，通过神经网络从输入数据预测变分参数来直接逼近真实标签的后验分布，保持架构无关性。

Result: 在合成和真实数据集上的广泛实验表明，该方法在准确性和效率方面都达到了最先进的性能。

Conclusion: 该方法成功结合了深度学习的表达能力和概率建模的严谨性，为部分标签学习提供了一种高效且理论严谨的解决方案。
...
### [220] [Data as a Lever: A Neighbouring Datasets Perspective on Predictive Multiplicity](https://arxiv.org/abs/2510.21303)
*Prakhar Ganesh,Hsiang Hsu,Golnoosh Farnadi*

Main category: cs.LG

TL;DR: 本文提出了一个邻近数据集框架来研究单个数据点差异对模型多样性的影响，发现类间分布重叠更大的邻近数据集反而表现出更低的模型多样性。基于此，作者将框架扩展到主动学习和数据插补两个应用领域，提出了多样性感知的新方法。


<details>
  <summary>Details</summary>
Motivation: 虽然已有研究主要关注建模选择对模型多样性的影响，但数据在塑造多样性方面的关键作用相对被忽视。本文旨在通过邻近数据集框架，研究单个数据点差异对模型多样性的影响。

Method: 引入邻近数据集框架来分析单个数据点差异对模型多样性的影响，通过严格的数学证明验证发现。然后将该框架扩展到主动学习和数据插补领域，提出了多样性感知的数据获取策略和插补技术。

Result: 发现了一个反直觉的结果：类间分布重叠更大的邻近数据集反而表现出更低的模型多样性，这源于共享的Rashomon参数。在主动学习和数据插补应用中，提出的多样性感知方法能够有效管理模型多样性。

Conclusion: 数据在模型多样性中扮演着关键角色，单个数据点的变化可以显著影响模型多样性。提出的邻近数据集框架为理解和控制模型多样性提供了新的视角，并在实际应用中展示了有效性。
...
### [221] [Revisiting Social Welfare in Bandits: UCB is (Nearly) All You Need](https://arxiv.org/abs/2510.21312)
*Dhruv Sarkar,Nishant Pandey,Sayak Ray Chowdhury*

Main category: cs.LG

TL;DR: 本文提出了一种简单而通用的方法来解决随机多臂老虎机中的公平性问题，通过初始均匀探索后使用标准UCB算法，实现了对Nash遗憾的近乎最优控制，并能扩展到更广泛的p-均值遗憾度量。


<details>
  <summary>Details</summary>
Motivation: 传统遗憾度量无法解决奖励分配的公平性问题，特别是在需要公平分配奖励的场景中（如临床试验）。现有方法需要专门算法设计和强假设，限制了其适用性。

Method: 采用初始均匀探索阶段后接标准上置信界(UCB)算法，仅依赖加性Hoeffding界，可自然扩展到次高斯奖励分布。

Result: 该方法实现了近乎最优的Nash遗憾，并能推广到更广泛的p-均值遗憾度量，在所有p值上实现统一的（近乎）最优遗憾界。

Conclusion: 简单的初始均匀探索加标准UCB算法足以解决公平性多臂老虎机问题，无需复杂算法设计或强假设，为公平性老虎机问题提供了通用解决方案。
...
### [222] [A Convergence Analysis of Adaptive Optimizers under Floating-point Quantization](https://arxiv.org/abs/2510.21314)
*Xuan Tang,Jichu Li,Difan Zou*

Main category: cs.LG

TL;DR: 该论文提出了首个分析自适应优化器在浮点量化下的收敛性理论框架，解释了为什么低精度训练仍然有效，并推导了Adam和Muon在梯度、权重和优化器状态量化下的收敛速率。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的快速扩展，低精度训练对于减少内存、提高效率至关重要，但现有收敛理论假设所有组件都是精确的，忽略了硬件感知量化，无法解释低精度训练为何有效。

Method: 引入理论框架分析Adam和Muon等自适应优化器在梯度、权重和优化器状态浮点量化下的收敛性，在平滑非凸目标下推导收敛速率，明确量化不同组件的量化误差对收敛的影响。

Result: 分析表明，只要尾数长度与迭代次数呈对数关系，两种算法都能保持接近全精度版本的收敛速率。Adam对权重和第二矩量化高度敏感，而Muon需要更弱的误差控制，可能更鲁棒。数值实验验证了理论。

Conclusion: 该研究缩小了低精度训练方法的实证成功与理论理解之间的差距，为低精度训练提供了理论基础，并揭示了不同优化器对量化误差的敏感性差异。
...
### [223] [SCORENF: Score-based Normalizing Flows for Sampling Unnormalized distributions](https://arxiv.org/abs/2510.21330)
*Vikas Kanaujia,Vipul Arora*

Main category: cs.LG

TL;DR: 提出了ScoreNF框架，结合归一化流和独立Metropolis-Hastings模块，用于从未归一化概率分布中进行高效无偏采样，减少对大量训练数据的依赖。


<details>
  <summary>Details</summary>
Motivation: 传统MCMC方法收敛慢、存在临界减速和模式混合问题，而基于似然的机器学习方法需要大量数据且容易出现模式覆盖和模式崩溃问题。

Method: 基于归一化流架构构建评分学习框架，集成独立Metropolis-Hastings模块，并提出了评估模式覆盖和模式崩溃行为的方法。

Result: 在合成2D分布（MOG-4和MOG-8）和高维φ4晶格场论分布上验证了方法的有效性，即使在小训练集下也能保持高性能。

Conclusion: ScoreNF框架能够从未归一化目标分布中进行高效无偏采样，减少了对计算昂贵的MCMC生成训练数据的依赖。
...
### [224] [Weak-to-Strong Generalization under Distribution Shifts](https://arxiv.org/abs/2510.21332)
*Myeongho Jeon,Jan Sobotka,Suhwan Choi,Maria Brbić*

Main category: cs.LG

TL;DR: RAVEN是一个鲁棒的弱到强泛化框架，通过动态学习弱模型的最优组合来解决分布偏移下弱到强泛化失败的问题，在多个任务上显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 随着超人类模型日益复杂，人类可能无法有效监督其行为。现有弱到强泛化方法在分布偏移下会失效，导致强模型性能比弱监督者更差。
...
### [225] [$α$-LoRA: Effective Fine-Tuning via Base Model Rescaling](https://arxiv.org/abs/2510.21345)
*Aymane El Firdoussi,El Mahdi Chayti,Mohamed El Amine Seddik,Martin Jaggi*

Main category: cs.LG

TL;DR: 提出了一种新的重参数化方法，旨在提升微调模型的泛化能力，通过随机矩阵理论在高维二元分类场景中验证有效性，并在LLM微调等实际实验中进一步证实。
...
Method: 引入了一类新的重参数化方法，通过在高维二元分类设置中使用随机矩阵理论工具来建立方法的有效性。

Result: 理论分析和实验验证表明，该方法能够有效提升微调模型的泛化性能。

Conclusion: 新提出的重参数化方法在提升模型泛化能力方面具有显著效果，为迁移学习提供了更优的解决方案。
...
### [226] [Robust Yield Curve Estimation for Mortgage Bonds Using Neural Networks](https://arxiv.org/abs/2510.21347)
*Sina Molavipour,Alireza M. Javid,Cassie Ye,Björn Löfdahl,Mikhail Nechaev*

Main category: cs.LG

TL;DR: 提出基于神经网络的鲁棒收益率曲线估计框架，专门针对小型抵押债券市场，通过新的损失函数确保平滑性和稳定性。


<details>
  <summary>Details</summary>
Motivation: 传统方法如bootstrap和Nelson-Siegel模型在债券稀疏、价格波动或存在噪声时容易过拟合或不稳定，需要更鲁棒的估计方法。

Method: 使用神经网络框架，每日独立估计收益率曲线，引入新损失函数强制平滑性和稳定性，可集成领域特定约束如无风险基准对齐。

Result: 在瑞典抵押债券上的实证结果显示，相比NSS和KR方法，该方法提供更鲁棒和稳定的收益率曲线估计。

Conclusion: 该框架为小型抵押债券市场提供了有效的收益率曲线估计解决方案，允许从业者在平滑性和准确性之间进行权衡。
...
### [227] [Compositional Monte Carlo Tree Diffusion for Extendable Planning](https://arxiv.org/abs/2510.21361)
*Jaesik Yoon,Hyeonseo Cho,Sungjin Ahn*

Main category: cs.LG

TL;DR: C-MCTD通过引入三个组件（在线组合器、分布式组合器、预规划组合器）将规划从单轨迹优化提升到完整计划组合推理，解决了MCTD在训练轨迹长度和局部规划方面的限制。


<details>
  <summary>Details</summary>
Motivation: 解决MCTD受限于训练轨迹长度和局部规划的问题，MCTD在单个轨迹内搜索而缺乏全局上下文，限制了其规划能力。

Method: 提出C-MCTD框架，包含三个组件：在线组合器进行全局感知规划，分布式组合器通过并行探索降低搜索复杂度，预规划组合器利用缓存计划图加速推理。

Result: C-MCTD能够进行完整计划组合的推理，提升了规划能力，解决了MCTD的局限性。

Conclusion: C-MCTD通过组合式方法有效提升了扩散模型与树搜索结合的规划能力，实现了从单轨迹优化到完整计划组合推理的跨越。
...
### [228] [Randomized Neural Network with Adaptive Forward Regularization for Online Task-free Class Incremental Learning](https://arxiv.org/abs/2510.21367)
*Junda Wang,Minghui Hu,Ning Li,Abdulaziz Al-Ali,Ponnuthurai Nagaratnam Suganthan*

Main category: cs.LG

TL;DR: 提出一种基于随机神经网络和前向正则化的在线任务无关类增量学习框架，通过集成深度随机向量函数链接网络和贝叶斯自适应调整，有效解决非独立同分布数据流中的灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 解决类增量学习在实际应用中的两个关键问题：(1) 非独立同分布批量数据流且无边界提示的在线任务无关场景；(2) 长任务流中的记忆丢失问题，旨在实现高效决策并降低累积遗憾。

Method: 提出随机神经网络前向正则化框架，集成无监督知识到递归凸优化中。开发edRVFL-kF算法，生成单次闭式增量更新和可变学习率。进一步改进为edRVFL-kF-Bayes，通过贝叶斯学习自适应确定正则化强度。

Result: 在2个图像数据集上进行了6个指标的实验，包括动态性能、消融测试和兼容性验证，结果表明-kF-Bayes和-kF风格的OTCIL框架具有显著有效性。

Conclusion: 所提出的OTCIL框架能够有效避免过去重放和灾难性遗忘，在在线任务无关类增量学习场景中实现优越性能，且通过贝叶斯自适应机制解决了非独立同分布数据带来的不稳定惩罚问题。
...
### [229] [Cost-Sensitive Freeze-thaw Bayesian Optimization for Efficient Hyperparameter Tuning](https://arxiv.org/abs/2510.21379)
...
Method: 在冻结-解冻框架中引入效用函数，结合新的采集函数和停止准则，动态继续训练预期能最大程度提高未来效用的配置，并自动在最大效用附近停止HPO过程。同时通过迁移学习提高样本效率。

Result: 在已建立的多保真度HPO基准测试中，算法优于所有考虑的先前冻结-解冻BO和迁移BO基线，在成本和性能之间实现了显著更好的权衡。

Conclusion: 提出的成本敏感HPO方法有效解决了计算成本与性能的权衡问题，在多个基准测试中表现出优越性能。
...
### [230] [Disentangled Representation Learning via Modular Compositional Bias](https://arxiv.org/abs/2510.21402)
...
<details>
  <summary>Details</summary>
Motivation: 现有解耦表示学习方法依赖特定因子策略，当新变化因子不符合先验假设或多个因子共存时，需要重新设计架构或目标，造成显著开销。

Method: 引入组合偏置作为模块化归纳偏置，通过因子特定的混合策略随机重混合潜在变量，使用先验损失和组合一致性损失使编码器发现因子结构。

Result: 在属性和对象解耦方面表现出竞争力，并独特地实现了全局风格和对象的联合解耦。

Conclusion: 该方法通过简单调整混合策略即可实现不同因子的解耦，提供了一种灵活且通用的解耦表示学习框架。
...
### [231] [Large Language Models as Model Organisms for Human Associative Learning](https://arxiv.org/abs/2510.21408)
*Camila Kolling,Vy Ai Vo,Mariya Toneva*

Main category: cs.LG

TL;DR: 该研究使用大语言模型研究联想学习中的表征变化，发现符合非单调可塑性假说的模式，并揭示了词汇干扰对表征分化的调节作用。
...
Method: 基于大语言模型的上下文学习能力，改编认知神经科学联想学习范式，在六个模型中研究表征如何演化，并分析词汇干扰的影响。

Result: 发现非单调模式与NMPH一致，中等相似项目在学习后发生分化。词汇干扰（新关联与先验知识竞争）调节这种分化，干扰越高分化越强。

Conclusion: 大语言模型不仅是研究类人学习系统中表征动态的强大工具，也是生成关于大脑记忆重组原理新假说的可访问通用计算模型。
...
### [232] [Self-diffusion for Solving Inverse Problems](https://arxiv.org/abs/2510.21417)
*Guanxiong Luo,Shoujin Huang,Yanlong Yang*

Main category: cs.LG

TL;DR: 提出了一种无需预训练生成模型的自扩散框架，通过交替加噪和去噪步骤迭代优化解，使用随机初始化的单卷积网络作为自去噪器。


<details>
  <summary>Details</summary>
Motivation: 传统扩散方法需要预训练模型来学习逆过程，而自扩散旨在开发自包含的迭代过程，不依赖预训练模型，提高灵活性和适应性。

Method: 自扩散框架交替进行加噪和去噪步骤，使用随机初始化的卷积网络作为自去噪器，通过数据保真度损失训练，利用神经网络的频谱偏置和调度噪声过程。

Result: 在线性逆问题上，自扩散方法取得了与其他方法相当或更优的性能表现。

Conclusion: 自扩散提供了一种不依赖预训练模型的有效逆问题求解框架，具有高度灵活性和广泛适用性。
...
### [233] [DreamerV3-XP: Optimizing exploration through uncertainty estimation](https://arxiv.org/abs/2510.21418)
*Lukas Bierling,Davide Pasero,Jan-Henrik Bertrand,Kiki Van Gerwen*

Main category: cs.LG

TL;DR: DreamerV3-XP是DreamerV3的扩展版本，通过优先回放缓冲器和基于世界模型分歧的内在奖励机制，提高了探索和学习效率。
...
Method: 使用基于回报、重建损失和价值误差的优先回放缓冲器，以及基于世界模型预测奖励分歧的内在奖励机制。

Result: 在Atari100k和DeepMind Control Visual Benchmark任务上验证了DreamerV3的结果，并显示出更快的学习和更低的动态模型损失。

Conclusion: DreamerV3-XP扩展有效提升了学习效率，尤其在稀疏奖励环境中表现更佳。
...
### [234] [A Rapid Physics-Informed Machine Learning Framework Based on Extreme Learning Machine for Inverse Stefan Problems](https://arxiv.org/abs/2510.21426)
...
<details>
  <summary>Details</summary>
Motivation: 传统物理信息神经网络（PINNs）在解决Stefan问题时存在超参数依赖性强、训练效率低和预测精度不足的问题，需要开发更高效准确的求解方法。

Method: 使用极限学习机网络替代深度神经网络，固定输入权重，通过最小二乘法优化输出权重，将逆Stefan问题转化为寻找Moore-Penrose广义逆的问题。

Result: 与传统PINNs相比，PIELM将相对L2误差提高了3-7个数量级，同时节省了超过94%的训练时间。

Conclusion: PIELM框架为逆Stefan问题提供了一种快速、准确的物理信息学习方法，在精度和效率方面均优于传统PINNs。
...
### [235] [Causality Meets Locality: Provably Generalizable and Scalable Policy Learning for Networked Systems](https://arxiv.org/abs/2510.21427)
...
<details>
  <summary>Details</summary>
Motivation: 解决大规模网络系统（如交通、电力、无线网格）中强化学习面临的规模和环境变化挑战，需要同时实现可扩展性和领域泛化能力。

Method: 每个智能体学习稀疏局部因果掩码识别影响其动态的最小邻域变量，生成近似紧凑表示；元actor-critic在多个源域上训练共享策略，并在测试时通过少量轨迹估计新域因子来部署适应策略。

Result: GSAC能够快速适应新领域，显著优于从头学习和传统适应基准方法，并建立了因果恢复、actor-critic收敛和适应差距的有限样本保证。

Conclusion: GSAC框架成功解决了大规模网络系统中强化学习的可扩展性和领域泛化问题，通过因果表示和元学习实现了高效的学习和快速适应能力。
...
### [236] [Unified token representations for sequential decision models](https://arxiv.org/abs/2510.21448)
*Zhuojing Tian,Yushu Chen*

Main category: cs.LG

TL;DR: 提出统一令牌表示(UTR)方法，将离线强化学习中的回报目标、状态和动作合并为单一令牌，显著减少序列长度和模型复杂度，在保持性能的同时大幅降低计算成本。
...
Method: 开发统一令牌表示(UTR)，将回报目标、状态和动作合并为单一令牌，并基于此构建UDT(Transformer骨干)和UDC(门控CNN骨干)两个变体。

Result: 两种变体在性能上达到或超过现有最优方法，同时计算成本显著降低。理论分析显示UTR具有更紧的Rademacher复杂度边界，表明更好的泛化能力。

Conclusion: UTR方法能很好地跨架构泛化，为未来大规模决策模型的可扩展控制提供了高效基础。
...
### [237] [ParaRNN: Unlocking Parallel Training of Nonlinear RNNs for Large Language Models](https://arxiv.org/abs/2510.21450)
*Federico Danieli,Pau Rodriguez,Miguel Sarabia,Xavier Suau,Luca Zappella*

Main category: cs.LG

TL;DR: ParaRNN是一个突破非线性RNN序列并行化障碍的框架，通过将非线性递推关系转换为方程组并使用牛顿迭代和并行归约求解，实现了665倍的加速，成功训练了70亿参数的LSTM和GRU模型。


<details>
  <summary>Details</summary>
Motivation: 传统RNN的序列性限制了并行计算，而现有的并行架构如Transformers和状态空间模型虽然能并行化，但线性约束限制了表达能力，无法建模复杂的非线性序列依赖关系。

Method: 将非线性递推关系序列转换为单个方程组，使用牛顿迭代结合自定义并行归约进行并行求解，实现了非线性RNN的高效并行训练。

Result: 实现了高达665倍的速度提升，成功训练了70亿参数的LSTM和GRU模型，其困惑度与同等规模的Transformers和Mamba2架构相当。

Conclusion: ParaRNN突破了非线性RNN的并行化障碍，为高效序列建模研究提供了开源框架，使研究人员能够大规模探索新的非线性RNN模型。
...
### [238] [Towards Explainable Personalized Recommendations by Learning from Users' Photos](https://arxiv.org/abs/2510.21455)
*Jorge Díez,Pablo Pérez-Núñez,Oscar Luaces,Beatriz Remeseiro,Antonio Bahamonde*

Main category: cs.LG

TL;DR: 提出了一种通过预测用户会拍摄什么照片来解释推荐系统的新方法，将个性化解释本身作为推荐来学习


<details>
  <summary>Details</summary>
Motivation: 解释复杂系统（如推荐系统）的输出对用户和公司都至关重要，用户上传的照片可以强化或证明他们对物品的看法

Method: 构建形式化框架来估计给定（用户，照片）对的作者概率，使用TripAdvisor餐厅评论数据（包含照片）进行验证

Result: 能够预测用户会为物品拍摄什么照片，这些图像是最能说服用户相信物品品质的论据

Conclusion: 该方法不仅能解释推荐结果提高系统可靠性，还能让公司了解客户关注的产品特点
...
### [239] [Estimating Treatment Effects in Networks using Domain Adversarial Training](https://arxiv.org/abs/2510.21457)
...
Conclusion: HINet方法成功解决了网络环境中异质处理效应估计的关键挑战，特别是在未知暴露映射和协变量偏移问题上的表现优于现有方法。
...
### [240] [Parameter-Free Hypergraph Neural Network for Few-Shot Node Classification](https://arxiv.org/abs/2510.21462)
*Chaewoon Bae,Doyun Choi,Jaehyun Lee,Jaemin Yoo*

Main category: cs.LG

TL;DR: ZEN是一种零参数超图神经网络，通过闭式解和冗余感知传播方案，在少样本节点分类任务中实现了高准确率和显著加速。


<details>
  <summary>Details</summary>
Motivation: 现有的超图神经网络在编码高阶结构时存在过拟合和可扩展性问题，需要一种既表达力强又高效的模型。

Method: 基于线性化超图神经网络的统一公式，提出闭式解权重矩阵和冗余感知传播方案，避免迭代训练并消除冗余信息。

Result: 在11个真实世界超图基准测试中，ZEN在分类准确率上持续优于8个基线模型，同时实现高达696倍的加速。

Conclusion: ZEN不仅性能优越，而且决策过程完全可解释，为理解数据集特性提供了洞见。
...
### [241] [Benchmarking Catastrophic Forgetting Mitigation Methods in Federated Time Series Forecasting](https://arxiv.org/abs/2510.21491)
...
<details>
  <summary>Details</summary>
Motivation: 灾难性遗忘在持续学习中是一个持续挑战，特别是在具有非独立同分布时间序列数据的联邦学习环境中。现有研究主要集中在视觉领域的分类任务，而物联网和边缘应用中普遍存在的基于回归的预测设置仍未得到充分探索。

Method: 使用北京多站点空气质量数据集在12个去中心化客户端上，系统评估了多种灾难性遗忘缓解策略，包括重放、弹性权重巩固、无遗忘学习和突触智能等方法。

Result: 建立了首个针对时间序列联邦学习中灾难性遗忘的基准测试，进行了最先进方法的全面比较分析，并发布了可复现的开源框架。
...
### [242] [Uniform Convergence Beyond Glivenko-Cantelli](https://arxiv.org/abs/2510.21506)
*Tanmay Devale,Pramith Devulapalli,Steve Hanneke*

Main category: cs.LG

TL;DR: 该论文研究了分布集合在{0,1}^N上的均值一致估计问题，提出了UME-learnability概念，证明了均值向量可分性是UME-learnability的充分条件但非必要条件，并解决了关于可数并集UME-learnability的猜想。


<details>
  <summary>Details</summary>
Motivation: 扩展Vapnik和Chervonenkis(1971)关于经验均值估计器的一致收敛框架，研究更一般的均值一致估计问题，即允许使用任意估计器而非仅限于经验均值估计器。

Method: 在分布集合的均值向量空间上工作，分析均值向量可分性与UME-learnability的关系，构造反例证明可分性非必要条件，并使用与可分性分析不同的技术方法。

Result: 证明了均值向量可分性是UME-learnability的充分条件；构造了均值向量不可分但UME-learnable的分布集合；证明了UME-learnable集合的可数并集也是UME-learnable的。

Conclusion: 该研究建立了更一般的均值一致估计理论框架，解决了相关猜想，并为理解分布集合的估计性质提供了新的理论工具。
...
### [243] [Generative Correlation Manifolds: Generating Synthetic Data with Preserved Higher-Order Correlations](https://arxiv.org/abs/2510.21610)
...
Method: 使用Cholesky分解目标相关矩阵的计算高效方法，生成能数学证明保留完整相关结构的合成数据集。

Result: 该方法能够保留从简单成对关系到高阶交互的整个相关结构，为合成数据生成提供了新途径。

Conclusion: GCM方法在隐私保护数据共享、鲁棒模型训练和仿真方面具有潜在应用价值。
...
### [244] [Surrogate-based quantification of policy uncertainty in generative flow networks](https://arxiv.org/abs/2510.21523)
...
TL;DR: 提出一种使用多项式混沌展开构建代理模型的方法，用于量化生成流网络在奖励函数存在不确定性时的策略不确定性。
...
Method: 构建基于多项式混沌展开的代理模型，在少量训练好的流网络上拟合，学习低维参数化奖励函数与流网络轨迹中各步骤动作概率分布之间的关系。

Result: 代理模型可用于廉价的蒙特卡洛采样，估计在不确定奖励下策略的不确定性，在离散和连续网格世界、符号回归和贝叶斯结构学习任务中验证了性能。

Conclusion: 该方法能有效量化生成流网络在不确定奖励函数下的策略不确定性，为策略可靠性评估提供实用工具。
...
### [245] [A Unified Model for Multi-Task Drone Routing in Post-Disaster Road Assessment](https://arxiv.org/abs/2510.21525)
*Huatian Gong,Jiuh-Biing Sheu,Zheng Wang,Xiaoguang Yang,Ran Yan*

Main category: cs.LG

TL;DR: 提出统一模型(UM)解决8种灾后道路评估变体问题，通过单一神经网络实现多任务学习，减少8倍训练时间和参数，在解决方案质量上优于单任务DRL方法6-14%，优于传统优化方法24-82%。


<details>
  <summary>Details</summary>
Motivation: 传统优化方法扩展性差且需要领域专业知识，现有深度强化学习方法采用单任务范式，需要为每个问题变体训练单独模型，缺乏对不断变化的操作需求的适应性。

Method: 使用现代transformer编码器-解码器架构，通过跨多个问题配置训练单一神经网络来捕获共享结构知识，同时通过轻量级适配器机制适应变体特定约束，实现高效微调。

Result: UM在多达1000个节点的网络中实现实时解决方案(1-10秒)，通过敏感性分析确认了鲁棒性。微调实验显示可以以最小成本有效整合未见属性，同时保持高解决方案质量。

Conclusion: 所提出的UM推进了时间关键应用的神经组合优化，为基于无人机的灾后道路评估提供了计算高效、高质量且适应性强的解决方案。
...
### [246] [Probe-based Fine-tuning for Reducing Toxicity](https://arxiv.org/abs/2510.21531)
*Jan Wehner,Mario Fritz*

Main category: cs.LG

TL;DR: 论文探讨了使用探针作为训练目标时可能出现的Goodhart定律问题，并提出了两种基于监督微调和直接偏好优化的训练方法。研究发现基于探针的偏好优化比基于分类器的方法更好地保持探针检测能力，且探针多样性益处有限，重新训练探针即可恢复高检测精度。


<details>
  <summary>Details</summary>
Motivation: 探针能够检测模型内部的不良行为（如欺骗或偏见），这些行为难以仅从输出中识别。然而，当探针成为训练目标时，可能因Goodhart定律而变得不可靠。

Method: 提出了两种训练方法：基于监督微调和直接偏好优化。在毒性减少测试平台上评估这些方法，并尝试通过探针集成、保留未用于训练的探针以及训练后重新训练新探针来保持探针检测器的准确性。

Result: 基于探针的偏好优化意外地比基于分类器的方法更好地保持了探针检测能力；探针多样性提供的实际益处有限，只需在优化后重新训练探针即可恢复高检测精度。

Conclusion: 基于探针的训练对于某些对齐方法是可行的，但在重新训练可行的情况下，探针集成基本上是不必要的。
...
### [247] [DEEDEE: Fast and Scalable Out-of-Distribution Dynamics Detection](https://arxiv.org/abs/2510.21638)
*Tala Aljaafari,Varun Kanade,Philip Torr,Christian Schroeder de Witt*

Main category: cs.LG

TL;DR: DEEDEE是一个用于强化学习时间序列的OOD检测器，使用两个统计量（episodewise均值和RBF核相似度）来检测分布偏移，在保持高性能的同时大幅降低计算成本。
...
Method: 使用两个统计量的检测器：episodewise均值和RBF核相似度，捕捉全局和局部偏差，避免复杂的表示学习。

Result: 在标准RL OOD测试套件中，DEEDEE匹配或超越当代检测器，计算量减少600倍，准确率平均提升5%。

Conclusion: 结果表明，RL轨迹中的各种异常类型通常通过少量低阶统计量体现，为复杂环境中的OOD检测提供了紧凑基础。
...
### [248] [FrameShield: Adversarially Robust Video Anomaly Detection](https://arxiv.org/abs/2510.21532)
*Mojtaba Nafez,Mobina Poulaei,Nikan Vasei,Bardia Soltani Moakhar,Mohammad Sabokrou,MohammadHossein Rohban*

Main category: cs.LG

TL;DR: 提出了一种名为时空区域扭曲(SRD)的伪异常生成方法，通过增强弱监督视频异常检测模型的对抗鲁棒性，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 弱监督视频异常检测模型容易受到对抗攻击，但由于只有视频级标签而缺乏帧级监督，传统对抗训练方法效果不佳。伪标签虽然能实现帧级对抗训练，但噪声严重影响性能。

Method: 使用时空区域扭曲(SRD)方法，通过在正常视频的局部区域应用严重增强来创建合成异常，同时保持时间一致性。将这些精确标注的合成异常与噪声伪标签结合，显著减少标签噪声。

Result: 在多个基准测试中，该方法将WSVAD模型的整体AUROC性能平均提升了71.0%，显著优于现有最先进方法。

Conclusion: 提出的SRD方法通过生成精确标注的合成异常，有效减少了伪标签噪声，实现了有效的对抗训练，显著增强了WSVAD模型的鲁棒性。
...
### [249] [Excision Score: Evaluating Edits with Surgical Precision](https://arxiv.org/abs/2510.21537)
*Nikolai Gruzinov,Ksenia Sycheva,Earl T. Barr,Alex Bezzubov*

Main category: cs.LG

TL;DR: 提出了修订相似度问题，并开发了Excision Score (ES)来衡量文档修订的相似性。ES通过移除共享内容后比较差异区域，解决了传统指标如BLEU被共享内容主导的问题。


<details>
  <summary>Details</summary>
Motivation: 现有相似度指标如BLEU在评估文档修订时存在根本缺陷，因为它们被共享内容主导，无法准确反映人类对修订相似度的判断。

Method: 提出Excision Score (ES)方法，使用最长公共子序列(LCS)移除现有文档与修订版本之间的共享内容，然后比较剩余的差异区域。通过近似方法将标准立方LCS计算加速到二次复杂度。

Result: 在代码编辑评估中，ES显著优于现有指标。在HumanEvalFix数据集上，ES比最接近的竞争对手SARI提高了12%的皮尔逊相关性，比标准指标如BLEU提高了>21%。当增加共享上下文时，ES的改进进一步增加到20%和>30%。

Conclusion: ES是一个有效的修订相似度度量方法，能够正确处理共享内容、代码块移动、插入/删除匹配等复杂情况，与人类判断更好地对齐。
...
### [250] [Cost Minimization for Space-Air-Ground Integrated Multi-Access Edge Computing Systems](https://arxiv.org/abs/2510.21541)
*Weihong Qin,Aimin Wang,Geng Sun,Zemin Sun,Jiacheng Wang,Dusit Niyato,Dong In Kim,Zhu Han*

Main category: cs.LG

TL;DR: 提出MADDPG-COCG算法解决SAGIN-MEC系统中的用户设备成本最小化问题，通过多智能体深度确定性策略梯度与凸优化联盟博弈相结合的方法，优化任务卸载比例、无人机轨迹规划等混合决策变量。


<details>
  <summary>Details</summary>
Motivation: 低空经济快速发展需要灵活广域计算服务，但SAGIN-MEC系统面临异构节点协调、移动性和网络变化建模、部分可观测环境下实时决策等挑战。

Method: 提出分层SAGIN-MEC架构，使用MADDPG算法优化连续时域决策，结合凸优化和联盟博弈方法处理混合变维决策，形成MADDPG-COCG算法。

Result: 相比基准算法，显著降低了用户设备总成本、任务完成延迟和能耗，仅略微增加无人机能耗，且具有更好的收敛稳定性和可扩展性。

Conclusion: MADDPG-COCG算法能有效解决SAGIN-MEC系统中的复杂优化问题，为低空经济提供高效的计算服务解决方案。
...
### [251] [Interpretable Multimodal Zero-Shot ECG Diagnosis via Structured Clinical Knowledge Alignment](https://arxiv.org/abs/2510.21551)
...
<details>
  <summary>Details</summary>
Motivation: 当前自动化ECG诊断系统在透明性和对未见病症的泛化能力方面存在不足，需要更符合临床工作流程的可解释方法。

Method: 使用LLM辅助、专家验证的过程构建结构化临床观察数据集，利用预训练多模态模型对齐ECG和文本嵌入，无需疾病特定微调。

Result: ZETA在零样本分类性能上具有竞争力，定性和定量证据表明其增强了可解释性，将预测基于具体的临床相关诊断特征。
...
### [252] [Leveraging Classical Algorithms for Graph Neural Networks](https://arxiv.org/abs/2510.21574)
...
<details>
  <summary>Details</summary>
Motivation: 神经网络擅长处理非结构化数据但泛化能力不足，而经典算法保证正确性但缺乏灵活性。探索将经典算法的先验知识嵌入GNN是否能提升分子图数据的预测性能。

Method: 在CLRS算法推理基准的24个经典算法上预训练GNN，然后将预训练模型的选定层初始化和冻结，用于分子属性预测任务的第二个GNN。

Result: 相比随机初始化基线，预训练模型在ogbg-molhiv（HIV抑制）任务上获得6%绝对增益，在ogbg-molclintox（临床毒性）任务上获得3%增益。

Conclusion: 将经典算法先验嵌入GNN能为复杂真实世界图数据提供有用的归纳偏置，显著提升性能。
...
### [253] [An unsupervised tour through the hidden pathways of deep neural networks](https://arxiv.org/abs/2510.21582)
*Diego Doimo*

Main category: cs.LG

TL;DR: 该论文研究了深度神经网络内部机制，包括数据内在维度估计、隐藏层概率密度演化以及泛化性能分析。


<details>
  <summary>Details</summary>
Motivation: 理解深度神经网络如何创建有意义的表示并实现泛化，重点在于表征隐藏表示的语义内容。

Method: 开发了Gride方法估计数据内在维度；分析隐藏层概率密度演化；研究宽神经网络中的冗余表示。

Result: 发现初始层生成单峰概率密度，后续层以分层方式产生密度峰；宽网络学习冗余表示而非过拟合伪相关。

Conclusion: 深度神经网络通过分层密度峰构建语义层次，冗余神经元在正则化且训练误差为零时出现，有助于泛化。
...
### [254] [REVE: A Foundation Model for EEG -- Adapting to Any Setup with Large-Scale Pretraining on 25,000 Subjects](https://arxiv.org/abs/2510.21585)
*Yassine El Ouahidi,Jonathan Lys,Philipp Thölke,Nicolas Farrugia,Bastien Pasdeloup,Vincent Gripon,Karim Jerbi,Giulia Lioi*

Main category: cs.LG

TL;DR: REVE是一个专门为处理多样化EEG信号设计的预训练模型，通过创新的4D位置编码方案和掩码自编码目标，在92个数据集上预训练，在10个下游EEG任务中取得最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 现有EEG基础模型难以泛化到不同协议、设备和电极配置的数据集，限制了在EEG领域的应用效果。

Method: 引入4D位置编码方案处理任意长度和电极排列的信号，使用掩码自编码目标在92个数据集上预训练超过60,000小时EEG数据。

Result: 在10个下游EEG任务中达到最先进性能，包括运动想象分类、癫痫检测、睡眠分期等，展示了强大的泛化能力和精细的时空建模。

Conclusion: REVE推动了标准化EEG研究，为临床神经科学提供了加速发展的工具。
...
### [255] [Accelerating Data Generation for Nonlinear temporal PDEs via homologous perturbation in solution space](https://arxiv.org/abs/2510.21592)
*Lei Liu,Zhenxin Huang,Hong Wang,huanshuo dong,Haiyang Xin,Hongwei Zhao,Bin Li*

Main category: cs.LG

TL;DR: 提出HOPSS算法，通过同源扰动方法直接生成训练数据集，减少时间步数，显著加速数据生成过程，同时保持模型训练所需的精度。
...
Conclusion: HOPSS算法能够显著加速PDE训练数据集的生成，同时保持数据精度，为数据驱动的深度学习方法提供了更高效的数据生成方案。
...
### [256] [SHAP Meets Tensor Networks: Provably Tractable Explanations with Parallelism](https://arxiv.org/abs/2510.21599)
*Reda Marzouk,Shahaf Bassan,Guy Katz*

Main category: cs.LG

TL;DR: 本文提出了计算张量网络(TNs)SHAP解释的通用框架，证明了在张量列车(TT)结构下SHAP计算可在多对数时间内并行完成，并揭示了神经网络宽度而非深度是SHAP计算的主要计算瓶颈。


<details>
  <summary>Details</summary>
Motivation: 虽然SHAP对于简单模型可在多项式时间内计算，但对于神经网络等更复杂的黑盒模型，SHAP计算变为NP难问题。本研究旨在解决张量网络这一更广泛模型类的SHAP计算问题。

Method: 引入计算任意结构张量网络精确SHAP解释的通用框架，特别关注张量列车(TT)结构下的高效并行算法，并利用二值化神经网络到张量网络的归约进行分析。

Result: 在TT结构下，SHAP计算可在多对数时间内并行完成；对于宽度固定的二值化神经网络，SHAP计算变得高效可处理，而深度固定的情况下计算仍然困难。

Conclusion: 张量网络的SHAP计算复杂度分析揭示了神经网络宽度而非深度是SHAP计算的主要计算瓶颈，为更广泛模型类的可解释性提供了新的理论洞见。
...
### [257] [Generalised Flow Maps for Few-Step Generative Modelling on Riemannian Manifolds](https://arxiv.org/abs/2510.21608)
...
<details>
  <summary>Details</summary>
Motivation: 当前几何生成模型在推理时计算成本高昂，需要复杂的数值模拟步骤，因为它们基于黎曼流形上的动力学测度传输框架。
...
Conclusion: GFM为几何生成建模提供了一种高效且有效的少步生成方法，显著降低了推理成本，同时保持了高质量的样本生成能力。
...
### [258] [Optimal Graph Clustering without Edge Density Signals](https://arxiv.org/abs/2510.21669)
*Maximilien Dreveton,Elaine Siyu Liu,Matthias Grossglauser,Patrick Thiran*

Main category: cs.LG

TL;DR: 本文建立了在流行度调整块模型(PABM)下图聚类的理论极限，解决了现有模型的局限性。PABM引入了簇内和簇间连接的独立流行度参数，即使在传统边密度信号消失时，只要簇内和簇间流行度系数不同，聚类恢复仍然可能。


<details>
  <summary>Details</summary>
Motivation: 现有图聚类模型如SBM假设顶点度均匀，DCBM在簇间应用统一的度校正，无法捕捉局部连接模式的差异。PABM旨在解决这些局限性，通过引入独立的簇内和簇间流行度参数来更准确地建模真实网络的度异质性。

Method: 提出PABM模型，该模型为簇内和簇间连接分别引入流行度参数。理论分析了PABM下聚类的最优错误率，并通过数值实验验证了使用k^2个特征向量的谱聚类方法优于传统方法。

Result: 证明了在PABM中，即使传统边密度信号消失，只要簇内和簇间流行度系数不同，聚类恢复仍然可能。实验表明，基于k^2个特征向量的谱聚类算法在合成和真实数据集上优于传统谱方法。

Conclusion: PABM捕捉了DCBM忽略的度异质性维度：局部连接模式的差异可以独立于全局边密度增强簇可分性。由于PABM的期望邻接矩阵秩在k到k^2之间，传统使用k个特征向量的谱嵌入可能丢失重要结构信息。
...
### [259] [On Uncertainty Calibration for Equivariant Functions](https://arxiv.org/abs/2510.21691)
*Edward Berman,Jacob Ginesin,Marco Pacini,Robin Walters*

Main category: cs.LG

TL;DR: 本文研究了等变性与模型校准之间的关系，通过理论证明和数值实验揭示了等变模型在不确定性估计方面的泛化界限，以及对称性不匹配如何导致分类和回归任务中的校准错误。


<details>
  <summary>Details</summary>
Motivation: 在数据稀疏场景（如机器人操作、分子物理等）中，等变网络能改善建模效果，不确定性估计能防止过度自信。但等变性与模型置信度、校准之间的关系尚未被研究，本文旨在填补这一空白。

Method: 通过理论分析证明等变条件下不确定性校准误差（ECE和ENCE）的上下界，并结合真实和模拟数据集进行数值实验，分析对称性不匹配、群大小以及偶然和认知不确定性等因素的影响。

Result: 建立了等变性与不确定性估计的理论联系，揭示了等变模型的泛化界限，证明对称性不匹配会导致分类和回归任务中的校准错误，并通过实验验证了理论发现。

Conclusion: 等变性对模型校准具有重要影响，对称性不匹配是导致模型校准错误的关键因素，这为等变模型在不确定性估计方面的应用提供了理论指导。
...
### [260] [Mechanistic Interpretability for Neural TSP Solvers](https://arxiv.org/abs/2510.21693)
*Reuben Narad,Leonard Boussioux,Michael Wagner*

Main category: cs.LG

TL;DR: 该论文应用稀疏自编码器（SAE）来解释基于Transformer的旅行商问题（TSP）求解器，首次揭示了神经网络在组合优化中学习到的几何特征。


<details>
  <summary>Details</summary>
Motivation: 解决神经网络TSP求解器作为黑盒的问题，理解其学习的几何模式和启发式策略，为运筹学模型提供可解释性。

Method: 训练指针网络解决100节点TSP实例，然后在编码器残差流上拟合稀疏自编码器，发现过完备的、可解释的特征字典。

Result: 发现求解器自然发展出反映基本TSP概念的特征：边界检测器（凸包节点）、聚类敏感特征（局部密集区域）和分隔符特征（几何分区）。

Conclusion: 首次从模型内部解释了神经TSP求解器在节点选择前的计算过程，证明几何结构无需显式监督即可出现，为结合神经效率和算法可解释性的透明混合系统提供了路径。
...
### [261] [Equivariance by Contrast: Identifiable Equivariant Embeddings from Unlabeled Finite Group Actions](https://arxiv.org/abs/2510.21706)
*Tobias Schmidt,Steffen Schneider,Matthias Bethge*

Main category: cs.LG

TL;DR: 提出了Equivariance by Contrast (EbC)方法，从观察对(y, g·y)中学习等变嵌入，其中g来自有限群。该方法无需依赖群特定的归纳偏置，联合学习潜在空间和群表示。


<details>
  <summary>Details</summary>
Motivation: 旨在开发一种通用方法，仅从群作用观察中学习等变表示，无需特定群的先验知识，包括处理非阿贝尔群和乘积群。

Method: 通过对比学习框架，从变换对(y, g·y)中联合学习潜在空间和群表示，使群操作在潜在空间中对应可逆线性映射。

Result: 在无限dSprites数据集上验证了方法有效性，实现了高保真等变性，群操作在潜在空间中准确再现。在合成数据上成功验证了正交群O(n)和一般线性群GL(n)。
...
### [262] [NeuroPilot: A Realtime Brain-Computer Interface system to enhance concentration of students in online learning](https://arxiv.org/abs/2510.20958)
*Asif Islam,Farhan Ishtiaque,Md. Muhyminul Haque,Kaled Masukur Rahman,Ravi Vaidyanathan,Khondaker A. Mamun*

Main category: cs.HC

TL;DR: 开发了一种基于脑机接口(BCI)的实时注意力监测系统，使用非侵入式EEG头带来检测学习者的注意力状态，通过实时反馈显著提高了注意力水平。


<details>
  <summary>Details</summary>
Motivation: 在线学习普及使得实时监测学生注意力变得重要，传统问卷方法需要人工干预，摄像头监控无法准确反映认知参与度，现有BCI方法缺乏实时验证评估。

Method: 使用FocusCalm EEG头带采集脑电信号，结合视频内问卷验证，采用滑动窗口分割、带通滤波、伪影去除，提取时频特征并通过SVM分类器识别注意力状态。

Result: 留一交叉验证准确率达到88.77%，实时反馈实验显示反馈阶段注意力显著改善(t=5.73, p=0.007)。

Conclusion: 该BCI系统能有效实时监测和改善学习者注意力，为在线教育提供可靠的注意力评估工具。
...
### [263] [Race and Gender in LLM-Generated Personas: A Large-Scale Audit of 41 Occupations](https://arxiv.org/abs/2510.21011)
*Ilona van der Linden,Sahana Kumar,Arnav Dixit,Aadi Sudan,Smruthi Danda,David C. Anastasiu,Kai Lukoff*

Main category: cs.HC

TL;DR: 对4个大型语言模型生成的150万份职业人物画像进行审计，发现AI在职业人物刻画中存在种族和性别的系统性偏差和刻板印象放大问题。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI工具越来越多地被用于创建职业人物画像，需要研究这些工具如何呈现种族和性别，以及不同AI安全承诺和来源国模型之间的差异。

Method: 对来自美国、中国、法国的4个大型语言模型生成的41种美国职业的150多万份人物画像进行大规模审计，并与美国劳工统计局数据进行对比。

Result: 发现两个重复模式：系统性偏移（某些群体持续被低估或高估）和刻板印象放大（现有的人口统计偏差被加剧）。白人（-31pp）和黑人（-9pp）工人代表性不足，而西班牙裔（+17pp）和亚裔（+12pp）工人代表性过高。

Conclusion: AI提供者的选择会实质性地改变可见性，需要针对具体模型进行审计和负责任的设讠实践。
...
### [264] [Designing and Evaluating Hint Generation Systems for Science Education](https://arxiv.org/abs/2510.21087)
*Anubhav Jangra,Smaranda Muresan*

Main category: cs.HC

TL;DR: 研究大型语言模型在教育中自动生成提示的作用，通过静态和动态提示策略促进主动学习，避免直接给出答案。


<details>
  <summary>Details</summary>
Motivation: 通用语言模型在教育中倾向于直接给出答案，这可能阻碍学生的概念理解和批判性思维发展。

Method: 比较静态提示（预生成）和动态提示（根据学习进度调整）两种策略，通过41名参与者的定量研究进行评估。

Result: 发现学习者对提示策略有不同的偏好，自动评估指标难以捕捉这些差异。

Conclusion: 研究为未来提示生成和智能辅导系统的学习者中心设计提供了关键考虑因素。
...
### [265] [Co-Designing with Multiple Stakeholders and Datasets: A Community-Centered Process to Understand Youth Deviance in the Italian City of Turin](https://arxiv.org/abs/2510.21467)
...
TL;DR: 本文介绍了Sbocciamo Torino公民工具的协同设计和设计评估，该工具通过多利益相关方协作和数据协作分析，帮助理解和应对意大利都灵市的青少年偏差问题。


<details>
  <summary>Details</summary>
Motivation: 解决都灵市青少年偏差这一复杂社会问题，通过多利益相关方协作和数据驱动的方法来理解和干预这一问题。

Method: 采用研究型设计和参与式设计方法，整合数据仪表板、利益相关方委员会和结构化协同设计会议，促进协作分析和干预规划。

Result: 利益相关方重视包容性设计方法和数据驱动协作，但发现沟通、数据素养和运营协调方面存在障碍，政治和制度支持对工具成功至关重要。

Conclusion: 公民工具可以通过参与式设计协作开发，为应对复杂社会问题提供有效途径，为社区技术研究做出贡献。
...
### [266] [Actionable Cybersecurity Notifications for Smart Homes: A User Study on the Role of Length and Complexity](https://arxiv.org/abs/2510.21508)
*Victor Jüttner,Charlotte S. Löffler,Erik Buchmann*

Main category: cs.HC

TL;DR: 研究通过用户实验发现，中等复杂度的LLM生成安全通知在智能家居用户中最有效，无论用户技术水平如何。新手用户偏好较长的初级通知，而专家用户偏好较短的专家级通知。


<details>
  <summary>Details</summary>
Motivation: 智能家居设备存在安全风险，但入侵检测系统的警报通常使用技术术语，普通用户难以理解。需要找到LLM生成的安全通知的最佳格式，使其既易于理解又能促使用户采取行动。

Method: 通过在线用户研究，对130名参与者测试不同长度和复杂度的LLM生成安全通知，评估用户喜好度、理解度和行动动机。

Result: 中等复杂度的通知在所有用户群体中效果最好。新手用户认为较长的初级通知更有效，而专家用户认为较短的专家级通知稍好。

Conclusion: 设计智能家居安全通知时，应采用中等复杂度的格式，这样既能保证可操作性，又能广泛适用于不同技术水平的用户。
...
### [267] [Human and AI Trust: Trust Attitude Measurement Instrument](https://arxiv.org/abs/2510.21535)
*Retno Larasati*

Main category: cs.HC

TL;DR: 开发并验证了一个用于测量非专家对AI医疗支持系统信任度的16项量表工具


<details>
  <summary>Details</summary>
Motivation: 随着AI技术发展和应用扩大，信任成为AI使用、接受和部署的关键标准，需要从人本视角准确评估信任的测量工具

Method: 遵循心理测量学原则，通过六个研究阶段开发信任量表：项目开发、项目评估、调查实施、维度测试、可靠性测试和有效性测试

Result: 六阶段评估结果表明，所提出的信任测量工具在系统测量和比较非专家对AI医疗支持系统的信任度方面具有实证可靠性和有效性

Conclusion: 成功开发了一个可靠有效的信任测量工具，专门用于人机交互研究中测量非专家对AI系统的信任态度
